...
def get_winerror(exception: OSError) -> int | None:
    try:
...
class Registrar:
    ...
    _frozen: None | int | str
    _frozendllhandle: int | None
    ...
    def nodebug(self, cls: type) -> None:
        ...
        try:
            _debug(f'DeleteKey( {_explain(HKCR)}\\CLSID\\{clsid}\\Logging"')
    ...
    def debug(self, cls: type, levels: list[str], format: str | None) -> None:
        """Write entries in the registry to setup logging for this clsid."""
        # handlers
        # format
        clsid = cls._reg_clsid_
        _debug(f'CreateKey( {_explain(HKCR)}\\CLSID\\{clsid}\\Logging"')
        hkey = winreg.CreateKey(HKCR, rf"CLSID\{clsid}\Logging")
        for item in levels:
            _name, value = item.split("=")
            v = getattr(logging, value)
            assert isinstance(v, int)
        _debug(f"SetValueEx(levels, {levels})")
        winreg.SetValueEx(hkey, "levels", None, winreg.REG_MULTI_SZ, levels)
        if format:
            _debug(f"SetValueEx(format, {format})")
    ...
    def register(self, cls: type, executable: str | None = None) -> None:
    ...
    def _register(self, cls: type, executable: str | None = None) -> None:
...
def UseCommandLine(*classes: type) -> int:
    ...
    opts, _args = w_getopt(sys.argv[1:], "regserver unregserver embedding l: f: nodebug")
    if not opts:
