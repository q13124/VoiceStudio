...
OLECHAR = WCHAR
PVOID = c_void_p
# See https://msdn.microsoft.com/en-us/library/windows/desktop/aa383751(v=vs.85).aspx#ULONG_PTR
...
class ITypeComp(IUnknown):
    ...
    ) -> tuple[Literal["function"], "FUNCDESC"] | tuple[Literal["variable"], "VARDESC"] | tuple[Literal["type"], "ITypeComp"] | None:
        """Bind to a name"""
        bindptr = BINDPTR()
        desckind = DESCKIND()
...
def RegisterTypeLib(
    tlib: ITypeLib, fullpath: str, helpdir: str | None = None
) -> int:
...
def GetModuleFileName(handle: int | None, maxsize: int) -> str:
