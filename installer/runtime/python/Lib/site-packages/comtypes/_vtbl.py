...
def HRESULT_FROM_WIN32(errcode: int | None) -> int:
...
def catch_errors(
    ...
    has_outargs = False if paramflags is None else bool([x[0] for x in paramflags if x[0] & 2])
...
def hack(
    ...
    def call_without_this(this, *args):
        ...
        try:
            ...
            try:
                descr, source, _helpfile, _helpcontext, _progid = details
...
def _make_dispmthentry(
    ...
    if "propget" in m.idlflags:
        ...
        argspec = (*m.argspec, (["out"], m.restype, "")) if m.restype else m.argspec
        mthname = m.name
