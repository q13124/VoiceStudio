...
def _unpack_argspec(
    ...
    name: str | None = None,
    defval: Any = _NOTHING,
) -> tuple[list[str], type["_CDataType"], str | None, Any]:
...
def _resolve_argspec(
    ...
    for item in items:
        ...
        if "optional" in idl and defval is _NOTHING:
            if typ is VARIANT:
...
class _ComMemberSpec(NamedTuple):
    ...
    restype: type["_CDataType"] | None
    name: str
    argtypes: tuple[type["_CDataType"], ...]
    paramflags: tuple["hints.ParamFlagType", ...] | None
...
class _DispMemberSpec(NamedTuple):
    ...
    def memid(self) -> int:
        try:
            return next(x for x in self.idlflags if isinstance(x, int))
        except IndexError:
    ...
    def call_with_inout(self, *args, **kw):
        ...
        for i, info in enumerate(paramflags):
            ...
            if dir_in and dir_out:
                ...
                atyp: type[_CDataType] = argtypes[i]._type_
...
class DispMemberGenerator:
    def __init__(self, cls_name: str) -> None:
        ...
        self._items: list[tuple[str, Callable[..., Any] | property, bool]] = []
