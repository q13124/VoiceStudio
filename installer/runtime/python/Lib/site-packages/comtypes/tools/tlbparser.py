...
class Parser:
    ...
    def ParseRecord(
        ...
        struct_name, _doc, _helpcntext, _helpfile = tinfo.GetDocumentation(-1)
    ...
    def ParseModule(self, tinfo: typeinfo.ITypeInfo, ta: typeinfo.TYPEATTR) -> None:
        ...
        for i in range(ta.cFuncs):
            ...
            dllname, func_name, _ordinal = tinfo.GetDllEntry(fd.memid, fd.invkind)
            func_doc = tinfo.GetDocumentation(fd.memid)[1]
            assert fd.cParamsOpt == 0  # XXX
    ...
    def ParseUnion(
        ...
        union_name, _doc, _helpcntext, _helpfile = tinfo.GetDocumentation(-1)
        members = []
    ...
    def _typelib_module(self, tlib: typeinfo.ITypeLib | None = None) -> str:
        if tlib is None:
            tlib = self.tlib
    ...
    def _register(
        self, name: str | None, value: Any, tlib: typeinfo.ITypeLib | None = None
    ) -> None:
...
def get_tlib_filename(tlib: typeinfo.ITypeLib) -> str | None:
