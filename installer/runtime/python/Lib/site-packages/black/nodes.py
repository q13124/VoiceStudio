...
class Visitor(Generic[T]):
    ...
    def visit(self, node: LN) -> Iterator[T]:
        ...
        """
        name = token.tok_name[node.type] if node.type < 256 else str(type_repr(node.type))
...
def whitespace(leaf: Leaf, *, complex_subscript: bool, mode: Mode) -> str:
    ...
    elif p.type == syms.sliceop or (p.type == syms.except_clause and t == token.STAR):
        return NO
...
def is_tuple(node: LN) -> bool:
    ...
    if node.type != syms.atom:
        return False
    gexp = unwrap_singleton_parenthesis(node)
    return not (gexp is None or gexp.type != syms.testlist_gexp)
