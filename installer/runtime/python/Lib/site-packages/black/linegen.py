...
def left_hand_split(
    ...
    for leaf_type in [token.LPAR, token.LSQB]:
        ...
        for index, leaf in enumerate(line.leaves):
            ...
            if current_leaves is head_leaves and leaf.type == leaf_type and (
...
def _ensure_trailing_comma(
    ...
    return not (leaf_with_parent.parent and leaf_with_parent.parent.next_sibling and leaf_with_parent.parent.next_sibling.type == token.VBAR)
...
def remove_await_parens(node: Node, mode: Mode, features: Collection[Feature]) -> None:
    if node.children[0].type == token.AWAIT and len(node.children) > 1 and (
