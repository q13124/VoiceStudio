...
) -> Callable[["XmpInformation"], list[str] | None]:
    def get(self: "XmpInformation") -> list[str] | None:
...
) -> Callable[["XmpInformation"], list[Any] | None]:
    def get(self: "XmpInformation") -> list[Any] | None:
        cached = self.cache.get(namespace, {}).get(name)
        if cached:
            return cached
        retval = []
...
def _getter_langalt(
    namespace: str, name: str
) -> Callable[["XmpInformation"], dict[Any, Any] | None]:
    def get(self: "XmpInformation") -> dict[Any, Any] | None:
        cached = self.cache.get(namespace, {}).get(name)
        if cached:
            return cached
        retval = {}
...
) -> Callable[["XmpInformation"], Any | None]:
    def get(self: "XmpInformation") -> Any | None:
        cached = self.cache.get(namespace, {}).get(name)
        if cached:
            return cached
        value = None
