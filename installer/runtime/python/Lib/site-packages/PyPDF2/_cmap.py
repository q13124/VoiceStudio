...
def build_char_map(
    ...
    str, float, str | dict[int, str], dict, DictionaryObject
...
unknown_char_map: tuple[str, float, str | dict[int, str], dict[Any, Any]] = (
    "Unknown",
    9999,
    dict(zip(range(256), ["�"] * 256, strict=False)),
    {},
)
...
def parse_encoding(
    ...
    encoding: str | list[str] | dict[int, str] = []
    if "/Encoding" not in ft:
        try:
            if "/BaseFont" in ft and cast(str, ft["/BaseFont"]) in charset_encoding:
                encoding = dict(
                    zip(range(256), charset_encoding[cast(str, ft["/BaseFont"])], strict=False)
                )
            else:
                encoding = "charmap"
    ...
    if isinstance(encoding, list):
        encoding = dict(zip(range(256), encoding, strict=False))
...
def parse_to_unicode(
    ...
    multiline_rg: None | tuple[int, int] = None  # tuple = (current_char, remaining size) ; cf #1285 for example of file
    cm = prepare_cm(ft)
    for l in cm.split(b"\n"):
...
def process_cm_line(
    ...
    process_rg: bool,
    process_char: bool,
    multiline_rg: None | tuple[int, int],
    map_dict: dict[Any, Any],
    int_entry: list[int],
) -> tuple[bool, bool, None | tuple[int, int]]:
