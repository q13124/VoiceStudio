...
def pytest_report_header(config):
    ...
    if config._env_timeout:
        timeout_header.append(
            f"timeout: {config._env_timeout}s\ntimeout method: {config._env_timeout_method}\ntimeout func_only: {config._env_timeout_func_only}"
        )
...
def _parse_marker(marker):
    ...
    for kw, val in marker.kwargs.items():
        if kw == "timeout":
            ...
            raise TypeError(f"Invalid keyword argument for timeout marker: {kw}")
...
def _validate_timeout(timeout, where):
    ...
    try:
        ...
        raise ValueError(f"Invalid timeout {timeout} from {where}")
...
def _validate_method(method, where):
    ...
    if method not in ["signal", "thread"]:
        raise ValueError(f"Invalid method {method} from {where}")
    return method
...
def _validate_disable_debugger_detection(disable_debugger_detection, where):
    ...
    if not isinstance(disable_debugger_detection, bool):
        raise ValueError(
            f"Invalid disable_debugger_detection value {disable_debugger_detection} from {where}"
        )
