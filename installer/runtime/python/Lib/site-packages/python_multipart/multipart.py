...
def parse_options_header(value: str | bytes | None) -> tuple[bytes, dict[bytes, bytes]]:
    ...
    for param in params:
        ...
        if key == "filename" and (value[1:3] == ":\\" or value[:2] == "\\\\"):
            value = value.split("\\")[-1]
...
class File:
    ...
    def __init__(self, file_name: bytes | None, field_name: bytes | None = None, config: FileConfig = None) -> None:
    ...
    def _get_disk_file(self) -> BufferedRandom:
        ...
        if file_dir is not None and keep_filename:
            ...
            try:
                ...
                raise FileError(f"Error opening temporary file: {path!r}")
        else:
            # Build options array.
...
class OctetStreamParser(BaseParser):
    ...
    def __init__(self, callbacks: OctetStreamCallbacks = None, max_size: float = float("inf")):
        ...
        if not isinstance(max_size, Number) or max_size < 1:
            raise ValueError(f"max_size must be a positive number, not {max_size!r}")
...
class QuerystringParser(BaseParser):
    ...
    def __init__(
        self, callbacks: QuerystringCallbacks = None, strict_parsing: bool = False, max_size: float = float("inf")
    ) -> None:
        if callbacks is None:
            callbacks = {}
        super().__init__()
    ...
    def _internal_write(self, data: bytes, length: int) -> int:
        ...
        while i < length:
            ...
            if state == QuerystringState.BEFORE_FIELD:
                ...
                # data state.
                equals_pos = data.find(b"=", i, sep_pos) if sep_pos != -1 else data.find(b"=", i)
    ...
    def __repr__(self) -> str:
        return f"{self.__class__.__name__}(strict_parsing={self.strict_parsing!r}, max_size={self.max_size!r})"
...
class MultipartParser(BaseParser):
    ...
    def __init__(
        self, boundary: bytes | str, callbacks: MultipartCallbacks = None, max_size: float = float("inf")
    ) -> None:
    ...
    def __init__(
        ...
        if content_type == "application/octet-stream":
            ...
            def on_headers_finished() -> None:
                ...
                _disp, options = parse_options_header(content_disp)
...
def parse_form(
    ...
    content_length = int(content_length) if content_length is not None else float("inf")
    bytes_read = 0
