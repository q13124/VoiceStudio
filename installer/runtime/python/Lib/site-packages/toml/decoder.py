...
def _detect_pathlib_path(p):
    import pathlib
    return bool(isinstance(p, pathlib.PurePath))
...
def loads(s, _dict=dict, decoder=None):
    ...
    for idx, line in enumerate(s):
        ...
        if multikey:
            ...
            if closed:
                try:
                    value, _vtype = decoder.load_value(multilinestr)
...
class TomlDecoder:
    ...
    def load_line(self, line, currentlevel, multikey, multibackslash):
        ...
        if k > -1:
            ...
            value, _vtype = self.load_value(pair[1], strictly_valid)
        try:
            currentlevel[pair[0]]
