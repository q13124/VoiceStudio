...
class ProgressBar(t.Generic[V]):
    ...
    def make_step(self, n_steps: int) -> None:
        ...
        # self.length.
        step = (time.time() - self.start) / self.pos if self.pos else time.time() - self.start
...
def _pipepager(generator: t.Iterable[str], cmd: str, color: bool | None) -> bool:
...
def _nullpager(
    stream: t.TextIO, generator: t.Iterable[str], color: bool | None
) -> None:
...
class Editor:
    ...
    def edit(self, text: t.AnyStr | None) -> t.AnyStr | None:
        ...
        if not text:
            ...
            data = text.replace("\n", "\r\n").encode("utf-8-sig") if WIN else text.encode("utf-8")
...
def open_url(url: str, wait: bool = False, locate: bool = False) -> int:
    ...
    try:
        url = os.path.dirname(_unquote_file(url)) or "." if locate else _unquote_file(url)
...
def _translate_ch_to_exc(ch: str) -> BaseException | None:
    if ch == "\x03":
