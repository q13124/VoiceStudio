...
class Detector:
    ...
    def _update_lang_prob(self, prob, word, alpha):
        ...
        if self.verbose:
            six.print_(f'{word}({self._unicode_encode(word)}): {self._word_prob_to_string(lang_prob_map)}')
    ...
    def _sort_probability(self, prob):
        result = [Language(lang, p) for (lang, p) in zip(self.langlist, prob, strict=False) if p > self.PROB_THRESHOLD]
