...
def LocateWebServerPath(description):
    ...
    for site in iis:
        ...
        if description in site_attributes:
            return site.AdsPath
    msg = f"No web sites match the description '{description}'"
...
def AssignScriptMaps(script_maps, target, update="replace"):
    ...
    try:
        ...
        msg = f"Unknown ScriptMapUpdate option '{update}'"
...
def _DeleteExtensionFileRecord(module, options):
    try:
        ...
        log(2, f"Deleted extension file record for '{module}'")
    except (pythoncom.com_error, AttributeError) as details:
...
def RemoveDirectory(params, options):
    ...
    try:
        ...
        if rc != winerror.ERROR_PATH_NOT_FOUND:
            raise
        log(2, f"VirtualDirectory '{params.Name}' did not exist")
        directory = None
...
def RemoveScriptMaps(vd_params, options):
    ...
    parent, _name = vd_params.split_path()
...
def GetLoaderModuleName(mod_name, check_module=None):
    ...
    if hasattr(sys, "frozen"):
        ...
        base, _ext = os.path.splitext(mod_name)
        path, base = os.path.split(base)
...
def HandleCommandLine(
    ...
    process.
    """
    if custom_arg_handlers is None:
        custom_arg_handlers = {}
    global verbose
    ...
    try:
        ...
        parser.error(f"Invalid arg '{arg}'")
