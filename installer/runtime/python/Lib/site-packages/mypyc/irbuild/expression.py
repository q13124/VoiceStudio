...
def try_gen_slice_op(builder: IRBuilder, base: Value, index: SliceExpr) -> Value | None:
    ...
    begin_type = builder.node_type(index.begin_index) if index.begin_index else int_rprimitive
    end_type = builder.node_type(index.end_index) if index.end_index else int_rprimitive

    # Both begin and end index must be int (or missing).
    if is_int_rprimitive(begin_type) and is_int_rprimitive(end_type):
        begin = builder.accept(index.begin_index) if index.begin_index else builder.load_int(0)
        if index.end_index:
