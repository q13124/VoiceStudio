...
def gen_func_item(
    ...
    contains_nested = fitem in builder.encapsulating_funcs
    ...
    func_name = singledispatch_main_func_name(name) if is_singledispatch else name
...
def gen_func_body(
    ...
    args, _, blocks, _ret_type, fn_info = builder.leave()
...
def handle_ext_method(builder: IRBuilder, cdef: ClassDef, fdef: FuncDef) -> None:
    ...
    sig = builder.mapper.fdef_to_sig(fdef, builder.options.strict_dunders_typing)
    func_ir, _func_reg = gen_func_item(builder, fdef, name, sig, cdef)
    builder.functions.append(func_ir)
...
def generate_dispatch_glue_native_function(
    ...
    arg_regs, _, blocks, _, _fn_info = builder.leave()
...
def gen_property_getter_ir(
    ...
    if not is_trait:
        ...
        builder.add(Unreachable())
    args, _, blocks, _ret_type, _fn_info = builder.leave()
