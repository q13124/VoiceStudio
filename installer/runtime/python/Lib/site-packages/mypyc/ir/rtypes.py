...
def deserialize_type(data: JsonDict | str, ctx: DeserMaps) -> RType:
    ...
    if isinstance(data, str):
        if data in ctx.classes:
            ...
            raise AssertionError(f"Can't find class {data}")
    ...
    def __init__(
        ...
        *,
        ...
        if ctype == "CPyTagged":
            ...
            raise AssertionError(f"Unrecognized ctype: {ctype!r}")
...
class TupleNameVisitor(RTypeVisitor[str]):
    ...
    def visit_rstruct(self, t: RStruct) -> str:
        raise AssertionError("RStruct not supported in tuple")

    def visit_rarray(self, t: RArray) -> str:
        raise AssertionError("RArray not supported in tuple")

    def visit_rvoid(self, t: RVoid) -> str:
        raise AssertionError("rvoid in tuple?")
...
def compute_rtype_alignment(typ: RType) -> int:
    ...
    if isinstance(typ, RPrimitive):
        return typ.size
    elif isinstance(typ, (RInstance, RUnion)):
        ...
        if isinstance(typ, RTuple):
            ...
            raise AssertionError("invalid rtype for computing alignment")
...
def compute_rtype_size(typ: RType) -> int:
    ...
    if isinstance(typ, RPrimitive):
        ...
        raise AssertionError("invalid rtype for computing size")
