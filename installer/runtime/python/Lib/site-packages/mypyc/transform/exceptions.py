        ...
        for op in ops:
            ...
            if isinstance(op, RegisterOp) and op.error_kind != ERR_NEVER:
                ...
                if op.error_kind == ERR_MAGIC:
                    ...
                    raise AssertionError("unknown error kind %d" % op.error_kind)
...
def adjust_error_kinds(block: BasicBlock) -> None:
    ...
    """
    for op in block.ops:
        if isinstance(op, GetAttr) and op.class_type.class_ir.is_always_defined(op.attr):
            op.error_kind = ERR_NEVER
        if isinstance(op, SetAttr) and op.class_type.class_ir.is_always_defined(op.attr):
            op.error_kind = ERR_NEVER
