...
def emit_attr_defaults_func_call(defaults_fn: FuncIR, self_name: str, emitter: Emitter) -> None:
    ...
    """
    emitter.emit_lines(
        f"if ({NATIVE_PREFIX}{defaults_fn.cname(emitter.names)}((PyObject *){self_name}) == 0) {{",
        "Py_DECREF(self);",
        "return NULL;",
        "}",
    )
...
def generate_constructor_for_class(
    ...
    if init_fn is not None:
        ...
        emitter.emit_line(
            f"char res = {emitter.get_group_prefix(init_fn.decl)}{prefix}{init_fn.cname(emitter.names)}({args}){cast};"
        )
