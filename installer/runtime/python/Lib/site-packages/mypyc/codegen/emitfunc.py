...
class FunctionEmitterVisitor(OpVisitor[None]):
    ...
    def visit_branch(self, op: Branch) -> None:
        ...
        if op.traceback_entry is not None or op.rare:
            cond = f"unlikely({cond})" if not negated_rare else f"likely({cond})"
        ...
        if cl.get_method(op.attr, prefer_method=prefer_method):
            ...
            if not always_defined:
                ...
                if not merged_branch:
                    ...
                    self.emitter.emit_line(
                        f'PyErr_SetString({exc_class}, "attribute {op.attr.removeprefix(GENERATOR_ATTRIBUTE_PREFIX)!r} of {cl.name!r} undefined");'
                    )
    ...
    def visit_call_c(self, op: CallC) -> None:
        dest = "" if op.is_void else self.get_dest_assign(op)
        args = ", ".join(self.reg(arg) for arg in op.args)
