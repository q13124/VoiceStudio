    ...
    try:
        ...
        if not os.path.exists(source_file):
            ...
            if source_file.startswith("<doctest "):
                ...
                line_no = int(line_no)
                source_file = f"<doctest {source_file}>"
...
def filter_args(func, ignore_lst, args=(), kwargs=None):
    ...
    **kwargs: dict
    ...
    for arg_position, arg_name in enumerate(arg_names):
        if arg_position < len(args):
            ...
            if arg_name not in arg_kwonlyargs:
                ...
                raise ValueError(
                    f"Keyword-only parameter '{arg_name}' was passed as "
                    f"positional parameter for {_signature_str(name, arg_sig)}:\n"
    ...
    for arg_name, arg_value in sorted(kwargs.items()):
        if arg_name in arg_dict:
            ...
            raise TypeError(
                f"Ignore list for {name}() contains an unexpected "
                f"keyword argument '{arg_name}'"
            )
...
def _format_arg(arg):
    ...
    if len(formatted_arg) > 1500:
        formatted_arg = f"{formatted_arg[:700]}..."
...
def format_signature(func, *args, **kwargs):
    ...
    arg_str.extend([f"{v}={_format_arg(i)}" for v, i in kwargs.items()])
    arg_str = ", ".join(arg_str)

    signature = f"{name}({arg_str})"
...
def format_call(func, args, kwargs, object_name="Memory"):
    ...
    msg = "{}\n[{}] Calling {}...\n{}".format(80 * "_", object_name, path, signature)
    return msg
