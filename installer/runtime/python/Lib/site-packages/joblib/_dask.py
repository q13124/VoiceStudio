...
class DaskDistributedBackend(AutoBatchingMixin, ParallelBackendBase):
    ...
    def __init__(
        ...
        **submit_kwargs,
        ...
        if scatter is not None and not isinstance(scatter, (list, tuple)):
            raise TypeError(
                f"scatter must be a list/tuple, got `{type(scatter).__name__}`"
            )
        ...
        if scatter is not None and len(scatter) > 0:
            ...
            self.data_futures = {id(x): f for x, f in zip(scatter, scattered, strict=False)}
        else:
            self._scatter = []
            self.data_futures = {}
    ...
    async def _to_func_args(self, func):
        ...
        for f, args, kwargs in func.items:
            ...
            kwargs = dict(zip(kwargs.keys(), await maybe_to_futures(kwargs.values()), strict=False))
