...
class BatchCompletionCallBack:
    ...
    def get_result(self, timeout):
        ...
        try:
            ...
            outcome = {"result": result, "status": TASK_DONE}
        except BaseException as e:
            outcome = {"result": e, "status": TASK_ERROR}
    ...
    def get_status(self, timeout):
        ...
        if (now - self._completion_timeout_counter) > timeout:
            outcome = {"result": TimeoutError(), "status": TASK_ERROR}
            self._register_outcome(outcome)
    ...
    def _retrieve_result(self, out):
        ...
        try:
            ...
            outcome = {"status": TASK_DONE, "result": result}
        except BaseException as e:
...
class Parallel(Logger):
    ...
    * Ability to use shared memory efficiently with worker
        ...
        **backend_kwargs,
        ...
        if backend is default_parallel_config["backend"] or backend is None:
            ...
            try:
                ...
                raise ValueError(
                    f"Invalid backend: {backend}, expected one of {sorted(BACKENDS.keys())!r}"
                ) from e
        ...
        if require == "sharedmem" and not getattr(backend, "supports_sharedmem", False):
            raise ValueError(f"Backend {backend} does not support shared memory")
        ...
        try:
            ...
            if self.timeout is not None and not self._backend.supports_timeout:
                warnings.warn(
                    ...
                    "the 'timeout' parameter will not be used.", stacklevel=2
                )
        ...
        with self._lock:
            ...
            try:
                ...
                try:
                    ...
                    self._register_new_job(batch_tracker)
                    batch_tracker._register_outcome({"result": e, "status": TASK_ERROR})
                    return True
    ...
    def _wait_retrieval(self):
        ...
        return bool(not self._backend.supports_retrieve_callback and len(self._jobs) > 0)
    ...
    def __call__(self, iterable):
        ...
        if n_jobs == 0:
            raise RuntimeError(f"{backend_name} has no active worker.")
    ...
    def __repr__(self):
        return f"{self.__class__.__name__}(n_jobs={self.n_jobs})"
