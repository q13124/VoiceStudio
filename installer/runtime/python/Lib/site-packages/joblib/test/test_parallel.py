...
def test_nested_loop_with_exception_with_loky():
    with raises(ValueError), Parallel(n_jobs=2, backend="loky") as parallel:
...
def test_error_capture(backend):
    ...
    if mp is not None:
        with raises(ZeroDivisionError):
            Parallel(n_jobs=2, backend=backend)(
                [delayed(division)(x, y) for x, y in zip((0, 1), (1, 0), strict=False)]
            )
        ...
        with Parallel(n_jobs=2, backend=backend) as parallel:
            ...
            with raises(ZeroDivisionError):
                parallel([delayed(division)(x, y) for x, y in zip((0, 1), (1, 0), strict=False)])
    ...
    with raises(ZeroDivisionError):
        Parallel(n_jobs=2)([delayed(division)(x, y) for x, y in zip((0, 1), (1, 0), strict=False)])
...
def test_invalid_backend():
    ...
    with raises(ValueError, match="Invalid backend:"), parallel_config(backend="unit-testing"):
        pass
...
def test_auto_memmap_on_arrays_from_generator(backend):
    ...
    for result, expected in zip(results, generate_arrays(len(results)), strict=False):
...
def test_parallel_return_order_with_return_as_generator_parameter(n_jobs):
    ...
    assert all(v == r for v, r in zip(input_list, result, strict=False))
...
def _test_parallel_unordered_generator_returns_fastest_first(backend, n_jobs):
    ...
    assert all(v == r for v, r in zip(expected_quickly_returned, quickly_returned, strict=False))
...
def test_multiple_generator_call_separated(backend, return_as_1, return_as_2, n_jobs):
    ...
    assert all(res == i for res, i in zip(g, range(10), strict=False))
    assert all(res == i for res, i in zip(g2, range(10, 20), strict=False))
...
def test_multiple_generator_call_separated_gc(backend, return_as_1, return_as_2, error):
    ...
    with ctx:
        ...
        assert all(res == i for res, i in zip(g, range(10, 20), strict=False))
...
def test_thread_bomb_mitigation(context, backend):
    ...
    # of threads.
    with context(backend, n_jobs=2), raises(BaseException) as excinfo:
        _recursive_parallel()
    exc = excinfo.value
    if backend == "loky":
...
def test_zero_worker_backend(context):
    ...
    with context(ZeroWorkerBackend()), pytest.raises(RuntimeError, match=expected_msg):
