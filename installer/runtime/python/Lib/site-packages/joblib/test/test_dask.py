...
from joblib.parallel import AutoBatchingMixin, ThreadingBackend
...
def test_simple(loop):
    with cluster() as (s, [_a, _b]), Client(s["address"], loop=loop) as client:  # noqa: F841
...
def test_dask_funcname(loop, mixed):
    ...
    with cluster() as (s, [_a, _b]), Client(s["address"], loop=loop) as client:
        with parallel_config(backend="dask"):
...
def test_manual_scatter(loop):
    ...
    with cluster() as (s, _), Client(s["address"], loop=loop) as client:
...
def test_auto_scatter(loop_in_thread):
    ...
    with cluster() as (s, [a, b]), Client(s["address"], loop=loop_in_thread) as client:
        with parallel_config(backend="dask"):
...
def test_nested_scatter(loop, retry_no):
    ...
    with cluster() as (s, [_a, _b]), Client(s["address"], loop=loop) as _:
        with parallel_config(backend="dask"):
...
def test_errors(loop):
    with pytest.raises(ValueError) as info, parallel_config(backend="dask"):
        pass
...
def test_dask_backend_keywords(loop):
    with cluster() as (s, [a, b]), Client(s["address"], loop=loop) as client:  # noqa: F841
