...
def test_parallel_config_nested():
    ...
    with parallel_config(backend="threading"), parallel_config(n_jobs=2):
        p = Parallel()
        assert isinstance(p._backend, ThreadingBackend)
        assert p.n_jobs == 2

    with parallel_config(verbose=100), parallel_config(n_jobs=2):
        p = Parallel()
...
def test_parallel_n_jobs_none(context):
    ...
    with context(backend="threading", n_jobs=2), Parallel(n_jobs=None) as p:
        assert p.n_jobs == 2
...
def test_parallel_config_n_jobs_none(context):
    ...
    # non regression test for #1473
    with context(backend="threading", n_jobs=2), context(backend="threading", n_jobs=None):
