...
def test_value_error():
    ...
    with raises(ValueError):
        numpy_pickle.dump("foo", {})


@parametrize("wrong_compress", [-1, 10, {}])
...
def test_numpy_persistence(tmpdir, compress):
    ...
    for _index, obj in enumerate(((a,), (a.T,), (a, a), [a, a, a])):
...
def test_compressed_pickle_dump_and_load(tmpdir):
    ...
    for result, expected in zip(result_list, expected_list, strict=False):
...
def test_file_handle_persistence_compressed_mmap(tmpdir):
    ...
    with closing(gzip.GzipFile(filename, "rb")) as f:
        ...
        assert (
            ...
            == '"{fileobj!r}" is not a raw file, mmap_mode "{mmap_mode}" '
            "flag will be ignored.".format(fileobj=f, mmap_mode="r+")
        )
...
def test_file_handle_persistence_in_memory_mmap():
    ...
    assert (
        ...
        '"{mmap_mode}" flag passed. mmap_mode option will be '
        "ignored.".format(mmap_mode="r+")
    )
...
def test_pickle_in_socket():
    ...
    with socket.create_connection(_ADDR) as client:
        server, _client_addr = listener.accept()
