...
def test_call_and_shelve(tmpdir):
    ...
    for func, Result in zip(
        ...
        (MemorizedResult, NotMemorizedResult, MemorizedResult, NotMemorizedResult), strict=False,
    ):
        assert func(2) == 5
...
def test_cached_function_race_condition_when_persisting_output_2(tmpdir, capfd):
    ...
    Parallel(n_jobs=2)(
        delayed(func_cached)(i % 2 == 0) for i in range(3)
    )
...
@parametrize("invalid_prefix", [None, {}, []])
...
def test_memory_pickle_dump_load(tmpdir, memory_kwargs):
    ...
    compare(
        memory,
        memory_reloaded,
        ignored_attrs={"store_backend", "timestamp", "_func_code_id"},
    )
