...
class StoreBackendMixin:
    ...
    def dump_item(self, call_id, item, verbose=1):
        ...
        try:
            ...
            if verbose > 10:
                print(f"Persisting in {item_path}")
    ...
    def _get_items_to_delete(self, bytes_limit, items_limit=None, age_limit=None):
        ...
        to_delete_size = size - bytes_limit if bytes_limit is not None else 0

        to_delete_items = len(items) - items_limit if items_limit is not None else 0
    ...
    def configure(self, location, verbose=1, backend_options=None):
        ...
        if not os.path.exists(gitignore):
            try:
                ...
                warnings.warn(f"Unable to write {gitignore}. Exception: {e}.", stacklevel=2)
