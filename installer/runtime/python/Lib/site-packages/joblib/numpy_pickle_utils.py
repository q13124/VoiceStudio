        ...
        if mmap_mode is not None:
            ...
            if isinstance(fileobj, io.BytesIO):
                warnings.warn(
                    ...
                    'mmap_mode "{mmap_mode}" flag passed. '
                    "mmap_mode option will be ignored.".format(**locals()),
                    stacklevel=2,
                )
            elif compressor != "not-compressed":
                warnings.warn(
                    'mmap_mode "{mmap_mode}" is not compatible '
                    "with compressed file {filename}. "
                    '"{mmap_mode}" flag will be ignored.'.format(**locals()),
                    stacklevel=2,
                )
...
def _write_fileobject(filename, compress=("zlib", 3)):
    ...
    if compressmethod in _COMPRESSORS:
