...
class ResourceTracker(_ResourceTracker):
    ...
    def _teardown_dead_process(self):
        ...
        warnings.warn(
            ...
            "Some folders/semaphores might leak.", stacklevel=2
        )
                    ...
                    if cmd == "PROBE":
                        ...
                        if registry[rtype][name] == 0:
                            ...
                            try:
                                ...
                                warnings.warn(
                                    f"resource_tracker: {name}: {e!r}", stacklevel=2
                                )
        ...
        def _unlink_resources(rtype_registry, rtype):
            if rtype_registry:
                with contextlib.suppress(Exception):
                    warnings.warn(
                        ...
                        "clean up at shutdown", stacklevel=2
                    )
            for name in rtype_registry:
                ...
                try:
                    ...
                    warnings.warn(f"resource_tracker: {name}: {e!r}", stacklevel=2)
