...
class Logger:
    ...
    def warn(self, msg):
        logging.getLogger(self._name).warning(f"[{self}]: {msg}")
    ...
    def debug(self, msg):
        ...
        logging.getLogger(self._name).debug(f"[{self}]: {msg}")
...
class PrintTime:
    ...
    def __call__(self, msg="", total=False):
        ...
        if not total:
            ...
            full_msg = f"{msg}: {format_time(time_lapse)}"
        else:
            # FIXME: Too much logic duplicated
            time_lapse = time.time() - self.start_time
            full_msg = f"{msg}: {time_lapse:.2f}s, {time_lapse / 60:.1f} min"
