...
RE_EMOTICONS = re.compile(r"({})($|\s)".format("|".join(RE_EMOTICONS)))
    ...
    def assessments(self, words=None, negation=True):
        ...
        for w, pos in words:
            ...
            if w in self and pos in self[w]:
                ...
                if m is None:
                    a.append({"w": [w], "p": p, "s": s, "i": i, "n": 1, "x": self.labeler.get(w)})
                ...
                if w == "(!)":
                    a.append({"w": [w], "p": 0.0, "s": 1.0, "i": 1.0, "n": 1, "x": IRONY})
                ...
                if (
                    ...
                    for (_type, p), e in EMOTICONS.items():
                        if w in (e.lower() for e in e):
                            a.append({"w": [w], "p": p, "s": 1.0, "i": 1.0, "n": 1, "x": MOOD})
                            break
