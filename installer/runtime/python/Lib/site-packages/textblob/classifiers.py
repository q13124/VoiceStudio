...
def _get_document_tokens(document):
    if isinstance(document, basestring):
        ...
        tokens = {strip_punc(w, all=False) for w in document}
    return tokens
...
def basic_extractor(document, train_set):
    ...
    features = {f"contains({word})": (word in tokens) for word in word_features}
    return features
...
def contains_extractor(document):
    ...
    """
    tokens = _get_document_tokens(document)
    features = {f"contains({w})": True for w in tokens}
    return features
