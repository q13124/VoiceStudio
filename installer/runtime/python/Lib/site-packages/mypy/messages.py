...
class MessageBuilder:
    ...
    def unsupported_operand_types(
        ...
        *,
        ...
        """
        left_str = ""
        left_str = left_type if isinstance(left_type, str) else format_type(left_type, self.options)
    ...
    def readonly_keys_mutated(self, keys: set[str], context: Context) -> None:
        suffix = "is" if len(keys) == 1 else "are"
        self.fail(
    ...
    def deleted_as_rvalue(self, typ: DeletedType, context: Context) -> None:
        ...
        s = "" if typ.source is None else f' "{typ.source}"'
    ...
    def format_long_tuple_type(self, typ: TupleType) -> str:
        ...
        if item_cnt > MAX_TUPLE_ITEMS:
            return f'"tuple[{format_type_bare(typ.items[0], self.options)}, {format_type_bare(typ.items[1], self.options)}, ... <{item_cnt - 2!s} more items>]"'
        else:
...
def format_type_inner(
    ...
    if isinstance(typ, Instance):
    ...
    elif isinstance(typ, (TypeVarType, TypeVarTupleType)):
        ...
        type_name = "TypeForm" if typ.is_type_form else "type"
        ...
        if func.is_type_obj():
            ...
            if use_pretty_callable and any(
                not should_format_arg_as_type(kind, name, verbosity)
                for kind, name in zip(func.arg_kinds, func.arg_names, strict=False)
            ):
    ...
    if "." in fullname:
        for i in range(fullname.count(".")):
            ...
            try:
                ...
                pass
        raise AssertionError("Couldn't determine module from CallableType")
    return None
