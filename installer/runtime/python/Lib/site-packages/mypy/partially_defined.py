...
class BranchStatement:
    ...
    def is_defined_in_a_branch(self, name: str) -> bool:
        assert len(self.branches) > 0
        return any(name in b.must_be_defined or name in b.may_be_defined for b in self.branches)
...
class DefinedVariableTracker:
    ...
    def is_defined_in_different_branch(self, name: str) -> bool:
        ...
        if not stmt.is_undefined(name):
            return False
        return any(stmt.is_defined_in_a_branch(name) for stmt in self._scope().branch_stmts)
