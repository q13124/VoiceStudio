...
from typing import TYPE_CHECKING, Any, ClassVar, Final, NewType, TypeGuard, TypeVar, cast, overload
...
class CallableArgument(ProperType):
    ...
    def serialize(self) -> JsonDict:
        raise AssertionError("Synthetic types don't serialize")
...
class TypedDictType(ProperType):
    ...
    def copy_modified(
        ...
        *,
        ...
        items = self.items if item_types is None else dict(zip(self.items, item_types, strict=False))
...
class PlaceholderType(ProperType):
    ...
    def serialize(self) -> str:
        ...
        raise AssertionError(f"Internal error: unresolved placeholder type {self.fullname}")
...
class TypeStrVisitor(SyntheticTypeVisitor[str]):
    ...
    def visit_callable_type(self, t: CallableType, /) -> str:
        ...
        num_skip = 2 if param_spec is not None else 0
    ...
    def visit_type_type(self, t: TypeType, /) -> str:
        type_name = "TypeForm" if t.is_type_form else "type"
...
def extend_args_for_prefix_and_suffix(
    ...
    start = (item,) * (prefix - idx) if idx < prefix else ()
    end = (item,) * (suffix - len(types) + idx + 1) if len(types) - idx - 1 < suffix else ()
...
def flatten_nested_unions(
    ...
    typelist = list(types) if not isinstance(types, list) else cast("list[Type]", types)
...
def read_function_like(data: ReadBuffer, tag: Tag) -> FunctionLike:
    ...
    if tag == OVERLOADED:
        return Overloaded.read(data)
    raise AssertionError(f"Invalid type tag for FunctionLike {tag}")
...
def read_type_var_likes(data: ReadBuffer) -> list[TypeVarLikeType]:
    ...
    for _ in range(read_int_bare(data)):
        ...
        if tag == TYPE_VAR_TYPE:
            ...
            raise AssertionError(f"Invalid type tag for TypeVarLikeType {tag}")
    return ret
