...
class ExpressionChecker(ExpressionVisitor[Type], ExpressionCheckerSharedApi):
    ...
    def analyze_ref_expr(self, e: RefExpr, lvalue: bool = False) -> Type:
        ...
        if isinstance(node, Var):
            ...
            if isinstance(node, PlaceholderNode):
                raise AssertionError(f"PlaceholderNode {node.fullname!r} leaked to checker")
        ...
        if not (
            ...
            if not (callee.required_keys <= always_present_keys):
                self.msg.unexpected_typeddict_keys(
                    callee,
                    expected_keys=[
                        key for key in callee.items if key in callee.required_keys
                    ],
                    actual_keys=[
    ...
    def apply_inferred_arguments(
        ...
        for inferred_type, tv in zip(inferred_args, callee_type.variables, strict=False):
    ...
    def split_for_callable(
        ...
        for tv, arg in zip(t.variables, args, strict=False):
            if isinstance(tv, ParamSpecType) and not isinstance(
    ...
    def visit_super_expr(self, e: SuperExpr) -> Type:
        ...
        if type_info in mro:
            ...
            ) and e.info and type_info in e.info.mro:
                mro = e.info.mro
                index = mro.index(type_info)
        if index is None:
            if (
...
def has_bytes_component(typ: Type) -> bool:
    ...
    return bool(isinstance(typ, Instance) and typ.type.fullname in byte_types)
