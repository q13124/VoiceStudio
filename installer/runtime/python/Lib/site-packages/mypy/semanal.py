...
class SemanticAnalyzer(
    ...
    def process_static_or_class_method_in_overload(self, defn: OverloadedFuncDef) -> None:
        ...
        for item in defn.items:
            if isinstance(item, Decorator):
                ...
                raise AssertionError(f"The 'item' variable is an unexpected type: {type(item)}")
    ...
    def recalculate_metaclass(self, defn: ClassDef, declared_metaclass: Instance | None) -> None:
        ...
        if any(info.is_protocol for info in defn.info.mro) and (
    ...
    def report_missing_module_attribute(
        ...
        if module:
            if source_id in module.names and not module.names[source_id].module_public:
                message = (
    ...
    def visit_assignment_expr(self, s: AssignmentExpr) -> None:
        s.value.accept(self)
        if self.is_func_scope() and not self.check_valid_comprehension(s):
            return
    ...
    def analyze_lvalues(self, s: AssignmentStmt) -> None:
        ...
        if s.rvalue:
            has_explicit_value = not s.rvalue.no_rhs if isinstance(s.rvalue, TempNode) else True
        else:
    ...
    def unwrap_final(self, s: AssignmentStmt) -> bool:
        ...
        if self.type and self.type.is_protocol and self.is_class_scope():
    ...
    def check_type_alias_type_call(self, rvalue: Expression, *, name: str) -> TypeGuard[CallExpr]:
        ...
        if not self.check_typevarlike_name(rvalue, name, rvalue):
            return False
        return rvalue.arg_kinds.count(ARG_POS) == 2
    ...
    def visit_type_alias_stmt(self, s: TypeAliasStmt) -> None:
        ...
        try:
            ...
            tag = self.track_incomplete_refs()
            res, alias_tvars, depends_on, _empty_tuple_index = self.analyze_alias(
                s.name.name,
                s.value.expr(),
                allow_placeholder=True,
                declared_type_vars=type_params,
    ...
    def _trivial_typevarlike_like(self, tv: TypeVarLikeType) -> TypeVarLikeType:
        ...
        if isinstance(tv, TypeVarType):
            ...
            raise AssertionError(f"Unknown TypeVarLike: {tv!r}")
    ...
    def implicit_symbol(
        ...
        basename = None if sym.node is None else sym.node.fullname
        fullname = name if basename is None else basename + "." + ".".join(parts)
    ...
    def add_redefinition(self, names: SymbolTable, name: str, symbol: SymbolTableNode) -> None:
        ...
        while True:
            new_name = f"{name}-redefinition" if i == 1 else f"{name}-redefinition{i}"
    ...
    def current_symbol_table(
        ...
        if type_param and self.scope_stack[-1] == SCOPE_ANNOTATION:
            ...
            n = self.locals[-2] if self.scope_stack[-1] == SCOPE_ANNOTATION else self.locals[-1]
            assert n is not None
            if escape_comprehensions:
                ...
                for i, scope_type in enumerate(reversed(self.scope_stack)):
                    ...
                    raise AssertionError("Should have at least one non-comprehension scope")
            else:
                names = n
