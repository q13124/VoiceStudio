...
class TypeChecker(NodeVisitor[None], TypeCheckerSharedApi):
    ...
    def check_overlapping_overloads(self, defn: OverloadedFuncDef) -> None:
        ...
        if defn.impl:
            if isinstance(defn.impl, FuncDef):
                ...
                raise AssertionError("Impl isn't the right type")
    ...
    def check_func_item(
        ...
        if name == "__post_init__" and dataclasses_plugin.is_processed_dataclass(defn.info):
    ...
    def check_reverse_op_method(
        ...
        if isinstance(ret_type, AnyType):
            return
        if isinstance(ret_type, Instance) and ret_type.type.fullname == "builtins.object":
            return
    ...
    def check_method_override_for_base_with_name(
        ...
        # of type ignores).
        context = defn if not isinstance(defn, Decorator) else defn.func
        ...
        ) and (is_custom_settable_property(defn) or (is_custom_settable_property(original_node))):
    ...
    def get_op_other_domain(self, tp: FunctionLike) -> Type | None:
        if isinstance(tp, CallableType):
            ...
            raise AssertionError("Need to check all FunctionLike subtypes here")
    ...
    def visit_class_def(self, defn: ClassDef) -> None:
        ...
        with (
            ...
            old_binder = self.binder
            self.binder = ConditionalTypeBinder(self.options)
            with self.binder.top_frame_context(), self.scope.push_class(defn.info):
    ...
    def is_noop_for_reachability(self, s: Statement) -> bool:
        ...
        """
        if (isinstance(s, AssertStmt) and is_false_literal(s.expr)) or (isinstance(s, ReturnStmt) and is_literal_not_implemented(s.expr)) or isinstance(s, (RaiseStmt, PassStmt)):
            return True
        ...
        if isinstance(lvalue, (TupleExpr, ListExpr)):
            ...
            if isinstance(lvalue, NameExpr) and lvalue.node:
                ...
                if name in ("__setattr__", "__getattribute__", "__getattr__"):
                    ...
                    signature = lvalue_type or self.expr_checker.accept(rvalue)
                    if signature:
                        if name == "__setattr__":
    ...
    def node_type_from_base(
        ...
        *,
        ...
        instance = tuple_fallback(self_type) if isinstance(self_type, TupleType) else self_type
        ...
        if self.check_rvalue_count_in_assignment(
            ...
            if infer_lvalue_type:
                for lv in lvalues:
                    ...
                    elif isinstance(lv, StarExpr) and (
    ...
    def lvalue_type_for_inference(self, lvalues: list[Lvalue], rvalue_type: TupleType) -> Type:
        ...
        def append_types_for_inference(lvs: list[Expression], rv_types: list[Type]) -> None:
            for lv, rv_type in zip(lvs, rv_types, strict=False):
                sub_lvalue_type, _index_expr, _inferred = self.check_lvalue(lv)
        ...
        if star_lv:
            sub_lvalue_type, _index_expr, _inferred = self.check_lvalue(star_lv.expr)
            if sub_lvalue_type and not isinstance(sub_lvalue_type, PartialType):
    ...
    def infer_context_dependent(
        ...
        ) as msg, self.local_type_map as type_map:
            typ = get_proper_type(
                self.expr_checker.accept(
    ...
    def visit_decorator_inner(
        ...
        for d in reversed(e.decorators[1:] if skip_first_item else e.decorators):
            ...
            sig, _t2 = self.expr_checker.check_call(
    ...
    def intersect_instances(
        ...
        def _get_base_classes(instances_: tuple[Instance, Instance]) -> list[Instance]:
            ...
            for inst in instances_:
                expanded = inst.type.bases if inst.type.is_intersection else [inst]
        ...
        def _make_fake_typeinfo_and_full_name(
            ...
            _cdef, info_ = self.make_fake_typeinfo(curr_module_.fullname, name, name, base_classes_)
            return info_, name
    ...
    def find_type_equals_check(
        ...
        else_map = combine_maps(else_maps) if is_final else {}
    ...
    def find_isinstance_check_helper(
        ...
        if isinstance(node, CallExpr) and len(node.args) != 0:
            ...
            yes_type = true_only(yes_type) if yes_type is not None else UninhabitedType()
            no_type = false_only(no_type) if no_type is not None else UninhabitedType()
    ...
    def narrow_with_len(self, typ: Type, op: str, size: int) -> tuple[Type | None, Type | None]:
        ...
        if isinstance(typ, TupleType):
            ...
            yes_type = make_simplified_union(yes_types) if yes_types else None
            no_type = make_simplified_union(no_types) if no_types else None
            return yes_type, no_type
        else:
            raise AssertionError("Unsupported type for len narrowing")
    ...
    def is_writable_attribute(self, node: Node) -> bool:
        ...
        if isinstance(node, Var):
            return not (node.is_property and not node.is_settable_property)
...
def conditional_types(
    ...
    *,
    ...
    if isinstance(proper_type, UnionType):
        ...
        yes_types, _no_types = zip(*result, strict=False)
...
def builtin_item_type(tp: Type) -> Type | None:
    ...
    if isinstance(tp, Instance):
        ...
        raise AssertionError("No Mapping base class found for TypedDict fallback")
    return None
...
def is_more_general_arg_prefix(t: FunctionLike, s: FunctionLike) -> bool:
    ...
    elif isinstance(t, FunctionLike) and isinstance(s, FunctionLike):
...
def group_comparison_operands(
    ...
    for i, (operator, _left_expr, _right_expr) in enumerate(pairwise_comparisons):
    ...
    for operator, disjoint_dict in groups.items():
        for _keys, indices in disjoint_dict.items():
