...
FUNCITEM_FLAGS: Final = [*FUNCBASE_FLAGS, "is_overload", "is_generator", "is_coroutine", "is_async_generator", "is_awaitable_coroutine"]
...
FUNCDEF_FLAGS: Final = [*FUNCITEM_FLAGS, "is_decorated", "is_conditional", "is_trivial_body", "is_trivial_self", "is_mypy_only"]
...
class NameExpr(RefExpr):
    ...
    def serialize(self) -> JsonDict:
        raise AssertionError(f"Serializing NameExpr: {self}")
...
class ArgKind(Enum):
    ...
    def is_positional(self, star: bool = False) -> bool:
        return self in (ARG_POS, ARG_OPT) or (star and self == ARG_STAR)

    def is_named(self, star: bool = False) -> bool:
        return self in (ARG_NAMED, ARG_NAMED_OPT) or (star and self == ARG_STAR2)
...
class IndexExpr(Expression):
    ...
    __slots__ = ("analyzed", "as_type", "base", "index", "method_type")
...
class TypeInfo(SymbolNode):
    ...
    def get_method(self, name: str) -> FuncBase | Decorator | None:
        for cls in self.mro:
            if name in cls.names:
                ...
                if isinstance(node, (SYMBOL_FUNCBASE_TYPES, Decorator)):
                    return node
                else:
                    return None
        return None
    ...
    def read(cls, data: ReadBuffer) -> TypeInfo:
        ...
        ti.abstract_attributes = list(zip(attrs, statuses, strict=False))
...
class PlaceholderNode(SymbolNode):
    ...
    def serialize(self) -> JsonDict:
        raise AssertionError("PlaceholderNode can't be serialized")
...
def read_symbol(data: ReadBuffer) -> SymbolNode:
    ...
    raise AssertionError(f"Unknown symbol tag {tag}")
...
def read_overload_part(data: ReadBuffer, tag: Tag | None = None) -> OverloadPart:
    ...
    if tag == FUNC_DEF:
        return FuncDef.read(data)
    raise AssertionError(f"Invalid tag for an OverloadPart {tag}")
