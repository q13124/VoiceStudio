...
def constant_fold_binary_op(
    ...
    if (isinstance(left, float) and isinstance(right, float)) or (isinstance(left, float) and isinstance(right, int)) or (isinstance(left, int) and isinstance(right, float)):
        return constant_fold_binary_float_op(op, left, right)

    # String concatenation and multiplication.
    if op == "+" and isinstance(left, str) and isinstance(right, str):
        return left + right
    elif (op == "*" and isinstance(left, str) and isinstance(right, int)) or (op == "*" and isinstance(left, int) and isinstance(right, str)):
        return left * right
    ...
    if (op == "+" and isinstance(left, (int, float)) and isinstance(right, complex)) or (op == "+" and isinstance(left, complex) and isinstance(right, (int, float))):
        return left + right
    elif (op == "-" and isinstance(left, (int, float)) and isinstance(right, complex)) or (op == "-" and isinstance(left, complex) and isinstance(right, (int, float))):
        return left - right
...
def constant_fold_binary_float_op(op: str, left: int | float, right: int | float) -> float | None:
    ...
    if op == "+":
        ...
        if right != 0:
            return left % right
    elif op == "**" and ((left < 0 and isinstance(right, int)) or left > 0):
        try:
            ret = left**right
