    ...
    def yield_uselessness_error_infos(self) -> Iterator[tuple[str, Context, ErrorCode]]:
        ...
        for candidate in set(chain(*self.uselessness_errors)):
            if all(
                ...
                for errors, lines in zip(self.uselessness_errors, self.unreachable_lines, strict=False)
            ):
...
class Errors:
    ...
    def has_many_errors(self) -> bool:
        ...
        if len(self.error_info_map) >= self.options.many_errors_threshold:
            return True
        return sum(len(errors) for errors in self.error_info_map.values()) >= self.options.many_errors_threshold
    ...
    def format_messages(
        ...
        if self.options.pretty and self.read_source:
            ...
            source_lines = self.read_source(mapped_path) if mapped_path else self.read_source(path)
    ...
    def new_messages(self) -> list[str]:
        ...
        """
        msgs = []
        for path in self.error_info_map:
...
def report_internal_error(
    ...
    prefix = (f"{file}:{line}: " if line else f"{file}: ") if file else ""
