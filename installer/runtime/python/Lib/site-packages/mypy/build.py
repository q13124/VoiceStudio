...
from typing import TYPE_CHECKING, Any, ClassVar, Final, NoReturn, TextIO, TypedDict
...
def write_deps_cache(
    ...
    for id, st in graph.items():
        ...
        hash = st.source_hash or (st.meta.hash if st.meta else "")
...
def get_cache_names(id: str, path: str, options: Options) -> tuple[str, str, str | None]:
    ...
    """
    pair = options.cache_map.get(normpath(path, options)) if options.cache_map else None
    if pair is not None:
    ...
    if manager.options.fixed_format_cache:
        ...
        if not isinstance(meta, dict):
            manager.log(  # type: ignore[unreachable]
                f"Could not load cache for {id}: meta cache is not a dict: {meta!r}"
            )
            return None
    t1 = time.time()
...
def validate_meta(
    ...
    if not bazel and (mtime != meta.mtime or path != meta.path):
        ...
        try:
            ...
            source_hash = "" if stat.S_ISDIR(st.st_mode) else manager.fscache.hash_digest(path)
        ...
        if source_hash != meta.hash:
            ...
            manager.log(
                f"Updating mtime for {id}: file {path}, meta {meta_file}, mtime {meta.mtime}"
            )
...
class State:
    ...
    def __init__(
        ...
        if self.meta:
            ...
            self.priorities = dict(zip(all_deps, self.meta.dep_prios, strict=False))
            assert len(all_deps) == len(self.meta.dep_lines)
            self.dep_line_map = dict(zip(all_deps, self.meta.dep_lines, strict=False))
            assert len(self.meta.dep_hashes) == len(self.meta.dependencies)
            self.dep_hashes = dict(zip(self.meta.dependencies, self.meta.dep_hashes, strict=False))
