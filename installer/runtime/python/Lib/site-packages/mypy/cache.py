...
def read_literal(data: ReadBuffer, tag: Tag) -> int | str | bool | float:
    ...
    raise AssertionError(f"Unknown literal tag {tag}")
...
JsonValue: _TypeAlias = Value | list["JsonValue"] | dict[str, "JsonValue"] | tuple["JsonValue", ...]
...
def read_json_value(data: ReadBuffer) -> JsonValue:
    ...
    raise AssertionError(f"Invalid JSON tag: {tag}")
...
def write_json_value(data: WriteBuffer, value: JsonValue) -> None:
    if value is None:
        ...
        raise AssertionError(f"Invalid JSON value: {value}")
