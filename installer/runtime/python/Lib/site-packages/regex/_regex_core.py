...
class RegexFlag(enum.IntFlag):
    ...
    def __repr__(self):
        ...
        for m in self.__class__:
            if value & m._value_:
                value &= ~m._value_
                members.append(f'regex.{m._name_}')
...
def is_actually_fuzzy(constraints):
    ...
    return (constraints.get("s"), constraints.get("i"), constraints.get("d")) != ((0, 0), (0, 0), (0, 0))
...
def parse_conditional(source, info):
    ...
    try:
        ...
        no_branch = parse_sequence(source, info) if source.match("|") else Sequence()
...
def parse_call_group(source, info, ch, pos):
    ...
    group = "0" if ch == "R" else ch + source.get_while(DIGITS)
...
def parse_escape(source, info, in_set):
    ...
    if ch in HEX_ESCAPES:
        ...
        raise error(f"bad escape \\{ch}", source.string, source.pos)
    elif ch in DIGITS:
...
def parse_numeric_escape(source, info, ch, in_set):
    ...
    if ch in DIGITS:
        ...
        if is_octal(digits) and ch in OCT_DIGITS:
            ...
            octal_mask = 255 if encoding in (ASCII, LOCALE) else 511
...
def parse_octal_escape(source, info, digits, in_set):
    ...
    try:
        ...
        if digits[0] in OCT_DIGITS:
            raise error("incomplete escape \\{}".format(''.join(digits)),
              source.string, source.pos)
        else:
            raise error(f"bad escape \\{digits[0]}", source.string,
              source.pos)
...
_POSIX_CLASSES = {'ALNUM', 'DIGIT', 'PUNCT', 'XDIGIT'}

_BINARY_VALUES = {'YES', 'Y', 'NO', 'N', 'TRUE', 'T', 'FALSE', 'F'}
...
def _compile_replacement(source, pattern, is_unicode):
    ...
    octal_mask = 255 if isinstance(source.sep, bytes) else 511
...
class RegexBase:
    ...
    def with_flags(self, positive=None, case_flags=None, zerowidth=None):
        positive = self.positive if positive is None else bool(positive)
        if case_flags is None:
            case_flags = self.case_flags
        else:
            case_flags = CASE_FLAGS_COMBINATIONS[case_flags & CASE_FLAGS]
        zerowidth = self.zerowidth if zerowidth is None else bool(zerowidth)
...
class Branch(RegexBase):
    ...
    def _can_split(items, count):
        ...
        return not (Branch._is_full_case(items, count - 2) and Branch._is_folded(items[count - 2:count + 1]))
    ...
    def _can_split_rev(items, count):
        ...
        # Check whether a 2-1 split would be OK.
        return not (Branch._is_full_case(items, end - count - 2) and Branch._is_folded(items[end - count - 2:end - count + 1]))
    ...
    def _is_folded(items):
        ...
        return any(folded == _regex.fold_case(FULL_CASE_FOLDING, c) for c in expanding_chars)
...
class GreedyRepeat(RegexBase):
    ...
    def dump(self, indent, reverse):
        limit = "INF" if self.max_count is None else self.max_count
...
class Sequence(RegexBase):
    ...
    def _flush_characters(info, characters, case_flags, items):
        ...
        if case_flags & IGNORECASE and not any(is_cased_i(info, c) for c in characters):
            case_flags = NOCASE
...
class Info:
    ...
    def __init__(self, flags=0, char_type=None, kwargs=None):
        if kwargs is None:
            kwargs = {}
    ...
    def is_open_group(self, name):
        ...
        group = int(name) if name.isdigit() else self.group_index.get(name)
