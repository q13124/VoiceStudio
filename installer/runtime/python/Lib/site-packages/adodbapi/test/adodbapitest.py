...
class CommonDBTests(unittest.TestCase):
    ...
    def testErrorHandlerInherits(self):
        ...
        def mycallable(connection, cursor, errorclass, errorvalue):
            return 1
    ...
    def testOwnErrorHandlerConnection(self):
        def mycallable(connection, cursor, errorclass, errorvalue):
            return (1)  # does not raise anything
        conn = self.getConnection()
        conn.errorhandler = mycallable
        conn.close()
        conn.commit()  # Should not be able to use connection after it is closed
    ...
    def testUserDefinedConversions(self):
        try:
            def duplicatingConverter(aStringField):
                return aStringField * 2
            ...
            crsr = conn.cursor()
            tabdef = (
                f"CREATE TABLE xx_{config.tmp} (fldData VARCHAR(100) NOT NULL, fld2 VARCHAR(20))"
            )
            crsr.execute(tabdef)
            crsr.execute(
            ...
            def upcaseConverter(aStringField):
                return aStringField.upper()
    ...
    def helpTestDataType(
        ...
        for inParam in inputs:
            ...
            try:
                ...
                raise
            crsr.execute(
                f"SELECT fldData FROM xx_{config.tmp} WHERE ?=fldID", [fldId]
            )
            rs = crsr.fetchone()
...
class TestADOwithSQLServer(CommonDBTests):
    ...
    def testMultipleSetReturn(self):
        ...
        crsr.callproc("sp_DeleteMe_OnlyForTesting")
        row = crsr.fetchone()
