...
def _format_fallback_interval(
    ...
    if skeleton in locale.datetime_skeletons:  # Use the given skeleton
        def format(dt):
            return format_skeleton(skeleton, dt, tzinfo, locale=locale)
    elif all(
        # Both are just dates
        ...
        def format(dt):
            return format_time(dt, tzinfo=tzinfo, locale=locale)
    else:
        def format(dt):
            return format_datetime(dt, tzinfo=tzinfo, locale=locale)
...
def format_interval(
    ...
    if skeleton not in interval_formats or not skeleton:
        ...
        skeleton = match_skeleton(skeleton, interval_formats) if skeleton and fuzzy else None
    ...
    for field in PATTERN_CHAR_ORDER:  # These are in largest-to-smallest order
        if field in skel_formats and start_fmt.extract(field) != end_fmt.extract(field):
            ...
            return "".join(
                ...
                for pattern, instant in zip(skel_formats[field], (start, end), strict=False)
            )
    ...
    for tok_type, tok_value in tokens:
        if tok_type == "field":
            ...
            if not any(ch in PATTERN_CHARS for ch in tok_value):  # No need to quote
                ...
                output.append("'{}'".format(tok_value.replace("'", "''")))
    return "".join(output)
