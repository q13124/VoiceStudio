...
from websockets.datastructures import Headers, HeadersLike, MultipleValuesError
...
from websockets.extensions import Extension, ServerExtensionFactory
...
from websockets.http11 import SERVER
from websockets.protocol import State
from websockets.typing import ExtensionHeader, LoggerLike, Origin, StatusLike, Subprotocol
...
class WebSocketServerProtocol(WebSocketCommonProtocol):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        if origins is not None and "" in origins:
            warnings.warn("use None instead of '' in origins", DeprecationWarning, stacklevel=2)
        ...
        * to authenticate the request and return an HTTP 401 Unauthorized or an
        ...
        if self._process_request is not None:
            ...
            if isinstance(response, Awaitable):
                ...
                warnings.warn(
                    "declare process_request as a coroutine", DeprecationWarning, stacklevel=2
                )
                return response
        return None
    ...
    def process_extensions(
        ...
        if header_values and available_extensions:
            parsed_header_values: list[ExtensionHeader] = functools.reduce(operator.iadd, [parse_extension(header_value) for header_value in header_values], [])
    ...
    def process_subprotocol(
        ...
        if header_values and available_subprotocols:
            parsed_header_values: list[Subprotocol] = functools.reduce(operator.iadd, [parse_subprotocol(header_value) for header_value in header_values], [])
    ...
    async def handshake(
        ...
        if isinstance(early_response_awaitable, Awaitable):
            ...
            # For backwards compatibility with 7.0.
            warnings.warn("declare process_request as a coroutine", DeprecationWarning, stacklevel=2)
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        if timeout is None:
            ...
            warnings.warn("rename timeout to close_timeout", DeprecationWarning, stacklevel=2)
        ...
        if klass is None:
            ...
            warnings.warn("rename klass to create_protocol", DeprecationWarning, stacklevel=2)
...
def remove_path_argument(
    ...
    try:
        ...
        try:
            ...
            warnings.warn("remove second argument of ws_handler", DeprecationWarning, stacklevel=2)
