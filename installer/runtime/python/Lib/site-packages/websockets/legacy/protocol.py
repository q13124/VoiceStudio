...
from websockets.extensions import Extension
...
from websockets.typing import BytesLike, Data, DataLike, LoggerLike, Subprotocol
...
class WebSocketCommonProtocol(asyncio.Protocol):
    ...
    * on the server side, when the connection handler terminates.
    ...
    def __init__(
        ...
        *,
        ...
        if legacy_recv:  # pragma: no cover
            warnings.warn("legacy_recv is deprecated", DeprecationWarning, stacklevel=2)
        ...
        self.messages: collections.deque[Data] = collections.deque()
    ...
    def host(self) -> str | None:
        ...
        warnings.warn(f"use {alternative}[0] instead of host", DeprecationWarning, stacklevel=2)
        return self._host

    @property
    def port(self) -> int | None:
        alternative = "remote_address" if self.is_client else "local_address"
        warnings.warn(f"use {alternative}[1] instead of port", DeprecationWarning, stacklevel=2)
        return self._port

    @property
    def secure(self) -> bool | None:
        warnings.warn("don't use secure", DeprecationWarning, stacklevel=2)
        return self._secure
