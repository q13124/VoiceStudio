...
class ClientProtocol(Protocol):
    ...
    def process_response(self, response: Response) -> None:
        ...
        connection: list[ConnectionOption] = functools.reduce(operator.iadd, [parse_connection(value) for value in headers.get_all("Connection")], [])
        if not any(value.lower() == "upgrade" for value in connection):
            raise InvalidUpgrade(
                "Connection", ", ".join(connection) if connection else None
            )

        upgrade: list[UpgradeProtocol] = functools.reduce(operator.iadd, [parse_upgrade(value) for value in headers.get_all("Upgrade")], [])
    ...
    def process_extensions(self, headers: Headers) -> list[Extension]:
        ...
        if extensions:
            ...
            parsed_extensions: list[ExtensionHeader] = functools.reduce(operator.iadd, [parse_extension(header_value) for header_value in extensions], [])
    ...
    def process_subprotocol(self, headers: Headers) -> Subprotocol | None:
        ...
        if subprotocols:
            ...
            parsed_subprotocols: Sequence[Subprotocol] = functools.reduce(operator.iadd, [parse_subprotocol(header_value) for header_value in subprotocols], [])
