...
class TableauProver(Prover):
    ...
    def _attempt_proof_all(
        ...
        if accessible_vars:
            ...
            if bv_available:
                variable_to_use = next(iter(bv_available))
    ...
    def is_atom(e):
        ...
        return bool(isinstance(e, (AbstractVariableExpression, LambdaExpression)))
    ...
    def line(self, data, indent=0):
        if isinstance(data, tuple):
            ...
            if isinstance(ex, AllExpression):
                try:
                    used_vars = "[{}]".format(",".join(f"{ve.variable.name}" for ve in ex._used_vars))
                    data += f":   {used_vars}"
                except AttributeError:
                    data += ":   []"
...
def tableau_test(c, ps=None, verbose=False):
    ...
    if not ps:
        ps = []
    print(
        "{} |- {}: {}".format(", ".join(ps), pc, TableauProver().prove(pc, pps, verbose=verbose))
    )
