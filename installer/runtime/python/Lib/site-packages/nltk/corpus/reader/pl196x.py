...
class Pl196xCorpusReader(CategorizedCorpusReader, XMLCorpusReader):
    ...
    def __init__(self, *args, **kwargs):
        self._textids = kwargs.get("textid_file")
    ...
    def _resolve(self, fileids, categories, textids=None):
        ...
        if textids is not None:
            if isinstance(textids, str):
                textids = [textids]
            files = functools.reduce(operator.iadd, (self._t2f[t] for t in textids), [])
            tdict = {}
            for f in files:
    ...
    def textids(self, fileids=None, categories=None):
        ...
        if isinstance(fileids, str):
            fileids = [fileids]
        return sorted(functools.reduce(operator.iadd, (self._f2t[d] for d in fileids), []))
