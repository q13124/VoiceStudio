...
def _pretty_fulltext_sentence(sent):
    ...
    outstr = ""
    outstr += "full-text sentence ({.ID}) in {}:\n\n".format(
    ...
    for o, (j, k, fname, asetIndex) in enumerate(overt):
        if o > 0 and j <= overt[o - 1][1]:
            ...
            if (
                ...
                return s
    for o in sorted(duplicates, reverse=True):
        del overt[o]
...
warnings(True) to display corpus consistency warnings when loading data
    ...
    def exemplars(self, luNamePattern=None, frame=None, fe=None, fe2=None):
            ...
            if fe is not None:  # narrow to frames that define this FE
                if isinstance(fe, str):
                    frames = PrettyLazyIteratorList(
                        ...
                        for f in frames
                        if fe in f.FE
                        or any(re.search(fe, ffe, re.I) for ffe in f.FE)
                    )
                else:
                ...
                if fe2 is not None:  # narrow to frames that ALSO define this FE
                    if isinstance(fe2, str):
                        frames = PrettyLazyIteratorList(
                            ...
                            for f in frames
                            if fe2 in f.FE
                            or any(re.search(fe2, ffe, re.I) for ffe in f.FE)
                        )
        ...
        def _matching_exs():
            for f in frames:
                ...
                if fe is not None:
                    fes = (
                        {ffe for ffe in f.FE if re.search(fe, ffe, re.I)}
                        if isinstance(fe, str)
                        else {fe.name}
                    )
                    if fe2 is not None:
                        fes2 = (
                            {ffe for ffe in f.FE if re.search(fe2, ffe, re.I)}
                            if isinstance(fe2, str)
                            else {fe2.name}
                        )
    ...
    def _exemplar_of_fes(self, ex, fes=None):
        ...
        """
        overtNames = set(list(zip(*ex.FE[0], strict=False))[2]) if ex.FE[0] else set()
        if "FE2" in ex:
            overtNames |= set(list(zip(*ex.FE2[0], strict=False))[2]) if ex.FE2[0] else set()
            if "FE3" in ex:
                overtNames |= set(list(zip(*ex.FE3[0], strict=False))[2]) if ex.FE3[0] else set()
    ...
    def frame_relations(self, frame=None, frame2=None, type=None):
        ...
        if relation_type is not None and not isinstance(relation_type, dict):
            type = next(rt for rt in self.frame_relation_types() if rt.name == type)
        ...
        if frame2 is not None:
            ...
            if not isinstance(frame2, int):
                frame2 = frame2.ID if isinstance(frame2, dict) else self.frame_by_name(frame2).ID
            if frame == frame2:
                raise FramenetError(
    ...
    def _handle_fecoreset_elt(self, elt):
        ...
        self._load_xml_attributes(AttrDict(), elt)
        tmp = []
        for sub in elt:
...
def demo():
    ...
    #
    firstcorp = next(iter(allcorpora))
