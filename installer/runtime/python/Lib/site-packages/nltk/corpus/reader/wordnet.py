...
class Synset(_WordNetObject):
    ...
    def _needs_root(self):
        return not (self._pos == NOUN and self._wordnet_corpus_reader.get_version() != "1.6")
 ...
 Synset('physical_entity.n.01'), Synset('entity.n.01')]
    ...
    def lch_similarity(self, other, verbose=False, simulate_root=True):
        ...
        if self._pos != other._pos:
            raise WordNetError(
                ...
                f"{self} and {other} to have the same part of speech."
            )
    ...
    def res_similarity(self, other, ic, verbose=False):
        ...
        _ic1, _ic2, lcs_ic = _lcs_ic(self, other, ic)
        return lcs_ic
...
class WordNetCorpusReader(CorpusReader):
    ...
    def __init__(self, root, omw_reader):
        ...
        if self._omw_reader is None:
            warnings.warn(
                "The multilingual functions are not available with this Wordnet version", stacklevel=2
            )
    ...
    def lemma_from_key(self, key):
        ...
        _lemma_name, lex_sense = key.split("%")
        pos_number, _lexname_index, _lex_id, _, _ = lex_sense.split(":")
        ...
        if not synset_line:
            raise WordNetError(f"No synset found for key {key!r}")
        ...
        for lemma in synset._lemmas:
            if lemma._key == key:
                return lemma
        raise WordNetError(f"No lemma found for for key {key!r}")
    ...
    def synset_from_pos_and_offset(self, pos, offset):
        ...
        if (
            ...
            synset = None
            warnings.warn(f"No WordNet synset found for pos={pos} at offset={offset}.", stacklevel=2)
        data_file.seek(0)
        return synset
    ...
    def synonyms(self, word, lang="eng"):
        ...
        return [
            sorted(set(ss.lemma_names(lang=lang)) - {word})
    ...
    def tag2pos(self, tag, tagset="en-ptb"):
        ...
        return UNIVERSAL_TAG_TO_WN_POS.get(tag)
                ...
                if self.map30:
                    if offset_pos in self.map30:
                        ...
                        if (
                            ...
                            warnings.warn(
                                f"{lang}: invalid offset {offset_pos} in '{line}'", stacklevel=2
                            )
                        continue
                elif offset_pos[-1] == "a":
    ...
    def digraph(
        ...
        dot_string = edges2dot(sorted(edges), shapes=shapes, attr=attr)
        return dot_string
...
def _lcs_ic(synset1, synset2, ic, verbose=False):
    ...
    if synset1._pos != synset2._pos:
        raise WordNetError(
            ...
            f"{synset1} and {synset2} to have the same part of speech."
        )

    ic1 = information_content(synset1, ic)
    ic2 = information_content(synset2, ic)
    subsumers = synset1.common_hypernyms(synset2)
    subsumer_ic = 0 if len(subsumers) == 0 else max(information_content(s, ic) for s in subsumers)
