...
class StreamBackedCorpusView(AbstractLazySequence):
    ...
    def __len__(self):
        if self._len is None:
            ...
            # of the file:
            for _tok in self.iterate_from(self._toknum[-1]):
                pass
        return self._len
...
class ConcatenatedCorpusView(AbstractLazySequence):
    ...
    def __len__(self):
        if len(self._offsets) <= len(self._pieces):
            ...
            for _tok in self.iterate_from(self._offsets[-1]):
                pass
...
def concat(docs):
    ...
    # No method found!
    raise ValueError(f"Don't know how to concatenate types: {types!r}")
...
def read_whitespace_block(stream):
    toks = []
    for _i in range(20):  # Read 20 lines at a time.
...
def read_sexpr_block(stream, block_size=16384, comment_char=None):
    ...
    if encoding not in (None, "utf-8"):
        ...
        warnings.warn(
            ...
            f"of the {encoding} encoding!", stacklevel=2
        )
...
def find_corpus_fileids(root, regexp):
    ...
    # or symlinked) subdirectories, and match paths against the regexp.
        ...
        raise AssertionError(f"Don't know how to handle {root!r}")
