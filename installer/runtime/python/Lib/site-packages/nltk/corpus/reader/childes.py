...
class CHILDESCorpusReader(XMLCorpusReader):
    ...
    def _get_words(
        ...
        results = []
        for xmlsent in xmldoc.findall(f".//{{{NS}}}u"):
            sents = []
            # select speakers
            if speaker == "ALL" or xmlsent.get("who") in speaker:
                for xmlword in xmlsent.findall(f".//{{{NS}}}w"):
                    suffixStem = None
                    suffixTag = None
                    ...
                    if relation or stem:
                        try:
                            xmlstem = xmlword.find(f".//{{{NS}}}stem")
                            word = xmlstem.text
                        ...
                        try:
                            xmlsuffix = xmlword.find(
                                f".//{{{NS}}}mor/{{{NS}}}mor-post/{{{NS}}}mw/{{{NS}}}stem"
                            )
                    ...
                    if relation or pos:
                        try:
                            xmlpos = xmlword.findall(f".//{{{NS}}}c")
                            xmlpos2 = xmlword.findall(f".//{{{NS}}}s")
                            if xmlpos2 != []:
                        ...
                        try:
                            xmlsuffixpos = xmlword.findall(
                                f".//{{{NS}}}mor/{{{NS}}}mor-post/{{{NS}}}mw/{{{NS}}}pos/{{{NS}}}c"
                            )
                            xmlsuffixpos2 = xmlword.findall(
                                f".//{{{NS}}}mor/{{{NS}}}mor-post/{{{NS}}}mw/{{{NS}}}pos/{{{NS}}}s"
                            )
                            if xmlsuffixpos2:
                                suffixTag = (
