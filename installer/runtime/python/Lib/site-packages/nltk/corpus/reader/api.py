...
class CorpusReader:
    ...
    def __init__(self, root, fileids, encoding="utf8", tagset=None):
        ...
        if isinstance(root, str) and not isinstance(root, PathPointer):
            ...
            root = ZipFilePathPointer(zipfile, zipentry) if zipfile else FileSystemPathPointer(root)
    ...
    def abspaths(self, fileids=None, include_encoding=False, include_fileid=False):
        ...
        if include_encoding and include_fileid:
            return list(zip(paths, [self.encoding(f) for f in fileids], fileids, strict=False))
        elif include_fileid:
            return list(zip(paths, fileids, strict=False))
        elif include_encoding:
            return list(zip(paths, [self.encoding(f) for f in fileids], strict=False))
        else:
            return paths
            ...
            with self.open(self._file) as f:
                for line in f.readlines():
                    ...
                    if file_id not in self.fileids():
                        raise ValueError(
                            f"In category mapping file {self._file}: {file_id} "
                            "not found"
                        )
