...
class ConllCorpusReader(CorpusReader):
    ...
    def __init__(
        ...
        for columntype in columntypes:
            if columntype not in self.COLUMN_TYPES:
                raise ValueError(f"Bad column type {columntype!r}")
        ...
        for block in read_blankline_block(stream):
            ...
            for row in grid:
                if len(row) != len(grid[0]):
                    raise ValueError(f"Inconsistent number of columns:\n{block}")
            grids.append(grid)
        return grids
    ...
    def _get_tagged_words(self, grid, tagset=None):
        ...
        return list(zip(self._get_column(grid, self._colmap["words"]), pos_tags, strict=False))
    ...
    def _get_iob_words(self, grid, tagset=None):
        ...
        return list(
            zip(
                ...
                pos_tags,
                self._get_column(grid, self._colmap["chunk"]), strict=False,
            )
        )
    ...
    def _get_chunked_words(self, grid, chunk_types, tagset=None):
        ...
        for word, pos_tag, chunk_tag in zip(words, pos_tags, chunk_tags, strict=False):
            if chunk_tag == "O":
                state, chunk_type = "O", ""
            else:
    ...
    def _get_parsed_sent(self, grid, pos_in_tree, tagset=None):
        ...
        treestr = ""
        for word, pos_tag, parse_tag in zip(words, pos_tags, parse_tags, strict=False):
            if word == "(":
                word = "-LRB-"
            if word == ")":
                word = "-RRB-"
    ...
    def _get_srl_instances(self, grid, pos_in_tree):
        ...
        for wordnum, predicate in enumerate(predicates):
            ...
            for spanlist in spanlists:
                ...
                raise ValueError(f"No srl column found for {predicate!r}")
            instances.append(
    ...
    def _require(self, *columntypes):
        for columntype in columntypes:
            if columntype not in self._colmap:
                raise ValueError(
                    f"This corpus does not contain a {columntype} " "column."
                )
