...
class SwitchboardTurn(list):
    ...
    def __repr__(self):
        if len(self) == 0:
            ...
            text = " ".join("{}/{}".format(*w) for w in self)
        else:
            text = " ".join(self)
...
class SwitchboardCorpusReader(CorpusReader):
    ...
    def _words_block_reader(self, stream):
        return functools.reduce(operator.iadd, self._discourses_block_reader(stream)[0], [])

    def _tagged_words_block_reader(self, stream, tagset=None):
        return functools.reduce(operator.iadd, self._tagged_discourses_block_reader(stream, tagset)[0], [])
    ...
    def _parse_utterance(self, utterance, include_tag, tagset=None):
        ...
        if m is None:
            raise ValueError(f"Bad utterance {utterance!r}")
