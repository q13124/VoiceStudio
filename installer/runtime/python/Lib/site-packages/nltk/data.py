...
class FileSystemPathPointer(PathPointer, str):
    ...
    def __init__(self, _path):
        ...
        if not os.path.exists(_path):
            raise OSError(f"No such file or directory: {_path!r}")
        self._path = _path
    ...
    def __repr__(self):
        return f"FileSystemPathPointer({self._path!r})"
...
def load(
    ...
    if format == "auto":
        ...
        if format is None:
            raise ValueError(
                f"Could not determine format for {resource_url} based "
    ...
    _protocol, path_ = split_resource_url(resource_url)
    ...
    if format == "raw":
        ...
        if format == "text":
            ...
            raise AssertionError(
                f"Internal NLTK error: Format {format} isn't "
                "handled by nltk.data.load()"
            )
...
class OpenOnDemandZipFile(zipfile.ZipFile):
    ...
    def __repr__(self):
        return repr(f"OpenOnDemandZipFile({self.filename!r})")
