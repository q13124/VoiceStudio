...
def accuracy(reference, test):
    ...
    return sum(x == y for x, y in zip(reference, test, strict=False)) / len(test)
...
def log_likelihood(reference, test):
    ...
    total_likelihood = sum(dist.logprob(val) for (val, dist) in zip(reference, test, strict=False))
...
def approxrand(a, b, **kwargs):
    ...
    if verbose:
        print(f"actual statistic: {actual_stat:f}")
        print("-" * 60)
    ...
    for i in range(shuffles):
        ...
        if verbose and i % 10 == 0:
            print(f"pseudo-statistic: {pseudo_stat:f}")
    ...
    if verbose:
        print(f"significance: {significance:f}")
        if betai:
