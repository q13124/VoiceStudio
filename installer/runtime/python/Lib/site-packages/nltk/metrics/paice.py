...
class Paice:
    ...
    def __str__(self):
        text = [f"Global Unachieved Merge Total (GUMT): {self.gumt}\n"]
        text.append(f"Global Desired Merge Total (GDMT): {self.gdmt}\n")
        text.append(f"Global Wrongly-Merged Total (GWMT): {self.gwmt}\n")
        text.append(f"Global Desired Non-merge Total (GDNT): {self.gdnt}\n")
        text.append(f"Understemming Index (GUMT / GDMT): {self.ui}\n")
        text.append(f"Overstemming Index (GWMT / GDNT): {self.oi}\n")
        text.append(f"Stemming Weight (OI / UI): {self.sw}\n")
        text.append(f"Error-Rate Relative to Truncation (ERRT): {self.errt}\r\n")
        coordinates = " ".join(["({}, {})".format(*item) for item in self.coords])
        text.append(f"Truncation line: {coordinates}")
        return "".join(text)
