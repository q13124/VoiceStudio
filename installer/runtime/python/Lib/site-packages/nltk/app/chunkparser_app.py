...
class RegexpChunkApp:
    ...
    _STATUS_PARAMS = {"background": "#9bb", "relief": "groove", "border": 2}
    _FONT_PARAMS = {"family": "helvetica", "size": -20}
    _FRAME_PARAMS = {"background": "#777", "padx": 2, "pady": 2, "border": 3}
    _EVALBOX_PARAMS = {
        "background": "#eef",
        "highlightbackground": "#eef",
        "highlightthickness": 1,
    ...
    def _eval_plot(self, *e, **config):
        ...
        if self._autoscale.get() and len(self._history) > 1:
            ...
            for i in range(1, min(len(self._history), self._SCALE_N + 1)):
                _grammar, precision, recall, _fmeasure = self._history[-i]
    ...
    def _init_widgets(self, top):
        ...
        for i, (tab, _tabstops, _text) in enumerate(self.HELP):
    ...
    def show_trace(self, *e):
        ...
        tagseq = "\t"
        charnum = [1]
        for _wordnum, (_word, pos) in enumerate(gold_tree.leaves()):
            tagseq += f"{pos} "
        ...
        for i in range(len(rules) + 1):
            if i == 0:
                ...
                self.devsetbox.insert("end", f"Apply {rules[i - 1]}:\n")
                self.devsetbox.tag_add("trace", "end -2c linestart", "end -2c")
    ...
    def save_history(self, filename=None):
        ...
        with open(filename, "w") as outfile:
            ...
            outfile.write(f"# Saved {time.ctime()}\n")
            outfile.write(f"# Development set: {self.devset_name}\n")
