...
def wnb(port=8000, runBrowser=True, logfilename=None):
    ...
    if logfile:
        logfile.write(f"NLTK Wordnet browser server running serving: {url}\n")
    if runBrowser:
        server_ready.set()
...
def _collect_one_synset(word, synset, synset_relations):
    ...
    assert pos_tuple is not None, f"pos_tuple is null: synset.pos(): {synset.pos()}"
    descr = pos_tuple[2]
    ...
    gl = " ({}) <i>{}</i> ".format(
        synset.definition(),
        "; ".join(f'"{e}"' for e in synset.examples()),
    )
    return s + gl + _synset_relations(word, synset, synset_relations) + "</li>\n"


def _collect_all_synsets(word, pos, synset_relations=None):
    """
...
def page_from_reference(href):
    ...
    body = ""
    for pos, _pos_str, name in _pos_tuples():
        if pos in pos_forms:
            body += _hlev(3, name) + "\n"
...
"""
    shutdown_link = '<a href="SHUTDOWN THE SERVER">Shutdown</a>' if with_shutdown else ""
