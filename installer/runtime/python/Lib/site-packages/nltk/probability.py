...
class FreqDist(Counter):
    ...
    def pformat(self, maxlen=10):
        ...
        if len(self) > maxlen:
            items.append("...")
        return "FreqDist({{{}}})".format(", ".join(items))
...
class ProbDistI(metaclass=ABCMeta):
    ...
    def generate(self):
        ...
        if self.SUM_TO_ONE:
            warnings.warn(
                f"Probability distribution {self!r} sums to {p_init - p!r}; generate()"
                " is returning an arbitrary sample.", stacklevel=2
            )
    ...
    *gamma* to the count for each bin, and taking the maximum
    ...
    def __init__(self, freqdist, gamma, bins=None):
        ...
        if (bins is not None) and (bins < freqdist.B()):
            name = self.__class__.__name__[:-8]
            raise ValueError(
                f"\nThe number of bins in a {name} distribution "
...
class CrossValidationProbDist(ProbDistI):
    ...
    def samples(self):
        ...
        return set(reduce(operator.iadd, (list(fd) for fd in self._freqdists), []))
...
class SimpleGoodTuringProbDist(ProbDistI):
    ...
    def _r_Nr(self):
        ...
        if not nonzero:
            return [], []
        return zip(*sorted(nonzero.items()), strict=False)
    ...
    def find_best_fit(self, r, nr):
        ...
        for x, y in zip(log_r, log_zr, strict=False):
    ...
    def _renormalize(self, r, nr):
        ...
        """
        prob_cov = 0.0
        for r_, nr_ in zip(r, nr, strict=False):
...
class ConditionalFreqDist(defaultdict):
    ...
    def plot(
        ...
        **kwargs,
        ...
        conditions = self.conditions() if not conditions else [c for c in conditions if c in self]
        if not samples:
...
def demo(numsamples=6, numoutcomes=500):
    ...
    zvals = list(zip(*vals, strict=False))
