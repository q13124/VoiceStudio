...
class Downloader:
    ...
    def _download_package(self, info, download_dir, force):
        ...
        try:
            ...
            yield ErrorMessage(
                info,
                f"Error downloading {info.id!r} from <{info.url}>:" f"\n  {e}",
            )
            return
            ...
            for msg in self.incr_download(info_or_id, download_dir, force):
                ...
                if not quiet:
                    ...
                    if isinstance(msg, StartCollectionMessage):
                        show(f"Downloading collection {msg.collection.id!r}")
                        prefix += "   | "
                        print_to(prefix)
                        ...
                        if self._errors:
                            show(
                                f"Downloaded collection {msg.collection.id!r} with errors"
                            )
                        else:
                    ...
                    # Package downloading messages:
                        ...
                        show(
                            f"Downloading package {msg.package.id} to {download_dir}..."
                        )
    ...
    def status(self, info_or_id, download_dir=None):
        ...
        if isinstance(info, Collection):
            ...
            if self.STALE in pkg_status:
                return self.STALE
            elif self.PARTIAL in pkg_status or (self.INSTALLED in pkg_status and self.NOT_INSTALLED in pkg_status):
                return self.PARTIAL
    ...
    def info(self, id):
        ...
        raise ValueError(f"Package {id!r} not found in index")
        ...
        while True:
            ...
            try:
                if command == "l":
                    ...
                    print(f"Command {user_input!r} unrecognized")
        ...
        while True:
            ...
            if user_input == "s":
                ...
                if new_dl_dir in ("", "x", "q", "X", "Q"):
                    ...
                    print(f"Directory {new_dl_dir!r} not found!  Create it first.")
            elif user_input == "u":
...
class DownloaderGUI:
    ...
    def _info_edit(self, info_key):
        ...
        (entry, _callback) = self._info[info_key]
        entry["state"] = "normal"
        entry["relief"] = "sunken"
        entry.focus()
    ...
    def _show_info(self):
        ...
        for entry, _cb in self._info.values():
            entry["state"] = "normal"
            entry.delete(0, "end")
    ...
    def _fill_table(self):
        ...
        if self._tab == "all packages":
            ...
            assert 0, f"bad tab value {self._tab!r}"
    ...
    def _download_cb(self, download_iter, ids):
        ...
        #    show('Package %s is out-of-date or corrupt' % msg.package.id)
            ...
            show(f"Finished downloading {msg.package.id!r}.")
        elif isinstance(msg, StartUnzipMessage):
            show(f"Unzipping {msg.package.filename}")
        elif isinstance(msg, FinishCollectionMessage):
            self._log_indent -= 1
            show(f"Finished downloading collection {msg.collection.id!r}.")
    ...
    def _init_progressbar(self):
        c = self._progressbar
        _width, height = int(c["width"]), int(c["height"])
    ...
    def _progress_alive(self):
        ...
        if not self._downloading:
            ...
            x1, _y1, _x2, _y2 = c.bbox("gradient")
            if x1 <= -100:
    ...
    def _monitor_message_queue(self):
        ...
        for msg in self._download_msg_queue:
            ...
            # All other messages
                ...
                show(f"Downloading package {msg.package.id!r}")
            elif isinstance(msg, UpToDateMessage):
                show(f"Package {msg.package.id} is up-to-date!")
            # elif isinstance(msg, StaleMessage):
...
def build_index(root, base_url):
    ...
    for pkg_xml, zf, subdir in _find_packages(os.path.join(root, "packages")):
        ...
        pkg_xml.set("sha256_checksum", f"{sha256_hexdigest(zf.filename)}")
    ...
    for item in packages + collections:
        if item.get("id") in uids:
            raise ValueError("Duplicate UID: {}".format(item.get("id")))
...
def _check_package(pkg_xml, zipfilename, zf):
    ...
    if sum((name != uid and not name.startswith(uid + "/")) for name in zf.namelist()):
        raise ValueError(
            f"Zipfile {uid}.zip does not expand to a single "
            f"subdirectory {uid}/"
        )
        ...
        for filename in files:
            if filename.endswith(".xml"):
                ...
                if sum(
                    ...
                    raise ValueError(
                        f"Zipfile {uid}.zip does not expand to a "
                        f"single subdirectory {uid}/"
                    )
