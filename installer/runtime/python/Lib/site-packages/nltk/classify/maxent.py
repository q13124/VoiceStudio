...
class MaxentClassifier(ClassifierI):
    ...
    def explain(self, featureset, columns=4):
        ...
        print(
            "  TOTAL:".ljust(descr_width) + "".join(f"{sums[l]:8.3f}" for l in labels)
        )
        print(
            "  PROBS:".ljust(descr_width)
            + "".join(f"{pdist.prob(l):8.3f}" for l in labels)
        )
        ...
        **cutoffs,
        ...
        for key in cutoffs:
            if key not in (
                ...
                raise TypeError(f"Unexpected keyword arg {key!r}")
...
def calculate_deltas(
    ...
    *delta[i]*.  In particular, it starts with a guess of
    ...
    #                       nf exp(delta[i]nf)
    for _rangenum in range(MAX_NEWTON):
...
def train_maxent_classifier_with_megam(
    ...
    if gaussian_prior_sigma:
        ...
        inv_variance = 0
    options += ["-lambda", f"{inv_variance:.2f}", "-tune"]
    if trace < 3:
        options += ["-quiet"]
    if "max_iter" in kwargs:
        options += ["-maxi", "{}".format(kwargs["max_iter"])]
    if "ll_delta" in kwargs:
        # [xx] this is actually a perplexity delta, not a log
        # likelihood delta
        options += ["-dpp", "{}".format(abs(kwargs["ll_delta"]))]
...
class TadmMaxentClassifier(MaxentClassifier):
    ...
    def train(cls, train_toks, **kwargs):
        ...
        _trainfile_fd, trainfile_name = tempfile.mkstemp(
            prefix="nltk-tadm-events-", suffix=".gz"
        )
        _weightfile_fd, weightfile_name = tempfile.mkstemp(prefix="nltk-tadm-weights-")
        ...
        if ll_delta:
            options.extend(["-fatol", f"{abs(ll_delta):.6f}"])
...
def demo():
    ...
    names_demo(MaxentClassifier.train)
