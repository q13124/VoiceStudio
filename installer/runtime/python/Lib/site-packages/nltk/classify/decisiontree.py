...
class DecisionTreeClassifier(ClassifierI):
    ...
    def pretty_format(self, width=70, prefix="", depth=4):
        ...
        s = ""
        for _i, (fval, result) in enumerate(
            sorted(
    ...
    def pseudocode(self, prefix="", depth=4):
        ...
        if self._default is not None:
            if len(self._decisions) == 1:
                s += f"{prefix}if {self._fname} != {next(iter(self._decisions.keys()))!r}: "
            else:
                s += f"{prefix}else: "
            if self._default._fname is not None and depth > 1:
    ...
    def train(
        ...
        feature_names = set()
        for featureset, _label in labeled_featuresets:
    ...
    def best_binary_stump(
        ...
        if verbose:
            if best_stump._decisions:
                descr = f"{best_stump._fname}={next(iter(best_stump._decisions.keys()))}"
            else:
                descr = "(default)"
            print(
