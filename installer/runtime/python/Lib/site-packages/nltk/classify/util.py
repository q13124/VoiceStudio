...
def log_likelihood(classifier, gold):
    ...
    ll = [pdist.prob(l) for ((fs, l), pdist) in zip(gold, results, strict=False)]
    return math.log(sum(ll) / len(ll))


def accuracy(classifier, gold):
    results = classifier.classify_many([fs for (fs, l) in gold])
    correct = [l == r for ((fs, l), r) in zip(gold, results, strict=False)]
    if correct:
...
def names_demo(trainer, features=names_demo_features):
    ...
    try:
        ...
        ll = [pdist.logprob(gold) for ((name, gold), pdist) in zip(test, pdists, strict=False)]
        print("Avg. log likelihood: %6.4f" % (sum(ll) / len(test)))
        print()
        print("Unseen Names      P(Male)  P(Female)\n" + "-" * 40)
        for (name, gender), pdist in list(zip(test, pdists, strict=False))[:5]:
            fmt = "  %-15s *%6.4f   %6.4f" if gender == "male" else "  %-15s  %6.4f  *%6.4f"
...
def partial_names_demo(trainer, features=names_demo_features):
    ...
    try:
        ...
        print()
        print("Unseen Names      P(Male)  P(Female)\n" + "-" * 40)
        for (name, is_male), pdist in zip(test, pdists, strict=False)[:5]:
            fmt = "  %-15s *%6.4f   %6.4f" if is_male else "  %-15s  %6.4f  *%6.4f"
