        ...
        for graph in graphs:
            for head_node in graph.nodes.values():
                for child_index, child_node in graph.nodes.items():
                    label = "T" if child_index in head_node["deps"] else "F"
...
class ProbabilisticNonprojectiveParser:
    ...
    def update_edge_scores(self, new_node, cycle_path):
        ...
        for i, _row in enumerate(self.scores):
            for j, _column in enumerate(self.scores[i]):
        ...
        for i, _row in enumerate(self.scores):
            for j, _cell in enumerate(self.scores[i]):
    ...
    def parse(self, tokens):
        ...
        for analysis in analyses:
            ...
            for address, (token, head_index) in enumerate(
                zip(tokens, analysis, strict=False), start=1
            ):
                ...
                rel = "ROOT" if head_address == 0 else ""
