...
class Configuration:
    ...
    def _check_informative(self, feat, flag=False):
        ...
        if feat == "":
            return False
        return not (flag is False and feat == "_")
...
class Transition:
    ...
    def __init__(self, alg_option):
        ...
        if alg_option not in [
            ...
            raise ValueError(
                f" Currently we only support {TransitionParser.ARC_STANDARD} and {TransitionParser.ARC_EAGER} "
            )
    ...
    def left_arc(self, conf, relation):
        ...
        if self._algo == TransitionParser.ARC_EAGER:
            for _idx_parent, _r, idx_child in conf.arcs:
                if idx_child == idx_wi:
                    flag = False
...
class TransitionParser(ParserI):
    ...
    def __init__(self, algorithm):
        ...
        if algorithm not in [self.ARC_STANDARD, self.ARC_EAGER]:
            raise ValueError(
                f" Currently we only support {self.ARC_STANDARD} and {self.ARC_EAGER} "
            )
    ...
    def parse(self, depgraphs, modelFile):
        ...
        for depgraph in depgraphs:
            ...
            while len(conf.buffer) > 0:
                ...
                for y_pred_idx, _confidence in sorted_Prob:
