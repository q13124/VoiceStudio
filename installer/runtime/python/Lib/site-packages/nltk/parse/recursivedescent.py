...
class RecursiveDescentParser(ParserI):
    ...
    def _expand(self, remaining_text, tree, frontier, production=None):
        ...
        productions = self._grammar.productions() if production is None else [production]
    ...
    def _trace_start(self, tree, frontier, text):
        print("Parsing {!r}".format(" ".join(text)))
        if self._trace > 2:
            print("Start:")
        if self._trace > 1:
    ...
    def _trace_backtrack(self, tree, frontier, toks=None):
        if self._trace > 2:
            if toks:
                print(f"Backtrack: {toks[0]!r} match failed")
            else:
                print("Backtrack")
...
class SteppingRecursiveDescentParser(RecursiveDescentParser):
    ...
    def expand(self, production=None):
        ...
        productions = self.untried_expandable_productions() if production is None else [production]
