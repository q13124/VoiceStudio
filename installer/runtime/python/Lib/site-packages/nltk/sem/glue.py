...
class GlueFormula:
    def __init__(self, meaning, glue, indices=None):
        ...
        if isinstance(meaning, str):
            ...
            raise RuntimeError(
                f"Meaning term neither string or expression: {meaning}, {meaning.__class__}"
            )
        ...
        if isinstance(glue, str):
            ...
            raise RuntimeError(
                f"Glue term neither string or expression: {glue}, {glue.__class__}"
            )
        ...
        if return_indices:
            for dep in self.glue.simplify().antecedent.dependencies[
                ...
                arg_meaning_abstracted = self.make_LambdaExpression(
                    Variable(f"v{dep}"), arg_meaning_abstracted
                )
...
class GlueDict(dict):
    ...
    def lookup(self, node, depgraph, counter):
        ...
        if not len(lookup):
            raise KeyError(
                "There is no GlueDict entry for sem type of '{}' "
                "with tag '{}', and rel '{}'".format(node["word"], node["tag"], node["rel"])
            )
                    ...
                    try:
                        ...
                        for atomic in atomics[key]:
                            if not (
                                ...
                                with contextlib.suppress(linearlogic.LinearLogicApplicationException):
