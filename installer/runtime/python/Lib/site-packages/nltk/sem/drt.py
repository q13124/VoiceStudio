...
class DrtExpression:
    ...
    def type(self):
        raise AttributeError(
            f"'{self.__class__.__name__}' object has no attribute 'type'"
        )
...
class DRS(DrtExpression, Expression):
    ...
    def fol(self):
        if self.consequent:
            ...
            accum = ImpExpression(accum, self.consequent.fol()) if accum else self.consequent.fol()
    ...
    def __eq__(self, other):
        ...
        if isinstance(other, DRS) and len(self.refs) == len(other.refs):
            converted_other = other
            for r1, r2 in zip(self.refs, converted_other.refs, strict=False):
            ...
            if self.consequent == converted_other.consequent and len(
                ...
                return all(c1 == c2 for c1, c2 in zip(self.conds, converted_other.conds, strict=False))
        return False
    ...
    def __str__(self):
        drs = "([{}],[{}])".format(
            ...
            ", ".join(f"{cond}" for cond in self.conds),
        )  # map(str, self.conds)))
        if self.consequent:
            return (
                DrtTokens.OPEN
...
class DrtProposition(DrtExpression, Expression):
    ...
    def _pretty(self):
        ...
        blank = " " * len(f"{self.variable}")
        return (
            [blank + " " + line for line in drs_s[:1]]
            + [f"{self.variable}" + ":" + line for line in drs_s[1:2]]
...
class DrtLambdaExpression(DrtExpression, LambdaExpression):
    ...
    def _pretty(self):
        ...
        while term.__class__ == self.__class__:
            ...
            term = term.term
        var_string = " ".join(f"{v}" for v in variables) + DrtTokens.DOT
...
class DrtBinaryExpression(DrtExpression, BinaryExpression):
    ...
    def _assemble_pretty(first_lines, op, second_lines):
        ...
        blank = " " * len(op)
        first_second_lines = list(zip(first_lines, second_lines, strict=False))
        return (
            [
    ...
    def __eq__(self, other):
        ...
        if isinstance(other, DrtConcatenation):
            ...
            if len(self_refs) == len(other_refs):
                converted_other = other
                for r1, r2 in zip(self_refs, other_refs, strict=False):
                    varex = self.make_VariableExpression(r1)
                    converted_other = converted_other.replace(r2, varex, True)
                return (
...
class DrtApplicationExpression(DrtExpression, ApplicationExpression):
    ...
    def _pretty(self):
        ...
        func_args_lines = list(zip(function_lines, list(zip(*args_lines, strict=False)), strict=False))
        return (
            [
...
class PossibleAntecedents(list, DrtExpression, Expression):
    ...
    def __str__(self):
        return "[" + ",".join(f"{it}" for it in self) + "]"
        ...
        if expression.is_pronoun_function():
            ...
            for ancestor in trail:
                for ref in ancestor.get_refs():
                    ...
                    # ==========================================================
                    if refex.__class__ == expression.argument.__class__ and refex != expression.argument:
                ...
                if isinstance(r_cond.second, PossibleAntecedents):
                    if not r_cond.second:
                        raise AnaphoraResolutionException(
                            f"Variable '{r_cond.first}' does not "
                            "resolve to anything."
                        )
...
class DrsDrawer:
    ...
    def _handle_VariableExpression(self, expression, command, x, y):
        return command(f"{expression}", x, y)
    ...
    def _handle_DRS(self, expression, command, x, y):
        ...
        refs = " ".join(f"{r}" for r in expression.refs) if expression.refs else "     "
    ...
    def _handle_LambdaExpression(self, expression, command, x, y):
        ...
        variables = DrtTokens.LAMBDA + f"{expression.variable}" + DrtTokens.DOT
    ...
    def _handle_BinaryExpression(self, expression, command, x, y):
        ...
        # Handle the operator
        right = command(f" {expression.getOp()} ", right, centred_string_top)[0]
