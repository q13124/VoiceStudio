...
class ImpExpression(Expression):
    ...
    def compile_neg(self, index_counter, glueFormulaFactory):
        ...
        new_v = glueFormulaFactory(f"v{fresh_index}", a, {fresh_index})
...
class ApplicationExpression(Expression):
    def __init__(self, function, argument, argument_indices=None):
        ...
        if argument_indices:
            ...
            if not set(function_simp.antecedent.dependencies) < argument_indices:
                raise LinearLogicApplicationException(
                    f"Dependencies unfulfilled when attempting to apply Linear Logic formula {function_simp} to {argument_simp}"
                )
            if set(function_simp.antecedent.dependencies) == argument_indices:
                raise LinearLogicApplicationException(
                    f"Dependencies not a proper subset of indices when attempting to apply Linear Logic formula {function_simp} to {argument_simp}"
                )
    ...
    def __str__(self):
        return f"{self.function}" + Tokens.OPEN + f"{self.argument}" + Tokens.CLOSE
    ...
    def __setitem__(self, variable, binding):
        ...
        if not existing or binding == existing:
            ...
            raise VariableBindingException(
                f"Variable {variable} already bound to another value"
            )
    ...
    def __add__(self, other):
        ...
        try:
            ...
            raise VariableBindingException(
                ...
                f" VariableBindingsLists: {self}, {other}"
            ) from e
