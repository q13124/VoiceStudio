...
class Boxer:
    ...
    def _call_candc(self, inputs, discourse_ids, question, verbose=False):
        ...
        return self._call(
            "\n".join(
                reduce(operator.iadd, ([f"<META>'{id}'", *d] for d, id in zip(inputs, discourse_ids, strict=False)), [])
            ),
            self._candc_bin,
            args,
            verbose,
...
class AbstractBoxerDrs:
    ...
    def variable_types(self):
        vartypes = {}
        for t, vars in zip(("z", "e", "p"), self.variables(), strict=False):
            for v in vars:
                vartypes[v] = t
        return vartypes
...
class BoxerDrs(AbstractBoxerDrs):
    ...
    def _variables(self):
        ...
        for cond in self.conds:
            for s, v in zip(variables, cond._variables(), strict=False):
                s.update(v)
        if self.consequent is not None:
            for s, v in zip(variables, self.consequent._variables(), strict=False):
                s.update(v)
        return variables
    ...
    def __eq__(self, other):
        return (
            ...
            and reduce(
                operator.and_, (c1 == c2 for c1, c2 in zip(self.conds, other.conds, strict=False))
            )
...
class BoxerIndexed(AbstractBoxerDrs):
    ...
    def __eq__(self, other):
        return (
            ...
            and reduce(operator.and_, (s == o for s, o in zip(self, other, strict=False)))
        )
    ...
    def __repr__(self):
        s = "{}({}, {}, [{}]".format(
            ...
            self.discourse_id,
            self.sent_index,
            ", ".join(f"{wi}" for wi in self.word_indices),
        )
        for v in self:
            s += f", {v}"
        return s + ")"
...
class NltkDrtBoxerDrsInterpreter:
    ...
    def interpret(self, ex):
        ...
        if isinstance(ex, BoxerDrs):
            ...
            pred = self._add_occur_indexing(f"{ex.rel}", ex)
        ...
        raise AssertionError(f"{ex.__class__.__name__}: {ex}")
    ...
    def _add_occur_indexing(self, base, ex):
        if self._occur_index and ex.sent_index is not None:
            if ex.discourse_id:
                base += f"_{ex.discourse_id}"
            base += f"_s{ex.sent_index}"
            base += f"_w{sorted(ex.word_indices)[0]}"
        return base
