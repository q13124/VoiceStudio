...
def boolean_ops():
    ...
    for pair in zip(names, [Tokens.NOT, Tokens.AND, Tokens.OR, Tokens.IMP, Tokens.IFF], strict=False):
        print("%-15s\t%s" % pair)
...
def equality_preds():
    ...
    for pair in zip(names, [Tokens.EQ, Tokens.NEQ], strict=False):
        print("%-15s\t%s" % pair)
...
def binding_ops():
    ...
    for pair in zip(names, [Tokens.EXISTS, Tokens.ALL, Tokens.LAMBDA, Tokens.IOTA], strict=False):
        print("%-15s\t%s" % pair)
...
class LogicParser:
        ...
        for start, end, escape, incl_quotes in self.quote_chars:
            if c == start:
                ...
                while data[i] != end:
                    if data[i] == escape:
                        ...
                        if len(data) == i:  # if there are no more chars
                            raise LogicalExpressionException(
                                ...
                                f"Escape character [{escape}] found at end.",
                            )
                        token += data[i]
                    else:
                        token += data[i]
                    ...
                    if len(data) == i:
                        raise LogicalExpressionException(
                            None, "End of input reached.  " f"Expected: [{end}]"
                        )
                if incl_quotes:
                    token += data[i]
                i += 1
    ...
    def handle_variable(self, tok, context):
        ...
        if self.inRange(0) and self.token(0) == Tokens.OPEN:
            ...
            if not isinstance(accum, FunctionVariableExpression) and not isinstance(
                ...
                raise LogicalExpressionException(
                    self._currentIndex,
                    f"'{tok}' is an illegal predicate name.  "
    ...
    def get_next_token_variable(self, description):
        ...
        if isinstance(self.make_VariableExpression(tok), ConstantExpression):
            raise LogicalExpressionException(
                self._currentIndex,
                f"'{tok}' is an illegal variable name.  "
                f"Constants may not be {description}.",
            )
        return Variable(tok)
    ...
    def handle_quant(self, tok, context):
        ...
        if not self.inRange(0):
            raise ExpectedMoreTokensException(
                self._currentIndex + 2,
                message=f"Variable and Expression expected following quantifier '{tok}'.",
            )
    ...
    def assertNextToken(self, expected):
        try:
            ...
            raise ExpectedMoreTokensException(
                e.index, message=f"Expected token '{expected}'."
            ) from e
    ...
    def __repr__(self):
        msg = "Next token: " + self.token(0) if self.inRange(0) else "No more tokens"
...
def unique_variable(pattern=None, ignore=None):
    ...
    if pattern is not None:
        if is_indvar(pattern.name):
            ...
            raise AssertionError("Cannot generate a unique constant")
    else:
        prefix = "z"
...
def skolem_function(univ_scope=None):
    ...
    param univ_scope
    """
    skolem = VariableExpression(Variable(f"F{_counter.get()}"))
    if univ_scope:
...
class ComplexType(Type):
    def __init__(self, first, second):
        assert isinstance(first, Type), f"{first} is not a Type"
        assert isinstance(second, Type), f"{second} is not a Type"
        self.first = first
        self.second = second
...
class BasicType(Type):
    def __eq__(self, other):
        return isinstance(other, BasicType) and (f"{self}") == (f"{other}")
...
def read_type(type_string):
    ...
    elif type_string[0] == f"{ENTITY_TYPE}":
        return ENTITY_TYPE
    elif type_string[0] == f"{TRUTH_TYPE}":
        return TRUTH_TYPE
    elif type_string[0] == f"{ANY_TYPE}":
        return ANY_TYPE
    else:
        raise LogicalExpressionException(
            None, f"Unexpected character: '{type_string[0]}'."
        )
...
class InconsistentTypeHierarchyException(TypeException):
    def __init__(self, variable, expression=None):
        if expression:
            msg = (
                f"The variable '{variable}' was found in multiple places with different"
                f" types in '{expression}'."
            )
        else:
            msg = (
                f"The variable '{variable}' was found in multiple places with different"
...
class TypeResolutionException(TypeException):
    def __init__(self, expression, other_type):
        super().__init__(
            f"The type of '{expression}', '{expression.type}', cannot be resolved with type '{other_type}'"
        )
...
class IllegalTypeException(TypeException):
    def __init__(self, expression, other_type, allowed_type):
        super().__init__(
            f"Cannot set type of {expression.__class__.__name__} '{expression}' to '{other_type}'; must match type '{allowed_type}'."
        )
...
class Expression(SubstituteBindingsI):
    ...
    def applyto(self, other):
        assert isinstance(other, Expression), f"{other} is not an Expression"
    ...
    def __and__(self, other):
        if not isinstance(other, Expression):
            raise TypeError(f"{other} is not an Expression")
            ...
            if var in bindings:
                ...
                if isinstance(val, Variable):
                    ...
                    raise ValueError(
                        ...
                        f"value into an expression: {val!r}"
                    )
    ...
    def replace(self, variable, expression, replace_bound=False, alpha_convert=True):
        ...
        assert isinstance(expression, Expression), (
            f"{expression} is not an Expression"
        )
...
class ApplicationExpression(Expression):
    ...
    def __init__(self, function, argument):
        ...
        """
        assert isinstance(function, Expression), f"{function} is not an Expression"
        assert isinstance(argument, Expression), f"{argument} is not an Expression"
    ...
    def _set_type(self, other_type=ANY_TYPE, signature=None):
        ...
        try:
            ...
            raise TypeException(
                f"The function '{self.function}' is of type '{self.function.type}' and cannot be applied "
                f"to '{self.argument}' of type '{self.argument.type}'.  Its argument must match type '{self.function.type.first}'."
            ) from e
...
class AbstractVariableExpression(Expression):
    ...
    def replace(self, variable, expression, replace_bound=False, alpha_convert=True):
        ...
        assert isinstance(variable, Variable), f"{variable} is not an Variable"
        assert isinstance(expression, Expression), (
            f"{expression} is not an Expression"
        )
...
class VariableBinderExpression(Expression):
    ...
    def __init__(self, variable, term):
        ...
        """
        assert isinstance(variable, Variable), f"{variable} is not a Variable"
        assert isinstance(term, Expression), f"{term} is not an Expression"
        self.variable = variable
        self.term = term
        ...
        if self.variable == variable:
            if replace_bound:
                assert isinstance(expression, AbstractVariableExpression), (
                    f"{expression} is not a AbstractVariableExpression"
                )
    ...
    def alpha_convert(self, newvar):
        ...
        """
        assert isinstance(newvar, Variable), f"{newvar} is not a Variable"
        return self.__class__(
...
class NegatedExpression(Expression):
    ...
    def __str__(self):
        return Tokens.NOT + f"{self.term}"


class BinaryExpression(Expression):
    def __init__(self, first, second):
        assert isinstance(first, Expression), f"{first} is not an Expression"
        assert isinstance(second, Expression), f"{second} is not an Expression"
        self.first = first
        self.second = second
...
class UnexpectedTokenException(LogicalExpressionException):
    def __init__(self, index, unexpected=None, expected=None, message=None):
        if unexpected and expected:
            msg = f"Unexpected token: '{unexpected}'.  " f"Expected token '{expected}'."
        elif unexpected:
            msg = f"Unexpected token: '{unexpected}'."
            if message:
                msg += "  " + message
        else:
            msg = f"Expected token '{expected}'."
...
def is_indvar(expr):
    ...
    """
    assert isinstance(expr, str), f"{expr} is not a string"
