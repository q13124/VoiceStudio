...
def root_semrep(syntree, semkey="SEM"):
    ...
    try:
        ...
        print(node, end=" ")
        print(f"has no specification for the feature {semkey}")
    raise
...
def evaluate_sents(inputs, grammar, model, assignment, trace=0):
    ...
    return [
        [
            (syn, sem, model.evaluate(f"{sem}", assignment, trace=trace))
...
def demo():
    ...
    if options.model:
        exec(f"import {options.model} as model")
    ...
    for i, sent in enumerate(sents):
        ...
        print(SPACER)
        if options.evaluate:
            for _syntree, semrep, value in evaluations[i]:
            ...
            for _syntree, semrep in semreps[i]:
                print("%d:  %s" % (n, semrep))
                n += 1
