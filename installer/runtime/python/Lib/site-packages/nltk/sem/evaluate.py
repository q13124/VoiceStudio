...
def trace(f, *args, **kw):
    ...
    d = dict(zip(argspec[0], args, strict=False))
    if d.pop("trace", None):
        print()
        for item in d.items():
            print("{} => {}".format(*item))
...
def is_rel(s):
    ...
    if len(s) == 0 or (all(isinstance(el, tuple) for el in s) and len(max(s)) == len(min(s))):
        ...
        raise ValueError(f"Set {s!r} contains sequences of different lengths")
...
class Valuation(dict):
        ...
        for sym, val in xs:
            if isinstance(val, (str, bool)):
                ...
                msg = textwrap.fill(
                    ...
                    f"Unrecognized value for symbol '{sym}':\n{val}",
                    width=66,
                )
    ...
    def __getitem__(self, key):
        if key in self:
            ...
            raise Undefined(f"Unknown expression: '{key}'")
    ...
    *g*. *g* only assigns values to individual variables (i.e.,
        ...
        if assign:
            for var, val in assign:
                ...
                assert is_indvar(var), (
                    f"Wrong format for an Individual Variable: '{var}'"
                )
                self[var] = val
        self.variant = None
        self._addvariant()
    ...
    def __getitem__(self, key):
        if key in self:
            ...
            raise Undefined(f"Not recognized as a variable: '{key}'")
    ...
    def add(self, var, val):
        ...
        """
        assert val in self.domain, f"{val} is not in the domain {self.domain}"
        assert is_indvar(var), f"Wrong format for an Individual Variable: '{var}'"
        self[var] = val
        self._addvariant()
        return self
...
class Model:
    ...
    def __init__(self, domain, valuation):
        ...
        if not domain.issuperset(valuation.domain):
            raise Error(
                f"The valuation domain, {valuation.domain}, must be a subset of the model's domain, {domain}"
            )
    ...
    def satisfy(self, parsed, g, trace=None):
        ...
        if isinstance(parsed, ApplicationExpression):
        ...
        elif isinstance(parsed, (IffExpression, EqualityExpression)):
            ...
            for u in self.domain:
                ...
                if not self.satisfy(parsed.term, new_g):
                    return False
            return True
        elif isinstance(parsed, (ExistsExpression, IotaExpression)):
            new_g = g.copy()
            for u in self.domain:
                new_g.add(parsed.variable.name, u)
    ...
    def i(self, parsed, g, trace=False):
        ...
        if parsed.variable.name in self.valuation.symbols:
            ...
            raise Undefined(f"Can't find a value for {parsed}")
    ...
    def satisfiers(self, parsed, varex, g, trace=None, nesting=0):
        ...
        var = Variable(varex) if isinstance(varex, str) else varex
        ...
        if var in parsed.free():
            ...
            for u in self.domain:
                ...
                lowtrace = trace - 1 if trace and trace > 1 else 0
                value = self.satisfy(parsed, new_g, lowtrace)

                if trace:
                    print(indent + f"(trying assignment {new_g})")
...
def folmodel(quiet=False, trace=None):
    ...
    if not quiet:
        ...
        for parsed in parsed_exprs:
            try:
                print(
                    f"The interpretation of '{parsed}' in m2 is {m2.i(parsed, g2)}"
                )
            except Undefined:
                print(f"The interpretation of '{parsed}' in m2 is Undefined")
