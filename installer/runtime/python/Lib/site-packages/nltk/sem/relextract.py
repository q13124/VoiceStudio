...
short2long = {"LOC": "LOCATION", "ORG": "ORGANIZATION", "PER": "PERSON"}
long2short = {"LOCATION": "LOC", "ORGANIZATION": "ORG", "PERSON": "PER"}
...
def extract_rels(subjclass, objclass, doc, corpus="ace", pattern=None, window=10):
    ...
    if subjclass and subjclass not in NE_CLASSES[corpus]:
        if _expand(subjclass) in NE_CLASSES[corpus]:
            ...
            raise ValueError(
                f"your value for the subject type has not been recognized: {subjclass}"
            )
    if objclass and objclass not in NE_CLASSES[corpus]:
        if _expand(objclass) in NE_CLASSES[corpus]:
            ...
            raise ValueError(
                f"your value for the object type has not been recognized: {objclass}"
            )
...
def in_demo(trace=0, sql=True):
    ...
    if sql:
        try:
            ...
            warnings.warn("Cannot import sqlite; sql flag will be ignored.", stacklevel=2)
