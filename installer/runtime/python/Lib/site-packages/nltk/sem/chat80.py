...
class Concept:
    ...
    def __init__(self, prefLabel, arity, altLabels=None, closures=None, extension=None):
        ...
        self.extension = sorted(extension)
    ...
    def __repr__(self):
        return f"Concept('{self.prefLabel}')"
    ...
    def augment(self, data):
        ...
        self.extension = sorted(self._extension)
...
def cities2table(filename, rel_name, dbname, verbose=False, setup=False):
    ...
    for t in records:
        cur.execute(f"insert into {table_name} values (?,?,?)", t)
        if verbose:
            print(f"inserting values into {table_name}: ", t)
    connection.commit()
    if verbose:
        print(f"Committing update to {dbname}")
    cur.close()
...
def sql_query(dbname, query):
    ...
    try:
        ...
        warnings.warn(
            f"Make sure the database file {dbname} is installed and uncompressed.", stacklevel=2
        )
        raise
...
def binary_concept(label, closures, subj, obj, records):
    ...
    :rtype: Concept
    """
    if label != "border" and label != "contain":
        label = label + "_of"
...
def val_load(db):
    ...
    if not os.access(dbname, os.R_OK):
        sys.exit(f"Cannot read file: {dbname}")
    else:
...
Valuation object for use in the NLTK semantics package.
    ...
    if options.outdb:
        ...
        if options.verbose:
            ...
            print(f"Dumping a valuation to {outdb}")
