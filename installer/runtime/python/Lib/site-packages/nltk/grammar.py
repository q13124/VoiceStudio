...
class ProbabilisticProduction(Production, ImmutableProbabilisticMixIn):
    ...
    def __str__(self):
        return super().__str__() + (
            " [1.0]" if (self.prob() == 1.0) else f" [{self.prob():g}]"
        )
...
class CFG:
    ...
    def check_coverage(self, tokens):
        ...
        if missing:
            ...
            raise ValueError(
                "Grammar does not cover some of the " f"input words: {missing!r}."
            )
    ...
    def __str__(self):
        ...
        result += f" (start state = {self._start!r})"
    ...
    def __init__(self, start, productions, calculate_leftcorners=True):
        ...
        for lhs, p in probs.items():
            if not ((1 - PCFG.EPSILON) < p < (1 + PCFG.EPSILON)):
                raise ValueError(f"Productions for {lhs!r} do not sum to 1")
...
def _read_production(line, nonterm_parser, probabilistic=False):
    ...
    while pos < len(line):
        ...
        if probabilistic and m:
            ...
            if probabilities[-1] > 1.0:
                raise ValueError(
                    f"Production probability {probabilities[-1]:f}, "
    ...
    if probabilistic:
        return [
            ...
            for (rhs, probability) in zip(rhsides, probabilities, strict=False)
        ]
    else:
