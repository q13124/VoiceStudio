...
class PunktLanguageVars:
    ...
    def _re_sent_end_chars(self):
        return "[{}]".format(re.escape("".join(self.sent_end_chars)))
...
class PunktToken:
    ...
    def __repr__(self):
        ...
        annotations.
        """
        typestr = f" type={self.type!r}," if self.type != self.tok else ""
    ...
    def _train_tokens(self, tokens, verbose):
        ...
        for aug_tok1, aug_tok2 in _pair_iter(tokens):
            ...
            if self._is_rare_abbrev_type(aug_tok1, aug_tok2):
                ...
                if verbose:
                    print(f"  Rare Abbrev: {aug_tok1.type}")
    ...
    def _get_orthography_data(self, tokens):
        ...
        for aug_tok in tokens:
            ...
            if aug_tok.sentbreak:
                context = "initial" if not (aug_tok.is_number or aug_tok.is_initial) else "unknown"
...
def demo(text, tok_cls=PunktSentenceTokenizer, train_cls=PunktTrainer):
    ...
    def cleanup(s):
        return (re.compile(r"(?:\r|^\s+)", re.MULTILINE).sub("", s).replace("\n", " "))
    trainer = train_cls()
