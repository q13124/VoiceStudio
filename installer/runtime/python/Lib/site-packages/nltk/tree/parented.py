...
class ParentedTree(AbstractParentedTree):
    ...
    def copy(self, deep=False):
        if not deep:
            warnings.warn(
                f"{self.__class__.__name__} objects do not support shallow copies. Defaulting to a deep copy.", stacklevel=2
            )
    ...
    def parent_index(self):
        ...
        for i, child in enumerate(self._parent):
            if child is self:
                return i
        raise AssertionError("expected to find self in self._parent!")
