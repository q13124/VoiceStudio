...
def error_list(train_sents, test_sents):
    ...
    errors = [hdr]
    for train_sent, test_sent in zip(train_sents, test_sents, strict=False):
        for wordnum, (word, train_pos) in enumerate(train_sent):
            test_pos = test_sent[wordnum][1]
            if train_pos != test_pos:
                left = " ".join("{}/{}".format(*w) for w in train_sent[:wordnum])
                right = " ".join("{}/{}".format(*w) for w in train_sent[wordnum + 1 :])
