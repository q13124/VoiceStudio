...
class HiddenMarkovModelTagger(TaggerI):
    ...
    def _tag(self, unlabeled_sequence):
        ...
        return list(zip(unlabeled_sequence, path, strict=False))
    ...
    def _exhaustive_point_entropy(self, unlabeled_sequence):
        ...
        for labelling, lp in zip(labellings, log_probs, strict=False):
            lp -= normalisation
            for t, label in enumerate(labelling):
    ...
    def test(self, test_sequence, verbose=False, **kwargs):
        ...
        if verbose:
            for test_sent, predicted_sent in zip(test_sequence, predicted_sequence, strict=False):
                print(
                    "Test:",
                    " ".join(f"{token}/{tag}" for (token, tag) in test_sent),
                )
                print()
                print("Untagged:", " ".join(f"{token}" for (token, tag) in test_sent))
                print()
                print(
                    "HMM-tagged:",
...
def _create_hmm_tagger(states, symbols, A, B, pi):
    def pd(values, samples):
        d = dict(zip(samples, values, strict=False))
        return DictionaryProbDist(d)

    def cpd(array, conditions, samples):
        d = {}
        for values, condition in zip(array, conditions, strict=False):
...
def demo():
    ...
    model, _states, _symbols = _market_hmm_example()
...
def demo_bw():
    ...
    trainer.train_unsupervised(training, model=model, max_iterations=1000)
