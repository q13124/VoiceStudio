...
class BrillTagger(TaggerI):
    ...
    def tag(self, tokens):
        ...
        for i, (_token, tag) in enumerate(tagged_tokens):
    ...
    def print_template_statistics(self, test_stats=None, printunused=True):
        ...
        for tid, score in zip(tids, trainscores, strict=False):
        ...
        def print_testtrain_stats():
            ...
            for tid, score in zip(tids, testscores, strict=False):
    ...
    def batch_tag_incremental(self, sequences, gold):
        ...
        def counterrors(xs):
            return sum(t[1] != g[1] for pair in zip(xs, gold, strict=False) for (t, g) in zip(*pair, strict=False))
        ...
        testing_stats["rulescores"] = [
            err0 - err1 for (err0, err1) in itertools.pairwise(errors)
        ]
