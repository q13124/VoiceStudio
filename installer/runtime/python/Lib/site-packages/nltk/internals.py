...
def read_str(s, start_position):
    ...
    _STRING_END_RE = re.compile(rf"\\|{quotemark}")
    position = m.end()
    while True:
...
def find_file_iter(
    ...
    if not yielded:
        msg = (
            f"NLTK was unable to find the {filename} file!"
                        ...
                        if not is_regex:
                            ...
                            for file_name in os.listdir(cp):
                                if re.match(name_pattern, file_name):
                                    if verbose:
                                        print(
                                            f"[Found {name_pattern}: {os.path.join(cp, file_name)}]"
                                        )
                                    yielded = True
...
def is_writable(path):
    ...
    if hasattr(os, "getuid"):
        ...
        if perm & 0o002 or (statdata.st_uid == os.getuid() and (perm & 0o200)) or ((statdata.st_gid in [os.getgid(), *os.getgroups()]) and (perm & 0o020)):
            return True
