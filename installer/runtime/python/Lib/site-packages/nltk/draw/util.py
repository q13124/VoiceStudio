        ...
        if self.__draggable:
            if "drag" in self.__callbacks:
                ...
                try:
                    ...
                    print(f"Error in drag callback for {self!r}")
...
class SymbolWidget(TextWidget):
    ...
    def __repr__(self):
        return f"[Symbol: {self._symbol!r}]"
    ...
    def __setitem__(self, attr, value):
        if attr == "align":
            if value not in ("top", "bottom", "center"):
                raise ValueError(f"Bad alignment: {value!r}")
            self._align = value
        elif attr == "space":
            self._space = value
        elif attr == "ordered":
...
class CanvasFrame:
    ...
    def add_widget(self, canvaswidget, x=None, y=None):
        ...
        # Move to (x,y)
        (x1, y1, _x2, _y2) = canvaswidget.bbox()
