...
class DutchStemmer(_StandardStemmer):
    ...
    def stem(self, word):
        ...
        if len(word) >= 4 and word[-1] not in self.__vowels and word[-1] != "I":
...
class EnglishStemmer(_StandardStemmer):
    ...
    def stem(self, word):
        ...
        if word.startswith(("gener", "commun", "arsen")):
            r1 = word[5:] if word.startswith(("gener", "arsen")) else word[6:]
            ...
            if word.endswith(suffix):
                if suffix in ("eed", "eedly"):
                    if r1.endswith(suffix):
                        ...
                        r1 = suffix_replace(r1, suffix, "ee") if len(r1) >= len(suffix) else ""

                        r2 = suffix_replace(r2, suffix, "ee") if len(r2) >= len(suffix) else ""
                else:
        ...
        if len(word) > 2 and word[-1] in "yY" and word[-2] not in self.__vowels:
            word = "".join((word[:-1], "i"))
            r1 = "".join((r1[:-1], "i")) if len(r1) >= 1 else ""

            r2 = "".join((r2[:-1], "i")) if len(r2) >= 1 else ""
            ...
            if word.endswith(suffix):
                if r1.endswith(suffix):
                    if suffix == "tional":
                        ...
                        r1 = "".join((r1[:-1], "e")) if len(r1) >= 1 else ""

                        r2 = "".join((r2[:-1], "e")) if len(r2) >= 1 else ""
                        ...
                        r1 = suffix_replace(r1, suffix, "ize") if len(r1) >= len(suffix) else ""

                        r2 = suffix_replace(r2, suffix, "ize") if len(r2) >= len(suffix) else ""

                    elif suffix in ("ational", "ation", "ator"):
                        word = suffix_replace(word, suffix, "ate")

                        r1 = suffix_replace(r1, suffix, "ate") if len(r1) >= len(suffix) else ""

                        r2 = suffix_replace(r2, suffix, "ate") if len(r2) >= len(suffix) else "e"

                    elif suffix in ("alism", "aliti", "alli"):
                        word = suffix_replace(word, suffix, "al")

                        r1 = suffix_replace(r1, suffix, "al") if len(r1) >= len(suffix) else ""

                        r2 = suffix_replace(r2, suffix, "al") if len(r2) >= len(suffix) else ""
                        ...
                        r1 = suffix_replace(r1, suffix, "ous") if len(r1) >= len(suffix) else ""

                        r2 = suffix_replace(r2, suffix, "ous") if len(r2) >= len(suffix) else ""

                    elif suffix in ("iveness", "iviti"):
                        word = suffix_replace(word, suffix, "ive")

                        r1 = suffix_replace(r1, suffix, "ive") if len(r1) >= len(suffix) else ""

                        r2 = suffix_replace(r2, suffix, "ive") if len(r2) >= len(suffix) else "e"

                    elif suffix in ("biliti", "bli"):
                        word = suffix_replace(word, suffix, "ble")

                        r1 = suffix_replace(r1, suffix, "ble") if len(r1) >= len(suffix) else ""

                        r2 = suffix_replace(r2, suffix, "ble") if len(r2) >= len(suffix) else ""
            ...
            if word.endswith(suffix):
                if r1.endswith(suffix):
                    if suffix == "tional":
                        ...
                        r2 = suffix_replace(r2, suffix, "ate") if len(r2) >= len(suffix) else ""
                        ...
                        r1 = suffix_replace(r1, suffix, "ic") if len(r1) >= len(suffix) else ""

                        r2 = suffix_replace(r2, suffix, "ic") if len(r2) >= len(suffix) else ""
        ...
        # STEP 5
        if (r2.endswith("l") and word[-2] == "l") or r2.endswith("e") or (r1.endswith("e") and len(word) >= 4 and (
...
class FrenchStemmer(_StandardStemmer):
    ...
    def stem(self, word):
        ...
        for suffix in self.__step1_suffixes:
            if word.endswith(suffix):
                if suffix == "eaux":
                    ...
                    if word[-2:] == "ic":
                        word = word[:-2] if "ic" in r2 else "".join((word[:-2], "iqU"))
                    ...
                    if word[-4:] == "abil":
                        word = word[:-4] if "abil" in r2 else "".join((word[:-2], "l"))

                    elif word[-2:] == "ic":
                        word = word[:-2] if "ic" in r2 else "".join((word[:-2], "iqU"))

                    elif word[-2:] == "iv" and "iv" in r2:
                        word = word[:-2]
        ...
        # STEP 4: Residual suffixes
            ...
            for suffix in self.__step4_suffixes:
                if word.endswith(suffix) and suffix in rv:
...
class PortugueseStemmer(_StandardStemmer):
    ...
    def stem(self, word):
        ...
        # STEP 3
        if (step1_success or step2_success) and rv.endswith("i") and word[-2] == "c":
            word = word[:-1]
            rv = rv[:-1]
            ...
            if word.endswith(suffix):
                if suffix in r1:
                    if suffix in ("ul", "ului"):
                        ...
                        rv = rv[:-len(suffix)] if suffix in rv else ""
                        ...
                        rv = suffix_replace(rv, suffix, "e") if suffix in rv else ""

                    elif suffix in ("ii", "iua", "iei", "iile", "iilor", "ilor"):
                        word = suffix_replace(word, suffix, "i")

                        rv = suffix_replace(rv, suffix, "i") if suffix in rv else ""
...
class RussianStemmer(_LanguageSpecificStemmer):
    ...
    def stem(self, word):
        ...
        if not undouble_success and not superlative_removed and word.endswith("'"):
            word = word[:-1]
