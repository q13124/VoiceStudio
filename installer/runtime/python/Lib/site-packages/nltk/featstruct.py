...
class FeatStruct(SubstituteBindingsI):
    ...
    def _freeze(self, visited):
        ...
        visited.add(id(self))
        self._frozen = True
        for _fname, fval in sorted(self._items()):
            if isinstance(fval, FeatStruct):
    ...
    def _repr(self, reentrances, reentrance_ids):
        ...
        for fname, fval in sorted(self.items()):
            ...
            if id(fval) in reentrance_ids:
                ...
                suffix = f"/{fval.name}" if isinstance(fval, Variable) else f"/{fval!r}"
        ...
        # Case 5b: Feature value defines custom unification method
            ...
            if isinstance(fval2, CustomFeatureValue) and result != fval2.unify(fval1):
                raise AssertionError(
                    f"CustomFeatureValue objects {fval1!r} and {fval2!r} disagree "
                    f"about unification value: {result!r} vs. {fval2.unify(fval1)!r}"
                )
...
def _default_fs_class(obj):
    ...
    if isinstance(obj, (dict, list)):
        ...
        raise ValueError(
            f"To unify objects of type {obj.__class__.__name__}, you must specify "
            "fs_class explicitly."
        )
...
class SubstituteBindingsSequence(SubstituteBindingsI):
    ...
    def variables(self):
        return [elt for elt in self if isinstance(elt, Variable)] + reduce(operator.iadd, (
                list(elt.variables())
                for elt in self
...
class FeatureValueTuple(SubstituteBindingsSequence, tuple):
    ...
    def __repr__(self):  # [xx] really use %s here?
        if len(self) == 0:
            return "()"
        return "({})".format(", ".join(f"{b}" for b in self))
...
class FeatureValueSet(SubstituteBindingsSequence, frozenset):
    ...
    def __repr__(self):  # [xx] really use %s here?
        ...
        return "{{{}}}".format(", ".join(sorted(f"{b}" for b in self)))
...
class FeatStructReader:
    ...
    _START_FDICT_RE = re.compile(
        rf"({_BARE_PREFIX_RE.pattern})|({_START_FSTRUCT_RE.pattern}\s*({_FEATURE_NAME_RE.pattern}\s*(=|->)|[+-]{_FEATURE_NAME_RE.pattern}|\]))"
    )
    ...
    def read_app_value(self, s, position, reentrances, match):
        ...
        return self._logic_parser.parse("{}({})".format(*match.group(2, 3))), match.end()
    ...
    def _read_seq_value(
        ...
        m = re.compile(rf"\s*/?\s*{cp}").match(s, position)
        if m:
        ...
        while True:
            ...
            m = re.compile(rf"\s*{cp}").match(s, position)
            if m:
                if seen_plus:
            ...
            m = re.compile(rf"\s*(,|\+|(?={cp}))\s*").match(s, position)
            if not m:
...
def display_unification(fs1, fs2, indent="  "):
    ...
    for fs1_line, fs2_line in zip(fs1_lines, fs2_lines, strict=False):
