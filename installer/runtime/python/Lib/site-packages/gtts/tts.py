...
class gTTS:
    ...
    def __init__(
        ...
        if pre_processor_funcs is None:
            pre_processor_funcs = [pre_processors.tone_marks, pre_processors.end_of_line, pre_processors.abbreviations, pre_processors.word_sub]
    ...
    def write_to_fp(self, fp):
        ...
        try:
            ...
            raise TypeError(
                f"'fp' is not a file-like object or it does not take bytes: {e!s}"
            )
    ...
    def infer_msg(self, tts, rsp=None):
        ...
        if rsp is None:
            ...
            if status == 403:
                ...
                cause = (
                    f"No audio stream in response. Unsupported language '{self.tts.lang}'"
                )
            elif status >= 500:
