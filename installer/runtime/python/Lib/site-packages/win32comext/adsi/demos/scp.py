...
def _get_option(po, opt_name, default=_NoDefault):
    ...
    if not ret and default is _NoDefault:
        parser.error(f"The '{opt_name}' option must be specified for this operation")
    if not ret:
        ret = default
    return ret
...
def main():
    ...
    for arg in args:
        if arg.lower() not in _handlers_dict:
            parser.error(f"Invalid command '{arg}' (use --help for valid commands)")
    ...
    for arg in args:
        ...
        if handler is None:
            parser.error(f"Invalid command '{arg}'")
        err_msg = None
        try:
            try:
            ...
            (hr, desc, exc, _argerr) = xxx_todo_changeme.args
            extra_desc = exc[2] if exc else ""
            err_msg = desc
            if extra_desc:
                err_msg += "\n\t" + extra_desc
        except win32api.error as xxx_todo_changeme1:
            (_hr, func, msg) = xxx_todo_changeme1.args
            err_msg = msg
        if err_msg:
