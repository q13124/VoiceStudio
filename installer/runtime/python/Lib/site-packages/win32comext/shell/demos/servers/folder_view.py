...
def make_item_enum(level, flags):
    pidls = []
    nums = ["zero", "one", "two", "three", "four", "five", "size", "seven", "eight", "nine", "ten"]
...
class ContextMenu:
    ...
    def InvokeCommand(self, ci):
        _mask, hwnd, verb, _params, _dir, _nShow, _hotkey, _hicon = ci
        # this seems very convoluted, but it's what the sample does :)
        for verb_name, verb_id, _flag in folderViewImplContextMenuIDs:
            matches = verb == verb_id if isinstance(verb, int) else verb == verb_name
            if matches:
                break
        else:
