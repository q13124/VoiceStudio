...
def SendEMAPIMail(
    ...
    (_eid_tag, eid), (_name_tag, _name), (_def_store_tag, _def_store) = row
    msgstore = session.OpenMsgStore(
        0, eid, None, mapi.MDB_NO_DIALOG | mapi.MAPI_BEST_ACCESS
    )

    # get the outbox
    _hr, props = msgstore.GetProps((mapitags.PR_IPM_OUTBOX_ENTRYID), 0)
    (tag, eid) = props[0]
    # check for errors
    if mapitags.PROP_TYPE(tag) == mapitags.PT_ERROR:
        raise TypeError(f"got PT_ERROR instead of PT_BINARY: {eid}")
