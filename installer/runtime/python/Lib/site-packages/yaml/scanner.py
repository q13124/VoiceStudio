...
class Scanner:
    ...
    def scan_directive_ignored_line(self, start_mark):
        ...
        if ch not in '\0\r\n\x85\u2028\u2029':
            raise ScannerError("while scanning a directive", start_mark,
                    f"expected a comment or a line break, but found {ch!r}", self.get_mark())
    ...
    def scan_block_scalar_indicators(self, start_mark):
        ...
        if ch not in '\0 \r\n\x85\u2028\u2029':
            raise ScannerError("while scanning a block scalar", start_mark,
                    f"expected chomping or indentation indicators, but found {ch!r}", self.get_mark())
    ...
    def scan_uri_escapes(self, name, start_mark):
        ...
        while self.peek() == '%':
            ...
            for k in range(2):
                if self.peek(k) not in '0123456789ABCDEFabcdef':
                    raise ScannerError(f"while scanning a {name}", start_mark,
                            f"expected URI escape sequence of 2 hexadecimal numbers, but found {self.peek(k)!r}", self.get_mark())
