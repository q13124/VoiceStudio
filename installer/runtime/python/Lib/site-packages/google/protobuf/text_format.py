...
class _Printer:
  ...
  def __init__(
    ...
    if double_format is not None:
      warnings.warn(
          ...
          'turn into error in 7.34.0, please remove it before that.', stacklevel=2
      )
  ...
  def PrintFieldValue(self, field, value):
    ...
    if field.cpp_type == descriptor.FieldDescriptor.CPPTYPE_MESSAGE:
      ...
      out.write('\"')
      out_value = value.encode('utf-8') if isinstance(value, str) and not self.as_utf8 else value
  ...
  def _MergeField(self, tokenizer, message):
    ...
    if (message_descriptor.full_name == _ANY_FULL_TYPE_NAME and
      ...
      while not tokenizer.TryConsume(expanded_any_end_token):
        if tokenizer.AtEnd():
          raise tokenizer.ParseErrorPreviousToken(f'Expected "{expanded_any_end_token}".')
    ...
    if tokenizer.TryConsume('['):
      ...
      if not field:
        if self.allow_unknown_extension:
          ...
          raise tokenizer.ParseErrorPreviousToken(
              f'Extension "{name}" not registered. '
    ...
    if field:
      if not self._allow_multiple_scalars and field.containing_oneof:
        ...
        if which_oneof is not None and which_oneof != field.name:
          raise tokenizer.ParseErrorPreviousToken(
              f'Field "{field.name}" is specified along with field "{which_oneof}", another member '
              f'of oneof "{field.containing_oneof.name}" for message type "{message_descriptor.full_name}".')
...
def _ParseAbstractInteger(text):
  ...
  try:
    ...
    raise ValueError(f'Couldn\'t parse integer: {orig_text}')
...
def ParseFloat(text):
  ...
  if _FLOAT_OCTAL_PREFIX.match(text):
    raise ValueError(f'Invalid octal float: {text}')
  try:
    ...
    if _FLOAT_INFINITY.match(text):
      ...
      try:
        ...
        raise ValueError(f"Couldn't parse float: {text}")
...
def ParseEnum(field, value):
  ...
  try:
    ...
    if enum_value is None:
      raise ValueError(f'Enum type "{enum_descriptor.full_name}" has no value named {value}.')
  else:
