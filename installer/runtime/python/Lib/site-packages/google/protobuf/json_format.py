    ...
    try:
      for field, value in fields:
        ...
        if _IsMapEntry(field):
          ...
          js_map = {}
          for key in value:
            recorded_key = ('true' if key else 'false') if isinstance(key, bool) else str(key)
...
def Parse(
  ...
  try:
    ...
    raise ParseError(f'Failed to load JSON: {e!s}.') from e
  ...
  try:
    ...
    raise ParseError(
        f'Failed to parse JSON: {type(e).__name__}: {e!s}.'
    ) from e
...
class _Parser:
  ...
  def ConvertMessage(self, value, message, path):
    ...
    if self.recursion_depth > self.max_recursion_depth:
      raise ParseError(
          f'Message too deep. Max recursion depth is {self.max_recursion_depth}'
      )
    message_descriptor = message.DESCRIPTOR
    full_name = message_descriptor.full_name
    if not path:
      ...
      try:
        ...
        if _IsMapEntry(field):
          ...
          if not isinstance(value, _LIST_LIKE):
            raise ParseError(
                f'repeated field {name} must be in [] which is {value} at {path}'
            )
          if field.cpp_type == descriptor.FieldDescriptor.CPPTYPE_MESSAGE:
          ...
          self._ConvertAndSetScalar(
              message, field, value, f'{path}.{name}'
          )
      except ParseError as e:
  ...
  def _ConvertValueMessage(self, value, message, path):
    ...
    if isinstance(value, dict):
      ...
      raise ParseError(
          f'Value {value} has unexpected type {type(value)} at {path}'
      )
  ...
  def _ConvertWrapperMessage(self, value, message, path):
    ...
    self._ConvertAndSetScalar(
        message, field, value, path=f'{path}.value'
    )
      ...
      if enum_value is None:
        try:
          ...
          raise EnumStringValueParseError(
              f'Invalid enum value {value} for enum type {field.enum_type.full_name}'
          ) from e
        if enum_value is None:
          if field.enum_type.is_closed:
            raise ParseError(
