  ...
  class DescriptorMetaclass(type):
    ...
    def __instancecheck__(cls, obj):
      if super().__instancecheck__(obj):
        return True
      return bool(isinstance(obj, cls._C_DESCRIPTOR_CLASS))
...
def _Deprecated(
  ...
  if _Deprecated.count > 0:
    _Deprecated.count -= 1
    warnings.warn(
        f'Call to deprecated {name}, use {alternative} instead.',
...
class DescriptorBase(metaclass=DescriptorMetaclass):
  ...
  def _LazyLoadOptions(self):
    ...
    if not hasattr(descriptor_pb2, self._options_class_name):
      raise RuntimeError(
          f'Unknown options class name {self._options_class_name}!'
      )
