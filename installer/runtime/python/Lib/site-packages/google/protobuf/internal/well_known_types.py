...
class Timestamp:
  ...
  def FromJsonString(self, value):
    ...
    nanos = round(float('0.' + nano_value) * 1000000000.0) if nano_value else 0
    # Parse timezone offsets.
    if value[timezone_offset] == 'Z':
      if len(value) != timezone_offset + 1:
        raise ValueError(
            f'Failed to parse timestamp: invalid trailing data {value}.'
        )
    else:
  ...
  def FromDatetime(self, dt):
    ...
    try:
      ...
      raise AttributeError(
          ...
          f'object got {type(dt).__name__} : {e}'
      ) from e
    _CheckTimestampValid(seconds, nanos)
    self.seconds = seconds
    self.nanos = nanos
...
def _CheckTimestampValid(seconds, nanos):
  if seconds < _TIMESTAMP_SECONDS_MIN or seconds > _TIMESTAMP_SECONDS_MAX:
    raise ValueError(
        f'Timestamp is not valid: Seconds {seconds} must be in range '
        '[-62135596800, 253402300799].')
  if nanos < 0 or nanos >= _NANOS_PER_SECOND:
    raise ValueError(
        f'Timestamp is not valid: Nanos {nanos} must be in a range '
        '[0, 999999].'
    )
  ...
  def FromJsonString(self, value):
    ...
    try:
      ...
      if pos == -1:
        ...
        if value[0] == '-':
          nanos = round(float(f'-0{value[pos:-1]}') * 1e9)
        else:
          nanos = round(float(f'0{value[pos:-1]}') * 1e9)
  ...
  def FromTimedelta(self, td):
    ...
    try:
      ...
      raise AttributeError(
          ...
          f'object got {type(td).__name__}: {e}'
      ) from e
