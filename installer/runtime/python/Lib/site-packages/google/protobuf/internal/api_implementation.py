...
if _implementation_type not in ('python', 'cpp', 'upb'):
  raise ValueError(f'PROTOCOL_BUFFERS_PYTHON_IMPLEMENTATION {_implementation_type} is not '
                   'supported. Please set to \'python\', \'cpp\' or '
                   '\'upb\'.')

if 'PyPy' in sys.version and _implementation_type == 'cpp':
  warnings.warn('PyPy does not work yet with cpp protocol buffers. '
                'Falling back to the python implementation.', stacklevel=2)
  _implementation_type = 'python'
...
if _implementation_type == 'cpp':
  try:
    ...
    warnings.warn(
        'Selected implementation cpp is not available.', stacklevel=2)
    pass
