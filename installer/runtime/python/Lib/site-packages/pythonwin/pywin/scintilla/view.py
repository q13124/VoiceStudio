...
class CScintillaView(docview.CtrlView, control.CScintillaColorEditInterface):
    ...
    def HookHandlers(self):
        ...
        for command, reflection in command_reflectors:
            def handler(id, code, ss=self.SendScintilla, tosend=reflection):
                return (ss(tosend)
                            and 0)
    ...
    def OnCmdFileLocate(self, cmd, id):
        ...
        if m:
            ...
            if fileName is None:
                win32ui.SetStatusText(f"Can't locate module {modName}")
    ...
    def _GetObjectAtPos(self, pos=-1, bAllowCalls=0):
        left, _right = self._GetWordSplit(pos, bAllowCalls)
...
def LoadConfiguration():
    ...
    configName = win32ui.GetProfileVal("Editor", "Keyboard Config", "default")
    configManager = ConfigManager(configName)
    if configManager.last_error:
        bTryDefault = 0
        msg = f"Error loading configuration '{configName}'\n\n{configManager.last_error}"
        ...
        if bTryDefault:
            ...
            if configManager.last_error:
                win32ui.MessageBox(
                    f"Error loading configuration 'default'\n\n{configManager.last_error}"
                )
