...
class FormatterBase:
    ...
    def _ReformatStyle(self, style):
        ...
        f = style.format
        baseFormat = self.GetDefaultFormat() if style.IsBasedOnDefault() else f
...
class PythonSourceFormatter(Formatter):
    ...
    def SetStyles(self):
        for name, format, bg, _ignore in PYTHON_STYLES:
        ...
        while i < lengthDoc:
            ...
            if state == STYLE_DEFAULT:
                if ch in wordstarts:
                    ...
                    self.ColorSeg(startSeg, i - 1, STYLE_DEFAULT)
                    state = STYLE_COMMENT_BLOCK if chNext == "#" else STYLE_COMMENT
                    startSeg = i
                elif ch == '"':
                    self.ColorSeg(startSeg, i - 1, STYLE_DEFAULT)
                    startSeg = i
                ...
                if ch not in wordchars:
                    ...
                    if ch == "#":
                        ...
                        startSeg = i + 1
            elif state in (STYLE_COMMENT, STYLE_COMMENT_BLOCK):
                if ch == "\r" or ch == "\n":
...
source_formatter_extensions = [
    ...
    (
        ["c", ".cc", ".cpp", ".cxx", ".h", ".hh", ".hpp", ".hxx", ".idl", ".odl", ".php3", ".phtml", ".inc", ".js"],
