...
debugging_options = ["No debugging\r", "Step-through in the debugger\r", "Run in the debugger\r", "Post-Mortem of unhandled exceptions"]
...
def GetActiveView():
    ...
    try:
        childFrame, _bIsMaximised = win32ui.GetMainFrame().MDIGetActive()
...
def RunScript(defName=None, defArgs=None, bShowDialog=1, debuggingType=None):
    ...
    if len(path) == 0:
        try:
            ...
            if fullScript is None:
                win32ui.MessageBox(f"The file '{script}' can not be located")
                return
            script = fullScript
    else:
    ...
    win32ui.SetStatusText(f"Running script {base}...", 1)
    exitCode = 0
    ...
    if bWorked:
        ...
        win32ui.SetStatusText(f"Exception raised while running script  {base}")
...
def ImportFile():
    ...
    _path, modName = os.path.split(pathName)
    modName, _modExt = os.path.splitext(modName)
    newPath = None
...
def _HandlePythonFailure(what, syntaxErrorPathName=None):
    _typ, details, _tb = sys.exc_info()
    if isinstance(details, SyntaxError):
        try:
            msg, (fileName, line, col, _text) = details
...
def FindTabNanny():
    ...
    try:
        path = win32api.RegQueryValue(
            ...
            f"SOFTWARE\\Python\\PythonCore\\{sys.winver}\\InstallPath",
        )
    except win32api.error:
        print("WARNING - The Python registry does not have an 'InstallPath' setting")
        print(f"          The file '{filename}' can not be located")
        return None
    fname = os.path.join(path, f"Tools\\Scripts\\{filename}")
    try:
        os.stat(fname)
    except OSError:
