...
def LoadToolMenuItems():
    ...
    lookNo = 1
    while 1:
        menu = win32ui.GetProfileVal(f"Tools Menu\\{lookNo}", "", "")
        if menu == "":
            break
        cmd = win32ui.GetProfileVal(f"Tools Menu\\{lookNo}", "Command", "")
...
def WriteToolMenuItems(items):
    ...
    for menu, cmd in items:
        win32ui.WriteProfileVal(f"Tools Menu\\{itemNo}", "", menu)
        win32ui.WriteProfileVal(f"Tools Menu\\{itemNo}", "Command", cmd)
        itemNo += 1
...
def HandleToolCommand(cmd, code):
    ...
    global tools
    (_menuString, pyCmd, desc) = tools[cmd]
    ...
    text = "Completed successfully." if worked else f"Error executing {desc}."
