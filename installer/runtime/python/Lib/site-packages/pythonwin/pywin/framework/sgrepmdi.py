            ...
            if os.path.isdir(d):
                ...
                if d in os.environ:
                    ...
                    try:
                        ...
                        win32ui.MessageBox(
                            f"Can't interpret registry key name '{keystr[0]}'"
                        )
                    try:
                        ...
                        if val:
                            ...
                            win32ui.MessageBox(
                                f"Registry path '{d}' did not return a path entry"
                            )
                    except:
                        win32ui.MessageBox(
                            f"Can't interpret registry key value: {keystr[1:]}"
                        )
                else:
                    win32ui.MessageBox(f"Directory '{d}' not found")
                if x:
                    for xd in x:
                        if xd not in dirs:
                            dirs[xd] = None
    ...
    def doSearch(self):
        ...
        if not self.dp:
            self.GetFirstView().Append(
                f"# ERROR: '{self.dirpattern}' does not resolve to any search locations"
            )
