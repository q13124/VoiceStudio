...
class DebuggerListViewWindow(DebuggerWindow):
    ...
    def OnItemRightClick(self, notify_data, extra):
        ...
        flags, hItem, _subitem = self.HitTest(pt)
        ...
        hook_parent = win32ui.GetMainFrame() if dockbar.IsFloating() else self.GetParentFrame()
...
class Debugger(debugger_parent):
    ...
    def close(self, frameShutdown=0):
        ...
        for id, _klass, _float in DebuggerDialogInfos:
            try:
    ...
    def runexec(self, what, globs=None, locs=None):
        ...
        sys.settrace(self.trace_dispatch)
        try:
            with contextlib.suppress(bdb.BdbQuit):
    ...
    def set_cur_frame(self, frame):
        ...
        for f, index in self.stack:
            ...
            raise AssertionError("Can't find the frame in the stack.")
    ...
    def GetDebuggerBar(self, barName):
        frame = win32ui.GetMainFrame()
        for id, klass, _float in DebuggerDialogInfos:
            if klass.title == barName:
                return frame.GetControlBar(id)
        raise AssertionError("Can't find a bar of that name!")
