...
class RegistryValueView(docview.ListView):
    ...
    def EditValue(self, item):
        ...
        keyVal = "" if index == 0 else self.GetItemText(index, 0)
        ...
        if d.DoModal() == win32con.IDOK:
            try:
                ...
                win32ui.MessageBox(f"Error setting value\r\n\n{exc.strerror}")
...
class HLIRegistryKey(hierlist.HierListItem):
    ...
    def __repr__(self):
        return f"<{self.__class__.__name__} with root={self.keyRoot}, key={self.keyName}>"
...
def EditRegistry(root=None, key=None):
    template.OpenRegistryKey(root, key)
