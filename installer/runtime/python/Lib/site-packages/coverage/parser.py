...
class PythonParser:
    ...
    def _finish_action_msg(self, action_msg: str | None, end: TLineNo) -> str:
        ...
        if action_msg is None:
            action_msg = "jump to the function exit" if end < 0 else "jump to line {lineno}"
...
class AstArcAnalyzer:
    ...
    def node_exits(self, node: ast.AST) -> set[ArcStart]:
        ...
        if handler is not None:
            ...
            if env.TESTING and node_name not in self.OK_TO_DEFAULT:
    ...
    def process_body(
        ...
        if prev_starts is None:
            prev_starts = set() if from_start is None else {from_start}
        else:
    ...
    def _handle__Try(self, node: ast.Try) -> set[ArcStart]:
        handler_start = self.line_for_node(node.handlers[0]) if node.handlers else None

        final_start = self.line_for_node(node.finalbody[0]) if node.finalbody else None
