...
def handle_special_build(modname, pyxfilename):
    ...
    if os.path.exists(special_build):
        ...
        if make_setup_args:
            ...
            assert isinstance(setup_args,dict), (f"make_setup_args in {special_build} did not return a dict")
        assert ext or setup_args, (f"neither make_ext nor make_setup_args {special_build}")
...
class PyxImportLoader(ExtensionFileLoader):
    ...
    def create_module(self, spec):
        try:
            ...
            if pyxargs.load_py_module_on_import_failure and spec.origin.endswith(PY_EXT):
                ...
                tb = sys.exc_info()[2]
                import traceback
                exc = ImportError(f"Building module {spec.name} failed: {traceback.format_exception_only(*sys.exc_info()[:2])}")
    ...
    def exec_module(self, module):
        try:
            ...
            import traceback
            _debug(f"Failed to load extension module: {failure_exc!r}")
            raise ImportError(f"Executing module {module.__file__} failed {traceback.format_exception_only(*sys.exc_info()[:2])}")
