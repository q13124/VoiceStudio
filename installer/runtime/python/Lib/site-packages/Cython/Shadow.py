...
def c_compile_guard(_):
    return _EmptyDecoratorAndManager()

def exceptval(_=None, check=True):
    return _EmptyDecoratorAndManager()
...
class StructType(CythonType):
    ...
    def __init__(self, *posargs, **data):
        ...
        if data and len(data) == 1 and 'cast_from' in data:
            ...
            for key, arg in zip(self._members, posargs, strict=False):
                setattr(self, key, arg)
            return
        else:
            for key, value in data.items():
                if key not in self._members:
                    raise ValueError(f"Invalid struct attribute for {self.__class__.__name__}: {key}")
...
class CythonDotImportedFromElsewhere:
    ...
    def __getattr__(self, attr):
        ...
        try:
            ...
            # you can't use them
            raise AttributeError(f"{attr}: the standard library module {self.__name__} is not available")
