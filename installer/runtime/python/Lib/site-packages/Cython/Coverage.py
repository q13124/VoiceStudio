...
from coverage.plugin import CoveragePlugin, FileReporter, FileTracer  # requires coverage.py 4.0+
...
from .Utils import find_root_package_dir, is_cython_generated_file, is_package_dir, open_source_file
...
def _find_dep_file_path(main_file, file_path, relative_path_search=False):
    ...
    if not os.path.exists(abs_path) and (file_path.endswith('.pxi') or
        ...
        matching_paths = zip(reversed(abs_no_ext.split(os.sep)), reversed(file_no_ext.split(os.sep)), strict=False)
