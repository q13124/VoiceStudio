...
class DeclarationWriter(TreeVisitor):
    ...
    def visit_CDefExternNode(self, node):
        file = '*' if node.include_file is None else f'"{node.include_file}"'
    ...
    def visit_CStructOrUnionDefNode(self, node):
        decl = 'ctypedef ' if node.typedef_flag else 'cdef '
    ...
    def visit_CppClassNode(self, node):
        extras = ""
        if node.templates:
            extras = "[{}]".format(", ".join(node.templates))
        if node.base_classes:
            extras += "({})".format(", ".join(node.base_classes))
    ...
    def visit_FromCImportStatNode(self, node):
        ...
        self.put(" cimport ")
        first = True
        for _pos, name, as_name, kind in node.imported_names:
            assert kind is None
            if first:
                first = False
            else:
