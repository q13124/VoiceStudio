...
def make_command_file(path_to_debug_info, prefix_code='',
    ...
    if not no_import:
        ...
        if not debug_files:
            sys.exit(f'No cython debug files were found in {os.path.abspath(path_to_debug_info)}. Aborting.')
...
def main():
    ...
    if options.build_dir is None and gdb_argv and os.path.isdir(gdb_argv[0]):
        ...
        warnings.warn(f'Using deprecated positional parameter to find build directory. Use "--build-dir {path_to_debug_info}" argument instead.', stacklevel=2)
