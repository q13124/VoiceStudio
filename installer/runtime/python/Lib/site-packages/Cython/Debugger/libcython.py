...
_data_types = {"CObject": CObject, "PythonObject": PythonObject}
...
def simple_repr(self, renamed=None, state=True):
    ...
    def names_and_values(prefix, attrs, args=None):
        for attr, arg in zip(attrs, args or attrs, strict=False):
...
class CythonBase:
    ...
    def print_stackframe(self, frame, index, is_c=False):
        ...
        if not is_c and self.is_python_function(frame):
            ...
            cyfunc = self.get_cython_function(frame)
            def f(arg):
                return self.cy.cy_cvalue.invoke(arg, frame=frame)
        ...
        with contextlib.suppress(gdb.GdbError):
    ...
    def print_gdb_value(self, name, value, max_name_length=None, prefix=''):
        typename = '' if libpython.pretty_printer_lookup(value) else f'({value.type}) '
    ...
    def _get_source(self, start, stop, lex_source, mark_line, lex_entire):
        with open(self.filename) as f:
            ...
            for idx, line in enumerate(slice):
                prefix = '>' if start + idx == mark_line else ' '
...
class CyBreak(CythonCommand):
    ...
    def complete(self, text, word):
        ...
        all_names = itertools.chain(qnames, names) if parameters.complete_unqualified else qnames
...
class CyStep(CythonExecutionControlCommand, libpython.PythonStepperMixin):
    ...
    def invoke(self, args, from_tty):
        if self.is_python_function():
            ...
            command = 'step' if self.stepinto else 'next'
...
class CyLocals(CythonCommand):
    ...
    def invoke(self, args, from_tty):
        ...
        for _name, cyvar in sorted(local_cython_vars.items(), key=sortkey):
...
class CyExec(CythonCommand, libpython.PyExec, EvaluateOrExecuteCodeMixin):
    ...
    def invoke(self, expr, from_tty):
        expr, _input_type = self.readcode(expr)
        executor = libpython.PythonCodeExecutor()
