...
def test_gdb():
    ...
    if not have_gdb:
        warnings.warn('Skipping gdb tests, need gdb >= 7.2 with Python >= 2.7', stacklevel=2)
...
class GdbDebuggerTestCase(DebuggerTestCase):
    ...
    def setUp(self):
        ...
        code = textwrap.dedent(f'''\
            ...
            test_libcython_in_gdb.main(version={sys.version_info[:2]!r})
    ...
    def tearDown(self):
        ...
        try:
            super().tearDown()
            if self.p:
                with contextlib.suppress(BaseException): self.p.stdout.close()
                with contextlib.suppress(BaseException): self.p.stderr.close()
                self.p.wait()
        finally:
