...
try:
    ...
    def os_fsencode(filename):
        ...
        for char in filename:
            ...
            byte = chr(ord(char) - 56320) if 56448 <= ord(char) <= 56575 else char.encode(encoding)
            encoded.append(byte)
        return ''.join(encoded)
...
class HeapTypeObjectPtr(PyObjectPtr):
    ...
    def proxyval(self, visited):
        ...
        attr_dict = pyop_attr_dict.proxyval(visited) if pyop_attr_dict else {}
...
class BuiltInMethodProxy:
    ...
    def __repr__(self):
        return (f'<built-in method {self.ml_name} of {self.pyop_m_self.safe_tp_name()} object at remote 0x{self.pyop_m_self.as_address():x}>'
                )
...
class PyCodeObjectPtr(PyObjectPtr):
    ...
    def addr2line(self, addrq):
        ...
        addr = 0
        for addr_incr, line_incr in zip(co_lnotab[::2], co_lnotab[1::2], strict=False):
            addr += ord(addr_incr)
            if addr > addrq:
                return lineno
...
class PyFrameObjectPtr(PyObjectPtr):
    ...
    def write_repr(self, out, visited):
        ...
        out.write(f'Frame 0x{self.as_address():x}, for file {self.co_filename.proxyval(visited)}, line {lineno}, in {self.co_name.proxyval(visited)} (')
        first = True
        for pyop_name, pyop_value in self.iter_locals():
            if not first:
                out.write(', ')
    ...
    def print_traceback(self):
        ...
        lineno = self.current_line_num()
        lineno = str(lineno) if lineno is not None else "?"
        sys.stdout.write(f'  File "{self.co_filename.proxyval(visited)}", line {lineno}, in {self.co_name.proxyval(visited)}\n')
...
class Frame:
    ...
    def is_python_frame(self):
        ...
        if self.is_evalframe():
            return True
        return bool(self.is_other_python_frame())
...
class PyLocals(gdb.Command):
    ...
    def invoke(self, args, from_tty):
        ...
        for pyop_name, pyop_value in pyop_frame.iter_locals():
            print(f'{pyop_name.proxyval(set())} = {pyop_value.get_truncated_repr(MAX_OUTPUT_LEN)}')
        ...
        if is_evalframeex(frame):
            ...
            if pyframe is None:
                warnings.warn("Use a Python debug build, Python breakpoints "
                              "won't work otherwise.", stacklevel=2)
                return None
...
class PyModEquals(PyNameEquals):
    ...
    def invoke(self, modname):
        ...
        if attr is not None:
            filename, _ext = os.path.splitext(os.path.basename(attr))
...
class PyBreak(gdb.Command):
    ...
    def invoke(self, funcname, from_tty):
        if '.' in funcname:
            modname, _dot, funcname = funcname.rpartition('.')
...
class ExecutionControlCommandBase(gdb.Command):
    ...
    def step(self, stepinto, stepover_command='next'):
        ...
        while True:
            ...
            if m and is_relevant_function and m.group(1) in breakpoint_list:
...
class PythonCodeExecutor:
    ...
    def alloc_pystring(self, string):
        ...
        try:
            ...
            # Python 3
            PyString_FromStringAndSize = (f'PyUnicode{get_inferior_unicode_postfix()}_FromStringAndSize')
...
if hasattr(gdb, 'GdbError'):
    ...
    warnings.warn("Use gdb 7.2 or higher to use the py-exec command.", stacklevel=2)
