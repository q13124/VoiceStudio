...
from . import Builtin, Future, Naming, Nodes, PyrexTypes, StringEncoding, Symtab, TypeSlots
...
def find_coercion_error(type_tuple, default, env):
    ...
    if err is None:
        ...
        if type_tuple[1].is_pyobject or env.directives['c_string_encoding'] in ('ascii', 'utf8'):
            return default
        else:
            return f"'{type_tuple[0].name}' objects do not support coercion to C types with non-ascii or non-utf8 c_string_encoding"
    else:
        return err
...
def get_exception_handler(exception_value):
    if exception_value is None:
        ...
        return (
            ...
            f'PyErr_SetString((PyObject*){exception_value.entry.cname}, exn.what());'
            f'}} catch(...) {{ PyErr_SetNone((PyObject*){exception_value.entry.cname}); }}',
            False)
    else:
        return (
            f'{exception_value.entry.cname}(); if (!PyErr_Occurred())'
...
def translate_double_cpp_exception(code, pos, lhs_type, lhs_code, rhs_code, lhs_exc_val, assign_exc_val, nogil):
    ...
    code.putln("try {")
    code.putln(lhs_type.declaration_code(f"__pyx_local_lvalue = {lhs_code};"))
    maybe_check_py_error(code, lhc_check_py_exc, pos, nogil)
    code.putln("try {")
    code.putln(f"__pyx_local_lvalue = {rhs_code};")
...
class ExprNode(Node):
    ...
    def compile_time_value_error(self, e):
        error(self.pos, f"Error in compile-time expression: {e.__class__.__name__}: {e}")
...
#        self.release_subexpr_temps(env)
    ...
    def release_temp_result(self, code):
        if not self.temp_code:
            ...
            pos = (os.path.basename(self.pos[0].get_description()), *self.pos[1:]) if self.pos else '(?)'
            if self.old_temp:
                raise RuntimeError(f"temp {self.old_temp} released multiple times in {self.__class__.__name__} at {pos!r}")
            else:
                raise RuntimeError(f"no temp, but release requested in {self.__class__.__name__} at {pos!r}")
    ...
    def generate_post_assignment_code(self, code):
        if self.is_temp:
            if self.type.is_string or self.type.is_pyunicode_ptr:
                ...
                code.putln(f"{self.result()} = 0;")
            elif self.type.is_memoryviewslice:
                code.putln(f"{self.result()}.memview = NULL;")
                code.putln(f"{self.result()}.data = NULL;")
    ...
    def coerce_to(self, dst_type, env):
        ...
        if dst_type.is_memoryviewslice:
            ...
            if used_as_reference and dst_type.is_cpp_class:
                warning(
                    self.pos,
                    f"Cannot pass Python object as C++ data structure reference ({dst_type} &), will pass by copy.")
    ...
    def coerce_to_boolean(self, env):
        ...
        type = self.type
        if type.is_enum or type.is_error or type is PyrexTypes.c_bint_type:
            return self
            ...
            error(self.pos, f"Type '{type}' not acceptable as a boolean")
            return self
    ...
    def from_node(cls, node, **kwargs):
        ...
        """
        attributes = ["cf_state", "cf_maybe_null", "cf_is_null", "constant_result"]
...
class NoneNode(PyConstNode):
    ...
    def coerce_to(self, dst_type, env):
        if not (dst_type.is_pyobject or dst_type.is_memoryviewslice or dst_type.is_error):
            ...
            error(self.pos, f"Cannot assign None to {dst_type}")
...
class CharNode(ConstNode):
    ...
    def calculate_result_code(self):
        return f"'{StringEncoding.escape_char(self.value)}'"
...
class FloatNode(ConstNode):
    ...
    def compile_time_value(self, denv):
        ...
        str_float_value = (f"{float_value:.330f}").strip('0')
        str_value = Utils.normalise_float_repr(self.value)
        if str_value not in (str_float_value, repr(float_value).lstrip('0')):
            warning(self.pos, f"Using this floating point value with DEF may lose precision, using {float_value!r}")
        return float_value
...
def _analyse_name_as_type(name, pos, env):
    ...
    with local_errors(ignore=True):
        ...
        try:
            declaration = TreeFragment(f"sizeof({name})", name=pos[0].filename, initial_pos=pos)
        except CompileError:
            pass
        else:
    ...
    def generate_evaluation_code(self, code):
        if self.type.is_pyobject:
            if StringEncoding.string_contains_lone_surrogates(self.value):
                ...
                const_code.putln(
                    f"{self.result_code} = PyUnicode_DecodeUnicodeEscape({data_cname}, sizeof({data_cname}) - 1, NULL); {const_code.error_goto_if_null(self.result_code, self.pos)}")
                const_code.put_error_if_neg(
                    self.pos, f"__Pyx_PyUnicode_READY({self.result_code})")
...
class ImagNode(AtomicExprNode):
    ...
    def generate_result_code(self, code):
        if self.type.is_pyobject:
            code.putln(
                f"{self.result()} = PyComplex_FromDoubles(0.0, {float(self.value)!r}); {code.error_goto_if_null(self.result(), self.pos)}")
...
class NameNode(AtomicExprNode):
    ...
    def compile_time_value(self, denv):
        try:
            ...
            error(self.pos, f"Compile-time name '{self.name}' not defined")
    ...
    def _analyse_target_declaration(self, env, is_assignment_expression):
        ...
        if not self.entry:
            if env.directives['warn.undeclared']:
                warning(self.pos, f"implicit declaration of '{self.name}'", 1)
            type = unspecified_type if env.directives['infer_types'] else py_object_type
            if is_assignment_expression:
    ...
    def analyse_target_types(self, env):
        ...
        if not self.is_lvalue():
            error(self.pos, f"Assignment to non-lvalue '{self.name}'")
            self.type = PyrexTypes.error_type
        entry.used = 1
        if entry.type.is_buffer:
            from . import Buffer
    ...
    def check_identifier_kind(self):
        ...
        if entry.is_type and (entry.type.is_enum or entry.type.is_cpp_enum) and entry.create_wrapper:
            ...
            if self.entry.as_variable:
                ...
                error(self.pos, f"'{self.name}' is not a constant, variable or function identifier")
    ...
    def generate_result_code(self, code):
        ...
        if entry.is_builtin and entry.is_const:
            ...
            if not self.cf_is_null:
                code.putln(
                    f'{self.result()} = PyObject_GetItem({namespace}, {interned_cname});')
                code.putln(f'if (unlikely(!{self.result()})) {{')
                code.putln('PyErr_Clear();')
            code.globalstate.use_utility_code(
                UtilityCode.load_cached("GetModuleGlobalName", "ObjectHandling.c"))
            code.putln(
                f'__Pyx_GetModuleGlobalName({self.result()}, {interned_cname});')
            if not self.cf_is_null:
                code.putln("}")
            ...
            code.putln(
                f'{self.result()} = __Pyx_GetBuiltinName({interned_cname}); {code.error_goto_if_null(self.result(), self.pos)}')
            self.generate_gotref(code)
            ...
            if entry.scope.is_module_scope:
                code.globalstate.use_utility_code(
                    UtilityCode.load_cached("GetModuleGlobalName", "ObjectHandling.c"))
                code.putln(
                    f'__Pyx_GetModuleGlobalName({self.result()}, {interned_cname}); {code.error_goto_if_null(self.result(), self.pos)}')
            else:
    ...
    def generate_assignment_code(self, rhs, code, overloaded_assignment=False,
        ...
        if entry.is_pyglobal:
            ...
            if entry.is_member:
                ...
                raise AssertionError(repr(entry))
            ...
            code.put_error_if_neg(
                self.pos,
                f'{setter}({namespace}, {interned_cname}, {rhs.py_result()})')
            if debug_disposal_code:
                print("NameNode.generate_assignment_code:")
                print(f"...generating disposal code for {rhs}")
            ...
            if not self.type.is_memoryviewslice:
                ...
                if debug_disposal_code:
                    print("NameNode.generate_assignment_code:")
                    print(f"...generating post-assignment code for {rhs}")
    ...
    def generate_deletion_code(self, code, ignore_nonexisting=False):
        if self.entry is None:
            ...
            if ignore_nonexisting:
                ...
                key_error_code = (
                    f'{{ PyErr_Clear(); PyErr_Format(PyExc_NameError, "name \'%s\' is not defined", "{self.entry.name}"); }}')
            code.putln(
                f'if (unlikely(PyObject_DelItem({namespace}, {interned_cname}) < 0)) {{'
                f' if (likely(PyErr_ExceptionMatches(PyExc_KeyError))) {key_error_code}'
            ...
            interned_cname = code.intern_identifier(self.entry.name)
            del_code = f'__Pyx_PyObject_DelAttrStr({Naming.module_cname}, {interned_cname})'
            if ignore_nonexisting:
                code.putln(
                    f'if (unlikely({del_code} < 0)) {{'
                    f' if (likely(PyErr_ExceptionMatches(PyExc_AttributeError))) PyErr_Clear(); else {code.error_goto(self.pos)} '
                    '}')
            else:
...
class BackquoteNode(ExprNode):
    ...
    def generate_result_code(self, code):
        code.putln(
            f"{self.result()} = PyObject_Repr({self.arg.py_result()}); {code.error_goto_if_null(self.result(), self.pos)}")
        self.generate_gotref(code)
...
class ScopedExprNode(ExprNode):
    ...
    def generate_evaluation_code(self, code):
        ...
        for _, entry in sorted(item for item in self.expr_scope.entries.items() if item[0]):
            if not entry.in_closure and entry.type.is_pyobject and entry.used:
        ...
        for label, old_label in ([(code.error_label, old_error_label), *list(zip(code.get_loop_labels(), old_loop_labels, strict=False))]):
...
class IteratorNode(ScopedExprNode):
    ...
    def generate_result_code(self, code):
        ...
        if sequence_type.is_cpp_class:
            raise AssertionError("Should have been changed to CppIteratorNode")
        if sequence_type.is_array or sequence_type.is_ptr:
        ...
        if self.may_be_a_sequence:
            code.putln(
                f"if (likely(PyList_CheckExact({self.sequence.py_result()})) || PyTuple_CheckExact({self.sequence.py_result()})) {{")

        if is_builtin_sequence or self.may_be_a_sequence:
            code.putln(f"{self.result()} = {self.sequence.py_result()}; __Pyx_INCREF({self.result()});")
            ...
            if self.reversed:
                ...
                code.putln("#endif")
                code.putln(f"--{self.counter_cname};")  # len -> last item
            else:
                code.putln(f"{self.counter_cname} = 0;")

        if not is_builtin_sequence:
            self.iter_func_ptr = code.funcstate.allocate_temp(self._func_iternext_type, manage_ref=False)
            if self.may_be_a_sequence:
                code.putln(f"{self.iter_func_ptr} = NULL;")
                code.putln("} else {")
                code.put(f"{self.counter_cname} = -1; ")

            code.putln(f"{self.result()} = PyObject_GetIter({self.sequence.py_result()}); {code.error_goto_if_null(self.result(), self.pos)}")
            self.generate_gotref(code)
    ...
    def generate_iter_next_result_code(self, result_name, code):
        ...
        if self.may_be_a_sequence:
            code.putln(f"if (likely(!{self.iter_func_ptr})) {{")
            code.putln(f"if (likely(PyList_CheckExact({self.py_result()}))) {{")
            self.generate_next_sequence_item('List', result_name, code)
            code.putln("} else {")
            self.generate_next_sequence_item('Tuple', result_name, code)
            code.putln("}")
        ...
        code.putln(f"if (unlikely(!{result_name})) {{")
...
class CppIteratorNode(ExprNode):
    ...
    def analyse_types(self, env):
        ...
        if iter_type.is_cpp_class:
            ...
            if iter_type != end.type.return_type:
    ...
    def generate_result_code(self, code):
        ...
        if self.sequence.is_simple():
            ...
            #    of the user.
            code.putln(f"{self.result()} = {self.sequence.result()}{self.cpp_attribute_op}{begin_name}();")
            return
        ...
        code.putln(f"{self.result()} = {self.cpp_sequence_cname}{self.cpp_attribute_op}{begin_name}();")
    ...
    def generate_for_loop_header(self, code):
        ...
        code.put(f"; {self.extra_dereference}{self.result()} != {self.cpp_sequence_cname or self.sequence.result()}{self.cpp_attribute_op}{end_name}(); ++{self.extra_dereference}{self.result()}")

    def generate_iter_next_result_code(self, result_name, code):
        code.putln(f"{result_name} = *{self.extra_dereference}{self.result()};")
...
class AsyncIteratorNode(ScopedExprNode):
    ...
    def generate_result_code(self, code):
        ...
        code.putln(f"{self.result()} = __Pyx_Coroutine_GetAsyncIter({self.sequence.py_result()}); {code.error_goto_if_null(self.result(), self.pos)}")
        self.generate_gotref(code)
...
class AsyncNextNode(AtomicExprNode):
    ...
    def generate_result_code(self, code):
        code.globalstate.use_utility_code(UtilityCode.load_cached("AsyncIter", "Coroutine.c"))
        code.putln(f"{self.result()} = __Pyx_Coroutine_AsyncIterNext({self.iterator.py_result()}); {code.error_goto_if_null(self.result(), self.pos)}")
        self.generate_gotref(code)
...
class WithExitCallNode(ExprNode, Nodes.CopyWithUpTreeRefsMixin):
    ...
    def generate_evaluation_code(self, code):
        if self.test_if_run:
            ...
            code.putln(f"if ({self.with_stat.exit_var}) {{")
        ...
        code.putln(f"{result_var} = __Pyx_PyObject_Call({self.with_stat.exit_var}, {self.args.result()}, NULL);")
...
class TempNode(ExprNode):
    ...
    def result(self):
        try:
            ...
            raise AssertionError("Remember to call allocate/release on TempNode")
            raise
...
class JoinedStrNode(ExprNode):
    ...
    def generate_evaluation_code(self, code):
        ...
        if use_stack_memory:
            ...
            code.putln(f"if (unlikely(!{values_array})) {{")
            code.putln(f"PyErr_NoMemory(); {code.error_goto(self.pos)}")
            code.putln("}")
        ...
        if not use_stack_memory:
            code.putln(f"PyMem_Free({values_array});")
...
class FormattedValueNode(ExprNode):
    ...
    def generate_result_code(self, code):
        if self.c_format_spec is not None and not self.value.type.is_pyobject:
            ...
            code.putln(f"{self.result()} = {convert_func_call}; {code.error_goto_if_null(self.result(), self.pos)}")
            self.generate_gotref(code)
            return
        ...
        if conversion_char:
            ...
            assert fn is not None, f"invalid conversion character found: '{conversion_char}'"
        ...
        code.putln(f"{self.result()} = {format_func}({value_result}, {format_spec}); {code.error_goto_if_null(self.result(), self.pos)}")
        self.generate_gotref(code)
...
class ParallelThreadsAvailableNode(AtomicExprNode):
    ...
    def generate_result_code(self, code):
        ...
        code.putln(f"if (omp_in_parallel()) {self.temp_code} = omp_get_max_threads();")
        code.putln(f"else {self.temp_code} = omp_get_num_threads();")
        code.putln("#else")
        code.putln(f"{self.temp_code} = 1;")
        code.putln("#endif")
...
class ParallelThreadIdNode(AtomicExprNode):  #, Nodes.ParallelNode):
    ...
    def generate_result_code(self, code):
        code.putln("#ifdef _OPENMP")
        code.putln(f"{self.temp_code} = omp_get_thread_num();")
        code.putln("#else")
        code.putln(f"{self.temp_code} = 0;")
        code.putln("#endif")
...
class IndexNode(_IndexingBaseNode):
    ...
    def analyse_target_types(self, env):
        ...
        if node is self and not node.is_lvalue():
            error(self.pos, f"Assignment to non-lvalue of type '{node.type}'")
        return node
    ...
    def analyse_base_and_index_types(self, env, getting=False, setting=False):
        ...
        if base_type.is_pyobject:
            ...
            error(self.pos,
                  f"Attempting to index non-array type '{base_type}'")
            self.type = PyrexTypes.error_type
            return self
    ...
    def analyse_as_c_array(self, env, is_slice):
        ...
        if is_slice:
            ...
            error(self.pos, f"Invalid index type '{self.index.type}'")
        return self
    ...
    def analyse_as_cpp(self, env, setting):
        ...
        if setting and not func_type.return_type.is_reference:
            error(self.pos, f"Can't set non-reference result '{self.type}'")
        return self
    ...
    def analyse_as_c_function(self, env):
        ...
        if base_type.is_fused:
            ...
            if base_type.templates is None:
                ...
                error(self.pos, f"Wrong number of template arguments: expected {len(base_type.templates)}, got {len(self.type_indices)}")
                self.type = error_type
            else:
                self.type = base_type.specialize(dict(zip(base_type.templates, self.type_indices, strict=False)))
    ...
    def analyse_as_c_tuple(self, env, getting, setting):
        ...
        if isinstance(self.index, IntNode) and self.index.has_constant_result():
            ...
            if -base_type.size <= index < base_type.size:
                ...
                error(self.pos,
                      f"Index {index} out of bounds for '{base_type}'")
                self.type = PyrexTypes.error_type
            return self
        else:
            self.base = self.base.coerce_to_pyobject(env)
    ...
    def analyse_as_buffer_operation(self, env, getting):
        ...
        """
        indices = self.index.args if isinstance(self.index, TupleNode) else [self.index]
        ...
        if base_type.is_memoryviewslice:
            ...
            have_slices, indices, _newaxes = MemoryView.unellipsify(indices, base_type.ndim)
            if have_slices:
    ...
    def parse_indexed_fused_cdef(self, env):
        ...
        if len(specific_types) > len(fused_types):
            ...
            return error(self.pos, "Not enough types specified to specialize "
                                   f"the function, {t} is still fused")
        ...
        fused_to_specific = dict(zip(fused_types, specific_types, strict=False))
    ...
    def calculate_result_code(self):
        if self.base.type in (list_type, tuple_type, bytearray_type):
            ...
            if self.base.type is list_type:
                ...
                raise AssertionError(f"unexpected base type in indexing: {self.base.type}")
    ...
    def generate_result_code(self, code):
        ...
        if self.type.is_pyobject:
            ...
            raise AssertionError(f"unexpected type {self.type} and base type {base_type} for indexing ({self.pos})")

        if utility_code is not None:
            code.globalstate.use_utility_code(utility_code)

        index_code = self.index.result() if self.index.type.is_int else self.index.py_result()

        if base_type.is_cpp_class and self.exception_check:
            translate_cpp_exception(code, self.pos,
                f"{self.result()} = {self.base.result()}[{self.index.result()}];",
                self.result() if self.type.is_pyobject else None,
                self.exception_value, self.in_nogil_context)
        else:
            error_check = '!%s' if error_value == 'NULL' else f'%s == {error_value}'
            code.putln(
                f"{self.result()} = {function}({self.base.py_result()}, {index_code}{self.extra_index_params(code)}); {code.error_goto_if(error_check % self.result(), self.pos)}")
        if self.type.is_pyobject:
            self.generate_gotref(code)
    ...
    def generate_setitem_code(self, value_code, code):
        ...
        code.putln(code.error_goto_if_neg(
            f"{function}({self.base.py_result()}, {index_code}, {value_code}{self.extra_index_params(code)})",
            self.pos))
    ...
    def _check_byte_value(self, code, rhs):
        ...
        if rhs.has_constant_result():
            ...
            warning(rhs.pos,
                    ...
                    f" when assigning to byte: {rhs.constant_result}",
                    level=1)
        else:
        ...
        if not self.nogil:
            ...
            if rhs.is_literal or rhs.type.signed:
                conditions.append(f'{value_code} < 0')
            if (rhs.is_literal or not
                ...
                conditions.append(f'{value_code} > 255')
            if conditions:
                code.putln("if (unlikely({})) {{".format(' || '.join(conditions)))
                code.putln(
                    'PyErr_SetString(PyExc_ValueError,'
                    f' "byte must be in range(0, 256)"); {code.error_goto(self.pos)}')
                code.putln("}")

        if needs_cast:
            value_code = f'((unsigned char){value_code})'
        return value_code
    ...
    def generate_deletion_code(self, code, ignore_nonexisting=False):
        ...
        if self.index.type.is_int:
            ...
            index_code = self.index.py_result()
            function = "PyDict_DelItem" if self.base.type is dict_type else "PyObject_DelItem"
        code.putln(code.error_goto_if_neg(
            f"{function}({self.base.py_result()}, {index_code}{self.extra_index_params(code)})",
            self.pos))
        self.generate_subexpr_disposal_code(code)
        self.free_subexpr_temps(code)
...
class BufferIndexNode(_IndexingBaseNode):
    ...
    def nogil_check(self, env):
        if (self.is_buffer_access or self.is_memview_index) and self.type.is_pyobject:
            error(self.pos, "Cannot access buffer with object dtype without gil")
            self.type = error_type

    def calculate_result_code(self):
        return f"(*{self.buffer_ptr_code})"
    ...
    def generate_buffer_setitem_code(self, rhs, code, op=""):
        ...
        if is_pythran_expr(base_type) and is_pythran_supported_type(rhs.type):
            ...
            # case.
            code.putln(f"__Pyx_call_destructor({obj});")
            code.putln(f"new (&{obj}) decltype({obj}){{{self.base.pythran_result()}}};")
            code.putln(f"{obj}{pythran_indexing_code(self.indices)} {op}= {rhs.pythran_result()};")
        ...
        if self.buffer_type.dtype.is_pyobject:
            ...
            code.put_xgotref(f"*{ptr}", self.buffer_type.dtype)
            code.putln(f"__Pyx_INCREF({rhs_code}); __Pyx_XDECREF(*{ptr});")
            code.putln(f"*{ptr} {op}= {rhs_code};")
            code.put_xgiveref(f"*{ptr}", self.buffer_type.dtype)
    ...
    def generate_result_code(self, code):
        if is_pythran_expr(self.base.type):
            res = self.result()
            code.putln(f"__Pyx_call_destructor({res});")
            code.putln(f"new (&{res}) decltype({res}){{{self.base.pythran_result()}{pythran_indexing_code(self.indices)}}};")
            return
        _buffer_entry, self.buffer_ptr_code = self.buffer_lookup_code(code)
        if self.type.is_pyobject:
            ...
            code.putln(f"__Pyx_INCREF((PyObject*){res});")
...
class MemoryViewIndexNode(BufferIndexNode):
    ...
    def analyse_types(self, env, getting=True):
        ...
        _have_slices, indices, newaxes = MemoryView.unellipsify(indices, self.base.type.ndim)
        ...
        if len(indices) - len(newaxes) > self.base.type.ndim:
            ...
            error(indices[self.base.type.ndim].pos,
                  f"Too many indices specified for type {self.base.type}")
            return self
        ...
        for i, index in enumerate(indices):
            ...
            if index.is_slice:
                ...
                self.type = error_type
                error(index.pos, f"Invalid index for memoryview specified, type {index.type}")
                return self
...
class MemoryCopyScalar(MemoryCopyNode):
    ...
    def _generate_assignment_code(self, scalar, code):
        ...
        if dtype.is_pyobject:
            code.putln(f"Py_DECREF(*(PyObject **) {p});")
...
class SliceIndexNode(ExprNode):
    ...
    def calculate_constant_result(self):
        start = None if self.start is None else self.start.constant_result
        stop = None if self.stop is None else self.stop.constant_result
        self.constant_result = self.base.constant_result[start:stop]

    def compile_time_value(self, denv):
        base = self.base.compile_time_value(denv)
        start = 0 if self.start is None else self.start.compile_time_value(denv)
        stop = None if self.stop is None else self.stop.compile_time_value(denv)
        try:
    ...
    def analyse_as_type(self, env):
        base_type = self.base.analyse_as_type(env)
        if base_type and not self.start and not self.stop:
            # memory view
            from . import MemoryView
            env.use_utility_code(
                MemoryView.get_view_utility_code(
    ...
    def coerce_to(self, dst_type, env):
        if ((self.base.type.is_string or self.base.type.is_cpp_string)
            ...
            if (dst_type is unicode_type and not env.directives['c_string_encoding']):
                error(self.pos,
                    f"default encoding required for conversion from '{self.base.type}' to '{dst_type}'")
            self.type = dst_type
    ...
    def generate_result_code(self, code):
        if not self.type.is_pyobject:
            error(self.pos,
                  f"Slicing is not currently supported for '{self.type}'.")
            return
    ...
    def generate_assignment_code(self, rhs, code, overloaded_assignment=False,
        ...
        if self.type.is_pyobject:
            ...
            code.putln(f"memcpy(&({self.base.result()}[{start_offset}]), {rhs.result()}, sizeof({self.base.result()}[0]) * ({array_length}));")
    ...
    def generate_deletion_code(self, code, ignore_nonexisting=False):
        if not self.base.type.is_pyobject:
            error(self.pos,
                  f"Deleting slices is only supported for Python types, not '{self.type}'.")
            return
        self.generate_subexpr_evaluation_code(code)
        code.globalstate.use_utility_code(self.set_slice_utility_code)
    ...
    def get_slice_config(self):
        ...
        if self.start:
            ...
            if has_c_start:
                ...
                py_start = f'&{self.start.py_result()}'
        ...
        if self.stop:
            ...
            if has_c_stop:
                ...
                py_stop = f'&{self.stop.py_result()}'
        py_slice = (self.slice and f'&{self.slice.py_result()}') or 'NULL'
    ...
    def generate_slice_guard_code(self, code, target_size):
        ...
        if compile_time_check and slice_size < 0:
            ...
            if int_target_size != slice_size:
                error(self.pos, f"Assignment to slice of wrong length, expected {slice_size}, got {target_size}")
        ...
        if runtime_check:
            ...
            code.putln(
                ...
                f' (Py_ssize_t)({target_size}), (Py_ssize_t)({runtime_check}));')
            if self.nogil:
                code.put_release_ensured_gil()
...
class SliceNode(ExprNode):
    ...
    def generate_result_code(self, code):
        ...
        code.putln(
            f"{self.result()} = PySlice_New({self.start.py_result()}, {self.stop.py_result()}, {self.step.py_result()}); {code.error_goto_if_null(self.result(), self.pos)}")
        self.generate_gotref(code)
        if self.is_literal:
...
class CallNode(ExprNode):
    ...
    def analyse_as_type_constructor(self, env):
        ...
        if type and type.is_struct_or_union:
            ...
            items = []
            for arg, member in zip(args, type.scope.var_entries, strict=False):
            ...
            if not constructor:
                error(self.function.pos, f"no constructor found for C++  type '{self.function.name}'")
                self.type = error_type
                return self
    ...
    def nogil_check(self, env):
        ...
        if func_type.is_pyobject or (not func_type.is_error and not getattr(func_type, 'nogil', False)):
            self.gil_error()
...
class SimpleCallNode(CallNode):
    ...
    def analyse_c_function_call(self, env):
        ...
        if overloaded_entry:
            ...
            if not func_type.is_cfunction:
                error(self.pos, f"Calling non-function type '{func_type}'")
                self.type = PyrexTypes.error_type
                self.result_code = "<error>"
                return
        ...
        if self.type.is_pyobject:
            self.result_ctype = py_object_type
            self.is_temp = 1
        elif func_type.exception_value is not None or func_type.exception_check or self.type.is_memoryviewslice:
            self.is_temp = 1
    ...
    def c_call_code(self):
        ...
        arg_list_code = []
        args = list(zip(formal_args, self.args, strict=False))
        max_nargs = len(func_type.args)
        expected_nargs = max_nargs - func_type.optional_arg_count
        ...
        if func_type.optional_arg_count:
            optional_args = 'NULL' if expected_nargs == actual_nargs else f"&{self.opt_arg_struct}"
    ...
    def generate_evaluation_code(self, code):
        ...
        if self.overflowcheck and is_signed_int and function.result() in abs_function_cnames:
            ...
            code.putln(f'if (unlikely({self.args[0].result()} == __PYX_MIN({self.args[0].type.empty_declaration_code()}))) {{\
                PyErr_SetString(PyExc_OverflowError,\
                                "Trying to take the absolute value of the most negative integer is not defined."); {code.error_goto(self.pos)}; }}')
        ...
        if arg is None:
            ...
            code.putln(
                f"{self.result()} = __Pyx_PyObject_CallNoArg({function.py_result()}); {code.error_goto_if_null(self.result(), self.pos)}")
        else:
            code.globalstate.use_utility_code(UtilityCode.load_cached(
                "PyObjectCallOneArg", "ObjectHandling.c"))
            code.putln(
                f"{self.result()} = __Pyx_PyObject_CallOneArg({function.py_result()}, {arg.py_result()}); {code.error_goto_if_null(self.result(), self.pos)}")
    ...
    def generate_result_code(self, code):
        ...
        if func_type.is_pyobject:
            ...
            code.globalstate.use_utility_code(UtilityCode.load_cached(
                "PyObjectCall", "ObjectHandling.c"))
            code.putln(
                f"{self.result()} = __Pyx_PyObject_Call({self.function.py_result()}, {arg_code}, NULL); {code.error_goto_if_null(self.result(), self.pos)}")
            self.generate_gotref(code)
            ...
            if self.has_optional_args:
                ...
                args = list(zip(func_type.args, self.args, strict=False))
                for formal_arg, actual_arg in args[expected_nargs:actual_nargs]:
                    code.putln(f"{self.opt_arg_struct}.{func_type.opt_arg_cname(formal_arg.name)} = {actual_arg.result_as(formal_arg.type)};")
            exc_checks = []
            if self.type.is_pyobject and self.is_temp:
                exc_checks.append(f"!{self.result()}")
            elif self.type.is_memoryviewslice:
                assert self.is_temp
...
class NumPyMethodCallNode(ExprNode):
    ...
    def generate_evaluation_code(self, code):
        ...
        code.putln(f"__Pyx_call_destructor({self.result()});")
...
class PyMethodCallNode(CallNode):
    ...
    def generate_evaluate_function(self, code, self_arg) -> str:
        ...
        code.putln(f"{function} = {self.function.py_result()}; ")
    ...
    def generate_runtime_method_unpacking_code(self, code, self_arg, space_for_selfarg_var, method_obj):
        ...
        code.putln(f"if ({likely_method}(PyMethod_Check({method_obj}))) {{")
...
class InlinedDefNodeCallNode(CallNode):
    ...
    def generate_result_code(self, code):
        ...
        arg_code = [self_code]
        func_type = self.function.def_node
        for arg, proto_arg in zip(self.args, func_type.args, strict=False):
            if arg.type.is_pyobject:
        ...
        code.putln(
            f"{self.result()} = {self.function.def_node.entry.pyfunc_cname}({arg_code}); {code.error_goto_if_null(self.result(), self.pos)}")
        if not self.function.def_node.is_cyfunction:
            code.putln("}")
        self.generate_gotref(code)
...
class CachedBuiltinMethodCallNode(CallNode):
    ...
    def generate_result_code(self, code):
        ...
        code.putln(f"{self.result()} = {call_code}; {code.error_goto_if_null(self.result(), self.pos)}")
        self.generate_gotref(code)
...
class GeneralCallNode(CallNode):
    ...
    def map_to_simple_call_node(self):
        ...
        for decl_arg, arg in zip(unmatched_args, kwargs.key_value_pairs, strict=False):
            name = arg.key.value
            ...
            for decl_arg in unmatched_args:
                ...
                if name not in keywords:
                    ...
                    error(self.pos, "C function call is missing "
                                    f"argument '{first_missing_keyword}'")
                    return None
        ...
        for arg in kwargs.key_value_pairs:
            ...
            if name not in matched_args:
                has_errors = True
                error(arg.pos,
                      f"C function got unexpected keyword argument '{name}'")
    ...
    def generate_result_code(self, code):
        ...
        kwargs = self.keyword_args.py_result() if self.keyword_args else 'NULL'
        code.globalstate.use_utility_code(UtilityCode.load_cached(
            "PyObjectCall", "ObjectHandling.c"))
        code.putln(
            f"{self.result()} = __Pyx_PyObject_Call({self.function.py_result()}, {self.positional_args.py_result()}, {kwargs}); {code.error_goto_if_null(self.result(), self.pos)}")
        self.generate_gotref(code)
...
class AsTupleNode(ExprNode):
    ...
    def generate_result_code(self, code):
        ...
        code.putln(
            f"{self.result()} = {cfunc}({self.arg.py_result()}); {code.error_goto_if_null(self.result(), self.pos)}")
        self.generate_gotref(code)
...
class MergedDictNode(ExprNode):
        ...
        for item in self.keyword_args:
            ...
            for key, value in items:
                if reject_duplicates and key in result:
                    raise ValueError(f"duplicate keyword argument found: {key}")
                result[key] = value
    ...
    def generate_evaluation_code(self, code):
        ...
        if item.type is not dict_type:
            ...
            code.putln(f'if (likely(PyDict_CheckExact({item.py_result()}))) {{')
        ...
        if item.is_dict_literal:
            ...
            if item.result_in_temp():
                ...
                code.putln("#endif")
                code.putln("{")
            code.putln(f"{self.result()} = PyDict_Copy({item.py_result()}); {code.error_goto_if_null(self.result(), item.pos)}")
            self.generate_gotref(code)
        ...
        if item.type is not dict_type:
            ...
            code.globalstate.use_utility_code(UtilityCode.load_cached(
                "PyObjectCallOneArg", "ObjectHandling.c"))
            code.putln(f"{self.result()} = __Pyx_PyObject_CallOneArg((PyObject*)&PyDict_Type, {item.py_result()}); {code.error_goto_if_null(self.result(), self.pos)}")
            self.generate_gotref(code)
            item.generate_disposal_code(code)
            code.putln('}')
            ...
            if item.is_dict_literal:
                ...
                for arg in item.key_value_pairs:
                    ...
                    if self.reject_duplicates:
                        code.putln(f"if (unlikely(PyDict_Contains({self.result()}, {arg.key.py_result()}))) {{")
                        helpers.add("RaiseDoubleKeywords")
                        # FIXME: find out function name at runtime!
                        code.putln(f'__Pyx_RaiseDoubleKeywordsError("function", {arg.key.py_result()}); {code.error_goto(self.pos)}')
                        code.putln("}")
                    code.put_error_if_neg(arg.key.pos, f"PyDict_SetItem({self.result()}, {arg.key.py_result()}, {arg.value.py_result()})")
                    arg.generate_disposal_code(code)
                    arg.free_temps(code)
            else:
                item.generate_evaluation_code(code)
                if self.reject_duplicates:
                    ...
                    code.put_error_if_neg(item.pos, f"__Pyx_MergeKeywords({self.result()}, {item.py_result()})")
                else:
                    # simple case, just add all entries
                    helpers.add("RaiseMappingExpected")
                    code.putln(f"if (unlikely(PyDict_Update({self.result()}, {item.py_result()}) < 0)) {{")
                    code.putln("if (PyErr_ExceptionMatches(PyExc_AttributeError)) "
                               f"__Pyx_RaiseMappingExpectedError({item.py_result()});")
...
class AttributeNode(ExprNode):
    ...
    def as_cython_attribute(self):
        if (isinstance(self.obj, NameNode) and
                self.obj.is_cython_module and self.attribute != "parallel"):
    ...
    def compile_time_value(self, denv):
        ...
        if attr.startswith("__") and attr.endswith("__"):
            error(self.pos,
                  f"Invalid attribute name '{attr}' in compile-time expression")
            return None
    ...
    def analyse_target_types(self, env):
        ...
        if node.type.is_const:
            error(self.pos, f"Assignment to const attribute '{self.attribute}'")
        if not node.is_lvalue():
            error(self.pos, f"Assignment to non-lvalue of type '{self.type}'")
        return node
    ...
    def analyse_as_cimported_attribute_node(self, env, target):
        ...
        if module_scope:
            ...
            if self.is_cimported_module_without_shadow(env):
                ...
                error(self.pos, f"cimported module has no attribute '{self.attribute}'")
                return self
        return None
    ...
    def as_name_node(self, env, entry, target):
        ...
        node = node.analyse_target_types(env) if target else node.analyse_rvalue_entry(env)
        node.entry.used = 1
        return node
...
#            error(self.pos, "C method can only be called")
    ...
    def analyse_attribute(self, env, obj_type = None):
        ...
        if obj_type.is_ptr or obj_type.is_array:
            ...
            self.op = "->"
        elif obj_type.is_extension_type or obj_type.is_builtin_type or (obj_type.is_reference and obj_type.is_fake_reference):
            self.op = "->"
        else:
            self.op = "."
        if obj_type.has_attributes:
            if obj_type.attributes_known():
                ...
                error(self.pos,
                    f"Cannot select attribute of incomplete type '{obj_type}'")
                self.type = PyrexTypes.error_type
                return
            self.entry = entry
            if entry:
    ...
    def analyse_as_python_attribute(self, env, obj_type=None, immutable_obj=False):
        ...
        if not obj_type.is_pyobject and not obj_type.is_error:
            ...
            if (obj_type.is_string or obj_type.is_cpp_string
                ...
                error(self.pos,
                      f"Object of type '{obj_type}' has no attribute '{self.attribute}'")
    ...
    def calculate_access_code(self):
        ...
        if self.entry and self.entry.is_cmethod:
            if obj.type.is_extension_type and not self.entry.is_builtin_cmethod:
                ...
                return f"((struct {obj.type.vtabstruct_cname} *){obj_code}{self.op}{obj.type.vtabslot_cname})->{self.member}"
    ...
    def generate_result_code(self, code):
        if self.is_py_attr:
            ...
            code.putln(
                f'{self.result()} = {lookup_func_name}({self.obj.py_result()}, {code.intern_identifier(self.attribute)}); {code.error_goto_if_null(self.result(), self.pos)}')
            self.generate_gotref(code)
        elif self.type.is_memoryviewslice:
            if self.is_memslice_transpose:
                # transpose the slice
                for access, _packing in self.type.axes:
                    if access == 'ptr':
                ...
                T = f"__pyx_memslice_transpose(&{self.result()})"
                code.putln(code.error_goto_if_neg(T, self.pos))
            elif self.initialized_check:
                code.putln(
                    f'if (unlikely(!{self.result()}.memview)) {{'
    ...
    def generate_assignment_code(self, rhs, code, overloaded_assignment=False,
        ...
        if self.is_py_attr:
            code.globalstate.use_utility_code(
                UtilityCode.load_cached("PyObjectSetAttrStr", "ObjectHandling.c"))
            code.put_error_if_neg(self.pos,
                f'__Pyx_PyObject_SetAttrStr({self.obj.py_result()}, {code.intern_identifier(self.attribute)}, {rhs.py_result()})')
            rhs.generate_disposal_code(code)
            rhs.free_temps(code)
        elif self.obj.type.is_complex:
            code.putln(f"__Pyx_SET_C{self.member.upper()}{self.obj.type.implementation_suffix}({self.obj.result_as(self.obj.type)}, {rhs.result_as(self.ctype())});")
            rhs.generate_disposal_code(code)
            rhs.free_temps(code)
        else:
            ...
            if not self.type.is_memoryviewslice:
                code.putln(
                    f"{select_code} = {rhs.move_result_rhs_as(self.ctype())};")
                        #rhs.result()))
            rhs.generate_post_assignment_code(code)
            rhs.free_temps(code)
    ...
    def generate_deletion_code(self, code, ignore_nonexisting=False):
        ...
        if self.is_py_attr or (self.entry.scope.is_property_scope
            ...
            code.globalstate.use_utility_code(
                UtilityCode.load_cached("PyObjectSetAttrStr", "ObjectHandling.c"))
            code.put_error_if_neg(self.pos,
                f'__Pyx_PyObject_DelAttrStr({self.obj.py_result()}, {code.intern_identifier(self.attribute)})')
        else:
...
class SequenceNode(ExprNode):
    ...
    def coerce_to_ctuple(self, dst_type, env):
        ...
        coerced_args = [arg.coerce_to(type, env) for arg, type in zip(self.args, dst_type.components, strict=False)]
    ...
    def generate_sequence_packing_code(self, code, target=None, plain=False):
        ...
        if self.mult_factor and not plain:
            ...
            if mult_factor.type.is_int:
                ...
                if (isinstance(mult_factor.constant_result, int) and
                    ...
                    size_factor = f' * {mult_factor.constant_result}'
        ...
        if ((self.type is tuple_type or self.type is list_type) and
            ...
            for i, arg in enumerate(self.args):
                code.putln(f"{target}.f{i} = {arg.result()};")
        else:
            ...
            if self.type is list_type:
                ...
                raise InternalError(f"sequence packing for unexpected type {self.type}")
            arg_count = len(self.args)
            code.putln(f"{target} = {create_func}({arg_count}{size_factor}); {code.error_goto_if_null(target, self.pos)}")
            code.put_gotref(target, py_object_type)
            ...
            if c_mult:
                ...
                code.putln(f'{{ Py_ssize_t {counter};')
                offset = counter if arg_count == 1 else f'{counter} * {arg_count}'
                code.putln(f'for ({counter}=0; {counter} < {c_mult}; {counter}++) {{')
                self.needs_subexpr_disposal = True
            else:
                offset = ''
        ...
        if mult_factor is not None and mult_factor.type.is_pyobject:
            code.putln(f'{{ PyObject* {Naming.quick_temp_cname} = PyNumber_InPlaceMultiply({target}, {mult_factor.py_result()}); {code.error_goto_if_null(Naming.quick_temp_cname, self.pos)}')
    ...
    def generate_special_parallel_unpacking_code(self, code, rhs, use_loop):
        ...
        none_check = f"likely({rhs.py_result()} != Py_None)"
        if rhs.type is list_type:
            ...
            tuple_check = f'likely(PyTuple_CheckExact({rhs.py_result()}))'
            list_check  = f'PyList_CheckExact({rhs.py_result()})'
            sequence_type_test = f"({tuple_check}) || ({list_check})"

        code.putln(f"if ({sequence_type_test}) {{")
        code.putln(f"PyObject* sequence = {rhs.py_result()};")

        # list/tuple => check size
        code.putln(f"Py_ssize_t size = {get_size_func}(sequence);")
        ...
        if len(sequence_types) == 2:
            code.putln(f"if (likely(Py{sequence_types[0]}_CheckExact(sequence))) {{")
        ...
        if not use_loop:
            ...
            code.putln("PyObject* item = __Pyx_PySequence_ITEM(sequence, i); {}".format(code.error_goto_if_null('item', self.pos)))
        ...
        if sequence_type_test == '1':
            ...
            code.putln(f"__Pyx_RaiseNoneNotIterableError(); {code.error_goto(self.pos)}")
            code.putln("}")  # all done
        else:
    ...
    def generate_generic_parallel_unpacking_code(self, code, rhs, unpacked_items, use_loop, terminate=True):
        ...
        if use_loop:
            code.putln("PyObject** temps[{}] = {{{}}};".format(
                len(self.unpacked_items),
                ','.join([f'&{item.result()}' for item in unpacked_items])))

        iterator_temp = code.funcstate.allocate_temp(py_object_type, manage_ref=True)
        code.putln(
            f"{iterator_temp} = PyObject_GetIter({rhs.py_result()}); {code.error_goto_if_null(iterator_temp, self.pos)}")
        ...
        if use_loop:
            code.putln(f"for (index=0; index < {len(unpacked_items)}; index++) {{")
            code.put(f"PyObject* item = {unpack_code}; if (unlikely(!item)) ")
        ...
        if terminate:
            ...
            code.put_error_if_neg(self.pos, "__Pyx_IternextUnpackEndCheck(%s, %d)" % (
                unpack_code,
                len(unpacked_items)))
            code.putln(f"{iternext_func} = NULL;")
    ...
    def generate_starred_assignment_code(self, rhs, code):
        ...
        if unpacked_fixed_items_right:
            ...
            code.putln(f'{length_temp} = __Pyx_PyList_GET_SIZE({target_list});')
            ...
            code.putln('#else')
            code.putln(f'CYTHON_UNUSED_VAR({sublist_temp});')
    ...
    def generate_operation_code(self, code):
        ...
        if self.is_literal or self.is_partly_literal:
            ...
            if self.is_literal:
                ...
                code.putln(f'{self.result()} = __Pyx_PySequence_Multiply({tuple_target}, {self.mult_factor.result()}); {code.error_goto_if_null(self.result(), self.pos)}')
                self.generate_gotref(code)
            else:
                code.putln(f'{self.result()} = PyNumber_Multiply({tuple_target}, {self.mult_factor.py_result()}); {code.error_goto_if_null(self.result(), self.pos)}')
                self.generate_gotref(code)
        else:
            self.type.entry.used = True
    ...
    def coerce_to(self, dst_type, env):
        if dst_type.is_pyobject:
            ...
            if not self.type.subtype_of(dst_type):
                error(self.pos, f"Cannot coerce list to type '{dst_type}'")
            ...
            if self.mult_factor:
                if isinstance(self.mult_factor.constant_result, int):
                    if self.mult_factor.constant_result <= 0:
                        error(self.pos, f"Cannot coerce non-positively multiplied list to '{dst_type}'")
                    else:
                        array_length *= self.mult_factor.constant_result
                else:
                    error(self.pos, f"Cannot coerce dynamically multiplied list to '{dst_type}'")
            ...
            error(self.pos, f"Cannot coerce multiplied list to '{dst_type}'")
        elif dst_type.is_struct:
            if len(self.args) > len(dst_type.scope.var_entries):
                error(self.pos, f"Too many members for '{dst_type}'")
            else:
                if len(self.args) < len(dst_type.scope.var_entries):
                    warning(self.pos, f"Too few members for '{dst_type}'", 1)
                for i, (arg, coerced_arg, member) in enumerate(zip(self.original_args, self.args, dst_type.scope.var_entries, strict=False)):
                    if isinstance(arg, CoerceToPyTypeNode):
                        arg = arg.arg
        ...
        if self.type.is_pyobject:
            ...
            for i, arg in enumerate(self.args):
                if arg.type.is_array:
                    code.globalstate.use_utility_code(UtilityCode.load_cached("IncludeStringH", "StringTools.c"))
                    code.putln(f"memcpy(&({self.result()}[{i}{offset}]), {arg.result()}, sizeof({self.result()}[0]));")
                else:
                    code.putln(f"{self.result()}[{i}{offset}] = {arg.result()};")
            if self.mult_factor:
                code.putln("}")
                code.putln("}")
        elif self.type.is_struct:
            for arg, member in zip(self.args, self.type.scope.var_entries, strict=False):
                code.putln(f"{self.result()}.{member.cname} = {arg.result()};")
        else:
...
class ComprehensionNode(ScopedExprNode):
    ...
    def generate_operation_code(self, code):
        if self.type is Builtin.list_type:
            ...
            raise InternalError(f"illegal type for comprehension: {self.type}")
        code.putln(f'{self.result()} = {create_code}; {code.error_goto_if_null(self.result(), self.pos)}')
...
class ComprehensionAppendNode(Node):
    ...
    def generate_execution_code(self, code):
        if self.target.type is list_type:
            ...
            raise InternalError(
                f"Invalid type for comprehension node: {self.target.type}")

        self.expr.generate_evaluation_code(code)
        code.putln(code.error_goto_if(f"{function}({self.target.result()}, (PyObject*){self.expr.result()})", self.pos))
...
class DictComprehensionAppendNode(ComprehensionAppendNode):
    ...
    def generate_execution_code(self, code):
        ...
        code.putln(code.error_goto_if(f"PyDict_SetItem({self.target.result()}, (PyObject*){self.key_expr.result()}, (PyObject*){self.value_expr.result()})", self.pos))
...
class InlinedGeneratorExpressionNode(ExprNode):
    ...
    def generate_result_code(self, code):
        code.putln(f"{self.result()} = __Pyx_Generator_GetInlinedResult({self.gen.result()}); {code.error_goto_if_null(self.result(), self.pos)}")
        self.generate_gotref(code)
        ...
        for item in args:
            if ((is_set and (item.is_set_literal or item.is_sequence_constructor)) or
                ...
                for arg in item.args:
                    arg.generate_evaluation_code(code)
                    code.put_error_if_neg(arg.pos, f"{add_func}({self.result()}, {arg.py_result()})")
                    arg.generate_disposal_code(code)
                    arg.free_temps(code)
                continue
            ...
            item.generate_evaluation_code(code)
            code.put_error_if_neg(item.pos, f"{extend_func}({self.result()}, {item.py_result()})")
            item.generate_disposal_code(code)
            item.free_temps(code)

        if self.type is tuple_type:
            code.putln("{")
            code.putln(f"PyObject *{Naming.quick_temp_cname} = PyList_AsTuple({self.result()});")
            code.put_decref(self.result(), py_object_type)
            code.putln(f"{self.result()} = {Naming.quick_temp_cname}; {code.error_goto_if_null(self.result(), self.pos)}")
            self.generate_gotref(code)
            code.putln("}")
...
class SetNode(ExprNode):
    ...
    def generate_evaluation_code(self, code):
        ...
        self.allocate_temp_result(code)
        code.putln(
            f"{self.result()} = PySet_New(0); {code.error_goto_if_null(self.result(), self.pos)}")
        self.generate_gotref(code)
        for arg in self.args:
            code.put_error_if_neg(
                self.pos,
        ...
        if dst_type.is_pyobject:
            ...
            if self.type.is_struct_or_union:
                if not dict_type.subtype_of(dst_type):
                    error(self.pos, f"Cannot interpret struct as non-dict type '{dst_type}'")
                return DictNode(self.pos, key_value_pairs=[
                    ...
                    for item in self.key_value_pairs])
            if not self.type.subtype_of(dst_type):
                error(self.pos, f"Cannot interpret dict as type '{dst_type}'")
        elif dst_type.is_struct_or_union:
            self.type = dst_type
            if not dst_type.is_struct and len(self.key_value_pairs) != 1:
                error(self.pos, f"Exactly one field must be specified to convert to union '{dst_type}'")
            elif dst_type.is_struct and len(self.key_value_pairs) < len(dst_type.scope.var_entries):
                warning(self.pos, f"Not all members given for struct '{dst_type}'", 1)
            for item in self.key_value_pairs:
        ...
        for item in self.key_value_pairs:
            ...
            if is_dict:
                if self.exclude_null_values:
                    code.putln(f'if ({item.value.py_result()}) {{')
                key = item.key
                if self.reject_duplicates:
                    ...
                    if keys_seen is None:
                        code.putln(f'if (unlikely(PyDict_Contains({self.result()}, {key.py_result()}))) {{')
                        # currently only used in function calls
                        needs_error_helper = True
                        code.putln(f'__Pyx_RaiseDoubleKeywordsError("function", {key.py_result()}); {code.error_goto(item.pos)}')
                        code.putln("} else {")

                code.put_error_if_neg(self.pos, f"PyDict_SetItem({self.result()}, {item.key.py_result()}, {item.value.py_result()})")
...
class SortedDictKeysNode(ExprNode):
    ...
    def generate_result_code(self, code):
        ...
        if self.arg.type is Builtin.dict_type:
            code.putln(f'{self.result()} = PyDict_Keys({dict_result}); {code.error_goto_if_null(self.result(), self.pos)}')
            self.generate_gotref(code)
        else:
            ...
            code.putln(f'{self.result()} = __Pyx_PyObject_CallMethod0({dict_result}, {keys_cname}); {code.error_goto_if_null(self.result(), self.pos)}')
            self.generate_gotref(code)
            code.putln(f"if (unlikely(!PyList_Check({self.result()}))) {{")
            self.generate_decref_set(code, f"PySequence_List({self.result()})")
            code.putln(code.error_goto_if_null(self.result(), self.pos))
            self.generate_gotref(code)
            code.putln("}")
        code.put_error_if_neg(
            self.pos, f'PyList_Sort({self.py_result()})')
...
class ClassNode(ExprNode, ModuleNameMixin):
    ...
    def generate_result_code(self, code):
        ...
        code.putln(
            f'{self.result()} = __Pyx_CreateClass({class_def_node.bases.py_result()}, {class_def_node.dict.py_result()}, {cname}, {qualname}, {py_mod_name}); {code.error_goto_if_null(self.result(), self.pos)}')
        self.generate_gotref(code)
...
class PyClassMetaclassNode(ExprNode):
    ...
    def generate_result_code(self, code):
        ...
        if mkw:
            ...
            call = f"__Pyx_Py3MetaclassGet({bases.result()}, {mkw.result()})"
        else:
            code.globalstate.use_utility_code(
                UtilityCode.load_cached("CalculateMetaclass", "ObjectHandling.c"))
            call = f"__Pyx_CalculateMetaclass(NULL, {bases.result()})"
        code.putln(
            f"{self.result()} = {call}; {code.error_goto_if_null(self.result(), self.pos)}")
        self.generate_gotref(code)
...
class PyClassNamespaceNode(ExprNode, ModuleNameMixin):
    ...
    def generate_result_code(self, code):
        ...
        code.putln(
            f"{self.result()} = __Pyx_Py3MetaclassPrepare({metaclass}, {class_def_node.bases.result()}, {cname}, {qualname}, {mkw}, {py_mod_name}, {doc_code}); {code.error_goto_if_null(self.result(), self.pos)}")
        self.generate_gotref(code)
...
class ClassCellInjectorNode(ExprNode):
    ...
    def generate_result_code(self, code):
        ...
        code.putln(
            f'{self.result()} = PyList_New(0); {code.error_goto_if_null(self.result(), self.pos)}')
        self.generate_gotref(code)
    ...
    def generate_injection_code(self, code, classobj_cname):
        ...
        code.put_error_if_neg(self.pos, f'__Pyx_CyFunction_InitClassCell({self.result()}, {classobj_cname})')
...
class ClassCellNode(ExprNode):
    ...
    def generate_result_code(self, code):
        if not self.is_generator:
            code.putln(f'{self.result()} = __Pyx_CyFunction_GetClassObj({Naming.self_cname});')
        else:
            code.putln(f'{self.result()} =  {Naming.generator_cname}->classobj;')
        code.putln(
            f'if (!{self.result()}) {{ PyErr_SetString(PyExc_RuntimeError, '
            f'"super(): empty __class__ cell"); {code.error_goto(self.pos)} }}')
...
class PyCFunctionNode(ExprNode, ModuleNameMixin):
    ...
    def analyse_default_args(self, env):
        ...
        if nonliteral_objects or nonliteral_other:
            ...
            # sort by name
            arg_entries = sorted(defaults_class_scope.entries.items())
            arg_entries = [ e for name, e in arg_entries if name.startswith("arg") ]
            self.defaults = []
            for arg, entry in zip(nonliteral_objects + nonliteral_other, arg_entries, strict=False):
                arg.defaults_class_key = entry.cname
                self.defaults.append((arg, entry))

            self.defaults_pyobjects = len(nonliteral_objects)
            for arg, entry in self.defaults:
                arg.default_value = f'{Naming.dynamic_args_cname}->{entry.cname}'
    ...
    def generate_pycfunction_code(self, code):
        py_mod_name = self.get_py_mod_name(code)
        code.putln(
            f'{self.result()} = PyCFunction_NewEx(&{self.pymethdef_cname}, {self.closure_result_code()}, {py_mod_name}); {code.error_goto_if_null(self.result(), self.pos)}')

        self.generate_gotref(code)

    def generate_cyfunction_code(self, code):
        def_node = self.specialized_cpdefs[0] if self.specialized_cpdefs else self.def_node
        ...
        code.putln(
            f'{self.result()} = {constructor}(&{self.pymethdef_cname}, {flags}, {self.get_py_qualified_name(code)}, {self.closure_result_code()}, {self.get_py_mod_name(code)}, {moddict_cname}, {self.code_object.py_result()}); {code.error_goto_if_null(self.result(), self.pos)}')
        ...
        if def_node.requires_classobj:
            ...
            code.putln(
                f'PyList_Append({class_node.class_cell.result()}, {self.py_result()});')

        if self.defaults:
            code.putln(
                f'if (!__Pyx_CyFunction_InitDefaults({self.result()}, {code.name_in_module_state(self.defaults_entry.type.typeptr_cname)})) {code.error_goto(self.pos)}')
            defaults = f'__Pyx_CyFunction_Defaults(struct {self.defaults_entry.type.objstruct_cname}, {self.result()})'
            for arg, entry in self.defaults:
                if arg.is_dynamic:
                    arg.generate_assignment_code(code, cyfunc_struct_target=f'{defaults}->{entry.cname}')

        if self.defaults_tuple:
            code.putln(f'__Pyx_CyFunction_SetDefaultsTuple({self.result()}, {self.defaults_tuple.py_result()});')
        if not self.specialized_cpdefs:
            ...
            if self.defaults_kwdict:
                code.putln(f'__Pyx_CyFunction_SetDefaultsKwDict({self.result()}, {self.defaults_kwdict.py_result()});')
            if def_node.defaults_getter:
                code.putln(f'__Pyx_CyFunction_SetDefaultsGetter({self.result()}, {def_node.defaults_getter.entry.pyfunc_cname});')
            if self.annotations_dict:
                code.putln(f'__Pyx_CyFunction_SetAnnotationsDict({self.result()}, {self.annotations_dict.py_result()});')
...
class InnerFunctionNode(PyCFunctionNode):
    ...
    def closure_result_code(self):
        if self.needs_closure_code:
            return f"((PyObject*){Naming.cur_scope_cname})"
        return "NULL"
...
class CodeObjectNode(ExprNode):
    ...
    def generate_codeobj(self, code, error_label):
        ...
        flags = '(unsigned int)({})'.format('|'.join(flags)) or '0'
        ...
        code.putln("PyObject* const varnames[] = {{{}}};".format(', '.join(varnames)))
...
class DefaultNonLiteralArgNode(ExprNode):
    ...
    def result(self):
        return f'__Pyx_CyFunction_Defaults(struct {self.defaults_struct.name}, {Naming.self_cname})->{self.defaults_struct.lookup(self.arg.defaults_class_key).cname}'
...
class GeneratorExpressionNode(LambdaNode):
    ...
    def generate_result_code(self, code):
        ...
        code.putln(
            f'{self.result()} = {self.def_node.entry.pyfunc_cname}({args_to_call}); {code.error_goto_if_null(self.result(), self.pos)}')
        self.generate_gotref(code)
...
class YieldExprNode(ExprNode):
    ...
    def analyse_types(self, env):
        if not self.label_num or (self.is_yield_from and self.in_async_gen):
            error(self.pos, f"'{self.expr_keyword}' not supported here")
        self.is_temp = 1
    ...
    def generate_evaluation_code(self, code):
        if self.arg:
            ...
            code.putln(
                f"{Naming.retval_cname} = {self.arg.result_as(py_object_type)};")
            self.arg.generate_post_assignment_code(code)
            self.arg.free_temps(code)
        else:
    ...
    def generate_yield_code(self, code):
        ...
        for cname, type, _manage_ref in code.funcstate.temps_in_use():
            save_cname = code.funcstate.closure_temps.allocate_temp(type)
            saved.append((cname, save_cname, type))
            if type.is_cpp_class:
                code.globalstate.use_utility_code(
                    UtilityCode.load_cached("MoveIfSupported", "CppSupport.cpp"))
                cname = f"__PYX_STD_MOVE_IF_SUPPORTED({cname})"
            else:
        ...
        if code.funcstate.current_except is not None:
            ...
            code.putln(f"__Pyx_Coroutine_SwapException({Naming.generator_cname});")
        else:
            # no exceptions being handled => restore exception state of caller
            code.putln(f"__Pyx_Coroutine_ResetAndClearException({Naming.generator_cname});")
        ...
        if self.in_async_gen and not self.is_await:
            ...
            code.putln(f"return __Pyx__PyAsyncGenValueWrapperNew({Naming.retval_cname});")
        else:
        ...
        for cname, save_cname, type in saved:
            ...
            if type.is_cpp_class:
                save_cname = f"__PYX_STD_MOVE_IF_SUPPORTED({save_cname})"
            code.putln(f'{cname} = {save_cname};')
            if type.is_pyobject:
                code.putln(f'{save_cname} = 0;')
...
class _YieldDelegationExprNode(YieldExprNode):
    ...
    def generate_evaluation_code(self, code, source_cname=None, decref_source=False):
        ...
        code.putln(f"{result_temp} = {self.yield_from_func(code)}({Naming.generator_cname}, {self.arg.py_result() if source_cname is None else source_cname}, &{Naming.retval_cname});")
        ...
        code.putln(f"if (likely({result_temp} == PYGEN_NEXT)) {{")
        ...
        self.generate_yield_code(code)
        code.putln(f"}} else if (likely({result_temp} == PYGEN_RETURN)) {{")
        code.put_gotref(Naming.retval_cname, py_object_type)
        if self.result_is_used:
    ...
    def fetch_iteration_result(self, code):
        # YieldExprNode has allocated the result temp for us
        code.putln(f"{self.result()} = {Naming.retval_cname}; {Naming.retval_cname} = NULL;")
...
class AwaitIterNextExprNode(AwaitExprNode):
    ...
    def generate_sent_value_handling_code(self, code, value_cname):
        ...
        code.putln(f"if (unlikely(!{value_cname})) {{")
        self._generate_break(code)
...
class GlobalsExprNode(AtomicExprNode):
    ...
    def generate_result_code(self, code):
        code.putln(f'{self.result()} = __Pyx_Globals(); {code.error_goto_if_null(self.result(), self.pos)}')
        self.generate_gotref(code)
...
class UnopNode(ExprNode):
    ...
    def compile_time_value(self, denv):
        ...
        if not func:
            error(self.pos,
                f"Unary '{self.operator}' not supported in compile-time expression")
    ...
    def generate_result_code(self, code):
        if self.type.is_pythran_expr:
            ...
            code.putln(f"__Pyx_call_destructor({self.result()});")
            code.putln(f"new (&{self.result()}) decltype({self.result()}){{{self.operator}{self.operand.pythran_result()}}};")
    ...
    def generate_py_operation_code(self, code):
        ...
        code.putln(
            f"{self.result()} = {function}({self.operand.py_result()}); {code.error_goto_if_null(self.result(), self.pos)}")
        self.generate_gotref(code)

    def type_error(self):
        if not self.operand.type.is_error:
            error(self.pos, f"Invalid operand type for '{self.operator}' ({self.operand.type})")
        self.type = PyrexTypes.error_type
    ...
    def analyse_cpp_operation(self, env, overload_check=True):
        ...
        if overload_check and cpp_type is None:
            error(self.pos, f"'{self.operator}' operator not defined for {type}")
            self.type_error()
            return
        self.type = cpp_type
...
class DecrementIncrementNode(CUnopNode):
    ...
    def type_error(self):
        if not self.operand.type.is_error:
            if self.is_prefix:
                error(self.pos, f"No match for 'operator{self.operator}' (operand type is '{self.operand.type}')")
            else:
                error(self.pos, f"No 'operator{self.operator}(int)' declared for postfix '{self.operator}' (operand type is '{self.operand.type}')")
        self.type = PyrexTypes.error_type
    ...
    def analyse_types(self, env):
        ...
        if not (argtype.is_cfunction or argtype.is_reference or self.operand.is_addressable()):
            if argtype.is_memoryviewslice:
                ...
                self.error(f"Taking address of non-lvalue (type {argtype})")
            return self
        if argtype.is_pyobject:
            self.error("Cannot take address of Python %s" % (
                f"variable '{self.operand.name}'" if self.operand.is_name else
                f"object attribute '{self.operand.attribute}'" if self.operand.is_attribute else
                "object"))
            return self
...
class TypecastNode(ExprNode):
        ...
        if to_py and not from_py:
            if self.type is bytes_type and self.operand.type.is_int:
                ...
                if self.operand.type.is_ptr:
                    ...
                    warning(self.pos, f"No conversion from {self.operand.type} to {self.type}, python object pointer used.")
            ...
            if self.type.create_from_py_utility_code(env):
                ...
                warning(self.pos, f"No conversion from {self.type} to {self.operand.type}, python object pointer used.")
    ...
    def calculate_result_code(self, operand_result = None):
        ...
        if self.type.is_complex:
            ...
            if self.operand.type.is_complex:
                ...
                imag_part = "0"
            return f"{self.type.from_parts}({real_part}, {imag_part})"
        else:
    ...
    def generate_result_code(self, code):
        if self.is_temp:
            code.putln(
                f"{self.result()} = (PyObject *){self.operand.result()};")
            code.put_incref(self.result(), self.ctype())
...
class CythonArrayNode(ExprNode):
    ...
    def analyse_types(self, env):
        ...
        self.operand = self.operand.analyse_types(env)
        array_dtype = self.array_dtype or self.base_type_node.base_type_node.analyse(env)
        ...
        if base_type.is_array:
            ...
            error(self.pos, f"unexpected base type {base_type} found")
            return self
    ...
    def generate_result_code(self, code):
        ...
        itemsize = f"sizeof({dtype.empty_declaration_code()})"
        type_info = Buffer.get_type_information_cname(code, dtype)

        if self.operand.type.is_ptr:
            code.putln(f"if (!{self.operand.result()}) {{")
            code.putln(    'PyErr_SetString(PyExc_ValueError,'
                                '"Cannot create cython.array from NULL pointer");')
            code.putln(code.error_goto(self.operand.pos))
            code.putln("}")

        code.putln(f"{format_temp} = __pyx_format_from_typeinfo(&{type_info}); {code.error_goto_if_null(format_temp, self.pos)}")
        ...
        code.putln("#if CYTHON_COMPILING_IN_LIMITED_API")
        code.putln(f'{format_ptr_temp} = PyBytes_AsString({format_temp}); {code.error_goto_if_null(format_ptr_temp, self.pos)}')
        code.putln("#else")
        code.putln(f'{format_ptr_temp} = PyBytes_AS_STRING({format_temp});')
        code.putln("#endif")

        code.putln(f'{self.result()} = __pyx_array_new({shapes_temp}, {itemsize}, {format_ptr_temp}, "{self.mode}", (char *) {self.operand.result()}); {code.error_goto_if_null(self.result(), self.pos)}')
        self.generate_gotref(code)
...
class SizeofTypeNode(SizeofNode):
    ...
    def check_type(self):
        ...
        if arg_type.is_pyobject and not arg_type.is_extension_type:
            ...
            error(self.pos, f"Cannot take sizeof incomplete type '{arg_type}'")
...
class SizeofVarNode(SizeofNode):
    ...
    def calculate_result_code(self):
        return f"(sizeof({self.operand.result()}))"
    ...
    def analyse_types(self, env):
        ...
        if as_type:
            ...
            if self.arg_type.type.is_pyobject:
                ...
                self.error(f"Cannot use typeid on incomplete type '{self.arg_type.type}'")
                return self
        env.use_utility_code(UtilityCode.load_cached("CppExceptionConversion", "CppSupport.cpp"))
        return self
...
def get_compile_time_binop(node):
    ...
    if not func:
        error(node.pos,
            f"Binary '{node.operator}' not supported in compile-time expression")
    return func
...
class BinopNode(ExprNode):
    ...
    def generate_result_code(self, code):
        ...
        if self.type.is_pythran_expr:
            ...
            code.putln(f"__Pyx_call_destructor({self.result()});")
            if self.operator == '**':
                code.putln(f"new (&{self.result()}) decltype({self.result()}){{pythonic::numpy::functor::power{{}}({self.operand1.pythran_result()}, {self.operand2.pythran_result()})}};")
            else:
                code.putln(f"new (&{self.result()}) decltype({self.result()}){{{self.operand1.pythran_result()} {self.operator} {self.operand2.pythran_result()}}};")
            ...
            code.putln(
                f"{self.result()} = {function}({op1_result}, {op2_result}{extra_args}); {code.error_goto_if_null(self.result(), self.pos)}")
            self.generate_gotref(code)
        elif self.is_temp:
    ...
    def type_error(self):
        if not (self.operand1.type.is_error
            ...
            error(self.pos, f"Invalid operand types for '{self.operator}' ({self.operand1.type}; {self.operand2.type})")
        self.type = PyrexTypes.error_type
...
class CBinopNode(BinopNode):
    ...
    def calculate_result_code(self):
        return f"({self.operand1.result()} {self.operator} {self.operand2.result()})"
...
class NumBinopNode(BinopNode):
    ...
    def generate_evaluation_code(self, code):
        if self.overflow_check:
            ...
            code.putln(f"{self.overflow_bit} = 0;")
        super().generate_evaluation_code(code)
        if self.overflow_check:
            code.putln(f"if (unlikely({self.overflow_bit})) {{")
    ...
    def calculate_result_code(self):
        if self.overflow_bit_node is not None:
            return f"{self.func}({self.operand1.result()}, {self.operand2.result()}, &{self.overflow_bit_node.overflow_bit})"
            ...
            return f"{func}({self.operand1.result()}, {self.operand2.result()})"
        ...
        if not self.type.is_pyobject:
            if self.zerodivision_check:
                if not self.infix:
                    ...
                    zero_test = f"{self.operand2.result()} == 0"
                code.putln(f"if (unlikely({zero_test})) {{")
                if in_nogil:
                    code.put_ensure_gil()
                code.putln(f'PyErr_SetString(PyExc_ZeroDivisionError, "{self.zero_division_message()}");')
                if in_nogil:
                    code.put_release_ensured_gil()
                code.putln(code.error_goto(self.pos))
                code.putln("}")
                if self.type.is_int and self.type.signed and self.operator != '%':
                    ...
                    if self.operand2.type.signed == 2:
                        ...
                        minus1_check = f'unlikely({self.operand2.result()} == -1)'
                    else:
                        type_of_op2 = self.operand2.type.empty_declaration_code()
                        minus1_check = f'(!((({type_of_op2})-1) > 0)) && unlikely({self.operand2.result()} == ({type_of_op2})-1)'
                    code.putln(f"else if (sizeof({self.type.empty_declaration_code()}) == sizeof(long) && {minus1_check} "
                               f" && unlikely(__Pyx_UNARY_NEG_WOULD_OVERFLOW({self.operand1.result()}))) {{")
                    if in_nogil:
                        code.put_ensure_gil()
            ...
            if code.globalstate.directives['cdivision_warnings'] and self.operator != '/':
                ...
                code.putln(f"if (unlikely(({self.operand1.result()} < 0) ^ ({self.operand2.result()} < 0))) {{")
                warning_code = f"__Pyx_cdivision_warning({Naming.filename_cname}, {Naming.lineno_cname})"

                if in_nogil:
                    result_code = 'result'
                    code.putln(f"int {result_code};")
                    code.put_ensure_gil()
                ...
                code.put(f"if (unlikely({result_code})) ")
    ...
    def analyse_operation(self, env):
        ...
        if not self.type.is_pyobject:
            ...
            if not self.cdivision and not self.type.is_int and not self.type.is_float:
                error(self.pos, f"mod operator not supported for type '{self.type}'")
        return result  # should either be self, or wrap self
...
class PowNode(NumBinopNode):
    ...
    def analyse_c_operation(self, env):
        ...
        if self.type.is_complex:
            ...
            self.pow_func = "__Pyx_pow_{}".format(self.type.empty_declaration_code().replace(' ', '_'))
            env.use_utility_code(
            ...
            error(self.pos, f"got unexpected types for C power operator: {self.operand1.type}, {self.operand2.type}")
    ...
    def calculate_result_code(self):
        ...
        return f"{self.pow_func}({typecast(self.operand1)}, {typecast(self.operand2)})"
        ...
        if (self.is_cpow is None and self.type_was_inferred and
            ...
            if check_op1 and check_op2:
                warning(self.pos, "Treating '**' as if 'cython.cpow(True)' since it "
                    f"is directly assigned to a {msg_detail}. "
                    "This is likely to be fragile and we recommend setting "
                    "'cython.cpow' explicitly.")
                self.is_cpow = True
                self.operand1 = op1
                self.operand2 = op2
...
class BoolBinopNode(ExprNode):
    ...
    def generate_operand1_test(self, code):
        ...
        if self.type.is_pyobject:
            ...
            code.putln(
                f"{test_result} = __Pyx_PyObject_IsTrue({self.operand1.py_result()}); {code.error_goto_if_neg(test_result, self.pos)}")
        else:
    ...
    def generate_operand_test(self, code):
        ...
        if self.arg.type.is_pyobject:
            test_result = code.funcstate.allocate_temp(
                PyrexTypes.c_bint_type, manage_ref=False)
            code.putln(
                f"{test_result} = __Pyx_PyObject_IsTrue({self.arg.py_result()}); {code.error_goto_if_neg(test_result, self.pos)}")
        else:
...
class CondExprNode(ExprNode):
    ...
    def type_error(self):
        if not (self.true_val.type.is_error or self.false_val.type.is_error):
            error(self.pos, f"Incompatible types in conditional expression ({self.true_val.type}; {self.false_val.type})")
        self.type = PyrexTypes.error_type
    ...
    def generate_evaluation_code(self, code):
        ...
        code.putln(f"if ({self.test.result()}) {{")
...
class CmpNode:
    ...
    def find_common_int_type(self, env, op, operand1, operand2):
        ...
        if type1.is_int:
            ...
            if type1_can_be_int:
                return type2
        elif type1_can_be_int and type2_can_be_int:
    ...
    def invalid_types_error(self, operand1, op, operand2):
        error(self.pos, f"Invalid types for '{op}' ({operand1.type}, {operand2.type})")
    ...
    def generate_operation_code(self, code, result_code,
        ...
        if self.type.is_pyobject:
            ...
            got_ref = f"__Pyx_XGOTREF({result_code}); "
        ...
        if self.special_bool_cmp_function:
            result1 = operand1.py_result() if operand1.type.is_pyobject else operand1.result()
            result2 = operand2.py_result() if operand2.type.is_pyobject else operand2.result()
            ...
            code.putln(
                f"{result_code} = {coerce_result}({self.special_bool_cmp_function}({result1}, {result2}, {special_bool_extra_args_result if self.special_bool_extra_args else richcmp_constants[op]})); {got_ref}{error_clause(result_code, self.pos)}")
            ...
            statement = f"{result_code} = {coerce_result}({code1} {self.c_operator(op)} {code2});"
...
class PrimaryCmpNode(ExprNode, CmpNode):
    ...
    def analyse_cpp_comparison(self, env):
        ...
        entry = env.lookup_operator(self.operator, [self.operand1, self.operand2])
        if entry is None:
            error(self.pos, f"Invalid types for '{self.operator}' ({type1}, {type2})")
            self.type = PyrexTypes.error_type
            self.result_code = "<error>"
            return
        func_type = entry.type
    ...
    def calculate_result_code(self):
        ...
        if operand1.type.is_complex:
            ...
            negation = "!" if self.operator == "not_in" else ""
            return f"({negation}{method}({operand2.result()}, {operand1.result()}))"
        else:
            if is_pythran_expr(self.type):
                ...
                if self.is_memslice_nonecheck:
                    if operand1.type.is_memoryviewslice:
                        result1 = f"((PyObject *) {result1}.memview)"
                    else:
                        result2 = f"((PyObject *) {result2}.memview)"

            return f"({result1} {self.c_operator(self.operator)} {result2})"
...
class CascadedCmpNode(Node, CmpNode):
    ...
    def generate_evaluation_code(self, code, result, operand1, needs_evaluation=False):
        if self.type.is_pyobject:
            code.putln(f"if (__Pyx_PyObject_IsTrue({result})) {{")
            code.put_decref(result, self.type)
        else:
            code.putln(f"if ({result}) {{")
        if needs_evaluation:
...
class PyTypeTestNode(CoercionNode):
    ...
    def reanalyse(self):
        ...
        if self.arg.may_be_none() and self.notnone:
            return self.arg.as_none_safe_node(f"Cannot convert NoneType to {self.type.name:.200}")
        return self.arg
...
class NoneCheckNode(_TempModifierNode):
    ...
    def condition(self):
        if self.type.is_pyobject:
            ...
            return f"((PyObject *) {self.arg.result()}.memview)"
        else:
    ...
    def put_nonecheck(self, code):
        code.putln(
            f"if (unlikely({self.condition()} == Py_None)) {{")
        ...
        if self.exception_format_args:
            code.putln('PyErr_Format({}, "{}", {});'.format(
                ...
                ', '.join([ '"{}"'.format(escape(str(arg).encode('UTF-8')))
...
class CoerceToPyTypeNode(CoercionNode):
    ...
    def __init__(self, arg, env, type=py_object_type):
        if not arg.type.create_to_py_utility_code(env):
            error(arg.pos, f"Cannot convert '{arg.type}' to Python object")
        ...
        if type is py_object_type:
            ...
            if (type not in (bytes_type, bytearray_type)
                ...
                error(arg.pos,
                    f"default encoding required for conversion from '{arg.type}' to '{type}'")
...
class CoerceIntToBytesNode(CoerceToPyTypeNode):
    ...
    def generate_result_code(self, code):
        ...
        if arg.type not in (PyrexTypes.c_char_type,
                            ...
                            PyrexTypes.c_schar_type):
            if arg.type.signed:
                code.putln(f"if (({arg_result} < 0) || ({arg_result} > 255)) {{")
            else:
                code.putln(f"if ({arg_result} > 255) {{")
            code.putln('PyErr_SetString(PyExc_OverflowError, '
                       f'"value too large to pack into a byte"); {code.error_goto(self.pos)}')
            code.putln('}')
        temp = None
        if arg.type is not PyrexTypes.c_char_type:
            ...
            arg_result = temp
        code.putln(f'{self.result()} = PyBytes_FromStringAndSize(&{arg_result}, 1); {code.error_goto_if_null(self.result(), self.pos)}')
        if temp is not None:
            code.funcstate.release_temp(temp)
        self.generate_gotref(code)
...
class CoerceFromPyTypeNode(CoercionNode):
    ...
    def __init__(self, result_type, arg, env):
        ...
        if not result_type.create_from_py_utility_code(env):
            error(arg.pos,
                  f"Cannot convert Python object to '{result_type}'")
        if self.type.is_string or self.type.is_pyunicode_ptr:
            if self.arg.is_name and self.arg.entry and self.arg.entry.is_pyglobal:
                warning(arg.pos,
                        f"Obtaining '{result_type}' from externally modifiable global Python value",
                        level=1)
...
class CoerceToBooleanNode(CoercionNode):
    ...
    def generate_result_code(self, code):
        ...
        if test_func is not None:
            ...
            code.putln(
                f"{self.result()} = __Pyx_PyObject_IsTrue({self.arg.py_result()}); {code.error_goto_if_neg(self.result(), self.pos)}")
...
class CoerceToTempNode(CoercionNode):
    ...
    def generate_result_code(self, code):
        ...
        code.putln(f"{self.result()} = {self.arg.result_as(self.ctype())};")
        if self.use_managed_ref:
            if not self.type.is_memoryviewslice:
                code.put_incref(self.result(), self.ctype())
            else:
        ...
        if annotation.is_name:
            ...
            if not env.lookup(annotation.name):
                warning(annotation.pos,
                        f"Unknown type declaration '{self.string.value}' in annotation, ignoring", level=1)
            ...
            if not env.lookup(annotation.obj.name):
                ...
                if module_scope and not module_scope.lookup_type(annotation.attribute):
                    error(annotation.pos,
                            f"Unknown type declaration '{self.string.value}' in annotation")
            else:
                module_scope = annotation.obj.analyse_as_module(env)
