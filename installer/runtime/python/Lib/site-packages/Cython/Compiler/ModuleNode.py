...
from . import Annotate, Code, Future, Naming, Nodes, Options, PyrexTypes, Pythran, TypeSlots
...
def generate_c_code_config(env, options):
    emit_linenums = False if Options.annotate or options.annotate else options.emit_linenums
...
class ModuleNode(Nodes.Node, Nodes.BlockNode):
    ...
    def analyse_declarations(self, env):
        ...
        if not Options.docstrings:
            ...
            env.doc = EncodedString('File: {} (starting at line {})'.format(*Nodes.relative_position(self.pos)))
    ...
    def prepare_utility_code(self):
        ...
        for _name, entry in sorted(env.entries.items()):
    ...
    def assure_safe_target(self, path, allow_failed=False):
        ...
        if not is_cython_generated_file(path, allow_failed=allow_failed, if_not_found=True):
            ...
            raise CompileError(
                self.pos, f'The output file already exists and does not look like it was generated by Cython: "{os.path.basename(path)}"')
    ...
    def generate_h_code(self, env, options, result):
        ...
        if h_types or h_vars or h_funcs or h_extension_types:
            ...
            h_code_main.putln(f"#ifndef {api_guard}")
            h_code_main.putln("")
            ...
            warning_string = EncodedString(f'Use PyImport_AppendInittab({env.module_name.as_c_string_literal()}, {py3_mod_func_name}) instead of calling {py3_mod_func_name} directly.')
            h_code_main.putln('/* WARNING: {} from Python 3.5 */'.format(warning_string.rstrip('.')))
            ...
            h_code_main.putln(f'__attribute__ ((__deprecated__({warning_string.as_c_string_literal()}), __unused__)) __inline__')
            h_code_main.putln("#elif defined(_MSC_VER)")
            h_code_main.putln(f'__declspec(deprecated({warning_string.as_c_string_literal()})) __inline')
            h_code_main.putln('#endif')
            h_code_main.putln(f"static PyObject* __PYX_WARN_IF_{py3_mod_func_name}_INIT_CALLED(PyObject* res) {{")
            h_code_main.putln("return res;")
            h_code_main.putln("}")
            # Function call is converted to warning macro; uncalled (pointer) is not
            h_code_main.putln(f'#define {py3_mod_func_name}() __PYX_WARN_IF_{py3_mod_func_name}_INIT_CALLED({py3_mod_func_name}())')
            h_code_main.putln('#endif')
    ...
    def generate_public_declaration(self, entry, h_code, i_code):
        h_code.putln(f"{Naming.extern_c_macro} {entry.type.declaration_code(entry.cname)};")
        if i_code:
            i_code.putln(f"cdef extern {entry.type.declaration_code(entry.cname, pyrex=1)}")
    ...
    def generate_api_code(self, env, options, result):
        ...
        if api_extension_types:
            ...
            for entry in api_extension_types:
                ...
                h_code.putln(f"#define {type.typeobj_cname} (*{type.typeptr_cname})")
        ...
        h_code.putln("")
        h_code.putln("static int {}(void) {{".format(self.api_name("import", env)))
        ...
        for entry in api_funcs:
            ...
            h_code.putln(
                f'if (__Pyx_ImportFunction_{Naming.cyversion}(module, {entry.name.as_c_string_literal()}, (void (**)(void))&{cname}, "{sig}") < 0) goto bad;')
        for entry in api_vars:
            cname = env.mangle(Naming.varptr_prefix_api, entry.name)
            sig = entry.type.empty_declaration_code()
            h_code.putln(
                f'if (__Pyx_ImportVoidPtr_{Naming.cyversion}(module, {entry.name.as_c_string_literal()}, (void **)&{cname}, "{sig}") < 0) goto bad;')
    ...
    def generate_cclass_include_code(self, type, i_code):
        ...
        if var_entries:
            for entry in var_entries:
                i_code.putln(f"cdef {entry.type.declaration_code(entry.cname, pyrex=1)}")
        else:
            i_code.putln("pass")
        i_code.dedent()
    ...
    def generate_c_code(self, env, options, result):
        ...
        code.putln(f'#define __Pyx_MODULE_NAME {self.full_module_name.as_c_string_literal()}')
    ...
    def generate_module_preamble(self, env, options, cimported_modules, metadata, code):
        ...
        code.putln(f'static const char * const {Naming.cfilenm_cname} = {Naming.file_c_macro};')
    ...
    def generate_includes(self, env, cimported_modules, code, early=True, late=True):
        for inc in sorted(env.c_includes.values(), key=IncludeCode.sortkey):
            if inc.location == inc.EARLY:
                if early:
                    inc.write(code)
            elif inc.location == inc.LATE and late:
                inc.write(code)
        if early:
    ...
    def generate_filename_table(self, code):
        ...
        code.putln("")
        code.putln(f"static const char* const {Naming.filetable_cname}[] = {{")
    ...
    def generate_gcc33_hack(self, env, code):
        ...
        for entry in env.c_class_entries:
            ...
            if not type.typedef_flag:
                ...
                tail = name[6:] if name.startswith("__pyx_") else name
    ...
    def generate_struct_union_predeclaration(self, entry, code):
        ...
        if type.is_cpp_class and type.templates:
            code.putln("template <typename {}>".format(", typename ".join(
    ...
    def generate_cpp_class_definition(self, entry, code):
        ...
        if scope:
            if type.templates:
                code.putln("template <class {}>".format(", class ".join(
                    [T.empty_declaration_code() for T in type.templates])))
    ...
    def generate_enum_definition(self, entry, code):
        ...
        if entry.type.is_enum and entry.type.typedef_flag:
            # Not pre-declared.
    ...
    def generate_exttype_vtable_struct(self, entry, code):
        ...
        if type.vtabstruct_cname:
            code.putln("")
            code.putln(f"struct {type.vtabstruct_cname} {{")
            if type.base_type and type.base_type.vtabstruct_cname:
                code.putln(f"struct {type.base_type.vtabstruct_cname} {Naming.obj_base_cname};")
            for method_entry in scope.cfunc_entries:
                if not method_entry.is_inherited:
                    code.putln("{};".format(method_entry.type.declaration_code(f"(*{method_entry.cname})")))
            code.putln("};")
    ...
    def generate_objstruct_definition(self, type, code):
        ...
        for attr in type.scope.var_entries:
            attr_type = py_object_type if attr.is_declared_generic else attr.type
        ...
        for entry in env.c_class_entries:
            if definition or entry.defined_in_pxd:
                ...
                if entry.type.typeobj_cname is not None:
                    ...
                    module_state_traverse.putln(
                        f"Py_VISIT(traverse_module_state->{entry.type.typeobj_cname});")
    ...
    def generate_cvariable_declarations(self, env, code, definition):
        ...
        for entry in env.var_entries:
            ...
            if entry.visibility == 'extern':
                ...
                storage_class = Naming.extern_c_macro
                dll_linkage = "DL_EXPORT" if definition else "DL_IMPORT"
    ...
    def generate_cfunction_declarations(self, env, code, definition):
        for entry in env.cfunc_entries:
            from_pyx = Options.cimport_from_pyx and entry.visibility != 'extern'
            if (entry.used
    ...
    def generate_new_function(self, scope, code, cclass_entry):
        ...
        _have_entries, (py_attrs, py_buffers, memoryview_slices) = \
        ...
        if freelist_size:
            ...
            module_state.putln("%s[%d];" % (
                ...
                freelist_size))
            module_state.putln(f"int {freecount_name};")
        ...
        need_self_cast = (type.vtabslot_cname or
                          ...
                          explicitly_constructable_attrs)
        if need_self_cast:
            code.putln("{};".format(scope.parent_type.declaration_code("p")))
        if base_type:
        ...
        if freelist_size and not base_type:
            code.putln('}')
        if need_self_cast:
            code.putln("p = {};".format(type.cast_code("o")))
        #if need_self_cast:
        ...
        if type.vtabslot_cname:
            ...
            if vtab_base_type is not type:
                ...
                struct_type_cast = ""
            code.putln(f"p->{type.vtabslot_cname} = {struct_type_cast}{type.vtabptr_cname};")
        ...
        for entry in py_attrs:
            if entry.name == "__dict__":
                ...
                code.put(f"p->{entry.cname} = PyDict_New(); if (unlikely(!p->{entry.cname})) goto bad;")
            else:
        ...
        if cinit_func_entry:
            if cinit_func_entry.trivial_signature:
                ...
                cinit_args = "o, a, k"
            needs_error_cleanup = True
            code.putln(f"if (unlikely({cinit_func_entry.func_cname}({cinit_args}) < 0)) goto bad;")
    ...
    def generate_dealloc_function(self, scope, code):
        ...
        if not is_final_type or scope.may_have_finalize():
            ...
            code.putln(
                f"if (unlikely(__Pyx_PyObject_GetSlot(o, tp_finalize, destructor)) && {finalised_check}) {{")

            code.putln(f"if (__Pyx_PyObject_GetSlot(o, tp_dealloc, destructor) == {slot_func_cname}) {{")
        ...
        if base_type:
            ...
            if tp_dealloc is not None:
                ...
                # using module state).
                code.putln("#if !CYTHON_USE_MODULE_STATE")
                code.putln(f"if (likely({base_cname})) __Pyx_PyType_GetSlot({base_cname}, tp_dealloc, destructor)(o); else")
                code.putln("#endif")
                code.putln(f"__Pyx_call_next_tp_dealloc(o, {slot_func_cname});")
                code.globalstate.use_utility_code(
    ...
    def generate_traverse_function(self, scope, code, cclass_entry):
        ...
        _have_entries, (py_attrs, py_buffers, _memoryview_slices) = (
        ...
        if base_type:
            ...
            if static_call:
                ...
                code.putln("#if !CYTHON_USE_MODULE_STATE")
                code.putln("e = 0;")
                code.putln(f"if (likely({base_cname})) {{")
                code.putln(
                    f"traverseproc traverse = __Pyx_PyType_GetSlot({base_cname}, tp_traverse, traverseproc);")
                code.putln("if (traverse) { e = traverse(o, v, a); }")
                code.putln("} else")
                code.putln("#endif")
                code.putln(f"{{ e = __Pyx_call_next_tp_traverse(o, v, a, {slot_func}); }}")
    ...
    def generate_clear_function(self, scope, code, cclass_entry):
        ...
        unused = '' if py_attrs or py_buffers or base_type else 'CYTHON_UNUSED '
        ...
        if base_type:
            ...
            if static_call:
                ...
                code.putln("} else")
                code.putln("#endif")
                code.putln(f"{{ __Pyx_call_next_tp_clear(o, {slot_func}); }}")
                code.globalstate.use_utility_code(
    ...
    def generate_richcmp_function(self, scope, code):
        ...
        while cls is not None and cls.entry.visibility != 'extern':
        ...
        for cmp_method in TypeSlots.richcmp_special_methods:
            ...
            code.putln(f"case Py_{cmp_type}: {{")
            if entry is None:
        ...
        if '__eq__' in comp_entry and '__ne__' not in comp_entry and not extern_parent:
            ...
            code.putln("ret = {}(o1, o2);".format(comp_entry['__eq__'].func_cname))
            code.putln("if (likely(ret && ret != Py_NotImplemented)) {")
        ...
        if extern_parent and extern_parent.typeptr_cname:
            code.putln(f"if (likely({extern_parent.typeptr_cname}->tp_richcompare)) return {extern_parent.typeptr_cname}->tp_richcompare(o1, o2, op);")
    ...
    def generate_binop_function(self, scope, slot, code, pos):
        ...
        def call_slot_method(method_name, reverse):
            ...
            if func_cname:
                ...
                return f'{func_name}_maybe_call_slot(__Pyx_PyType_GetSlot({code.name_in_module_state(scope.parent_type.typeptr_cname)}, tp_base, PyTypeObject*), left, right {extra_arg})'

        if get_slot_method_cname(slot.left_slot.method_name) and not get_slot_method_cname(slot.right_slot.method_name):
            warning(pos, f"Extension type implements {slot.left_slot.method_name}() but not {slot.right_slot.method_name}(). "
    ...
    def generate_property_get_function(self, property_entry, code):
        ...
        code.putln("")
        code.putln(
            f"static PyObject *{property_entry.getter_cname}(PyObject *o, CYTHON_UNUSED void *x) {{")
        code.putln(
    ...
    def generate_property_set_function(self, property_entry, code):
        ...
        code.putln("")
        code.putln(
            f"static int {property_entry.setter_cname}(PyObject *o, PyObject *v, CYTHON_UNUSED void *x) {{")
        code.putln(
            "if (v) {")
        if set_entry:
            code.putln(
    ...
    def generate_typeobj_spec(self, entry, code):
        ...
        if generate_members:
            ...
            code.putln(f"static PyMemberDef {ext_type.typeobj_cname}_members[] = {{")
            code.putln("#if !CYTHON_USE_TYPE_SLOTS")
            if weakref_entry:
                ...
                code.putln(f'{{"__weaklistoffset__", T_PYSSIZET, offsetof({objstruct}, {weakref_entry.cname}), READONLY, 0}},')
            code.putln("#endif")
        ...
        code.putln(f"static PyType_Slot {ext_type.typeobj_cname}_slots[] = {{")
        for slot in TypeSlots.get_slot_table(code.globalstate.directives):
            slot.generate_spec(scope, code)
        if generate_members:
            code.putln(f"{{Py_tp_members, (void*){ext_type.typeobj_cname}_members}},")
        code.putln("{0, 0},")
        code.putln("};")

        classname = scope.class_name.as_c_string_literal()
        code.putln(f"static PyType_Spec {ext_type.typeobj_cname}_spec = {{")
        code.putln('"{}.{}",'.format(self.full_module_name, classname.replace('"', '')))
        code.putln(f"sizeof({objstruct}),")
        code.putln("0,")
        code.putln("{},".format(TypeSlots.get_slot_by_name("tp_flags", scope.directives).slot_code(scope)))
    ...
    def generate_typeobj_definition(self, modname, entry, code):
        ...
        objstruct = type.objstruct_cname if type.typedef_flag else f"struct {type.objstruct_cname}"
        code.putln(
    ...
    def generate_method_table(self, env, code):
        ...
        code.putln(
            f"static PyMethodDef {env.method_table_cname}[] = {{")
    ...
    def generate_dict_getter_function(self, scope, code):
        ...
        code.putln(f"static PyObject *{func_name}(PyObject *o, CYTHON_UNUSED void *x) {{")
        self.generate_self_cast(scope, code)
        code.putln(f"if (unlikely(!p->{dict_name})){{")
    ...
    def generate_getset_table(self, env, code):
        if env.property_entries:
            code.putln("")
            code.putln(
                f"static struct PyGetSetDef {env.getset_table_cname}[] = {{")
    ...
    def create_import_star_conversion_utility_code(self, env):
        ...
        for _name, entry in sorted(env.entries.items()):
            if entry.is_cglobal and entry.used and not entry.type.is_pyobject:
        ...
        for name, entry in sorted(env.entries.items()):
            if entry.is_cglobal and entry.used and not entry.type.is_const:
                ...
                if msvc_count % 100 == 0:
                    ...
                    code.putln("#endif")
                code.putln(f'else if (__Pyx_StrEq(name, "{name}")) {{')
                if entry.type.is_pyobject:
                    if entry.type.is_extension_type or entry.type.is_builtin_type:
                        type_test = entry.type.type_test_code(
                            env, "o")
                        code.putln(f"if (!({type_test})) {code.error_goto(entry.pos)};")
                    ...
                    code.putln(f'PyErr_Format(PyExc_TypeError, "Cannot convert Python object {name} to {entry.type}");')
    ...
    def generate_module_state_end(self, env, modules, globalstate):
        ...
        module_state.putln(f'}} {Naming.modulestatetype_cname};')
        module_state.putln('')
        ...
        module_state.putln('#endif')
        module_state.putln('')
        module_state.putln(f'#define {Naming.modulestateglobal_cname} (__Pyx_PyModule_GetState(__Pyx_State_FindModule(&{Naming.pymoduledef_cname})))')
        module_state.putln('')
        module_state.putln(f'#define {env.module_cname} (__Pyx_State_FindModule(&{Naming.pymoduledef_cname}))')
        module_state.putln("#else")
        module_state.putln(f'static {Naming.modulestatetype_cname} {Naming.modulestateglobal_cname}_static =')
        module_state.putln('#ifdef __cplusplus')
        ...
        module_state.putln('#endif')
        module_state.putln(f'static {Naming.modulestatetype_cname} * const {Naming.modulestateglobal_cname} = &{Naming.modulestateglobal_cname}_static;')
        module_state.putln("#endif")
    ...
    def generate_module_state_clear(self, env, code):
        code.putln("#if CYTHON_USE_MODULE_STATE")
        code.putln(f"static CYTHON_SMALL_CODE int {Naming.module_cname}_clear(PyObject *m) {{")
    ...
    def generate_module_state_traverse(self, env, code):
        code.putln("#if CYTHON_USE_MODULE_STATE")
        code.putln(f"static CYTHON_SMALL_CODE int {Naming.module_cname}_traverse(PyObject *m, visitproc visit, void *arg) {{")
    ...
    def generate_module_init_func(self, imported_modules, shared_utility_exporter, env, code):
        ...
        header3 = "__Pyx_PyMODINIT_FUNC {}(void)".format(self.mod_init_func_cname('PyInit', env))
        ...
        if self.scope.is_package:
            ...
            code.putln("__Pyx_PyMODINIT_FUNC PyInit___init__(void) {{ return {}(); }}".format(self.mod_init_func_cname('PyInit', env)))
            code.putln("#endif")
        ...
        if wrong_punycode_module_name:
            code.putln("#if !defined(CYTHON_NO_PYINIT_EXPORT) && (defined(_WIN32) || defined(WIN32) || defined(MS_WINDOWS))")
            code.putln(f"void {wrong_punycode_module_name}(void) {{}} /* workaround for https://bugs.python.org/issue39432 */")
            code.putln("#endif")
        code.putln(header3)
        ...
        code.putln(f"static CYTHON_SMALL_CODE int {self.module_init_func_cname()}(PyObject *{Naming.pymodinit_module_arg})")
        ...
        code.putln(f"if ({Naming.module_cname}) {{")
        # Hack: enforce single initialisation.
        code.putln(f"if ({Naming.module_cname} == {Naming.pymodinit_module_arg}) return 0;")
        code.putln('PyErr_SetString(PyExc_RuntimeError,'
                   f' "Module \'{env.module_name.as_c_string_literal()[1:-1]}\' has already been imported. Re-initialisation is not supported.");')
        code.putln("return -1;")
        code.putln("}")
        code.putln("#else")
        # Hack: enforce single initialisation also on reimports under different names (with PEP 3121/489).
        code.putln(f"if ({Naming.module_cname}) return __Pyx_NewRef({Naming.module_cname});")
        code.putln("#endif")
        ...
        code.putln(f"{empty_tuple} = PyTuple_New(0); {code.error_goto_if_null(empty_tuple, self.pos)}")
        empty_bytes = code.name_in_main_c_code_module_state(Naming.empty_bytes)
        code.putln(f"{empty_bytes} = PyBytes_FromStringAndSize(\"\", 0); {code.error_goto_if_null(empty_bytes, self.pos)}")
        empty_unicode = code.name_in_main_c_code_module_state(Naming.empty_unicode)
        code.putln(f"{empty_unicode} = PyUnicode_FromStringAndSize(\"\", 0); {code.error_goto_if_null(empty_unicode, self.pos)}")
        ...
        code.putln(f"if ({self.is_main_module_flag_cname()}) {{")
        ...
        code.putln(f'if ({env.module_cname}) {{')
        code.putln(
        ...
        code.putln(f'PyErr_SetString(PyExc_ImportError, "init {env.qualified_name.as_c_string_literal()[1:-1]}");')
        code.putln('}')
            ...
            def __exit__(self, exc_type, exc_value, exc_tb):
                ...
                if needs_error_handling:
                    self.call_code.putln(
                        self.call_code.error_goto_if_neg(f"{self.cfunc_name}({Naming.modulestatevalue_cname})", pos))
                else:
                    self.call_code.putln(
    ...
    def generate_module_import_setup(self, env, code):
        ...
        if module_path:
            ...
            if env.is_package:
                ...
                code.putln(
                    f'if (PyObject_SetAttrString({env.module_cname}, "__path__", {temp}) < 0) {code.error_goto(self.pos)};')
            ...
            code.putln(code.error_goto_if_neg(
                '__Pyx_SetPackagePathFromImportLib({})'.format(code.get_py_string_const(
        ...
        code.putln("{")
        code.putln("PyObject *modules = PyImport_GetModuleDict(); {}".format(code.error_goto_if_null("modules", self.pos)))
        code.putln(f'if (!PyDict_GetItemString(modules, {fq_module_name_cstring})) {{')
        code.putln(code.error_goto_if_neg(f'PyDict_SetItemString(modules, {fq_module_name_cstring}, {env.module_cname})', self.pos))
        code.putln("}")
        code.putln("}")
    ...
    def generate_module_cleanup_func(self, env, code):
        ...
        code.putln(f'static void {Naming.cleanup_cname}(CYTHON_UNUSED PyObject *self) {{')
        ...
        for entry in env.c_class_entries:
            ...
            if cclass_type.scope.directives.get('freelist', 0):
                ...
                freecount_name = code.name_in_main_c_code_module_state(
                    scope.mangle_internal(Naming.freecount_name))
                code.putln('#if CYTHON_USE_FREELISTS')
                code.putln(f"while ({freecount_name} > 0) {{")
                code.putln(f"PyObject* o = (PyObject*){freelist_name}[--{freecount_name}];")
                code.putln("PyTypeObject *tp = Py_TYPE(o);")
                code.putln("#if CYTHON_USE_TYPE_SLOTS")
                code.putln("(*tp->tp_free)(o);")
                code.putln("#else")
...
#                    code.entry_as_pyobject(entry), entry.cname))
    ...
    def generate_main_method(self, env, code):
        module_is_main = self.is_main_module_flag_cname()
        wmain = "wmain" if Options.embed == "main" else Options.embed
    ...
    def generate_pymoduledef_struct(self, env, code):
        doc = f"{code.get_string_const(env.doc)}" if env.doc else "0"
        if Options.generate_cleanup_code:
        ...
        code.putln(f"static PyObject* {Naming.pymodule_create_func_cname}(PyObject *spec, PyModuleDef *def); /*proto*/")
        code.putln(f"static int {exec_func_cname}(PyObject* module); /*proto*/")

        code.putln(f"static PyModuleDef_Slot {Naming.pymoduledef_slots_cname}[] = {{")
        code.putln(f"{{Py_mod_create, (void*){Naming.pymodule_create_func_cname}}},")
        code.putln(f"{{Py_mod_exec, (void*){exec_func_cname}}},")
        ...
        code.putln(f"{{Py_mod_multiple_interpreters, {subinterp_option}}},")
        code.putln("#endif")
        code.putln("{0, NULL}")
        code.putln("};")
    ...
    def generate_module_creation_code(self, env, code):
        ...
        f"{code.get_string_const(env.doc)}" if env.doc else "0"
        ...
        code.putln("#if CYTHON_PEP489_MULTI_PHASE_INIT")
        code.putln(f"{module_temp} = {Naming.pymodinit_module_arg};")
        code.put_incref(module_temp, py_object_type, nanny=False)
        code.putln("#else")
        code.putln(
            f"{module_temp} = PyModule_Create(&{Naming.pymoduledef_cname}); {code.error_goto_if_null(module_temp, self.pos)}")
        code.putln("#endif")

        code.putln("#if CYTHON_USE_MODULE_STATE")
        code.putln("{")
        # So that PyState_FindModule works in the init function:
        code.putln(f"int add_module_result = __Pyx_State_AddModule({module_temp}, &{Naming.pymoduledef_cname});")
        code.putln(f"{module_temp} = 0; /* transfer ownership from {module_temp} to {env.module_name.as_c_string_literal()} pseudovariable */")
        ...
        code.putln(
            f"{dict_cname} = PyModule_GetDict({env.module_cname}); {code.error_goto_if_null(dict_cname, self.pos)}")
        ...
        code.putln(
            f'{builtins_cname} = __Pyx_PyImport_AddModuleRef(__Pyx_BUILTIN_MODULE_NAME); {code.error_goto_if_null(builtins_cname, self.pos)}')
        runtime_cname = code.name_in_main_c_code_module_state(Naming.cython_runtime_cname)
        code.putln(
            f'{runtime_cname} = __Pyx_PyImport_AddModuleRef("cython_runtime"); {code.error_goto_if_null(runtime_cname, self.pos)}')
        code.putln(
            f'if (PyObject_SetAttrString({env.module_cname}, "__builtins__", {builtins_cname}) < 0) {code.error_goto(self.pos)}')
        if Options.pre_import is not None:
            code.putln(
                f'{Naming.preimport_cname} = __Pyx_PyImport_AddModuleRef("{Options.pre_import}"); {code.error_goto_if_null(Naming.preimport_cname, self.pos)}')
    ...
    def generate_wrapped_entries_code(self, env, code):
        for name, entry in sorted(env.entries.items()):
            if (entry.create_wrapper
                ...
                code.putln("{")
                code.putln(f"PyObject* wrapped = {entry.type.to_py_function}({entry.cname});")
                code.putln(code.error_goto_if_null("wrapped", entry.pos))
                code.putln(
                    f'if (PyObject_SetAttrString({env.module_cname}, "{name}", wrapped) < 0) {code.error_goto(entry.pos)};')
                code.putln("}")
    ...
    def generate_type_import_code(self, env, type, pos, code, import_generator):
        ...
        if type.vtabptr_cname:
            ...
            code.putln(f"{type.vtabptr_cname} = (struct {type.vtabstruct_cname}*)__Pyx_GetVtable({code.name_in_main_c_code_module_state(type.typeptr_cname)}); {code.error_goto_if_null(type.vtabptr_cname, pos)}")
    ...
    def generate_exttype_vtable_init_code(self, entry, code):
        ...
        if type.vtable_cname:
            ...
            if type.base_type and type.base_type.vtabptr_cname:
                code.putln(
                    f"{type.vtable_cname}.{Naming.obj_base_cname} = *{type.base_type.vtabptr_cname};")
            ...
            if c_method_entries:
                for meth_entry in c_method_entries:
                    ...
                    code.putln(
                        f"{type.vtable_cname}.{meth_entry.cname} = {cast}{meth_entry.func_cname};")
...
def _deduplicate_inout_signatures(item_tuples):
    ...
    signatures, names, items = zip(*sorted(item_tuples), strict=False)
...
class ModuleImportGenerator:
    ...
    def imported_module(self, module_name_string, error_code):
        ...
        code.putln(f'{temp} = PyImport_ImportModule({module_name_string}); if (unlikely(!{temp})) {error_code}')
        code.put_gotref(temp, py_object_type)
