...
class Signature:
    ...
    def method_function_type(self):
        ...
        for m in mflags:
            if m in (method_noargs, method_onearg):
                return "PyCFunction"
            if m == method_varargs:
...
class SlotDescriptor:
    ...
    def generate_spec(self, scope, code):
        ...
        if not preprocessor_guard and self.slot_name.startswith(('bf_', 'am_')):
    ...
    def generate_set_slot_code(self, value, scope, code):
        ...
        if scope.parent_type.typeptr_cname:
            target = f"{code.typeptr_cname_in_module_state(scope.parent_type)}->{self.slot_name}"
        else:
            assert scope.parent_type.typeobj_cname
            target = f"{code.name_in_module_state(scope.parent_type.typeobj_cname)}.{self.slot_name}"
...
class ConstructorSlot(InternalMethodSlot):
    ...
    def generate_dynamic_init_code(self, scope, code):
        ...
        if base_type.typeptr_cname:
            ...
            src = f'{base_typeptr_cname}->{self.slot_name}'
        elif base_type.is_extension_type and base_type.typeobj_cname:
            src = f'{code.typeptr_cname_in_module_state(base_type)}.{self.slot_name}'
        else:
            return
...
class SuiteSlot(SlotDescriptor):
    ...
    def is_empty(self, scope):
        return all(slot.slot_code(scope) == "0" for slot in self.sub_slots)
...
class MemberTableSlot(SlotDescriptor):
    ...
    def is_empty(self, scope):
        return all(not member_entry for member_entry in self.get_member_specs(scope))
    ...
    def generate_substructure_spec(self, scope, code):
        ...
        code.putln(f"static struct PyMemberDef {self.substructure_cname(scope)}[] = {{")
...
class BaseClassSlot(SlotDescriptor):
    ...
    def generate_dynamic_init_code(self, scope, code):
        ...
        if base_type:
            base_typeptr_cname = code.typeptr_cname_in_module_state(base_type)
            code.putln(f"{code.typeptr_cname_in_module_state(scope.parent_type)}->{self.slot_name} = {base_typeptr_cname};")
...
class DictOffsetSlot(SlotDescriptor):
    ...
    def members_slot_value(self, scope):
        ...
        if dict_offset == "0":
            return None
        return f'{{"__dictoffset__", T_PYSSIZET, {dict_offset}, READONLY, NULL}},'
...
def get_slot_by_name(slot_name, compiler_directives):
    ...
    for slot in get_slot_table(compiler_directives).slot_table:
        if slot.slot_name == slot_name:
            return slot
    raise AssertionError(f"Slot not found: {slot_name}")
