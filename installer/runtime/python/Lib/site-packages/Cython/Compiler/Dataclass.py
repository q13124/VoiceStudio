    ...
    def visit_SingleAssignmentNode(self, node):
        if node.lhs.is_name and node.lhs.name in self.names:
            if node.lhs.name in self.removed_assignments:
                warning(node.pos, (f"Multiple assignments for '{node.lhs.name}' in dataclass; "
                                   "using most recent"), 1)
...
class TemplateCode:
    ...
    def add_extra_statements(self, statements):
        if self.extra_stats is None:
            raise AssertionError("Can only use add_extra_statements on top-level writer")
...
def handle_cclass_dataclass(node, dataclass_args, analyse_decs_transform):
    ...
    kwargs = {"init": True, "repr": True, "eq": True,
...
def generate_init_code(code, init, node, fields, kw_only):
    ...
    * Call to user-defined __post_init__ function (if it exists) is copied from
    ...
    for name, field in fields.items():
        ...
        annotation = f": {entry.annotation.string.value}" if entry.annotation else ""
        assignment = ''
        if field.default is not MISSING or field.default_factory is not MISSING:
            ...
            error(entry.pos, (f"non-default argument '{name}' follows default argument "
...
def generate_match_args(code, match_args, node, fields, global_kw_only):
    ...
    code.add_code_line(f"__match_args__ = {tuple(positional_arg_names)!s}")
