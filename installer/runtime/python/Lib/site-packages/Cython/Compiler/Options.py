    ...
    for old_option in ShouldBeFromDirective.known_directives:
        ...
        if not isinstance(value, ShouldBeFromDirective):
            if old_option.disallow:
                raise RuntimeError(
                    f"Option '{old_option.option_name}' must be set from directive '{old_option.directive_name}'")
            else:
                # Warn?
    ...
    if type is bool:
        ...
        if relaxed_bool:
            ...
            if value in ("true", "yes"):
                ...
                return False
        raise ValueError(f"{name} directive must be set to True or False, got '{orig_value}'")
    elif type is int:
        try:
            return int(value)
        except ValueError:
            raise ValueError(f"{name} directive must be set to an integer, got '{orig_value}'")
    elif type is str:
        return str(value)
    elif callable(type):
...
def parse_directive_list(s, relaxed_bool=False, ignore_unknown=False,
    ...
    True
    """
    result = {} if current_settings is None else current_settings
    for item in s.split(','):
        item = item.strip()
        if not item:
            continue
...
class CompilationOptions:
    ...
    def __init__(self, defaults=None, **kw):
        ...
        if directives.get('np_pythran', False) and not options['cplus']:
            import warnings
            warnings.warn("C++ mode forced when in Pythran mode!", stacklevel=2)
            options['cplus'] = True
