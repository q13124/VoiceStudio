    ...
    def _print_node(self, node):
        ...
        if self._line_range is None or self._line_range[0] <= line <= self._line_range[1]:
            if len(self.access_path) == 0:
                ...
                _parent, attr, idx = self.access_path[-1]
                name = "%s[%d]" % (attr, idx) if idx is not None else attr
    ...
    def repr_of(self, node):
        if node is None:
            ...
            if isinstance(node, ExprNodes.NameNode):
                result += f"(type={node.type!r}, name=\"{node.name}\")"
                ...
                result += f"(type={node.type!r}, attribute=\"{node.attribute}\")"
            elif isinstance(node, (ExprNodes.ConstNode, ExprNodes.PyConstNode)):
                result += f"(type={node.type!r}, value={node.value!r})"
