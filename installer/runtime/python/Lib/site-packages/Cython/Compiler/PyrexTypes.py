...
class PyrexType(BaseType):
    ...
    def _assign_from_py_code(self, source_code, result_code, error_pos, code,
        ...
        convert_call = f"{from_py_function or self.from_py_function}({source_code}{args})"
        if self.is_enum:
            convert_call = typecast(self, c_long_type, convert_call)
        if special_none_cvalue:
            # NOTE: requires 'source_code' to be simple!
            convert_call = f"(__Pyx_Py_IsNone({source_code}) ? ({special_none_cvalue}) : ({convert_call}))"
        return f'{result_code} = {convert_call}; {code.error_goto_if(error_condition or self.error_condition(result_code), error_pos)}'

    def _generate_dummy_refcounting(self, code, *ignored_args, **ignored_kwds):
        if self.needs_refcounting:
            raise NotImplementedError(f"Ref-counting operation not yet implemented for type {self}")
...
def create_typedef_type(name, base_type, cname, is_external=0, namespace=None):
    if is_external and (base_type.is_complex or base_type.is_fused):
...
class CTypedefType(BaseType):
        ...
        if self.typedef_is_external:
            if not self.from_py_utility_code:
                ...
                if type(base_type) is CIntType:
                    ...
                    return True
                elif base_type.is_float or base_type.is_complex:
                    pass  # XXX implement!
                elif base_type.is_cpp_string:
    ...
    def error_condition(self, result_code):
        if self.typedef_is_external and self.exception_value is not None:
            condition = f"({result_code} == {self.cast_code(self.exception_value)})"
...
class MemoryViewSliceType(PyrexType):
    ...
    def assert_direct_dims(self, pos):
        for access, _packing in self.axes:
            if access != 'direct':
    ...
    def _generate_decref(self, code, cname, nanny, null_check=False,
        ...
        if clear:
            if clear_before_decref:
                ...
                code.putln(f"{prefix}_{X}DECREF({self.as_pyobject(cname)}); {cname} = 0;")
        else:
...
class PyExtensionType(PyObjectType):
    ...
    def assignable_from(self, src_type):
        if self == src_type:
            return True
        if isinstance(src_type, PyExtensionType) and src_type.base_type is not None:
...
class CType(PyrexType):
    ...
    def to_py_call_code(self, source_code, result_code, result_type, to_py_function=None):
        ...
        assert func
        if (self.is_string or self.is_cpp_string) and result_type.is_builtin_type:
...
class CVoidType(CType):
    ...
    def declaration_code(self, entity_code,
            for_display = 0, dll_linkage = None, pyrex = 0):
        base_code = "void" if pyrex or for_display else public_decl("void", dll_linkage)
        return self.base_declaration_code(base_code, entity_code)
...
class InvisibleVoidType(CVoidType):
    ...
    def declaration_code(self, entity_code,
            for_display = 0, dll_linkage = None, pyrex = 0):
        base_code = "[void]" if pyrex or for_display else public_decl("", dll_linkage)
        return self.base_declaration_code(base_code, entity_code)
...
class CIntLike:
    ...
    def can_coerce_to_pystring(self, env, format_spec=None):
        format_type, width, _padding = self._parse_format(format_spec)
...
class CReturnCodeType(CIntType):
    ...
    def convert_to_pystring(self, cvalue, code, format_spec=None, name_type=None):
        return "__Pyx_NewRef({})".format(code.get_py_string_const(StringEncoding.EncodedString("None")))
...
class CPyTSSTType(CType):
    ...
    def declaration_code(self, entity_code,
                         for_display=0, dll_linkage=None, pyrex=0):
        base_code = "Py_tss_t" if pyrex or for_display else public_decl("Py_tss_t", dll_linkage)
        return self.base_declaration_code(base_code, entity_code)
...
class CArrayType(CPointerBaseType):
    ...
    def __repr__(self):
        return f"<CArrayType {self.size} {self.base_type!r}>"
    ...
    def declaration_code(self, entity_code,
            for_display = 0, dll_linkage = None, pyrex = 0):
        dimension_code = self.size if self.size is not None else ""
    ...
    def to_py_call_code(self, source_code, result_code, result_type, to_py_function=None):
        ...
        return f'{result_code} = {self.to_tuple_function if target_is_tuple else func}({source_code}, {self.size})'
    ...
    def from_py_call_code(self, source_code, result_code, error_pos, code,
        ...
        call_code = f"{from_py_function or self.from_py_function}({source_code}, {result_code}, {self.size})"
...
class CPtrType(CPointerBaseType):
    ...
    def __repr__(self):
        return f"<CPtrType {self.base_type!r}>"
        ...
        if self.base_type.is_cfunction and src_type.is_cfunction:
            ...
            if self.base_type.pointer_assignable_from_resolved_type(copied_src_type):
                ...
                if not self.base_type.exception_check and self.base_type.exception_value is None:
                    src_name = "the value being assigned" if src_name is None else f"'{src_name}'"
    ...
    def can_coerce_to_pyobject(self, env):
        ...
        for arg in self.args:
            if not arg.type.is_pyobject and not arg.type.can_coerce_to_pyobject(env):
                return False
        return not (not self.return_type.is_pyobject and not self.return_type.can_coerce_to_pyobject(env))
    ...
    def create_to_py_utility_code(self, env):
        ...
        def declared_type(ctype):
            ...
            if ctype.is_pyobject:
                ...
                type_displayname = 'float' if ctype is c_double_type else repr(type_displayname)
...
def get_fused_cname(fused_cname, orig_cname):
    ...
    return StringEncoding.EncodedString(f'{Naming.fused_func_prefix}{fused_cname}{orig_cname}')
...
def get_specialized_types(type):
    ...
    if isinstance(type, FusedType):
        ...
        result = []
        for _cname, f2s in get_all_specialized_permutations(type.get_fused_types()):
...
class ToPyStructUtilityCode(AbstractUtilityCode):
    ...
    def put_code(self, globalstate, used_by=None):
        ...
        for member in self.type.scope.var_entries:
            ...
            code.putln("{}; if (unlikely(!member)) goto bad;".format(member.type.to_py_call_code(f's.{member.cname}', 'member', member.type)))
...
class CStructOrUnionType(CType):
    ...
    def declaration_code(self, entity_code,
        ...
        if pyrex or for_display:
            ...
            base_code = self.cname if self.typedef_flag else f"{self.kind} {self.cname}"
    ...
    def needs_explicit_construction(self, scope):
        return bool(self._needs_cpp_construction and scope.is_c_class_scope)
...
class CppClassType(CType):
    ...
    def specialize_here(self, pos, env, template_values=None):
        ...
        for value in template_values:
            if value.is_pyobject or value.needs_refcounting:
                ...
                error(pos,
                      f"{type_description} type '{value}' cannot be used as a template argument")
    ...
    def declaration_code(self, entity_code,
        ...
        if self.templates:
            ...
            brackets = "[%s]" if for_display else "<%s> "
...
class EnumMixin:
    ...
    def create_enum_to_py_utility_code(self, env):
        from .UtilityCode import CythonUtilityCode
        self.to_py_function = f"__Pyx_Enum_{type_identifier(self)}_to_py"
...
class CppScopedEnumType(CType, EnumMixin):
    ...
    def create_from_py_utility_code(self, env):
        ...
        if self.underlying_type.create_from_py_utility_code(env):
            self.from_py_function = f'({self.cname}){self.underlying_type.from_py_function}'
        return True
    ...
    def create_to_py_utility_code(self, env):
        ...
        if self.underlying_type.create_to_py_utility_code(env):
            ...
            self.to_py_function = f'[](const {self.cname}& x){{return {self.underlying_type.to_py_function}(({self.underlying_type.empty_declaration_code()})x);}}'
        return True
...
class CTupleType(CType):
    ...
    def declaration_code(self, entity_code,
            for_display = 0, dll_linkage = None, pyrex = 0):
        if pyrex or for_display:
            return f"{self!s} {entity_code}"
        else:
            return self.base_declaration_code(self.cname, entity_code)

    def can_coerce_to_pyobject(self, env):
        return all(component.can_coerce_to_pyobject(env) for component in self.components)

    def can_coerce_from_pyobject(self, env):
        return all(component.can_coerce_from_pyobject(env) for component in self.components)
    ...
    for func in functions:
        ...
        if func_type.templates:
            ...
            if deductions is None:
                ...
                errors.append((func, "Unable to deduce type parameter {}".format(", ".join([param.name for param in func_type.templates if param not in deductions]))))
            else:
                type_list = [deductions[param] for param in func_type.templates]
                from .Symtab import Entry
                specialization = Entry(
                    name = func.name + "[{}]".format(",".join([str(t) for t in type_list])),
                    cname = func.cname + "<{}>".format(",".join([t.empty_declaration_code() for t in type_list])),
...
def widest_numeric_type(type1, type2):
    ...
    if type1 == type2:
        ...
        widest_type = type2
    elif type1.rank > type2.rank or type1.signed < type2.signed:
        widest_type = type1
...
_escape_special_type_characters = partial(re.compile(
    ...
    " ?({}) ?".format("|".join(re.escape(s) for s in sorted(_special_type_characters, reverse=True)))
...
def write_noexcept_performance_hint(pos, env,
    ...
    if is_call:
        ...
        on_what = f"on {function_name} "
    else:
        on_what =''
    msg = (
        f"Exception check {on_what}will always require the GIL to be acquired."
    )
    ...
    solutions = [f"Declare {the_function} as 'noexcept' if you control the definition and "
                 "you're sure you don't want the function to raise exceptions."]
    if void_return:
        solutions.append(
            f"Use an 'int' return type on {the_function} to allow an error code to be returned.")
