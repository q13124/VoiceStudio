...
class MarkParallelAssignments(EnvTransform):
    ...
    def mark_assignment(self, lhs, rhs, inplace_op=None):
        if isinstance(lhs, (ExprNodes.NameNode, Nodes.PyArgDeclNode)):
            ...
            if self.parallel_block_stack:
                ...
                if previous_assignment:
                    ...
                    if (inplace_op and previous_inplace_op and
                        ...
                        error(lhs.pos,
                              "Reduction operator '{}' is inconsistent "
                              "with previous reduction operator '{}'".format(*t))
                else:
                    pos = lhs.pos
            ...
            for i, arg in enumerate(lhs.args):
                item_node = None if not rhs or arg.is_starred else rhs.inferable_item_node(i)
    ...
    def visit_YieldExprNode(self, node):
        if self.parallel_block_stack:
            error(node.pos, f"'{node.expr_keyword}' not allowed in parallel sections")
        return node
...
class SimpleAssignmentTypeInferer:
    ...
    def infer_types(self, scope):
        ...
        if verbose:
            for entry in inferred:
                message(entry.pos, f"inferred '{entry.name}' to be of type '{entry.type}'")
...
def safe_spanning_type(types, might_overflow, scope):
    ...
    if result_type.is_pyobject:
        ...
        return result_type
    elif result_type.is_memoryviewslice or result_type is PyrexTypes.soft_complex_type or result_type == PyrexTypes.c_double_complex_type or ((result_type.is_int or result_type.is_enum) and not might_overflow) or (not result_type.can_coerce_to_pyobject(scope)
