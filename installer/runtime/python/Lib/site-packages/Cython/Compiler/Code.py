...
class UtilityCodeBase(AbstractUtilityCode):
    ...
    def get_special_comment_matcher(line_comment_char):
        return re.compile((
            # section title
            r'^{C}{{5,30}}  \s*  (?P<name> (?:\w|\.)+ )  \s*  {C}{{5,30}} |'
            # section tags and dependencies
            r'^{C}+  @(?P<tag> .+)'
        ).format(C=re.escape(line_comment_char)), re.VERBOSE).match
    ...
    @classmethod
    def load_cached(cls, utility_code_name, from_file, __cache=None):
        """
...
def add_macro_processor(*macro_names, regex=None, is_module_specific=False, _last_macro_processor = None):
    ...
    """
    if _last_macro_processor is None:
        _last_macro_processor = [None]
...
def sub_tempita(s, context, file=None, name=None, __cache=None):
...
class FunctionState:
    ...
    def __init__(self, owner, names_taken=None, scope=None):
    ...
    def all_new_labels(self):
        ...
        new_labels = []
        for old_label, name in zip(old_labels, ['continue', 'break', 'return', 'error'], strict=False):
            if old_label:
    ...
    def temps_in_use(self):
        ...
        """
        used = []
        for name, type, manage_ref, _static in self.temps_allocated:
            freelist = self.temps_free.get((type, manage_ref))
...
class GlobalState:
    ...
    def get_string_const(self, text, c_used=True):
        ...
        byte_string = text.utf8encode() if text.is_unicode else text.byteencode()
        try:
    ...
    def new_num_const_cname(self, value, py_type):  # type: (str, str) -> str
        ...
        if len(value) > 42:
            ...
            cname = f"{prefix}{value}"
        return cname

    def new_const_cname(self, prefix='', value=''):
        value = replace_identifier('_', value)[:32].strip('_')
        name_suffix = self.unique_const_cname(value + "{sep}{counter}")
        prefix = Naming.interned_prefixes[prefix] if prefix else Naming.const_prefix
    ...
    def get_cached_unbound_method(self, type_cname, method_name):
        ...
        try:
            ...
            cname = self.cached_cmethods[key] = self.new_const_cname(
                'umethod', f'{type_cname}_{method_name}')
        return cname
    ...
    def add_cached_builtin_decl(self, entry):
        if entry.is_builtin and entry.is_const and self.should_declare(entry.cname, entry):
    ...
    def put_cached_builtin_init(self, pos, name, cname):
        ...
        w.putln(f'{cname} = __Pyx_GetBuiltinName({cname_in_modulestate}); if (!{cname}) {w.error_goto(pos)}')
    ...
    def generate_object_constant_decls(self):
        ...
        for prefix, count in sorted(self.const_array_counters.items()):
            ...
            if cleanup_level is not None and cleanup_level <= Options.generate_cleanup_code:
                ...
                part_writer.putln(
                    "{{ Py_CLEAR({}); }}".format(part_writer.name_in_main_c_code_module_state(f"{struct_attr_cname}[i]"))
                )
    ...
    def generate_cached_methods_decls(self):
        ...
        for (type_cname, method_name), cname in sorted(self.cached_cmethods.items()):
            ...
            decl.putln(f'__Pyx_CachedCFunction {cname};')
            # split type reference storage as it might not be static
            init.putln(f'{init.name_in_main_c_code_module_state(cname)}.type = (PyObject*){type_cname};')
    ...
    def generate_string_constants(self):
        ...
        for c, cname in sorted(self.pyunicode_ptr_const_index.items()):
            ...
            decls_writer.putln(f"static Py_UNICODE {cname}[] = {{ {utf32_array} }};")
            if utf16_array:
                decls_writer.putln("#else")
                decls_writer.putln(f"static Py_UNICODE {cname}[] = {{ {utf16_array} }};")
...
class CCodeWriter:
    ...
    def label_interceptor(self, new_labels, orig_labels, skip_to_label=None, pos=None, trace=True):
        ...
        """
        for label, orig_label in zip(new_labels, orig_labels, strict=False):
    ...
    def get_py_version_hex(self, pyversion):
        return "0x{:02X}{:02X}{:02X}{:02X}".format(*((*tuple(pyversion), 0, 0, 0, 0))[:4])
    ...
    def put_temp_declarations(self, func_context: FunctionState):
        for name, type, _manage_ref, static in func_context.temps_allocated:
        ...
        if func_context.should_declare_error_indicator:
            unused = '' if self.funcstate.uses_error_indicator else 'CYTHON_UNUSED '
            # Initialize these variables to silence compiler warnings
            self.putln(f"{unused}int {Naming.lineno_cname} = 0;")
            self.putln(f"{unused}const char *{Naming.filename_cname} = NULL;")
            self.putln(f"{unused}int {Naming.clineno_cname} = 0;")
    ...
    def build_function_modifiers(self, modifiers, mapper=modifier_output_mapper):
        if not modifiers:
            return ''
        return '{} '.format(' '.join([mapper(m,m) for m in modifiers]))
    ...
    def put_init_to_py_none(self, cname, type, nanny=True):
        ...
        if nanny:
            self.putln(f"{cname} = {py_none}; __Pyx_INCREF(Py_None);")
        else:
            self.putln(f"{cname} = {py_none}; Py_INCREF(Py_None);")
    ...
    def put_pymethoddef(self, entry, term, allow_skip=True, wrapper_code_writer=None):
        ...
        if cast != 'PyCFunction':
            func_ptr = f'(void(*)(void))({cast}){func_ptr}'
    ...
    def put_pymethoddef_wrapper(self, entry):
        ...
        if entry.is_special:
            ...
            if method_noargs in method_flags:
                ...
                self.putln(f"static PyObject *{func_cname}(PyObject *self, CYTHON_UNUSED PyObject *arg) {{")
    ...
    def put_error_if_neg(self, pos, value):
        ...
        return self.putln(f"if ({value} < (0)) {self.error_goto(pos)}")
    ...
    def put_add_traceback(self, qualified_name, include_cline=True):
        ...
        self.funcstate.uses_error_indicator = True
        self.putln('__Pyx_AddTraceback({}, {}, {}, {});'.format(*format_tuple))
