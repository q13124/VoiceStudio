...
class CmdLineParserTest(TestCase):
    ...
    def check_default_options(self, options, white_list=None):
        ...
        no_value = object()
        for name in default_options.__dict__:
    ...
    def test_embed_before_positional(self):
        _options, sources = parse_command_line([
            '--embed',
            'source.pyx',
        ])
        self.assertEqual(sources, ['source.pyx'])
    ...
    def test_no_annotate(self):
        _options, _sources = parse_command_line([
    ...
    def test_short_w(self):
        options, _sources = parse_command_line([
            '-w', 'my_working_path',
            'source.pyx'
        ])
    ...
    def test_directive_multiple(self):
        options, _source = parse_command_line([
               '-X', 'cdivision=True',
    ...
    def test_option_first(self):
        _options, sources = parse_command_line(['-V', 'file.pyx'])
        self.assertEqual(sources, ['file.pyx'])

    def test_file_inbetween(self):
        _options, sources = parse_command_line(['-V', 'file.pyx', '-a'])
        self.assertEqual(sources, ['file.pyx'])

    def test_option_trailing(self):
        _options, sources = parse_command_line(['file.pyx', '-V'])
        self.assertEqual(sources, ['file.pyx'])
