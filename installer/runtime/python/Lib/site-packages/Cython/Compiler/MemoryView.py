...
def valid_memslice_dtype(dtype, i=0):
    ...
    if dtype.is_struct and dtype.kind == 'struct':
        return all(valid_memslice_dtype(member.type) for member in dtype.scope.var_entries)
...
class MemoryViewSliceBufferEntry(Buffer.BufferEntry):
    ...
    def generate_buffer_slice_code(self, code, indices, dst, dst_type, have_gil,
        ...
        code.putln("{dst}.data = {src}.data;".format(**locals()))
        code.putln("{dst}.memview = {src}.memview;".format(**locals()))
...
class ContigSliceIter(SliceIter):
    def start_loops(self):
        ...
        code.putln(f"{type_decl} *__pyx_temp_pointer = ({type_decl} *) {self.slice_result}.data;")
...
def get_copy_new_utility(pos, from_memview, to_memview):
    ...
    for (access, _packing) in from_memview.axes:
        if access != 'direct':
