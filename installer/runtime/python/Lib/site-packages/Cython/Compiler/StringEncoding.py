...
def string_contains_lone_surrogates(ustring):
    ...
    """
    return any(c >= 55296 and c <= 57343 for c in map(ord, ustring))
...
class BytesLiteral(bytes):
    ...
    def utf8encode(self):
        raise AssertionError(f"this is not a unicode string: {self!r}")
...
def _build_specials_replacer():
    ...
    for special in _c_special:
        regexp = ''.join(['[{}]'.format(c.replace('\\', '\\\\')) for c in special])
        subexps.append(regexp)
        replacements[special.encode('ASCII')] = _to_escape_sequence(special).encode('ASCII')
    sub = re.compile(('({})'.format('|'.join(subexps))).encode('ASCII')).sub
