...
pythran_builtins = '__builtin__' if pythran_is_pre_0_9_6 else 'builtins'
...
def pythran_binop_type(op, tA, tB):
    if op == '**':
        return f'decltype(pythonic::numpy::functor::power{{}}(std::declval<{pythran_type(tA)}>(), std::declval<{pythran_type(tB)}>()))'
    else:
        return f"decltype(std::declval<{pythran_type(tA)}>() {op} std::declval<{pythran_type(tB)}>())"
...
def pythran_indexing_type(type_, indices):
    return type_remove_ref(f"decltype(std::declval<{pythran_type(type_)}>(){_index_access(_index_type_code, indices)})")
...
def is_type(type_, types):
    return any(getattr(type_, attr, False) for attr in types)
...
def pythran_get_func_include_file(func):
    func = np_func_to_list(func)
    return "pythonic/numpy/{}.hpp".format("/".join(func))
