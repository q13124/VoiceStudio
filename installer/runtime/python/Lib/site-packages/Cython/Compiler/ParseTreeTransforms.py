...
class _AssignmentExpressionChecker(TreeVisitor):
    ...
    def visit_AssignmentExpressionNode(self, node):
        ...
        if node.target_name in self.current_target_names:
            error(node.pos, f"assignment expression cannot rebind comprehension iteration variable '{node.target_name}'")
        elif node.target_name in self.all_target_names:
            error(node.pos, f"comprehension inner loop cannot rebind assignment expression target '{node.target_name}'")
...
def sort_common_subsequences(items):
    ...
    def contains(seq, x):
        for item in seq:
            if item is x or (item.is_sequence_constructor and contains(item.args, x)):
                return True
        return False
    def lower_than(a,b):
...
def flatten_parallel_assignments(input: list, output: list):
    ...
    for lhs in input[:-1]:
        ...
        if starred_targets > 1:
            ...
            for targets, expr in zip(lhs_targets, lhs.args, strict=False):
                targets.append(expr)
    ...
    for cascade, rhs in zip(lhs_targets, rhs_args, strict=False):
        if cascade:
            cascade.append(rhs)
...
def map_starred_assignment(lhs_targets: list, starred_assignments: list, lhs_args: list, rhs_args: list):
    ...
    pos = starred_rhs[0].pos if starred_rhs else target.pos
...
class InterpretCompilerDirectives(CythonTransform):
    ...
    def check_directive_scope(self, pos, directive, scope):
        ...
        if legal_scopes and scope not in legal_scopes:
            self.context.nonfatal_error(PostParseError(pos, f'The {directive} compiler directive '
                                        f'is not allowed in {scope} scope'))
            return False
        else:
        ...
        for name, value in directives.items():
            if name == 'locals':
                ...
                self.context.nonfatal_error(PostParseError(
                    ...
                    f"staticmethod, or final decorators, got {name}."))
    ...
    def _transform_with_gil(self, node, state):
        ...
        if isinstance(manager, ExprNodes.SimpleCallNode) and manager.args:
            if len(manager.args) > 1:
                self.context.nonfatal_error(
                    PostParseError(node.pos, f"Compiler directive {state} accepts one positional argument."))
...
class _HandleGeneratorArguments(VisitorTransform, SkipDeclarations):
    ...
    def visit_GeneratorExpressionNode(self, node):
        ...
        self._handle_ExprNode(node, do_visit_children=False)
                ...
                if handler_name:
                    if decorator.obj.name != node.name:
                        ...
                        error(decorator_node.pos,
                              f"Mismatching property names, expected '{decorator.obj.name}', got '{node.name}'")
...
if VALUE is not None:
    ...
    def _inject_pickle_methods(self, node):
        ...
        if cinit or non_py or (structs and not auto_pickle_forced):
            if cinit:
                ...
                msg = "{} cannot be converted to a Python object for pickling".format(','.join(f"self.{e.name}" for e in non_py))
            else:
                ...
                msg = ("Pickling of struct members such as {} must be explicitly requested "
                       "with @auto_pickle(True)".format(','.join(f"self.{e.name}" for e in structs)))

            if auto_pickle_forced:
                error(node.pos, msg)

            pickle_func = TreeFragment(f"""
    ...
    def visit_CStructOrUnionDefNode(self, node):
        ...
        init_assignments = []
        for entry, attr in zip(var_entries, attributes, strict=False):
...
class AdjustDefByDirectives(CythonTransform, SkipDeclarations):
    ...
    def visit_DefNode(self, node):
        ...
        has_explicit_exc_clause = except_val is not None
        ...
        if return_type_node is None and self.directives['annotation_typing']:
            ...
            except_val = (None, bool(return_type_node))
    ...
    def visit_GILStatNode(self, node):
        if node.condition is not None:
            error(node.condition.pos,
                  ...
                  f"`with {node.state}(<condition>)` statement")
            return node
        ...
        if was_nogil == is_nogil and self.nogil_state != Nodes.NoGilState.NoGilScope:
            if not was_nogil:
...
class TransformBuiltinMethods(EnvTransform):
    ...
    def visit_cython_attribute(self, node):
        ...
        if attribute:
            ...
            elif PyrexTypes.parse_basic_type(attribute) or self.context.cython_scope.lookup_qualified_name(attribute):
                pass
            else:
...
class DebugTransform(CythonTransform):
    ...
    def visit_FuncDefNode(self, node):
        ...
        pf_cname = '' if node.py_func is None else node.py_func.entry.func_cname
    ...
    def visit_NameNode(self, node):
        if (self.register_stepinto and
            ...
            attrs = {"name": node.entry.func_cname}
    ...
    def serialize_modulenode_as_function(self, node):
        ...
        """
        self._serialize_modulenode_as_function(node, {
    ...
    def serialize_local_variables(self, entries):
        for entry in entries.values():
            if not entry.cname:
                ...
                continue
            vartype = 'PythonObject' if entry.type.is_pyobject else 'CObject'
            ...
            if entry.from_closure:
                ...
                qname = f'{entry.scope.outer_scope.qualified_name}.{entry.scope.name}.{entry.name}'
            elif entry.in_closure:
