...
class Context:
    ...
    def find_module(self, module_name, from_module=None, pos=None, need_pxd=1,
        ...
        if debug_find_module:
            print(f"Context.find_module: module_name = {module_name}, from_module = {from_module}, pos = {pos}, need_pxd = {need_pxd}")
            ...
            if not pxd_pathname:
                ...
                if not pxd_pathname and need_pxd:
                    ...
                    if package_pathname and package_pathname.endswith(Utils.PACKAGE_FILES):
                        ...
                        error(pos, "'{}.pxd' not found".format(qualified_name.replace('.', os.sep)))
            if pxd_pathname:
                scope.pxd_file_loaded = True
                try:
                    if debug_find_module:
        ...
        try:
            with source_desc.get_file_object() as f:
                ...
                if self.options.formal_grammar:
                    try:
                        from Cython.Parser import ConcreteSyntaxTree
                    except ImportError:
                        raise RuntimeError(
...
def run_pipeline(source, options, full_module_name, context):
    ...
    if '.' in source.full_module_name and '.' in os.path.splitext(os.path.basename(abs_path))[0]:
        warning((source.source_desc, 1, 0),
                f"Dotted filenames ('{os.path.basename(abs_path)}') are deprecated."
                " Please use the normal Python package directory layout.", level=1)
    ...
    err, _enddata = Pipeline.run_pipeline(pipeline, source)
...
def get_fingerprint(cache, source, options):
    from Cython.Build.Cache import FingerprintFlags
    from Cython.Build.Dependencies import create_dependency_tree
