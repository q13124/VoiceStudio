...
def p_index(s: PyrexScanner, base):
    ...
    if is_single_value and len(subscripts[0]) == 2:
        ...
        index = indexes[0] if is_single_value else ExprNodes.TupleNode(pos, args=indexes)
...
def p_int_literal(s: PyrexScanner):
    ...
    # Python files.
    is_c_literal = None
    if unsigned or (longness and (longness == 'LL' or s.context.language_level >= 3)):
        is_c_literal = True
    if s.in_python_file:
        if is_c_literal:
...
def wrap_compile_time_constant(pos, value):
    ...
    error(pos, f"Invalid type for compile-time constant: {value!r} (type {value.__class__.__name__})")
    return None
    ...
    while s.sy == 'BEGIN_STRING' or s.sy == 'BEGIN_FT_STRING':
        ...
        if next_kind == 'c':
            ...
            if {kind, next_kind} in ({'f', 'u'}, {'f', ''}):
                ...
                error(pos, f"Cannot mix string literals of different types, expected {kind}'', got {next_kind}''")
                continue
...
def p_string_literal(s: PyrexScanner, kind_override=None) -> tuple:
    ...
    while 1:
        ...
        if s.sy == 'END_STRING':
            ...
            s.error(f"Unexpected token {s.sy!r}:{s.systring!r} in string literal")
    ...
    if not expr_string.strip():
        ...
        if s.sy == "!":
            ...
            if conversion_char in ['}', ':', '']:
                ...
                error(s.position(), f"invalid conversion character '{conversion_char}'")
                s.next()
...
def p_ft_string_middles(s: PyrexScanner,
    ...
    while True:
        ...
        if sy == "{":
            ...
            error(
                s.position(),
                f"Unexpected token {s.sy!r}:{s.systring!r} in {tf_string_kind}-string literal")
    return middles
    ...
    if c in "01234567":
        ...
        if c == 'N':
            ...
            try:
                ...
                s.error("Unknown Unicode character name {}".format(repr(escape_sequence[3:-1]).lstrip('u')), fatal=False)
...
def p_backquote_expr(s: PyrexScanner):
    ...
    s.expect('`')
    arg = args[0] if len(args) == 1 else ExprNodes.TupleNode(pos, args=args)
...
def p_expression_or_assignment(s: PyrexScanner):
    ...
    while s.sy == '=':
        s.next()
        expr = p_yield_expression(s) if s.sy == 'yield' else p_testlist_star_expr(s)
    ...
    if len(expr_list) == 1:
        if re.match(r"([-+*/%^&|]|<<|>>|\*\*|//|@)=", s.sy):
            ...
            operator = s.sy[:-1]
            s.next()
            rhs = p_yield_expression(s) if s.sy == 'yield' else p_testlist(s)
...
def p_return_statement(s: PyrexScanner):
    ...
    pos = s.position()
    s.next()
    value = p_testlist(s) if s.sy not in statement_terminators else None
...
def p_for_iterator(s: PyrexScanner, allow_testlist: cython.bint = True, is_async: cython.bint = False):
    pos = s.position()
    expr = p_testlist(s) if allow_testlist else p_or_test(s)
...
def p_except_clause(s: PyrexScanner):
    ...
    if s.sy != ':':
        ...
        exc_type = exc_type.args if isinstance(exc_type, ExprNodes.TupleNode) else [exc_type]
...
def p_positional_and_keyword_args(s: PyrexScanner, end_sy_set, templates = None) -> tuple:
    ...
    while s.sy not in end_sy_set:
        ...
        if s.sy != ',':
            if s.sy not in end_sy_set and parsed_type:
                s.error("Unmatched {}".format(" or ".join(end_sy_set)))
            break
        s.next()
...
def p_c_declarator(s: PyrexScanner, ctx = Ctx(),
    ...
    if not calling_convention_allowed and result.calling_convention and s.sy != '(':
        error(s.position(), f"{result.calling_convention} on something that is not a function")
    while s.sy in ('[', '('):
        pos = s.position()
        if s.sy == '[':
...
def p_c_array_declarator(s: PyrexScanner, base):
    pos = s.position()
    s.next()  # '['
    dim = p_testlist(s) if s.sy != ']' else None
    s.expect(']')
...
def p_cdef_statement(s: PyrexScanner, pos, ctx):
    ...
    if ctx.api and ctx.visibility not in ('private', 'public'):
...
def p_visibility(s: PyrexScanner, prev_visibility):
    ...
    if s.sy == 'IDENT' and s.systring in ('extern', 'public', 'readonly'):
        ...
        if prev_visibility != 'private' and visibility != prev_visibility:
            s.error(f"Conflicting visibility options '{prev_visibility}' and '{visibility}'", fatal=False)
        s.next()
    return visibility
...
@cython.cfunc
def p_def_statement(s: PyrexScanner, decorators: list | None = None, is_async_def: cython.bint = False):
    # s.sy == 'def'
...
def p_c_class_definition(s: PyrexScanner, pos,  ctx):
    ...
    if s.sy == ':':
        body_level = 'c_class_pxd' if ctx.level == 'module_pxd' else 'c_class'
...
def _extract_docstring(node) -> tuple:
    ...
    if node is None:
        ...
        stats = node.stats
        if isinstance(stats[0], Nodes.ExprStatNode) and stats[0].expr.is_string_literal:
...
def p_code(s: PyrexScanner, level=None, ctx=Ctx):
    ...
    if s.sy != 'EOF':
        s.error(f"Syntax error in statement [{s.sy!r},{s.systring!r}]")
    return body
        ...
        if m:
            ...
            for name in new_directives:
                if name not in result:
                    ...
                    s.error(f"Conflicting settings found for top-level directive {name}: {result[name]!r} and {new_directives[name]!r}", pos=pos)
...
def p_pattern_capture_target(s: PyrexScanner):
    ...
    s.next()
    if s.sy in ['.', '(', '=']:
        s.error(f"Illegal next symbol '{s.sy}'")
    return target
