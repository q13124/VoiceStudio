...
from Cython.Utils import add_metaclass, str_to_number
...
def embed_position(pos, docstring):
    if not Options.embed_pos_in_docstring:
        return docstring
    pos_line = 'File: {} (starting at line {})'.format(*relative_position(pos))
...
class Node:
    ...
    def gil_error(self, env=None):
        error(self.pos, f"{self.gil_message} not allowed without gil")
    ...
    def cpp_error(self):
        error(self.pos, f"{self.cpp_message} only allowed in c++")
    ...
    def clone_node(self):
        ...
        for attrname in result.child_attrs:
            ...
            if isinstance(value, list):
                setattr(result, attrname, list(value))
        return result
    ...
    def dump(self, level=0, filter_out=("pos",), cutoff=100, encountered=None):
        ...
        def dump_child(x, level):
            if isinstance(x, Node):
                ...
                return "[{}]".format(", ".join([dump_child(item, level) for item in x]))
            else:
                return repr(x)
...
class StatNode(Node):
    ...
    def generate_execution_code(self, code):
        raise InternalError(f"generate_execution_code not implemented for {self.__class__.__name__}")
...
class CArrayDeclaratorNode(CDeclaratorNode):
    ...
    def analyse(self, base_type, env, nonempty=0, visibility=None, in_pxd=False):
        ...
        if not base_type.is_complete():
            error(self.pos, f"Array element type '{base_type}' is incomplete")
        if base_type.is_pyobject:
...
class CFuncDeclaratorNode(CDeclaratorNode):
    ...
    def analyse(self, return_type, env, nonempty=0, directive_locals=None, visibility=None, in_pxd=False):
                    ...
                    if (visibility != 'extern'
                        ...
                        try:
                            ...
                            if self.exception_value.constant_result == type_default_value:
                                warning(self.pos, f"Ambiguous exception value, same as default return value: {self.exception_value.constant_result!r}")
        ...
        if callspec:
            ...
            if current and current != callspec:
                error(self.pos, f"cannot have both '{current}' and '{callspec}' "
                      "calling conventions")
    ...
    def declare_optional_arg_struct(self, func_type, env, fused_cname=None):
        ...
        arg_count_member = f'{Naming.pyrex_prefix}n'
...
class CArgDeclNode(Node):
    ...
    def calculate_default_value_code(self, code):
        if self.default_value is None and self.default:
                ...
                if type is not None:
                    ...
                    if self.templates:
                        if self.name not in self.templates:
                            error(self.pos, f"'{self.name}' is not a type identifier")
...
class CNestedBaseTypeNode(CBaseTypeNode):
    ...
    def analyse(self, env, could_be_name=None):
        ...
        if not base_type.is_cpp_class:
            error(self.pos, f"'{base_type}' is not a valid type scope")
            return PyrexTypes.error_type
        type_entry = base_type.scope.lookup_here(self.name)
        if not type_entry or not type_entry.is_type:
            error(self.pos, f"'{base_type}.{self.name}' is not a type identifier")
            return PyrexTypes.error_type
...
class TemplatedTypeNode(CBaseTypeNode):
    ...
    def _analyse_template_types(self, env, base_type):
        ...
        for template_node in self.positional_args:
            ...
            if ttype is None and base_type.is_cpp_class:
        ...
        for i, ttype in enumerate(template_types):
            ...
            if (require_python_types and not ttype.is_pyobject) or (require_optional_types and not ttype.can_be_optional()):
                if ttype.equivalent_type and not template_node.as_cython_attribute():
                    ...
                    error(template_node.pos, f"{base_type.python_type_constructor_name}[...] cannot be applied to type {ttype}")
    ...
    def analyse(self, env, could_be_name=False, base_type=None):
        ...
        if ((base_type.is_cpp_class and base_type.is_template_type()) or
            ...
            if self.keyword_args and self.keyword_args.key_value_pairs:
                ...
                error(self.pos, f"{tp} cannot take keyword arguments")
            ...
            if len(self.positional_args) > 1 or self.keyword_args.key_value_pairs:
                ...
                dimension = None if not self.positional_args else self.positional_args[0]
...
class CVarDefNode(StatNode):
    ...
    def analyse_declarations(self, env, dest_scope=None):
        ...
        templates = self.declarators[0].analyse_templates() if self.declarators else None
        ...
        for declarator in self.declarators:
            ...
            if not type.is_complete():
                if not ((self.visibility == 'extern' and type.is_array) or type.is_memoryviewslice):
                    error(declarator.pos, f"Variable type '{type}' is incomplete")
    ...
    def analyse_declarations(self, env):
        ...
        def base_ok(base_class):
            if base_class.is_cpp_class or base_class.is_struct:
                ...
                error(self.pos, f"Base class '{base_class}' not a struct or class.")
        ...
        if self.attributes is not None:
            ...
            for func in func_attributes(self.attributes):
                ...
                if self.templates is not None:
                    func.template_declaration = "template <typename {}>".format(", typename ".join(template_names))
...
class CEnumDefNode(StatNode):
    ...
    def generate_execution_code(self, code):
        ...
        for item in self.entry.enum_values:
            code.putln(f"{temp} = PyLong_FromLong({item.cname}); {code.error_goto_if_null(temp, item.pos)}")
            code.put_gotref(temp, PyrexTypes.py_object_type)
            code.putln(f'if (PyDict_SetItemString({code.name_in_module_state(Naming.moddict_cname)}, {item.name.as_c_string_literal()}, {temp}) < 0) {code.error_goto(item.pos)}')
...
class CEnumDefItemNode(StatNode):
    ...
    def analyse_enum_declarations(self, env, enum_entry, incremental_int_value):
        ...
        cname = f"{enum_entry.cname}::{self.name}" if enum_entry.type.is_cpp_enum else self.cname
...
class FuncDefNode(StatNode, BlockNode):
    ...
    def generate_function_definitions(self, env, code):
        ...
        if is_buffer_slot and 'cython_unused' not in self.modifiers:
        ...
        if not return_type.is_void:
            ...
            code.putln(f"{return_type.declaration_code(Naming.retval_cname)}{init};")
        ...
        if self.needs_closure:
            ...
            if not slot_func_cname:
                slot_func_cname = f'{code.name_in_module_state(lenv.scope_class.type.typeptr_cname)}->tp_new'
            code.putln(f"{Naming.cur_scope_cname} = ({lenv.scope_class.type.empty_declaration_code()}){slot_func_cname}({code.name_in_module_state(lenv.scope_class.type.typeptr_cname)}, {code.name_in_module_state(Naming.empty_tuple)}, NULL);")
            code.putln(f"if (unlikely(!{Naming.cur_scope_cname})) {{")
        ...
        if self.needs_outer_scope:
            if self.is_cyfunction:
                code.putln(f"{outer_scope_cname} = ({cenv.scope_class.type.empty_declaration_code()}) __Pyx_CyFunction_GetClosure({Naming.self_cname});")
            else:
                code.putln(f"{outer_scope_cname} = ({cenv.scope_class.type.empty_declaration_code()}) {Naming.self_cname};")
            if lenv.is_passthrough:
                code.putln(f"{Naming.cur_scope_cname} = {outer_scope_cname};")
        ...
        if not self.body.is_terminator:
            if return_type.is_pyobject:
                ...
                if val:
                    code.putln(f"{Naming.retval_cname} = {val};")
                elif not return_type.is_void:
                    code.putln(f"__Pyx_pretend_to_initialize(&{Naming.retval_cname});")
            ...
            if err_val is not None or exc_check:
                ...
                if code.funcstate.error_without_exception:
                    tempvardecl_code.putln(
                        f"int {Naming.error_without_exception_cname} = 0; /* StopIteration */"
                    )
                    code.putln(f"if (!{Naming.error_without_exception_cname}) {{")
                code.put_add_traceback(self.entry.qualified_name)
                if code.funcstate.error_without_exception:
                    code.putln("}")
            else:
                warning(self.entry.pos,
                        f"Unraisable exception in function '{self.entry.qualified_name}'.", 0)
                assure_gil('error')
        ...
        if self.entry.is_special and self.entry.name == "__hash__":
            ...
            code.putln(f"if (unlikely({Naming.retval_cname} == -1) && !PyErr_Occurred()) {Naming.retval_cname} = -2;")
        ...
        if preprocessor_guard:
            code.putln(f"#endif /*!({preprocessor_guard})*/")
    ...
    def declare_argument(self, env, arg):
        if arg.type.is_void:
            ...
            error(arg.pos, f"Argument type '{arg.type}' is incomplete")
    ...
    def generate_arg_type_test(self, arg, code):
        ...
        if arg.type.typeobj_is_available():
            ...
            arg_code = f"((PyObject *){arg.entry.cname})"
            exact = 0
    ...
    def generate_arg_none_check(self, arg, code):
        ...
        cname = f"{arg.entry.cname}.memview" if arg.type.is_memoryviewslice else arg.entry.cname

        code.putln(f'if (unlikely(((PyObject *){cname}) == Py_None)) {{')
    ...
    def getbuffer_check(self, code):
        ...
        code.putln(f"if (unlikely({view} == NULL)) {{")
    ...
    def getbuffer_init(self, code):
        ...
        if obj_type and obj_type.is_pyobject:
            code.put_init_to_py_none(f"{view}->obj", obj_type)
            code.put_giveref(f"{view}->obj", obj_type)  # Do not refnanny object within structs
        else:
            code.putln(f"{view}->obj = NULL;")

    def getbuffer_error_cleanup(self, code):
        py_buffer, obj_type = self._get_py_buffer_info()
        view = py_buffer.cname
        if obj_type and obj_type.is_pyobject:
            code.putln(f"if ({view}->obj != NULL) {{")
            code.put_gotref(f"{view}->obj", obj_type)
            code.put_decref_clear(f"{view}->obj", obj_type)
            code.putln("}")
        else:
            code.putln(f"Py_CLEAR({view}->obj);")

    def getbuffer_normal_cleanup(self, code):
        py_buffer, obj_type = self._get_py_buffer_info()
        view = py_buffer.cname
        if obj_type and obj_type.is_pyobject:
            code.putln(f"if ({view}->obj == Py_None) {{")
            code.put_gotref(f"{view}->obj", obj_type)
            code.put_decref_clear(f"{view}->obj", obj_type)
            code.putln("}")
...
class CFuncDefNode(FuncDefNode):
    ...
    def analyse_declarations(self, env):
        ...
        for formal_arg, type_arg in zip(self.args, typ.args, strict=False):
    ...
    def generate_function_header(self, code, with_pymethdef, with_opt_args=1, with_dispatch=1, cname=None):
        ...
        for arg in type.args[:len(type.args)-type.optional_arg_count]:
            ...
            if not entry.cf_used:
                arg_decl = f'CYTHON_UNUSED {arg_decl}'
        ...
        if with_dispatch and self.overridable:
            ...
            if self.override:
                ...
                arg_decls.append(f'CYTHON_UNUSED {dispatch_arg}')
        ...
        if needs_proto:
            ...
            code.globalstate.parts['module_declarations'].putln(
                f"{storage_class}{modifiers}{header}; /* proto*/")
            if preprocessor_guard:
                code.globalstate.parts['module_declarations'].putln("#endif")
        code.putln(f"{storage_class}{modifiers}{header} {{")
        ...
        for arg in self.args:
            if arg.default:
                ...
                if self.override or entry.cf_used:
                    ...
                    code.putln(f'{arg.type.declaration_code(arg.cname)} = {result};')
    ...
    def generate_argument_parsing_code(self, env, code):
        ...
        if self.type.optional_arg_count:
            code.putln(f'if ({Naming.optional_args_cname}) {{')
            for arg in self.args:
                if arg.default:
                    entry = scope.lookup(arg.name)
                    if self.override or entry.cf_used:
                        code.putln(f'if ({Naming.optional_args_cname}->{Naming.pyrex_prefix}n > {i}) {{')
                        declarator = arg.declarator
                        while not hasattr(declarator, 'name'):
                            declarator = declarator.base
                        code.putln(f'{arg.cname} = {Naming.optional_args_cname}->{self.type.opt_arg_cname(declarator.name)};')
                        used += 1
                    i += 1
            for _ in range(used):
                code.putln('}')
        ...
        def put_into_closure(entry):
            if entry.in_closure and not arg.default:
                code.putln(f'{entry.cname} = {entry.original_cname};')
                if entry.type.is_memoryviewslice:
    ...
    def generate_wrapper_functions(self, code):
        ...
        while entry.prev_entry is not None:
            ...
            entry.func_cname = f"{self.entry.func_cname}{Naming.pyrex_prefix}wrap_{k}"
            code.putln()
...
class DefNode(FuncDefNode):
    ...
    def as_cfunction(self, cfunc=None, scope=None, overridable=True, returns=None, except_val=None, has_explicit_exc_clause=False,
        ...
        if cfunc is None:
            ...
            for i, (formal_arg, type_arg) in enumerate(zip(self.args, cfunc_type.args, strict=False)):
                name_declarator, type = formal_arg.analyse(scope, nonempty=1,
    ...
    def generate_function_header(self, code, with_pymethdef, proto_only=0):
        ...
        def arg_decl_code(arg):
            entry = arg.entry
            cname = entry.original_cname if entry.in_closure else entry.cname
        ...
        decls_code.putln(
            f"static {dc}({arg_code}); /* proto */")
        if preprocessor_guard:
            decls_code.putln("#endif")
        code.putln(f"static {dc}({arg_code}) {{")
...
class DefNodeWrapper(FuncDefNode):
    ...
    def generate_function_body(self, code):
        ...
        for arg in self.args:
            if arg.type.is_cpp_class:
                ...
                args.append(f"__PYX_STD_MOVE_IF_SUPPORTED({arg.entry.cname})")
        ...
        args = ', '.join(args)
        if not self.return_type.is_void:
            code.put(f'{Naming.retval_cname} = ')
        code.putln(f'{self.target.entry.pyfunc_cname}({args});')
    ...
    def generate_function_definitions(self, env, code):
        ...
        if not self.return_type.is_void:
            code.putln(f'{self.return_type.declaration_code(Naming.retval_cname)}{retval_init};')
        code.put_declare_refcount_context()
        code.put_setup_refcount_context(EncodedString(f'{self.name} (wrapper)'))
    ...
    def generate_function_header(self, code, with_pymethdef, proto_only=0):
        ...
        if sig.has_dummy_arg or self.self_in_stararg:
            arg_code = f"PyObject *{Naming.self_cname}"
        ...
        for arg in self.args:
            if not arg.is_generic:
                if arg.is_self_arg or arg.is_type_arg:
                    arg_code_list.append(f"PyObject *{arg.hdr_cname}")
                else:
        ...
        if sig.has_generic_args:
            varargs_args = f"PyObject *{Naming.args_cname}, PyObject *{Naming.kwds_cname}"
            if sig.use_fastcall:
                fastcall_args = f"PyObject *const *{Naming.args_cname}, Py_ssize_t {Naming.nargs_cname}, PyObject *{Naming.kwds_cname}"
                arg_code_list.append(
        ...
        if entry.is_special:
            for n in range(len(self.args), sig.max_num_fixed_args()):
                arg_code_list.append(f"CYTHON_UNUSED PyObject *unused_arg_{n}")
        arg_code = ", ".join(arg_code_list)
        ...
        header = f"{mf}static {dc}({arg_code})"
        code.putln(f"{header}; /*proto*/")
    ...
    def generate_argument_declarations(self, env, code):
        for arg in self.args:
            if arg.is_generic:
                if arg.needs_conversion:
                    code.putln(f"PyObject *{arg.hdr_cname} = 0;")
                else:
        ...
        if self.signature_has_generic_args():
            ...
            nargs_code = f"CYTHON_UNUSED Py_ssize_t {Naming.nargs_cname};"
        ...
        code.putln(f'CYTHON_UNUSED PyObject *const *{Naming.kwvalues_cname};')
    ...
    def generate_argument_parsing_code(self, env, code, decl_code):
        ...
        if self.signature_has_generic_args():
            ...
            code.putln(f"{Naming.nargs_cname} = PyTuple_GET_SIZE({Naming.args_cname});")
            code.putln("#else")
            ...
            code.putln("{} = PyTuple_Size({}); if ({}) return {};".format(
                Naming.nargs_cname,
                Naming.args_cname,
                code.unlikely(f"{Naming.nargs_cname} < 0"),
                self.error_value(),
            ))
            code.putln("#endif")
            if self.signature.use_fastcall:
        ...
        code.putln(f'{Naming.kwvalues_cname} = __Pyx_KwValues_{self.signature.fastvar}({Naming.args_cname}, {Naming.nargs_cname});')
    ...
    def generate_tuple_and_keyword_parsing_code(self, args, code, decl_code):
        ...
        if accept_kwd_args:
            ...
            code.putln(f"PyObject ** const {Naming.pykwdlist_cname}[] = {{{non_pos_args_id}}};")
        ...
        if self.num_required_kw_args > 0:
            kw_unpacking_condition = f"likely({kw_unpacking_condition})"

        # --- optimised code when we receive keyword arguments
        code.putln(f"if ({kw_unpacking_condition}) {{")
    ...
    def generate_arg_assignment(self, arg, item, code):
        if arg.type.is_pyobject:
            ...
            entry = arg.entry
            code.putln(f"{entry.cname} = {item};")
        else:
            if arg.type.from_py_function:
                if arg.default:
                    # C-typed default arguments must be handled here
                    code.putln(f'if ({item}) {{')
                code.putln(arg.type.from_py_call_code(
                    item, arg.entry.cname, arg.pos, code))
                if arg.default:
                    code.putln('} else {')
                    code.putln(f"{arg.entry.cname} = {arg.calculate_default_value_code(code)};")
                    if arg.type.is_memoryviewslice:
                        code.put_var_incref_memoryviewslice(arg.entry, have_gil=True)
                    code.putln('}')
            else:
                error(arg.pos, f"Cannot convert Python object argument to type '{arg.type}'")

    def generate_stararg_init_code(self, max_positional_args, code):
        # If the "**kwargs" parameter is unused, we keep it as NULL to avoid useless overhead.
        if self.starstar_arg and self.starstar_arg.entry.cf_used:
            self.starstar_arg.entry.xdecref_cleanup = 0
            code.putln(f'{self.starstar_arg.entry.cname} = PyDict_New(); if (unlikely(!{self.starstar_arg.entry.cname})) return {self.error_value()};')
            code.put_var_gotref(self.starstar_arg.entry)
        if self.star_arg:
            self.star_arg.entry.xdecref_cleanup = 0
            if max_positional_args == 0:
                ...
                assert not self.signature.use_fastcall
                code.put_incref(Naming.args_cname, py_object_type)
                code.putln(f"{self.star_arg.entry.cname} = {Naming.args_cname};")
            else:
    ...
    def generate_argument_values_setup_code(self, args, code, decl_code):
        ...
        if self.target.defaults_struct:
            code.putln(f'struct {self.target.defaults_struct} *{Naming.dynamic_args_cname} = __Pyx_CyFunction_Defaults(struct {self.target.defaults_struct}, {Naming.self_cname});')
    ...
    def generate_keyword_unpacking_code(self, max_positional_args, all_args, code):
        ...
        if num_pos_only_args > 0:
            ...
            code.putln(f'const Py_ssize_t kwd_pos_args = {Naming.nargs_cname};')
    ...
    def generate_arg_conversion(self, arg, code):
        ...
        if old_type.is_pyobject:
            if arg.default:
                code.putln(f"if ({arg.hdr_cname}) {{")
            else:
                code.putln(f"assert({arg.hdr_cname}); {{")
            ...
            if new_type.assignable_from(old_type):
                code.putln(f"{arg.entry.cname} = {arg.hdr_cname};")
            else:
                error(arg.pos, f"Cannot convert 1 argument from '{old_type}' to '{new_type}'")
    ...
    def generate_arg_conversion_from_pyobject(self, arg, code):
        ...
        if new_type.from_py_function:
            ...
            error(arg.pos, f"Cannot convert Python object argument to type '{new_type}'")

    def generate_arg_conversion_to_pyobject(self, arg, code):
        old_type = arg.hdr_type
        func = old_type.to_py_function
        if func:
            code.putln(f"{arg.entry.cname} = {func}({arg.hdr_cname}); {code.error_goto_if_null(arg.entry.cname, arg.pos)}")
            code.put_var_gotref(arg.entry)
        else:
            error(arg.pos, f"Cannot convert argument of type '{old_type}' to Python object")
...
class GeneratorBodyDefNode(DefNode):
    ...
    def generate_function_header(self, code, proto=False):
        header = f"static PyObject *{self.entry.func_cname}(__pyx_CoroutineObject *{Naming.generator_cname}, CYTHON_UNUSED PyThreadState *{Naming.local_tstate_cname}, PyObject *{Naming.sent_value_cname})"
        if proto:
            code.putln(f'{header}; /* proto */')
        else:
            code.putln(f'{header} /* generator body */\n{{')
    ...
    def generate_function_definitions(self, env, code):
        ...
        # ----- Local variables
        code.putln(f"PyObject *{Naming.retval_cname} = NULL;")
        tempvardecl_code = code.insertion_point()
        code.put_declare_refcount_context()
        ...
        if self.is_inlined and self.inlined_comprehension_type is not None:
            ...
            if target_type is Builtin.list_type:
                ...
                raise InternalError(
                    f"invalid type of inlined comprehension: {target_type}")
            code.putln(f"{Naming.retval_cname} = {comp_init}; {code.error_goto_if_null(Naming.retval_cname, self.pos)}")
        ...
        if lenv.scope_class.type.scope.var_entries:
            closure_init_code.putln('{} = {};'.format(
                ...
                lenv.scope_class.type.cast_code(f'{Naming.generator_cname}->closure')))
            # FIXME: this silences a potential "unused" warning => try to avoid unused closures in more cases
            code.putln(f"CYTHON_MAYBE_UNUSED_VAR({Naming.cur_scope_cname});")
        ...
        code.putln("#endif")
        code.putln(f'{Naming.generator_cname}->resume_label = -1;')
        # clean up as early as possible to help breaking any reference cycles
        code.putln(f'__Pyx_Coroutine_clear((PyObject*){Naming.generator_cname});')
        code.put_finish_refcount_context()
        code.putln(f"return {Naming.retval_cname};")
        code.putln("}")
        ...
        resume_code.putln(f"switch ({Naming.generator_cname}->resume_label) {{")

        resume_code.putln(f"case 0: goto {first_run_label};")
...
class OverrideCheckNode(StatNode):
    ...
    def analyse_expressions(self, env):
        ...
        first_arg = 0 if self.py_func.is_module_scope else 1
        from . import ExprNodes
    ...
    def generate_execution_code(self, code):
        ...
        if self.py_func.is_module_scope:
            self_arg = f"((PyObject *){Naming.module_cname})"
        else:
            self_arg = f"((PyObject *){self.args[0].cname})"
        code.putln("/* Check if called by wrapper */")
        code.putln(f"if (unlikely({Naming.skip_dispatch_cname})) ;")
        ...
        # instance at a time.
        code.putln(f"static PY_UINT64_T {Naming.tp_dict_version_temp} = __PYX_DICT_VERSION_INIT, {Naming.obj_dict_version_temp} = __PYX_DICT_VERSION_INIT;")
        code.putln(f"if (unlikely(!__Pyx_object_dict_version_matches({self_arg}, {Naming.tp_dict_version_temp}, {Naming.obj_dict_version_temp}))) {{")
        code.putln(f"PY_UINT64_T {Naming.type_dict_guard_temp} = __Pyx_get_tp_dict_version({self_arg});")
        code.putln("#endif")
        ...
        code.putln(f"{func_node_temp} = __Pyx_PyObject_GetAttrStr({self_arg}, {interned_attr_cname}); {code.error_goto_if_null(func_node_temp, self.pos)}")
        code.put_gotref(func_node_temp, py_object_type)

        code.putln(f"if (!__Pyx_IsSameCFunction({func_node_temp}, (void(*)(void)) {method_entry.func_cname})) {{")
        self.body.generate_execution_code(code)
        code.putln("}")
        ...
        code.putln("#if CYTHON_USE_DICT_VERSIONS && CYTHON_USE_PYTYPE_LOOKUP && CYTHON_USE_TYPE_SLOTS")
        code.putln(f"{Naming.tp_dict_version_temp} = __Pyx_get_tp_dict_version({self_arg});")
        code.putln(f"{Naming.obj_dict_version_temp} = __Pyx_get_object_dict_version({self_arg});")
        ...
        code.putln(f"if (unlikely({Naming.type_dict_guard_temp} != {Naming.tp_dict_version_temp})) {{")
        code.putln(f"{Naming.tp_dict_version_temp} = {Naming.obj_dict_version_temp} = __PYX_DICT_VERSION_INIT;")
        code.putln("}")
        code.putln("#endif")
...
class PyClassDefNode(ClassDefNode):
    ...
    def generate_execution_code(self, code):
        ...
        if self.orig_bases:
            ...
            code.putln(f"if ({self.bases.result()} != {self.orig_bases.result()}) {{")
            code.putln(
                code.error_goto_if_neg(f'PyDict_SetItemString({self.dict.result()}, "__orig_bases__", {self.orig_bases.result()})',
                    self.pos
            ))
            code.putln("}")
...
class CClassDefNode(ClassDefNode):
    ...
    def analyse_declarations(self, env):
        ...
        if self.bases.args:
            ...
            if base_type is None:
                error(base.pos, f"First base of '{self.class_name}' is not an extension type")
                ...
                error(base.pos, f"'{base_type}' is not an extension type")
            elif not base_type.is_complete():
                error(base.pos, f"Base class '{base_type.name}' of type '{self.class_name}' is incomplete")
            elif base_type.scope and base_type.scope.directives and \
                     base_type.is_final_type:
                error(base.pos, f"Base class '{base_type}' of type '{self.class_name}' is final")
            elif base_type.is_builtin_type and \
                     base_type.name in ('tuple', 'bytes'):
                error(base.pos, f"inheritance from PyVarObject types like '{base_type.name}' is not currently supported")
            else:
        ...
        if self.visibility == 'extern':
            if (self.module_name == '__builtin__' and
                ...
                warning(self.pos, f"{self.class_name} already a builtin Cython type", 1)
    ...
    def generate_execution_code(self, code):
        ...
        if not self.entry.type.early_init:
            ...
            if self.type_init_args:
                ...
                code.putln(f"{bases} = __Pyx_PyTuple_GET_ITEM({self.type_init_args.result()}, 1);")
                ...
                code.putln("} else")
                code.putln("#endif")
                code.putln(f"{trial_type} = __Pyx_PyType_GetSlot(&PyType_Type, tp_new, newfunc)(&PyType_Type, {self.type_init_args.result()}, NULL);")
                code.putln(code.error_goto_if_null(trial_type, self.pos))
                code.put_gotref(trial_type, py_object_type)
                code.putln(f"if (__Pyx_PyType_GetSlot((PyTypeObject*) {trial_type}, tp_base, PyTypeObject*) != {first_base}) {{")
                # trial_type is a heaptype so GetSlot works in all versions of the limited API
                trial_type_base = f"__Pyx_PyType_GetSlot((PyTypeObject*) {trial_type}, tp_base, PyTypeObject*)"
                code.putln(f"__Pyx_TypeName base_name = __Pyx_PyType_GetFullyQualifiedName({trial_type_base});")
                code.putln(f"__Pyx_TypeName type_name = __Pyx_PyType_GetFullyQualifiedName({first_base});")
    ...
    def generate_type_ready_code(entry, code, bases_tuple_cname=None, check_heap_type_bases=False):
        ...
        if entry.visibility == 'extern':
            ...
            if type.typeobj_cname:
                ...
                code.putln(f"{type.typeptr_cname} = &{type.typeobj_cname};")
            return
        # TODO: remove 'else:' and dedent
        else:
            assert typeptr_cname
            assert type.typeobj_cname
            typespec_cname = f"{type.typeobj_cname}_spec"
            ...
            if not bases_tuple_cname and scope.parent_type.base_type:
                ...
                code.putln(f"{tuple_temp} = PyTuple_Pack(1, (PyObject *){code.typeptr_cname_in_module_state(scope.parent_type.base_type)}); {code.error_goto_if_null(tuple_temp, entry.pos)}")
            ...
            if bases_tuple_cname or tuple_temp:
                ...
                code.putln(f"{typeptr_cname} = (PyTypeObject *) __Pyx_PyType_FromModuleAndSpec({Naming.module_cname}, &{typespec_cname}, {bases_tuple_cname or tuple_temp});")
                if tuple_temp:
                ...
                code.putln(
                    f"{typeptr_cname} = (PyTypeObject *) __Pyx_PyType_FromModuleAndSpec({Naming.module_cname}, &{typespec_cname}, NULL); {code.error_goto_if_null(typeptr_cname, entry.pos)}")
            ...
            if not buffer_slot.is_empty(scope):
                ...
                code.putln(f"{typeptr_cname}->{buffer_slot.slot_name} = {buffer_slot.slot_code(scope)};")
                ...
                for buffer_method_name in ("__getbuffer__", "__releasebuffer__"):
                    ...
                    if buffer_slot.slot_code(scope) == "0" and not TypeSlots.get_base_slot_function(scope, buffer_slot):
                        code.putln(f"if (!{typeptr_cname}->tp_as_buffer->{buffer_slot.slot_name} &&"
                                   f" {typeptr_cname}->tp_base->tp_as_buffer &&"
                                   f" {typeptr_cname}->tp_base->tp_as_buffer->{buffer_slot.slot_name}) {{")
                        code.putln(f"{typeptr_cname}->tp_as_buffer->{buffer_slot.slot_name} = {typeptr_cname}->tp_base->tp_as_buffer->{buffer_slot.slot_name};")
                        code.putln("}")
            ...
            code.put_error_if_neg(entry.pos, f"__Pyx_fix_up_extension_type_from_spec(&{typespec_cname}, {typeptr_cname})")

            code.putln("#else")
            if bases_tuple_cname:
                code.put_incref(bases_tuple_cname, py_object_type)
                code.put_giveref(bases_tuple_cname, py_object_type)
                code.putln(f"{type.typeobj_cname}.tp_bases = {bases_tuple_cname};")
            code.putln(f"{typeptr_cname} = &{type.typeobj_cname};")
            ...
            while base_type:
                if base_type.is_external and base_type.objstruct_cname != "PyTypeObject":
                    ...
                    code.put_error_if_neg(entry.pos, f"__Pyx_validate_extern_base({base_typeptr_cname})")
                    code.putln("}")
                    break
                base_type = base_type.base_type
            ...
            code.put_error_if_neg(entry.pos, f"__Pyx_PyType_Ready({typeptr_cname})")
            code.putln("#endif")
            ...
            if getattr_slot_func == '0' and dictoffset_slot_func == '0':
                ...
                code.putln("if ((CYTHON_USE_TYPE_SLOTS && CYTHON_USE_PYTYPE_LOOKUP) &&"
                           f" likely(!{typeptr_cname}->tp_dictoffset && {typeptr_cname}->tp_getattro == PyObject_GenericGetAttr)) {{")
                code.putln(f"{typeptr_cname}->tp_getattro = PyObject_GenericGetAttr;")
                code.putln("}")
            ...
            for func in entry.type.scope.pyfunc_entries:
                ...
                if (func.is_special and Options.docstrings and
                    ...
                    code.putln(
                        f"{func.wrapperbase_cname} = *((PyWrapperDescrObject *)wrapper)->d_base;")
                    code.putln(
                        f"{func.wrapperbase_cname}.doc = {func.doc_cname};")
                    code.putln(
                        f"((PyWrapperDescrObject *)wrapper)->d_base = &{func.wrapperbase_cname};")
                    code.putln("}")
                    code.putln("}")
                    code.putln('#endif')
                    if preprocessor_guard:
            ...
            if type.vtable_cname:
                ...
                code.put_error_if_neg(entry.pos, f"__Pyx_SetVtable({typeptr_cname}, {type.vtabptr_cname})")
                code.globalstate.use_utility_code(
                    UtilityCode.load_cached('MergeVTables', 'ImportExport.c'))
                code.put_error_if_neg(entry.pos, f"__Pyx_MergeVtables({typeptr_cname})")
            if not type.scope.is_internal and not type.scope.directives.get('internal'):
                # scope.is_internal is set for types defined by
                # Cython (such as closures), the 'internal'
                # directive is set by users
                code.put_error_if_neg(entry.pos, f"PyObject_SetAttr({Naming.module_cname}, {code.intern_identifier(scope.class_name)}, (PyObject *) {typeptr_cname})")

            weakref_entry = scope.lookup_here("__weakref__") if not scope.is_closure_class_scope else None
            if weakref_entry:
                if weakref_entry.type is py_object_type:
                    tp_weaklistoffset = f"{typeptr_cname}->tp_weaklistoffset"
                    if type.typedef_flag:
                        objstruct = type.objstruct_cname
                    else:
                        objstruct = f"struct {type.objstruct_cname}"
                    code.putln("#if CYTHON_USE_TYPE_SLOTS")
                    code.putln(f"if ({tp_weaklistoffset} == 0) {tp_weaklistoffset} = offsetof({objstruct}, {weakref_entry.cname});")
                    code.putln("#endif")
                else:
            ...
            if scope.lookup_here("__reduce_cython__") if not scope.is_closure_class_scope else None:
                ...
                code.put_error_if_neg(entry.pos, f"__Pyx_setup_reduce((PyObject *) {typeptr_cname})")
...
class SingleAssignmentNode(AssignmentNode):
    ...
    def unroll(self, node, target_size, env):
        ...
        if slice_size != target_size:
            error(self.pos, f"Assignment to/from slice of wrong length, expected {slice_size}, got {target_size}")
            return
...
class CascadedAssignmentNode(AssignmentNode):
    ...
    def generate_assignment_code(self, code, overloaded_assignment=False):
        ...
        for lhs, rhs, overload in zip(self.lhs_list, self.cloned_values, self.assignment_overloads, strict=False):
            rhs.generate_evaluation_code(code)
    ...
    def annotate(self, code):
        for rhs in self.coerced_values:
            rhs.annotate(code)
        for lhs, rhs in zip(self.lhs_list, self.cloned_values, strict=False):
            lhs.annotate(code)
            rhs.annotate(code)
        self.rhs.annotate(code)
...
class InPlaceAssignmentNode(AssignmentNode):
    ...
    def generate_execution_code(self, code):
        ...
        if lhs.is_buffer_access or lhs.is_memview_index:
            ...
            code.putln(f"{lhs.result()} {c_op}= {rhs.result()};")
...
class PrintStatNode(StatNode):
    ...
    def generate_execution_code(self, code):
        ...
        if len(self.arg_tuple.args) == 1 and self.append_newline:
            ...
            code.putln(
                f"if (__Pyx_PrintOne({stream_result}, {arg.py_result()}) < 0) {code.error_goto(self.pos)}")
...
class ExecStatNode(StatNode):
    ...
    def generate_execution_code(self, code):
        ...
        code.putln("{} = __Pyx_PyExec3({}, {}, {});".format(temp_result, *args))
        for arg in self.args:
            arg.generate_disposal_code(code)
            arg.free_temps(code)
        code.putln(
    ...
    def generate_execution_code(self, code):
        ...
        for arg in self.args:
            if (arg.type.is_pyobject or
                ...
                arg.generate_evaluation_code(code)
                code.putln(f"delete {arg.result()};")
                arg.generate_disposal_code(code)
                arg.free_temps(code)
...
class ReturnStatNode(StatNode):
    ...
    def generate_execution_code(self, code):
        ...
        if value:
            ...
            if self.return_type.is_memoryviewslice:
                ...
                code.putln(f"{Naming.retval_cname} = {value.result_as(self.return_type)};")
                value.generate_post_assignment_code(code)
    ...
    def put_return(self, code, value):
        ...
        code.putln(f"{Naming.retval_cname} = {value};")
...
class RaiseStatNode(StatNode):
    ...
    def generate_execution_code(self, code):
        ...
        if self.builtin_exc_name == 'MemoryError':
            ...
            code.putln(f'{Naming.error_without_exception_cname} = 1;')
            code.putln(f'{code.error_goto(None)};')
        ...
        code.putln(
            f"__Pyx_Raise({type_code}, {value_code}, {tb_code}, {cause_code});")
...
class ReraiseStatNode(StatNode):
    ...
    def generate_execution_code(self, code):
        ...
        if vars:
            ...
            code.putln("__Pyx_ErrRestoreWithState({}, {}, {});".format(*tuple(vars)))
...
class AssertStatNode(StatNode):
    ...
    def generate_execution_code(self, code):
        ...
        code.putln(
            f"if (unlikely(!{self.condition.result()})) {{")
        ...
        code.putln(f"if ((1)); else {code.error_goto(self.pos, used=False)}")
        code.putln("#endif")
...
class SwitchStatNode(StatNode):
    ...
    def generate_execution_code(self, code):
        ...
        code.mark_pos(self.pos)
        code.putln(f"switch ({self.test.result()}) {{")
        for case in self.cases:
...
class WhileStatNode(LoopNode, StatNode):
    ...
    def generate_execution_code(self, code):
        ...
        if self.condition:
            ...
            self.condition.generate_disposal_code(code)
            code.putln(
                f"if (!{self.condition.result()}) break;")
            self.condition.free_temps(code)
        self.body.generate_execution_code(code)
...
class DictIterationNextNode(Node):
    ...
    def generate_execution_code(self, code):
        ...
        code.putln(f"{result_temp} = __Pyx_dict_iter_next({self.dict_obj.py_result()}, {self.expected_size.result()}, &{self.pos_index_var.result()}, {temp_addresses[0]}, {temp_addresses[1]}, {temp_addresses[2]}, {self.is_dict_flag.result()});")
        code.putln(f"if (unlikely({result_temp} == 0)) break;")
        code.putln(code.error_goto_if(f"{result_temp} == -1", self.pos))
...
class SetIterationNextNode(Node):
    ...
    def generate_execution_code(self, code):
        ...
        result_temp = code.funcstate.allocate_temp(PyrexTypes.c_int_type, False)
        code.putln(f"{result_temp} = __Pyx_set_iter_next({self.set_obj.py_result()}, {self.expected_size.result()}, &{self.pos_index_var.result()}, &{value_ref.result()}, {self.is_set_flag.result()});")
        code.putln(f"if (unlikely({result_temp} == 0)) break;")
        code.putln(code.error_goto_if(f"{result_temp} == -1", self.pos))
        code.funcstate.release_temp(result_temp)
...
class ForFromStatNode(LoopNode, StatNode):
    ...
    def generate_execution_code(self, code):
        ...
        if loopvar_type.is_int and not loopvar_type.signed and self.relation2[0] == '>':
            ...
            # is within step of 0.
            code.putln(f"for ({loopvar_name} = {self.bound1.result()}{offset} + {step}; {loopvar_name} {self.relation2} {self.bound2.result()} + {step}; ) {{ {loopvar_name}{incop};")
        else:
            code.putln(f"for ({loopvar_name} = {self.bound1.result()}{offset}; {loopvar_name} {self.relation2} {self.bound2.result()}; {loopvar_name}{incop}) {{")
...
class WithStatNode(StatNode):
    ...
    def generate_execution_code(self, code):
        ...
        if self.target:
            ...
            code.putln(f"{self.target_temp.result()} = {self.enter_call.result()};")
...
class TryExceptStatNode(StatNode):
    ...
    def generate_execution_code(self, code):
        ...
        if can_raise:
            ...
            save_exc.putln("__Pyx_ExceptionSave({});".format(', '.join([f'&{var}' for var in exc_save_vars])))
            ...
            def restore_saved_exception():
                ...
                code.putln("__Pyx_ExceptionReset({});".format(', '.join(exc_save_vars)))
        else:
            ...
            mark_vars_used = [f"(void){var};" for var in exc_save_vars]
            save_exc.putln("{} /* mark used */".format(' '.join(mark_vars_used)))
    ...
    def generate_handling_code(self, code, end_label):
        ...
        if self.pattern:
            ...
            if has_non_literals:
                ...
                code.putln("__Pyx_ErrFetch(&{}, &{}, &{});".format(*tuple(exc_vars)))
                exc_type = exc_vars[0]
            else:
            ...
            if exc_type:
                ...
                if len(patterns) == 2:
                    exc_tests.append(f"__Pyx_PyErr_GivenExceptionMatches2({exc_type}, {patterns[0]}, {patterns[1]})")
                else:
                    exc_tests.extend(
                        f"__Pyx_PyErr_GivenExceptionMatches({exc_type}, {pattern})"
                        for pattern in patterns
                    )
            elif len(patterns) == 2:
                code.globalstate.use_utility_code(
                    UtilityCode.load_cached("FastTypeChecks", "ModuleSetupCode.c"))
                exc_tests.append(f"__Pyx_PyErr_ExceptionMatches2({patterns[0]}, {patterns[1]})")
            else:
                code.globalstate.use_utility_code(
                    UtilityCode.load_cached("PyErrExceptionMatches", "Exceptions.c"))
                exc_tests.extend(
                    f"__Pyx_PyErr_ExceptionMatches({pattern})"
                    for pattern in patterns
                )
            ...
            if exc_vars:
                code.putln("__Pyx_ErrRestore({}, {}, {});".format(*tuple(exc_vars)))
                code.putln(' '.join([f"{var} = 0;" for var in exc_vars]))
        ...
        if needs_exception:
            ...
            exc_vars = [code.funcstate.allocate_temp(py_object_type, manage_ref=True)
                        for _ in range(3)]
            exc_args = "&{}, &{}, &{}".format(*tuple(exc_vars))
            code.putln(f"if (__Pyx_GetException({exc_args}) < 0) {code.error_goto(self.pos)}")
            for var in exc_vars:
        ...
        if self.excinfo_target is not None:
            for tempvar, node in zip(exc_vars, self.excinfo_target.args, strict=False):
...
class TryFinallyStatNode(StatNode):
    ...
    def generate_execution_code(self, code):
        ...
        for _i, (new_label, old_label) in enumerate(zip(new_labels, old_labels, strict=False)):
            if not code.label_used(new_label):
                continue
            if new_label == new_error_label and preserve_error:
                continue  # handled above

            code.putln(f'{new_label}: {{')
            ret_temp = None
            if old_label == return_label and not self.finally_clause.is_terminator:
                ...
                if (self.func_return_type and
                    ...
                    code.putln(f"{ret_temp} = {Naming.retval_cname};")
                    if self.func_return_type.is_pyobject:
                        code.putln(f"{Naming.retval_cname} = 0;")

            fresh_finally_clause().generate_execution_code(code)

            if old_label == return_label and ret_temp:
                code.putln(f"{Naming.retval_cname} = {ret_temp};")
                if self.func_return_type.is_pyobject:
                    code.putln(f"{ret_temp} = 0;")
    ...
    def put_error_catcher(self, code, temps_to_clean_up, exc_vars,
        ...
        code.putln(" __Pyx_ExceptionSwap(&{}, &{}, &{});".format(*exc_vars[3:]))
        code.putln("if ("
        ...
        for var in exc_vars:
            code.put_xgotref(var, py_object_type)
        if exc_lineno_cnames:
            code.putln(f"{exc_lineno_cnames[0]} = {Naming.lineno_cname}; {exc_lineno_cnames[1]} = {Naming.clineno_cname}; {exc_filename_cname} = {Naming.filename_cname};")
    ...
    def put_error_uncatcher(self, code, exc_vars, exc_lineno_cnames=None, exc_filename_cname=None):
        ...
        code.putln("__Pyx_ExceptionReset({}, {}, {});".format(*exc_vars[3:]))
        for var in exc_vars[:3]:
            code.put_xgiveref(var, py_object_type)
        code.putln("__Pyx_ErrRestore({}, {}, {});".format(*exc_vars[:3]))
        ...
        code.putln(' '.join([f"{var} = 0;" for var in exc_vars]))
        if exc_lineno_cnames:
            code.putln(f"{Naming.lineno_cname} = {exc_lineno_cnames[0]}; {Naming.clineno_cname} = {exc_lineno_cnames[1]}; {Naming.filename_cname} = {exc_filename_cname};")
...
class GILExitNode(StatNode):
    ...
    def generate_execution_code(self, code):
        variable = self.state_temp.result() if self.state_temp else None
            ...
            if name == "*":
                ...
                if entry:
                    ...
                    if submodule_scope.parent_module is module_scope:
                        ...
                        error(pos, f"Name '{name}' not declared in module '{module_name}'")
...
class FromImportStatNode(StatNode):
                ...
                if target.entry:
                    if target.get_known_standard_library_import() is None:
                        target.entry.known_standard_library_import = EncodedString(
                            f"{self.module.module_name.value}.{name}")
                else:
    ...
    def generate_execution_code(self, code):
        ...
        if self.import_star:
            code.putln(
                f'if ({Naming.import_star}({self.module.py_result()}) < 0) {code.error_goto(self.pos)};')
...
class ParallelStatNode(StatNode, ParallelNode):
    ...
    def analyse_declarations(self, env):
        ...
        if self.kwargs:
            ...
            for dictitem in self.kwargs.key_value_pairs:
                if dictitem.key.value in seen:
                    error(self.pos, f"Duplicate keyword argument found: {dictitem.key.value}")
        ...
        for kw, val in self.kwargs.items():
            if kw not in self.valid_keyword_arguments:
                error(self.pos, f"Invalid keyword argument: {kw}")
            else:
                setattr(self, kw, val)
    ...
    def _allocate_closure_temp(self, code, entry):
        ...
        code.putln(f"{cname} = {entry.cname};")
        entry.cname = cname
    ...
    def put_num_threads(self, code):
        ...
        """
        if self.num_threads is not None:
            code.put(f" num_threads({self.evaluate_before_block(code, self.num_threads)})")
    ...
    def release_closure_privates(self, code):
        ...
        for entry, original_cname in self.modified_entries:
            code.putln(f"{original_cname} = {entry.cname};")
    ...
    def privatize_temps(self, code, exclude_temps=()):
        ...
        if self.is_parallel:
            ...
            if privates:
                c.put(" private({})".format(", ".join(privates)))
            if firstprivates:
                c.put(" firstprivate({})".format(", ".join(firstprivates)))
            ...
            if self.breaking_label_used:
                ...
                if self.error_label_used:
                    ...
                    c.put(" private({}, {}, {})".format(*self.pos_info))

                c.put(" shared({})".format(', '.join(shared_vars)))
    ...
    def trap_parallel_exit(self, code, should_flush=False):
        ...
        if self.any_label_used:
            ...
            if should_flush and self.breaking_label_used:
                code.putln_openmp(f"#pragma omp flush({Naming.parallel_why})")
    ...
    def fetch_parallel_exception(self, code):
        ...
        code.putln_openmp(f"#pragma omp flush({Naming.parallel_exc_type})")
        code.putln(
            f"if (!{Naming.parallel_exc_type}) {{")

        code.putln("__Pyx_ErrFetchWithState(&{}, &{}, &{});".format(*self.parallel_exc))
        pos_info = chain(*zip(self.parallel_pos_info, self.pos_info, strict=False))
        code.funcstate.uses_error_indicator = True
        code.putln("{} = {}; {} = {}; {} = {};".format(*tuple(pos_info)))
    ...
    def restore_parallel_exception(self, code):
        ...
        code.putln("__Pyx_ErrRestoreWithState({}, {}, {});".format(*self.parallel_exc))
        pos_info = chain(*zip(self.pos_info, self.parallel_pos_info, strict=False))
        code.putln("{} = {}; {} = {}; {} = {};".format(*tuple(pos_info)))
    ...
    def end_parallel_control_flow_block(
        ...
        if self.error_label_used:
            c.putln("const char *{} = NULL; int {} = 0, {} = 0;".format(*self.parallel_pos_info))
            c.putln("PyObject *{} = NULL, *{} = NULL, *{} = NULL;".format(*self.parallel_exc))
            ...
            code.putln(f"{Naming.parallel_why} = 4;")
            code.putln(
                "}")

        any_label_used = self.any_label_used if continue_ else self.breaking_label_used

        if any_label_used:
            # __pyx_parallel_why is used, declare and initialize
            c.putln(f"int {Naming.parallel_why};")
            c.putln(f"{Naming.parallel_why} = 0;")

            code.putln(
                f"if ({Naming.parallel_why}) {{")
            ...
            for temp_cname, private_cname, temp_type in self.parallel_private_temps:
                if temp_type.is_cpp_class:
                    ...
                    temp_cname = f"__PYX_STD_MOVE_IF_SUPPORTED({temp_cname})"
                code.putln(f"{private_cname} = {temp_cname};")

            code.putln(f"switch ({Naming.parallel_why}) {{")
            if continue_:
                code.put("    case 1: ")
                code.put_goto(code.continue_label)
    ...
    redef_condition = f"({buggy_platform_macro_condition} && {have_expect_condition})"
    ...
    def _parameters_nogil_check(self, env, names, nodes):
        for name, node in zip(names, nodes, strict=False):
            if node is not None and node.type.is_pyobject:
                error(node.pos, f"{name} may not be a Python object "
...
class ParallelWithBlockNode(ParallelStatNode):
    ...
    def generate_execution_code(self, code):
        ...
        if self.threading_condition is not None:
            code.put(f"if({self.threading_condition.result()}) ")
...
class ParallelRangeNode(ParallelStatNode):
    ...
    def analyse_declarations(self, env):
        ...
        if self.schedule not in (None, 'static', 'dynamic', 'guided', 'runtime'):
            error(self.pos, f"Invalid schedule argument to prange: {self.schedule}")
    ...
    def analyse_expressions(self, env):
        ...
        if not self.target.type.is_numeric:
            ...
            if not self.target.type.is_pyobject:
                ...
                error(self.target.pos,
                      f"Must be of numeric type, not {self.target.type}")
        ...
        for node, name in zip(start_stop_step, self.names, strict=False):
            if node is not None:
                node.analyse_types(env)
                if not node.type.is_numeric:
                    error(node.pos, f"{name} argument must be numeric")
                    continue
    ...
    def generate_execution_code(self, code):
        ...
        start_stop_step = self.start, self.stop, self.step
        defaults = '0', '0', '1'
        for node, name, default in zip(start_stop_step, self.names, defaults, strict=False):
            if node is None:
                result = default
        ...
        code.putln("if ({nsteps} > 0)".format(**fmt_dict))
        ...
        if self.else_clause:
            if self.breaking_label_used:
                code.put(f"if ({Naming.parallel_why} < 2)")
    ...
    def generate_loop(self, code, fmt_dict):
            ...
            if op and op in "+*-&^|" and entry != self.target.entry:
                if entry.type.is_pyobject:
                    ...
                    reduction_codepoint.put(
                                f" reduction({op}:{entry.cname})")
            else:
                if not entry.type.is_pyobject:
                    code.put(f" firstprivate({entry.cname})")
                    code.put(f" lastprivate({entry.cname})")

        if self.schedule:
            if self.chunksize:
                chunksize = f", {self.evaluate_before_block(code, self.chunksize)}"
            else:
                chunksize = ""

            code.put(f" schedule({self.schedule}{chunksize})")
        ...
        code.put("for ({i} = 0; {i} < {nsteps}; {i}++)".format(**fmt_dict))
        ...
        code.putln("{target} = ({target_type})({start} + {step} * {i});".format(**fmt_dict))
        ...
        if self.breaking_label_used:
            ...
            guard_around_body_codepoint.putln(f"if ({Naming.parallel_why} < 2)")
...
class CnameDecoratorNode(StatNode):
    ...
    def analyse_declarations(self, env):
        ...
        if self.is_function:
            ...
            for _name, entry in scope.entries.items():
                if entry.func_cname:
