...
def analyse_buffer_options(globalpos, env, posargs, dictargs, defaults=None, need_complete=True):
    ...
    for name, (value, pos) in zip(buffer_options, posargs, strict=False):
        if name not in buffer_options:
            raise CompileError(pos, ERR_BUF_OPTION_UNKNOWN % name)
        if name in options:
    ...
    def generate_buffer_lookup_code(self, code, index_cnames):
        ...
        if mode == 'full':
            for i, s, o in zip(index_cnames,
                               ...
                               self.get_buf_suboffsetvars(), strict=False):
                params.append(i)
                params.append(s)
                params.append(o)
...
def put_assign_to_buffer(lhs_cname, rhs_cname, buf_entry,
    ...
    pybuffernd_struct = buffer_aux.buflocal_nd_var.cname
    get_flags(buffer_aux, buffer_type)
    ...
    if is_initialized:
        ...
        code.putln('if ({}) {{'.format(code.unlikely(f"{retcode_cname} < 0")))
        ...
        exc_temps = tuple(code.funcstate.allocate_temp(PyrexTypes.py_object_type, manage_ref=False)
                          for _ in range(3))
        code.putln('PyErr_Fetch(&{}, &{}, &{});'.format(*exc_temps))
        code.putln('if ({}) {{'.format(code.unlikely("%s == -1" % (getbuffer % lhs_cname))))
        code.putln('Py_XDECREF({}); Py_XDECREF({}); Py_XDECREF({});'.format(*exc_temps))  # Do not refnanny these!
        code.globalstate.use_utility_code(raise_buffer_fallback_code)
        code.putln('__Pyx_RaiseBufferFallbackError();')
        code.putln('} else {')
        code.putln('PyErr_Restore({}, {}, {});'.format(*exc_temps))
        code.putln('}')
        code.putln('{} = {} = {} = 0;'.format(*exc_temps))
        for t in exc_temps:
        ...
        code.putln('if ({}) {{'.format(code.unlikely("%s == -1" % (getbuffer % rhs_cname))))
        code.putln('{} = {}; __Pyx_INCREF(Py_None); {}.rcbuffer->pybuffer.buf = NULL;'.format(lhs_cname,
...
def put_buffer_lookup_code(entry, index_signeds, index_cnames, directives,
    ...
    if directives['boundscheck']:
        ...
        for dim, (signed, cname, shape) in enumerate(zip(index_signeds, index_cnames, entry.get_buf_shapevars(), strict=False)):
            if signed != 0:
                # not unsigned, deal with negative index
                code.putln(f"if ({cname} < 0) {{")
                if negative_indices:
                    code.putln(f"{cname} += {shape};")
                    code.putln("if (%s) %s = %d;" % (
                        code.unlikely(f"{cname} < 0"),
                        failed_dim_temp, dim))
                else:
                    code.putln("%s = %d;" % (failed_dim_temp, dim))
                code.put("} else ")
            # check bounds in positive direction
            cast = "" if signed != 0 else "(size_t)"
            code.putln("if (%s) %s = %d;" % (
        ...
        code.putln("if ({}) {{".format(code.unlikely(f"{failed_dim_temp} != -1")))
        ...
        for signed, cname, shape in zip(index_signeds, index_cnames, entry.get_buf_shapevars(), strict=False):
            if signed != 0:
...
def mangle_dtype_name(dtype):
    ...
    if dtype.is_pyobject:
        ...
        prefix = "nn_" if dtype.is_typedef or dtype.is_struct_or_union else ""
    ...
    if name not in code.globalstate.utility_codes:
        ...
        if dtype.is_simple_buffer_dtype():
            ...
            types = [get_type_information_cname(code, f.type, maxdepth - 1)
                     for f in fields]
            typecode.putln(f"static const __Pyx_StructField {structinfo_name}[] = {{", safe=True)
            ...
            for f, typeinfo in zip(fields, types, strict=False):
                typecode.putln(f'  {{&{typeinfo}, "{f.name}", offsetof({struct_type}, {f.cname})}},', safe=True)
...
context = {"max_dims": Options.buffer_max_dims}
