            ...
            if len(vars) == 1:
                ...
                if len(vars) != len(item):
                    raise ValueError(
                        ...
                        % (len(vars), len(item)))
                for name, value in zip(vars, item, strict=False):
                    ns[name] = value
            try:
    ...
    def _parse_signature(self, args, kw):
        ...
        while args:
            ...
            if sig_args:
                ...
                raise TypeError(
                    'Extra position arguments: {}'.format(', '.join([repr(v) for v in args])))
...
def parse_for(tokens, name, context):
    ...
    if '(' in vars:
        raise TemplateError(
            f'You cannot have () in the variable section of a for loop ({vars!r})', position=pos, name=name)
    vars = tuple([
...
def parse_default(tokens, name, context):
    ...
    if not var_re.search(var):
        raise TemplateError(
            f"Not a valid variable name for {{{{default}}}}: {var!r}", position=pos, name=name)
    expr = parts[1].strip()
...
def parse_signature(sig_text, name, pos):
    ...
    def get_token(pos=False):
        try:
            tok_type, tok_string, (srow, scol), (erow, ecol), _line = next(tokens)
