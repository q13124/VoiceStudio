...
from Cython.Utils import cached_function, safe_makedirs
...
class Cache:
    ...
    def fingerprint_file(self, cfile, fingerprint, ext):
        return (
            join_path(self.path, f"{os.path.basename(cfile)}-{fingerprint}") + ext
        )
    ...
    def cleanup_cache(self, ratio=0.85):
        ...
        if total_size > self.cache_size:
            for _time, size, file in sorted(all, reverse=True):
                os.unlink(file)
                total_size -= size
