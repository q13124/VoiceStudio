...
from Cython.Compiler.Tests.Utils import backup_Options, check_global_options, restore_Options
...
class TestCythonizeArgsParser(TestCase):
    ...
    def test_directive_value_invalid(self):
        with self.assertRaises(ValueError):
            _options, _args =  self.parse_args(['-X', 'cdivision=sadfasd'])

    def test_directive_key_invalid(self):
        with self.assertRaises(ValueError):
            _options, _args =  self.parse_args(['-X', 'abracadabra'])

    def test_directive_no_value(self):
        with self.assertRaises(ValueError):
            _options, _args =  self.parse_args(['-X', 'cdivision'])
    ...
    def test_directives_wrong(self):
        ...
        for key, value in directives:
            ...
            with self.assertRaises(ValueError, msg = "Error for option: "+cmd):
                _options, _args =  self.parse_args(['-X', cmd])
...
class TestParseArgs(TestCase):
    ...
    def test_build_set_for_inplace(self):
        options, _args = parse_args(['foo.pyx', '-i'])
        self.assertEqual(options.build, True)
        self.check_default_global_options()

    def test_lenient(self):
        _options, sources = parse_args(['foo.pyx', '--lenient'])
    ...
    def test_annotate(self):
        _options, sources = parse_args(['foo.pyx', '--annotate'])
        self.assertEqual(sources, ['foo.pyx'])
        self.assertEqual(Options.annotate, 'default')
        self.check_default_global_options(['annotate'])

    def test_annotate_fullc(self):
        _options, sources = parse_args(['foo.pyx', '--annotate-fullc'])
        self.assertEqual(sources, ['foo.pyx'])
        self.assertEqual(Options.annotate, 'fullc')
        self.check_default_global_options(['annotate'])

    def test_no_docstrings(self):
        _options, sources = parse_args(['foo.pyx', '--no-docstrings'])
        self.assertEqual(sources, ['foo.pyx'])
