...
class RE:
    ...
    def build_machine(self, machine, initial_state, final_state,
        ...
        """
        raise NotImplementedError(f"{self.__class__.__name__}.build_machine not implemented")
...
def Char(c):
    ...
    """
    result = CodeRange(ord(c), ord(c) + 1) if len(c) == 1 else SpecialSymbol(c)
    result.str = f"Char({c!r})"
    return result
...
class Alt(RE):
    ...
    def calc_str(self):
        return "Alt({})".format(','.join(map(str, self.re_list)))
...
class SwitchCase(RE):
    ...
    def calc_str(self):
        name = "NoCase" if self.nocase else "Case"
...
def Str(*strs):
    ...
    if len(strs) == 1:
        ...
        result.str = "Str({})".format(','.join(map(repr, strs)))
        return result
