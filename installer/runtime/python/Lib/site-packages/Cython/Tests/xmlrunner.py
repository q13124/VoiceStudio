...
class _XMLTestResult(TextTestResult):
    ...
    def _prepare_callback(self, test_info, target_list, verbose_str, short_str):
        ...
        def callback():
            ...
            if self.showAll:
                self.stream.writeln(f'({test_info.get_elapsed_time():.3f}s) {verbose_str}')
            elif self.dots:
    ...
    def _report_testsuite(suite_name, tests, xml_document):
        ...
        testsuite.setAttribute('time', f'{sum([e.get_elapsed_time() for e in tests]):.3f}')
    ...
    def generate_reports(self, test_runner):
        ...
        for suite, tests in all_results.items():
            ...
            if type(test_runner.output) is str:
                report_file = open(f'{test_runner.output}{os.sep}TEST-{suite}.xml', 'w')
                try:
