...
class NodeTypeWriter(TreeVisitor):
    ...
    def visit_Node(self, node):
        if not self.access_path:
            ...
            name = "%s[%d]" % tip[1:3] if tip[2] is not None else tip[1]
...
class CythonTest(unittest.TestCase):
    ...
    def assertLines(self, expected, result):
        ...
        if not isinstance(result, list):
            result = result.split("\n")
        for idx, (expected_line, result_line) in enumerate(zip(expected, result, strict=False)):
    ...
    def assertCode(self, expected, result_tree):
        ...
        for idx, (line, expected_line) in enumerate(zip(result_lines, expected_lines, strict=False)):
...
class TreeAssertVisitor(VisitorTransform):
    ...
    def create_c_file_validator(self):
        ...
        def fail(pos, pattern, found, file_path):
            Errors.error(pos, "Pattern '{}' {} found in {}".format(
                pattern,
