...
class ContainerExtractor:
    ...
    def extract(self, container):
        ...
        lst = []
        def locate(c):
            return c.__class__.__name__ in self.allowed
        def recursive(c):
            return c.__class__.__name__ in self.extracted
        def process(c):
            return self.process(c, lst)
...
class EndingList:
    ...
    def findending(self, pos):
        ...
        if len(self.endings) == 0:
            return None
        for _index, ending in enumerate(reversed(self.endings)):
...
class FormulaSymbol(FormulaBit):
    ...
    def detect(self, pos) -> bool:
        ...
        if pos.current() in FormulaSymbol.unmodified:
            return True
        return pos.current() in FormulaSymbol.modified
...
class Bracket(FormulaBit):
    ...
    def innertext(self, pos) -> None:
        ...
        specialchars = [*list(FormulaConfig.symbolfunctions.keys()), FormulaConfig.starts['command'], FormulaConfig.starts['bracket'], Comment.start]
        while not pos.finished():
...
class MultiRowFormula(CommandBit):
    ...
    def addempty(self) -> None:
        ...
        for index, _originalcell in enumerate(self.rows[-1].contents):
...
class EquationEnvironment(MultiRowFormula):
    ...
    def parsebit(self, pos) -> None:
        ...
        self.output = TaggedOutput().settag(
                        f'span class="environment {environment}"', False)
        if environment in FormulaConfig.environments:
            ...
            # print in red
            self.output = TaggedOutput().settag('span class="unknown"')
            self.add(FormulaConstant(f'\\begin{{{environment}}} '))
    ...
    def parsebit(self, pos) -> None:
        ...
        if not isinstance(parameter, FormulaConstant):
            try:
                ...
                Trace.error(f'No base character found for "{self.command}".')
                return
