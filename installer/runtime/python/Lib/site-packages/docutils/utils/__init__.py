    ...
    for field in field_list:
        ...
        if len(body) == 0:
            ...
            raise BadOptionDataError(
                  ...
                  f'a single paragraph only (option "{name}")')
        else:
...
def assemble_option_dict(option_list: list[tuple[str, str|None]],
    ...
    for name, value in option_list:
        ...
        if name in options:
            raise DuplicateOptionError(f'duplicate option "{name}"')
        try:
            options[name] = convertor(value)
        except (ValueError, TypeError) as detail:
            raise detail.__class__('(option: "{}"; value: {!r})\n{}'.format(name, value, ' '.join(detail.args)))
    return options
