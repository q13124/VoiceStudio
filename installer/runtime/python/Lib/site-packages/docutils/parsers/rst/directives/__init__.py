...
def directive(directive_name, language_module, document):
    ...
    try:
        ...
        msg_text.append('Problem retrieving directive entry from language '
                        f'module {language_module!r}: {error}.')
    except KeyError:
        msg_text.append(f'No directive entry for "{directive_name}" in module "{language_module.__name__}".')
    if not canonicalname:
        try:
            canonicalname = _fallback_language_module.directives[normname]
            msg_text.append(f'Using English fallback for directive "{directive_name}".')
        except KeyError:
            msg_text.append(f'Trying "{directive_name}" as canonical directive name.')
    ...
    try:
        ...
        messages.append(document.reporter.error(
            f'Error importing directive module "{modulename}" (directive "{directive_name}"):\n{detail}',
    ...
    try:
        ...
        messages.append(document.reporter.error(
            f'No directive class "{classname}" in module "{modulename}" (directive "{directive_name}").',
            line=document.current_line))
        return None, messages
...
def class_option(argument: str) -> list[str]:
    ...
    for name in names:
        ...
        if not class_name:
            raise ValueError(f'cannot make "{name}" into a class name')
...
def unicode_code(code):
    ...
    try:
        ...
        raise ValueError(f'code too large ({detail})')
...
def single_char_or_unicode(argument: str) -> str:
    ...
    if len(char) > 1:
        raise ValueError(f'{char!r} invalid; must be a single character or '
                         'a Unicode code')
    return char
...
def encoding(argument: str) -> str:
    ...
    try:
        ...
        raise ValueError(f'unknown encoding: "{argument}"')
    return argument
...
def choice(argument, values):
    ...
    try:
        ...
        raise ValueError(f'must supply an argument; choose from {format_values(values)}')
    if value in values:
        return value
    else:
        raise ValueError(f'"{argument}" unknown; choose from {format_values(values)}')


def format_values(values) -> str:
    return '{}, or "{}"'.format(', '.join(f'"{s}"' for s in values[:-1]),
                            values[-1])
