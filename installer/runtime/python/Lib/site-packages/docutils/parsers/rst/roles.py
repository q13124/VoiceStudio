...
def role(role_name, language_module, lineno, reporter):
    ...
    if role_name:
        ...
        try:
            ...
            msg_text.append(f'No role entry for "{role_name}" in module "{language_module.__name__}".')
    else:
    ...
    if not canonicalname:
        try:
            ...
            msg_text.append(f'Using English fallback for role "{role_name}".')
        except KeyError:
            msg_text.append(f'Trying "{role_name}" as canonical role name.')
...
def pep_reference_role(role, rawtext, text, lineno, inliner,
    ...
    try:
        ...
        msg = inliner.reporter.error(
            f'PEP number must be a number from 0 to 9999; "{text}" is invalid.', line=lineno)
...
def raw_role(role, rawtext, text, lineno, inliner, options=None, content=None):
    ...
    if 'format' not in options:
        msg = inliner.reporter.error(
            f'No format (Writer name) is associated with this role: "{role}".\n'
            'The "raw" role cannot be used directly.\n'
            'Instead, use the "role" directive to create a new role with '
            'an associated format.', line=lineno)
        prb = inliner.problematic(rawtext, rawtext, msg)
        return [prb], [msg]
...
def unimplemented_role(role, rawtext, text, lineno, inliner,
                       options=None, content=None):
    msg = inliner.reporter.error(
        f'Interpreted text role "{role}" not implemented.', line=lineno)
    prb = inliner.problematic(rawtext, rawtext, msg)
    return [prb], [msg]
