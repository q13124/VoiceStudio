...
class RSTState(StateWS):
    ...
    def no_match(self, context, transitions):
        ...
        self.reporter.severe(
            f'Internal error: no transition pattern match.  State: "{self.__class__.__name__}"; '
            f'transitions: {transitions}; context: {context}; current line: {self.state_machine.line!r}.')
        return context, None, []
    ...
    def nested_parse(self,
        ...
        if match_titles and node == self.state_machine.node:
    ...
    def unindent_warning(self, node_name):
        ...
        return self.reporter.warning(f'{node_name} ends without a blank line; '
                                     'unexpected unindent.',
                                     line=lineno)
...
def build_regexp(definition, compile_patterns=True):
    ...
    regexp = '{prefix}(?P<{name}>{or_group}){suffix}'.format(**locals())
    if compile_patterns:
...
class Inliner:
    ...
    def init_customizations(self, settings) -> None:
           ...
           [
            ...
            ('whole', '', end_string_suffix,  # whole constructs
             [  # reference name & end-string
              rf'(?P<refname>{self.simplename})(?P<refend>__?)',
              ('footnotelabel', r'\[', r'(?P<fnend>\]_)',
               [r'[0-9]+',                     # manually numbered
                rf'\#({self.simplename})?',  # auto-numbered (w/ label?)
                r'\*',                         # auto-symbol
                rf'(?P<citationlabel>{self.simplename})',  # citation ref
                ]
               )
              ]
             ),
            ('backquote',             # interpreted text or phrase reference
             f'(?P<role>(:{self.simplename}:)?)',  # optional role
        ...
        self.patterns = Struct(
          ...
          pep=re.compile(
                ...
                {end_string_suffix}""".format(**args), re.VERBOSE),
          rfc=re.compile(
                r"""
                {start_string_prefix}
                (RFC(-|\s+)?(?P<rfcnum>\d+))
    ...
    # uri_end_delim'):
    uri_end = r"""(?:{urilast}|{uric}(?={uri_end_delim}))""".format(**locals())
    ...
    def inline_obj(self, match, lineno, end_pattern, nodeclass,
        ...
        msg = self.reporter.warning(
              f'Inline {nodeclass.__name__} start-string without end-string.', line=lineno)
    ...
    def emphasis(self, match, lineno):
        before, inlines, remaining, sysmessages, _endstring = self.inline_obj(
    ...
    def interpreted(self, rawsource, text, role, lineno):
        ...
        if role_fn:
            ...
            msg = self.reporter.error(
                f'Unknown interpreted text role "{role}".',
                line=lineno)
    ...
    def footnote_reference(self, match, lineno):
        ...
        if match.group('citationlabel'):
            refnode = nodes.citation_reference(f'[{label}]_',
                                               refname=refname)
...
class Body(RSTState):
    ...
    pats['enum'] = ('({arabic}|{loweralpha}|{upperalpha}|{lowerroman}'
                    '|{upperroman}|#)'.format(**enum.sequencepats))
    pats['optname'] = '{alphanum}{alphanumplus}*'.format(**pats)
    # @@@ Loosen up the pattern?  Allow Unicode?
    pats['optarg'] = '({alpha}{alphanumplus}*|<[^<>]+>)'.format(**pats)
    pats['shortopt'] = r'(-|\+){alphanum}( ?{optarg})?'.format(**pats)
    pats['longopt'] = r'(--|/){optname}([ =]{optarg})?'.format(**pats)
    pats['option'] = r'({shortopt}|{longopt})'.format(**pats)
    ...
    patterns = {
          'bullet': '[-+*\u2022\u2023\u2043]( +|$)',
          'enumerator': r'({parens}|{rparen}|{period})( +|$)'.format(**pats),
          'field_marker': r':(?![: ])([^:\\]|\\.|:(?!([ `]|$)))*(?<! ):( +|$)',
          'option_marker': r'{option}(, {option})*(  +| ?$)'.format(**pats),
          'doctest': r'>>>( +|$)',
          'line_block': r'\|( +|$)',
          ...
          'explicit_markup': r'\.\.( +|$)',
          'anonymous': r'__( +|$)',
          'line': r'({nonalphanum7bit})\1* *$'.format(**pats),
          'text': r''}
    initial_transitions = (
          'bullet',
          'enumerator',
    ...
    def indent(self, match, context, next_state):
        """Block quote."""
        (indented, _indent, line_offset, blank_finish
    ...
    def enumerator(self, match, context, next_state):
        ...
        if ordinal != 1:
            ...
            msg = self.reporter.info(
                f'Enumerated list start value not ordinal-1: "{text}" (ordinal {ordinal})', base_node=enumlist)
            self.parent += msg
    ...
    def option_marker(self, match, context, next_state):
        ...
        try:
            ...
            msg = self.reporter.error(f'Invalid option list marker: {error}')
            self.parent += msg
            (indented, _indent, line_offset, blank_finish
             ) = self.state_machine.get_first_known_indented(match.end())
            elements = self.block_quote(indented, line_offset)
    ...
    def parse_option_marker(self, match):
        ...
        for optionstring in optionstrings:
            ...
            if 0 < len(tokens) <= 2:
                ...
                raise MarkupError(
                    f'wrong number of option tokens (={len(tokens)}), should be 1 or 2: '
                    f'"{optionstring}"')
        return optlist
    ...
    def line_block_line(self, match, lineno):
        ...
        (indented, _indent, _line_offset, blank_finish
         ) = self.state_machine.get_first_known_indented(match.end(),
                                                         until_blank=True)
        text = '\n'.join(indented)
    ...
    explicit.patterns = Struct(
          target=re.compile(r"""
                            ...
                            """.format(**vars(Inliner)), re.VERBOSE),
          reference=re.compile(r"""
                               (
                                 (?P<simple>{simplename})_
                               |                  # *OR*
                                 `                  # open backquote
                                 (?![ ])            # not space
    ...
    def footnote(self, match):
        src, srcline = self.state_machine.get_source_and_line()
        (indented, _indent, offset, blank_finish
         ) = self.state_machine.get_first_known_indented(match.end())
        label = match.group(1)
    ...
    def hyperlink_target(self, match):
        ...
        lineno = self.state_machine.abs_line_number()
        (block, _indent, _offset, blank_finish
        ...
        if targetname:
            ...
            if refuri:
                ...
                if uri:
                    ...
                    raise ApplicationError(f'problem with URI: {refuri!r}')
            self.document.note_explicit_target(target, self.parent)
    ...
    def substitution_def(self, match):
        ...
        src, srcline = self.state_machine.get_source_and_line()
        (block, _indent, offset, blank_finish
         ) = self.state_machine.get_first_known_indented(match.end(),
                                                         strip_indent=False)
        blocktext = (match.string[:match.end()] + '\n'.join(block))
        block.disconnect()
        ...
        if not block:
            msg = self.reporter.warning(
                f'Substitution definition "{subname}" missing contents.',
        ...
        _new_abs_offset, blank_finish = self.nested_list_parse(
        ...
        if len(substitution_node) == 0:
            msg = self.reporter.warning(
                  f'Substitution definition "{subname}" empty or invalid.',
                  nodes.literal_block(blocktext, blocktext),
                  source=src, line=srcline)
            return [msg], blank_finish
    ...
    def run_directive(self, directive, match, type_name, option_presets):
        ...
        assert isinstance(result, list), \
               f'Directive "{type_name}" must return a list of nodes.'
        for i in range(len(result)):
            assert isinstance(result[i], nodes.Node), \
                   (f'Directive "{type_name}" returned non-Node object (index {i}): {result[i]!r}')
        return (result,
    ...
    def parse_extension_options(self, option_spec, datalines):
        ...
        try:
            ...
            return 0, f'unknown option: "{detail.args[0]}"'
        except (ValueError, TypeError) as detail:
            return 0, 'invalid option value: {}'.format(' '.join(detail.args))
        except utils.ExtensionOptionError as detail:
            return 0, 'invalid option data: {}'.format(' '.join(detail.args))
        if blank_finish:
            return 1, options
        else:
    ...
    def unknown_directive(self, type_name):
        lineno = self.state_machine.abs_line_number()
        (indented, _indent, _offset, blank_finish
         ) = self.state_machine.get_first_known_indented(0, strip_indent=False)
        text = '\n'.join(indented)
        error = self.reporter.error(f'Unknown directive type "{type_name}".',
    ...
    explicit.constructs = [
          (footnote,
           re.compile(rf"""
                      ...
                      (                 # footnote label:
                        ...
                        |               # *OR*
                          \#{Inliner.simplename}            # auto-number ed?) footnote label
                        |               # *OR*
          ...
          (citation,
           re.compile(rf"""
                      \.\.[ ]+          # explicit markup start
                      \[({Inliner.simplename})\]          # citation label
                      ([ ]+|$)          # whitespace or end of line
                      """, re.VERBOSE)),
          (hyperlink_target,
           re.compile(r"""
          ...
          (directive,
           re.compile(rf"""
                      \.\.[ ]+          # explicit markup start
                      ({Inliner.simplename})              # directive name
                      [ ]?              # optional space
                      ::                # directive delimiter
                      ([ ]+|$)          # whitespace or end of line
                      """, re.VERBOSE))]
...
class EnumeratedList(SpecializedBody):
    ...
    def enumerator(self, match, context, next_state):
        """Enumerated list item."""
        format, sequence, _text, ordinal = self.parse_enumerator(
...
class SubstitutionDef(Body):
    ...
    patterns = {
          'embedded_directive': re.compile(rf'({Inliner.simplename})::( +|$)'),
          'text': r''}
...
class QuotedLiteralBlock(RSTState):
    ...
    patterns = {'initial_quoted': r'({nonalphanum7bit})'.format(**Body.pats),
                'text': r''}
