...
class Table(Directive):
    ...
    def check_table_dimensions(self, rows, header_rows, stub_columns):
        if len(rows) < header_rows:
            error = self.reporter.error(f'{header_rows} header row(s) specified but '
                f'only {len(rows)} row(s) of data supplied ("{self.name}" directive).',
        ...
        for row in rows:
            ...
            if len(row) == stub_columns > 0:
                error = self.reporter.error(
                    f'Insufficient data supplied ({len(row)} columns(s)); '
                    'no data remaining for table body, required '
                    f'by "{self.name}" directive.',
                    nodes.literal_block(self.block_text, self.block_text),
                    line=self.lineno)
                raise SystemMessagePropagation(error)
    ...
    def get_column_widths(self, n_cols):
        if isinstance(self.widths, list):
            if len(self.widths) != n_cols:
                ...
                error = self.reporter.error(f'"{self.name}" widths do not match the '
                    f'number of columns in table ({n_cols}).',
                    nodes.literal_block(self.block_text, self.block_text),
                    line=self.lineno)
                raise SystemMessagePropagation(error)
    ...
    def run(self):
        if not self.content:
            warning = self.reporter.warning('Content block expected '
                f'for the "{self.name}" directive; none found.',
                nodes.literal_block(self.block_text, self.block_text),
                line=self.lineno)
            return [warning]
        ...
        if len(node) != 1 or not isinstance(node[0], nodes.table):
            error = self.reporter.error('Error parsing content block for the '
                f'"{self.name}" directive: exactly one table expected.',
                nodes.literal_block(self.block_text, self.block_text),
                line=self.lineno)
            return [error]
        table_node = node[0]
        ...
        try:
            if (not self.state.document.settings.file_insertion_enabled
                ...
                warning = self.reporter.warning('File and URL access '
                    f'deactivated; ignoring "{self.name}" directive.',
                    nodes.literal_block(self.block_text, self.block_text),
                    line=self.lineno)
                return [warning]
            title, messages = self.make_title()
    ...
    def get_csv_data(self):
        ...
        if self.content:
            ...
            if 'file' in self.options or 'url' in self.options:
                error = self.reporter.error(f'"{self.name}" directive may not both '
                    'specify an external file and have content.',
                    nodes.literal_block(self.block_text, self.block_text),
                    line=self.lineno)
                raise SystemMessagePropagation(error)
            ...
            try:
                ...
                severe = self.reporter.severe(
                      'Problems with "{}" directive URL "{}":\n{}.'.format(self.name, self.options['url'], error),
                      nodes.literal_block(self.block_text, self.block_text),
                      line=self.lineno)
                raise SystemMessagePropagation(severe)
            ...
            error = self.reporter.warning(
                f'The "{self.name}" directive requires content; none supplied.',
                nodes.literal_block(self.block_text, self.block_text),
                line=self.lineno)
            raise SystemMessagePropagation(error)
...
class ListTable(Table):
    ...
    def run(self):
        if not self.content:
            error = self.reporter.error(f'The "{self.name}" directive is empty; '
                'content required.',
                nodes.literal_block(self.block_text, self.block_text),
                line=self.lineno)
            return [error]
    ...
    def check_list_content(self, node):
        if len(node) != 1 or not isinstance(node[0], nodes.bullet_list):
            error = self.reporter.error(
                f'Error parsing content block for the "{self.name}" directive: '
        ...
        for item_index in range(len(list_node)):
            ...
            if len(item) != 1 or not isinstance(item[0], nodes.bullet_list):
                error = self.reporter.error(
                    f'Error parsing content block for the "{self.name}" directive: '
                    f'two-level bullet list expected, but row {item_index + 1} does not '
                ...
                if len(item[0]) != num_cols:
                    error = self.reporter.error(
                        f'Error parsing content block for the "{self.name}" directive: '
                        f'uniform two-level bullet list expected, but row {item_index + 1} '
                        'does not contain the same number of items as row 1 '
                        f'({len(item[0])} vs {num_cols}).',
                        nodes.literal_block(self.block_text, self.block_text),
                        line=self.lineno)
                    raise SystemMessagePropagation(error)
            else:
