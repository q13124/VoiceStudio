...
from docutils.parsers.rst import Directive, convert_directive_function, directives, roles, states
    ...
    def run(self):
        ...
        if self.content:
            if 'file' in self.options or 'url' in self.options:
                raise self.error(
                    f'"{self.name}" directive may not both specify an external file '
                    'and have content.')
            text = '\n'.join(self.content)
            ...
            if 'url' in self.options:
                raise self.error(
                    ...
                    f'specified for the "{self.name}" directive.')
...
class Replace(Directive):
    ...
    def run(self):
        if not isinstance(self.state, states.SubstitutionDef):
            raise self.error(
                f'Invalid context: the "{self.name}" directive can only be used within '
    ...
    def run(self):
        ...
        for code in codes:
            try:
                ...
                raise self.error(f'Invalid character code: {code}\n{io.error_string(error)}')
...
class Role(Directive):
    ...
    def run(self):
        ...
        if self.content_offset > self.lineno or not self.content:
            raise self.error(f'"{self.name}" directive requires arguments on the first '
                             'line.')
        args = self.content[0]
        match = self.argument_pattern.match(args)
        if not match:
            raise self.error(f'"{self.name}" directive arguments not valid role names: '
                             f'"{args}".')
        ...
        if base_role_name:
            ...
            if base_role is None:
                error = self.state.reporter.error(
                    f'Unknown interpreted text role "{base_role_name}".',
        ...
        assert not hasattr(base_role, 'arguments'), (
            f'Supplemental directive arguments for "{self.name}" directive not '
            f'supported (specified by "{base_role!r}" role).')
        try:
            converted_role = convert_directive_function(base_role)
            (_arguments, options, content, _content_offset
             ) = self.state.parse_directive_block(
                    self.content[1:], self.content_offset,
                    converted_role, option_presets={})
        except states.MarkupError as detail:
            error = self.reporter.error(
                f'Error in "{self.name}" directive:\n{detail}.',
                nodes.literal_block(self.block_text, self.block_text),
                line=self.lineno)
            return [*messages, error]
        if 'class' not in options:
            try:
                ...
                error = self.reporter.error(
                    f'Invalid argument for "{self.name}" directive:\n{detail}.',
                    nodes.literal_block(self.block_text, self.block_text),
                    line=self.lineno)
                return [*messages, error]
...
class MetaBody(states.SpecializedBody):
    ...
    def field_marker(self, match, context, next_state):
        """Meta element."""
        node, _blank_finish = self.parsemeta(match)
        self.parent += node
        return [], next_state, []
    ...
    def parsemeta(self, match):
        ...
        if not indented:
            ...
            msg = self.reporter.info(
                  f'No content for meta tag "{name}".',
        ...
        for token in tokens[1:]:
            try:
                ...
                msg = self.reporter.error(
                      f'Error parsing meta tag attribute "{token}": {detail}.', nodes.literal_block(line, line))
                return msg, blank_finish
...
class Meta(Directive):
    ...
    def run(self):
        self.assert_has_content()
        node = nodes.Element()
        new_line_offset, _blank_finish = self.state.nested_list_parse(
    ...
    def run(self):
        if self.content:
            ...
            info = self.reporter.info(
                f'Directive processed. Type="{self.name}", arguments={self.arguments!r}, options={self.options!r}, '
                'content:',
