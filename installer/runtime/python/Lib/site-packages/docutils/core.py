...
class Publisher:
    ...
    def report_Exception(self, error) -> None:
        if isinstance(error, utils.SystemMessage):
            ...
            print(f'{io.error_string(error)}', file=self._stderr)
            print(f"""\
...
and the command line used.""", file=self._stderr)
    ...
    def report_SystemMessage(self, error) -> None:
        print(f'Exiting due to level-{error.level} ({utils.Reporter.levels[error.level]}) system message.',
              file=self._stderr)
