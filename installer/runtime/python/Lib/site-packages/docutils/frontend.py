...
def validate_boolean(setting: str | bool,
    ...
    try:
        ...
        raise LookupError(f'unknown boolean value: "{value}"')
...
def validate_strip_class(setting: str,
    ...
    for cls in value:
        ...
        if cls != normalized:
            raise ValueError(f'Invalid class value {cls!r} (perhaps {normalized!r}?)')
    return value
    ...
    for item in value:
        try:
            ...
            raise ValueError('Invalid value "{}".'
                             ' Format is "<language>:<quotes>".'.format(item.encode('ascii', 'backslashreplace')))
        ...
        if len(multichar_quotes) == 4:
            ...
            raise ValueError('Invalid value "{}". Please specify 4 quotes\n'
                             '    (primary open/close; secondary open/close).'.format(item.encode('ascii', 'backslashreplace')))
...
class OptionParser(optparse.OptionParser, docutils.SettingsSpec):
    ...
    version_template: ClassVar[str] = '%prog (Docutils {}{}, Python {}, on {})'.format(
