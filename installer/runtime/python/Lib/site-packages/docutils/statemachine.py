...
class StateMachine:
    ...
    def check_line(self, context, state, transitions=None):
        ...
        if self.debug:
            print(f'\nStateMachine.check_line: state="{state.__class__.__name__}", transitions={transitions!r}.', file=sys.stderr)
        for name in transitions:
            ...
            if self.debug:
                print(f'\nStateMachine.check_line: No match in state "{state.__class__.__name__}".', file=sys.stderr)
    ...
    def error(self) -> None:
        ...
        print(f'{type_name}: {value}', file=sys.stderr)
        print(f'input line {self.abs_line_number()}', file=sys.stderr)
        print(f'module {module}, line {line}, function {function}',
              file=sys.stderr)
...
class StateWS(State):
    ...
    def indent(self, match, context, next_state):
        ...
        (`self.indent_sm`).
        """
        (indented, _indent, line_offset, _blank_finish
    ...
    def known_indent(self, match, context, next_state):
        ...
        match, ``match.end()``.
        """
        (indented, line_offset, _blank_finish
...
class ViewList:
    ...
    def trim_start(self, n=1):
        ...
        """
        if n > len(self.data):
            raise IndexError(f"Size of trim too large; can't trim {n} items "
                             f"from a list of size {len(self.data)}.")
        elif n < 0:
