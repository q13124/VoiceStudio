    ...
    def apply(self) -> None:
        ...
        if len(anonymous_refs) != len(anonymous_targets):
            msg = self.document.reporter.error(
                  f'Anonymous hyperlink mismatch: {len(anonymous_refs)} references but {len(anonymous_targets)} '
...
class IndirectHyperlinks(Transform):
    ...
    def indirect_target_error(self, target, explanation) -> None:
        ...
        reflist = []
        if target['names']:
            naming = '"{}" '.format(target['names'][0])
        for name in target['names']:
            reflist.extend(self.document.refnames.get(name, []))
        for id in target['ids']:
            reflist.extend(self.document.refids.get(id, []))
        if target['ids']:
            naming += '(id="{}")'.format(target['ids'][0])
        msg = self.document.reporter.error(
              'Indirect hyperlink target {} refers to target "{}", {}.'.format(naming, target['refname'], explanation), base_node=target)
        msgid = self.document.set_id(msg)
    ...
    def symbolize_footnotes(self) -> None:
        ...
        for ref in self.document.symbol_footnote_refs:
            try:
                ...
                msg = self.document.reporter.error(
                      f'Too many symbol footnote references: only {len(labels)} '
...
class Substitutions(Transform):
    ...
    def apply(self):
        ...
        for ref in subreflist:
            ...
            if key is None:
                msg = self.document.reporter.error(
                      f'Undefined substitution referenced: "{refname}".', base_node=ref)
            else:
                subdef = defs[key]
                if len(subdef.astext()) > line_length_limit:
                    msg = self.document.reporter.error(
                            f'Substitution definition "{key}" exceeds the'
                            ' line-length-limit.')
            if msg:
                msgid = self.document.set_id(msg)
                prb = nodes.problematic(
            ...
            if ('ltrim' in subdef.attributes
                or 'trim' in subdef.attributes) and index > 0 and isinstance(parent[index - 1],
                                        nodes.Text):
                parent[index - 1] = parent[index - 1].rstrip()
            if ('rtrim' in subdef.attributes
                or 'trim' in subdef.attributes) and (len(parent) > index + 1
            ...
            try:
                ...
                if isinstance(parent, nodes.substitution_definition):
                    ...
                    msg = self.document.reporter.error(
                        ...
                        f'"{refname}".', base_node=ref_origin)
                    msgid = self.document.set_id(msg)
                    prb = nodes.problematic(
                        ref.rawsource, ref.rawsource, refid=msgid)
                    prbid = self.document.set_id(prb)
        ...
        for target in self.document.findall(nodes.target):
            if not target.referenced:
                ...
                self.document.reporter.info(
                    f'Hyperlink target "{naming}" is not referenced.', base_node=target)
