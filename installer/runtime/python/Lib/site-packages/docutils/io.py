...
class Input(TransformSpec):
    ...
    def __repr__(self) -> str:
        return f'{self.__class__}: source={self.source!r}, source_path={self.source_path!r}'
...
class Output(TransformSpec):
    ...
    def __repr__(self) -> str:
        return (f'{self.__class__}: destination={self.destination!r}, destination_path={self.destination_path!r}')
...
class FileInput(Input):
    ...
    def __init__(
        ...
        if source is None:
            ...
            raise UnicodeError(f'Encoding clash: encoding given is "{self.encoding}" '
                               f'but source is opened with encoding "{self.source.encoding}".')
        if not source_path:
            with contextlib.suppress(AttributeError):
...
class FileOutput(Output):
    ...
    def __init__(self,
        ...
        if destination is None:
            ...
            print(f'Warning: Destination mode "{self.destination.mode}" differs from specified '
                  f'mode "{mode}"',
                  file=self._stderr)
        if not destination_path:
            with contextlib.suppress(AttributeError):
