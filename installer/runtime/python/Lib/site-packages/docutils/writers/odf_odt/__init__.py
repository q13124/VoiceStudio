...
_ElementInterface = ET.Element if isinstance(ET.Element, type) else ET._ElementInterface
...
TABLENAMEDEFAULT = f'{TABLESTYLEPREFIX}0'
TABLEPROPERTYNAMES = (
...
class Writer(writers.Writer):
    ...
    settings_spec = (
        ...
        (
            ('Specify a stylesheet.  '
                f'Default: "{default_stylesheet_path}"',
                ['--stylesheet'],
    ...
    def assemble_my_parts(self) -> None:
        ...
        updated, new_dom_styles, _updated_node = self.update_stylesheet(
...
class ODFTranslator(nodes.GenericNodeVisitor):
        ...
        if self.settings.odf_config_file:
            ...
            for rststyle, format in parser.items("Formats"):
                if rststyle not in self.used_styles:
                    self.document.reporter.warning(
                        f'Style "{rststyle}" is not a style used by odtwriter.')
    ...
    def retrieve_styles(self, extension):
        ...
        if ext == '.xml':
            ...
            raise RuntimeError(f'stylesheet path ({stylespath}) must be {extension} or '
                               '.xml file')
    ...
    def rststyle(self, name, parameters=()):
        ...
        return self.format_map.get(name1, f'rststyle-{name1}')
    ...
    def setup_paper(self, root_el) -> None:
        ...
        def walk(el) -> None:
            if el.tag == "{{{}}}page-layout-properties".format(SNSD["style"]) and \
                    "{{{}}}page-width".format(SNSD["fo"]) not in el.attrib:
                el.attrib["{{{}}}page-width".format(SNSD["fo"])] = f"{w:.3f}pt"
                el.attrib["{{{}}}page-height".format(SNSD["fo"])] = f"{h:.3f}pt"
                el.attrib["{{{}}}margin-left".format(SNSD["fo"])] = \
                    el.attrib["{{{}}}margin-right".format(SNSD["fo"])] = \
                    "%.3fpt" % (.1 * w)
                el.attrib["{{{}}}margin-top".format(SNSD["fo"])] = \
                    el.attrib["{{{}}}margin-bottom".format(SNSD["fo"])] = \
                    "%.3fpt" % (.1 * h)
            else:
                for subel in el:
                    walk(subel)
    ...
    def add_header_footer(self, root_el) -> None:
        automatic_styles = root_el.find(
            '{{{}}}automatic-styles'.format(SNSD['office']))
            ...
            if len(self.bumped_list_level_stack) > 0:
                ...
                if level_obj.get_sibling():
                    ...
                    for level_obj1 in self.bumped_list_level_stack:
                        for _idx in range(level_obj1.get_level()):
    ...
    def visit_image(self, node) -> None:
        ...
        if uri_parts.scheme in ('', 'file'):
            ...
            if self.settings.root_prefix and source.startswith('/'):
                ...
                docsource, _line = utils.get_source_line(node)
                if docsource:
        ...
        if isinstance(node.parent, docutils.nodes.figure):
            _el3, _el4, el5, caption = self.generate_figure(
                node, source,
                destination, el2)
            attrib = {}
            el6, _width = self.generate_image(
    ...
    def get_image_scaled_width_height(self, node, source):
        ...
        if width is None or height is None:
            ...
            filename, _destination = self.image_dict[source]
    ...
    def generate_figure(self, node, source, destination, current_element):
        caption = None
        width, _height = self.get_image_scaled_width_height(node, source)
    ...
    def visit_option_list(self, node) -> None:
        ...
        if not self.optiontablestyles_generated:
            ...
            el = SubElement(self.automatic_styles, 'style:style', attrib={
                'style:name': self.rststyle(f'{table_name}.%c', ('A', )),
    ...
    def depart_reference(self, node) -> None:
        if self.settings.create_links and 'refuri' in node:
            self.set_to_parent()
    ...
    def visit_table(self, node) -> None:
        ...
        table_name = f'{TABLESTYLEPREFIX}%d'
        el1 = SubElement(self.automatic_styles, 'style:style', attrib={
            'style:name': self.rststyle(
                f'{table_name}', (self.table_count, )),
            'style:family': 'table',
        }, nsdict=SNSD)
        ...
        el2 = SubElement(self.automatic_styles, 'style:style', attrib={
            'style:name': self.rststyle(
                f'{table_name}.%c%d', (self.table_count, 'A', 1, )),
    ...
    def visit_colspec(self, node) -> None:
        ...
        SubElement(el1, 'style:table-column-properties',
                   attrib={'style:column-width': f'{colwidth:.4f}in'},
                   nsdict=SNSD)
        ...
        if isinstance(node.parent, docutils.nodes.section):
            ...
            if section_level > 7:
                self.document.reporter.warning(
                    ...
                    f'    Reducing to heading level 7 for heading: "{node.astext()}"', base_node=node)
                section_level = 7
    ...
    def find_title_label(self, node, class_type, label_key):
        ...
        for child in node.children:
            if isinstance(child, class_type):
                title_node = child
                break
        label = title_node.astext() if title_node is not None else self.language.labels[label_key]
        return label
    ...
    def depart_topic(self, node) -> None:
        if 'classes' in node.attributes and 'contents' in node.attributes['classes']:
            if self.settings.generate_oowriter_toc:
