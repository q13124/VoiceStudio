...
class Translator(nodes.NodeVisitor):
    ...
    def astext(self):
        ...
        for i in range(len(self.body)-1, 0, -1):
            ...
            if self.body[i] == '.sp\n':
                if self.body[i - 1][:4] in ('.BI ', '.IP ') or (self.body[i - 1][:3] == '.B '
    ...
    def list_start(self, node) -> None:
        class EnumChar:
            ...
            def __repr__(self) -> str:
                return f'enum_style-{list(self._style)}'
    ...
    def header(self):
        th = (".TH \"{title}\" \"{manual_section}\""
              " \"{date}\" \"{version}\"").format(**self._docinfo)
        if self._docinfo["manual_group"]:
            th += " \"{manual_group}\"".format(**self._docinfo)
        th += "\n"
        sh_tmpl = (".SH Name\n"
                   "%(title)s \\- %(subtitle)s\n")
    ...
    def depart_document(self, node) -> None:
        if self._docinfo['author']:
            self.body.append('.SH Author\n{}\n'.format(', '.join(self._docinfo['author'])))
        ...
        if self._docinfo['copyright']:
            self.body.append('.SH Copyright\n{}\n'.format(self._docinfo['copyright']))
    ...
    def visit_footnote(self, node) -> None:
        ...
        num = num.strip()
        self.body.append(f'.IP [{self.deunicode(num)}] 5\n')
    ...
    def visit_image(self, node):
        ...
        if 'alt' in node.attributes:
            self.document.reporter.info(msg,
                                        base_node=node)
            self.body.append('.sp\n    {}\n'.format(node.attributes['alt']))
        elif 'uri' in node.attributes:
            self.body.append('.sp\n    image: {}\n'.format(node.attributes['uri']))
            self.document.reporter.warning(
    ...
    def visit_option_argument(self, node) -> None:
        ...
        if node['delimiter'] != ' ':
            self.body.append('\\fB{} '.format(node['delimiter']))
    ...
    def _visit_reference_no_macro(self, node) -> None:
        ...
        if 'refuri' in node:
            ...
            if (node['refuri'] == node.astext()
                ...
                # without mailto:
                self.body.append(rf"\%<{insert_URI_breakpoints(node.astext())}>")
                raise nodes.SkipNode
    ...
    def _depart_reference_no_macro(self, node) -> None:
        if 'refuri' in node:
            self.body.append(r" \%<{}>".format(insert_URI_breakpoints(node['refuri'])))
        # elif 'refid' in node:
    ...
    def _visit_reference_with_macro(self, node) -> None:
        ...
        if 'refuri' in node:
            ...
            self.body.append(f"\\%{insert_URI_breakpoints(_uri)}\n")
            if (node['refuri'] == node.astext()
                or node['refuri'] == "mailto:"+node.astext()):
    ...
    def visit_system_message(self, node) -> None:
        ...
        self.body.append('.IP "System Message: {}/{} ({}:{})"\n'.format(node['type'], node['level'], node['source'], line))
    ...
    def visit_title(self, node):
        if isinstance(node.parent, nodes.topic):
            ...
            self.body.append(f'.SH {self.deunicode(node.astext())}\n')
            raise nodes.SkipNode
        else:
            self.body.append('.SS ')
