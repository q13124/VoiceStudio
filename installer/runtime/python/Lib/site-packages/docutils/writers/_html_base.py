    ...
    settings_spec = (
        ...
        (('Specify the template file (UTF-8 encoded). '
         ...
         ('Specify the initial header level. '
          ...
          {'choices': ['1', '2', '3', '4', '5', '6', 'auto'], 'default': '2',
           'metavar': '<level>'}),
...
class HTMLTranslator(writers.DoctreeTranslator):
        ...
        if ids:
            ...
            for id in ids[1:]:
                ...
                if empty or isinstance(node, (nodes.Sequential,
                    ...
                    prefix.append(f'<span id="{id}"></span>')
                else:
    ...
    def visit_entry(self, node) -> None:
        ...
        self.context.append(f'</{tagname.lower()}>\n')
        ...
        for i, cls in enumerate(classes):
            if cls.startswith('field-indent-'):
                try:
                    ...
                    break
                atts['style'] = f'--field-indent: {indent_length};'
                classes.pop(i)
                break
    ...
    def visit_generated(self, node):
        if 'sectnum' in node['classes']:
            ...
            self.body.append(f'<span class="sectnum">{self.encode(sectnum)} </span>')
            # Content already processed:
            raise nodes.SkipNode
    ...
    def depart_label(self, node) -> None:
        ...
        if len(backrefs) > 1:
            ...
            self.body.append('<span class="backrefs">({})</span>\n'.format(','.join(backlinks)))
    ...
    def visit_math(self, node):
        ...
        if format == 'html':
            ...
            math_code = wrap_math_code(math_code, is_block) if is_block else rf'\({math_code}\)'
            math_code = self.encode(math_code)
        elif format == 'mathml':
            ...
            try:
                ...
                format = 'problematic' if self.settings.report_level <= 2 else 'latex'
    ...
    def visit_problematic(self, node) -> None:
        if node.hasattr('refid'):
            self.body.append('<a href="#{}">'.format(node['refid']))
    ...
    def visit_table(self, node) -> None:
        ...
        if 'align' in node:
            atts['classes'].append('align-{}'.format(node['align']))
        if 'width' in node:
            width = node['width']
