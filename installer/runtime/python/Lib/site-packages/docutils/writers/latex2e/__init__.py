...
class Babel:
    ...
    def __call__(self):
        ...
        self.setup = [r'\usepackage[{}]{{babel}}'.format(','.join(languages))]
        ...
        for c in ''.join(self.active_chars.get(lng, '') for lng in languages):
            if c not in shorthands:
                shorthands.append(c)
        if shorthands:
            self.setup.append(r'\AtBeginDocument{{\shorthandoff{{{}}}}}'.format(''.join(shorthands)))
...
class Table:
    ...
    def get_opening(self, width=r'\linewidth'):
        ...
        if align and not latex_type.startswith("longtable"):
            opening.append(rf'\noindent\makebox[\linewidth]{align}{{%')
            align = ''
        if not self.colwidths_auto:
            if self.borders == 'standard' and not self.legacy_column_widths:
                ...
                opening.append(rf'\setlength{{\DUtablewidth}}{{{width}}}%')
        opening.append(rf'\begin{{{latex_type}}}{align}')
        return '\n'.join(opening)
    ...
    def get_closing(self):
        ...
        closing.append(rf'\end{{{self.get_latex_type()}}}')
        if (self.get('align')
    ...
    def get_colspecs(self, node):
        ...
        if self.colwidths_auto:
            ...
            latex_colspecs = [f'p{{{colwidth:.3f}\\DUtablewidth}}'
            ...
            if total_width > 1 or 'width' in node.parent.parent.attributes:
                self._colwidths = [colwidth/total_width
                                   for colwidth in self._colwidths]
            latex_colspecs = [f'p{{\\DUcolumnwidth{{{colwidth:.3f}}}}}'
                              for colwidth in self._colwidths]
    ...
    def get_column_width(self) -> str:
        ...
        try:
            if self.legacy_column_widths:
                return f'{self._colwidths[self._cell_in_row]:.2f}\\DUtablewidth'
            return f'\\DUcolumnwidth{{{self._colwidths[self._cell_in_row]:.2f}}}'
        except IndexError:
            return '*'
    ...
    def get_multicolumn_width(self, start, len_) -> str:
        ...
        try:
            ...
            if self.legacy_column_widths:
                return f'p{{{multicol_width:.2f}\\DUtablewidth}}'
            return f'p{{\\DUcolumnwidth{{{multicol_width:.3f}}}}}'
        except IndexError:
            return 'l'
...
class LaTeXTranslator(writers.DoctreeTranslator):
    ...
    def __init__(self, document, babel_class=Babel) -> None:
        ...
        if settings.graphicx_option == '':
            ...
            self.graphicx_package = (rf'\usepackage[{settings.graphicx_option}]{{graphicx}}')
        ...
        if settings.language_code != 'en':
            self.pdfinfo.append(f'  pdflang={{{settings.language_code}}},')
        ...
        if self.latex_encoding not in ('ascii', 'unicode', 'utf8'):
            self.requirements['_inputenc'] = (rf'\usepackage[{self.latex_encoding}]{{inputenc}}')
        # TeX font encoding
        if not self.is_xetex:
            if self.font_encoding:
                self.requirements['_fontenc'] = (rf'\usepackage[{self.font_encoding}]{{fontenc}}')
            ...
            for ch in text:
                ...
                # preamble-definitions for unsupported Unicode characters
                    ...
                    self.requirements['_inputenc'+str(cp)] = (
                        f'\\DeclareUnicodeCharacter{{{cp:04X}}}{{{CharMaps.unsupported_unicode[cp]}}}')
    ...
    def append_hypertargets(self, node) -> None:
        ...
        self.out.append('%\n'.join(f'\\raisebox{{1em}}{{\\hypertarget{{{id}}}{{}}}}' for id in node['ids']))
    ...
    def visit_admonition(self, node) -> None:
        ...
        if self.settings.legacy_class_functions:
            ...
            self.out.append('\n\\DUadmonition[{}]{{'.format(','.join(node['classes'])))
            return
    ...
    def visit_citation_reference(self, node) -> None:
        ...
        if self.use_latex_citations:
            ...
            self.out.append(f'\\hyperlink{{{href}}}{{[')
    ...
    def depart_document(self, node) -> None:
        ...
        if self.document.get('title', ''):
            self.pdfinfo.insert(0, '  pdftitle={{{}}},'.format(self.encode(self.document.get('title', ''))))
        if self.pdfauthor:
            authors = (self.author_separator + ' ').join(self.pdfauthor)
            self.pdfinfo.append(f'  pdfauthor={{{authors}}}')
        if self.pdfinfo:
    ...
    def make_title(self) -> None:
        ...
        if self.subtitle:
            title_arg += [r'\\', r'\DUdocumentsubtitle{{{}}}'.format(''.join(self.subtitle)), *self.subtitle_labels]
        self.titledata.append(r'\title{{{}}}'.format('%\n  '.join(title_arg)))
        # \author
        ...
        self.titledata.append(r'\author{{{}}}'.format(' \\and\n'.join(authors)))
        # \date
        self.titledata.append(r'\date{{{}}}'.format(', '.join(self.date)))
        # \maketitle
    ...
    def append_bibliogaphy(self) -> None:
        ...
        if self.bibtex:
            ...
            self.out.append('\\bibliography{{{}}}\n'.format(','.join(self.bibtex[1:])))
            ...
            for bibitem in self._bibitems:
                ...
                self.out.append(f'\\bibitem[{bibitem[0]}]{{{cite_key}}}{{{bibitem[1]}}}\n')
    ...
    def visit_enumerated_list(self, node) -> None:
        ...
        label = rf'{prefix}\{enumtype}{{{counter_name}}}{suffix}'
        ...
        if enum_level <= 4:
            ...
            if (prefix, enumtype, suffix) != default_labels[enum_level-1]:
                self.out.append(f'\n\\renewcommand{{\\label{counter_name}}}{{{label}}}')
        else:
            self.fallbacks[counter_name] = f'\\newcounter{{{counter_name}}}'
            self.out.append('\\begin{list}')
            self.out.append(f'{{{label}}}')
            self.out.append(f'{{\\usecounter{{{counter_name}}}}}')
        if 'start' in node:
            self.out.append('\n\\setcounter{%s}{%d}' %
    ...
    def visit_figure(self, node) -> None:
        ...
        if alignment != 'center':
            ...
            self.out.append(f'\\begin{{figure}} % align = "{alignment}"')
        else:
    ...
    def visit_footnote(self, node) -> None:
        ...
        if self.docutils_footnotes:
            ...
            if self.settings.footnote_references == 'brackets':
                num = f'[{num}]'
            self.out.append('%\n\\DUfootnotetext{{{}}}{{{}}}{{{}}}{{'.format(node['ids'][0], backref, self.encode(num)))
    ...
    def visit_footnote_reference(self, node) -> None:
        ...
        if format == 'brackets':
            ...
            self.provide_fallback('footnotes')
            self.out.append(r'\DUfootnotemark{{{}}}{{{}}}{{'.format(node['ids'][0], href))
            self.context.append('}')
    ...
    def visit_inline(self, node) -> None:
        ...
        for cls in node['classes']:
            if cls.startswith('language-'):
                ...
                if language:
                    self.babel.otherlanguages[language] = True
                    self.out.append(rf'\foreignlanguage{{{language}}}{{')
            else:
                self.provide_fallback('inline')
                self.out.append(rf'\DUrole{{{cls}}}{{')
    ...
    def visit_literal_block(self, node) -> None:
        ...
        if literal_env and _use_env and (not _autowidth_table
            ...
            self.out.append(f'\\begin{{{literal_env}}}{self.literal_block_options}\n')
    ...
    def visit_meta(self, node) -> None:
        ...
        if name in ('author', 'keywords', 'producer', 'subject', 'title'):
            ...
            self.pdfinfo.append(f'  pdf{name}={{{content}}},')
        elif name == 'creator':
            self.pdfinfo.append(f'  addtopdfcreator={{{content}}},')
        else:
            # generic interface (case sensitive!)
            self.pdfinfo.append(f'  pdfinfo={{{name}={{{content}}}}},')
    ...
    def visit_paragraph(self, node) -> None:
        ...
        if (index == 0 and isinstance(node.parent,
                                     (nodes.list_item, nodes.description,
                                      nodes.compound, nodes.container))) or (index > 0
    ...
    def visit_problematic(self, node) -> None:
        ...
        self.out.append('%\n')
        self.append_hypertargets(node)
        self.out.append(r'\hyperlink{{{}}}{{\textbf{{\color{{red}}'.format(node['refid']))
    ...
    def visit_reference(self, node):
        ...
        if 'refuri' in node:
            ...
            if node['refuri'] == node.astext():
                self.out.append(rf'\url{{{href}}}')
                raise nodes.SkipNode
            self.out.append(rf'\href{{{href}}}{{')
            return
        # internal reference
        if 'refid' in node:
            href = node['refid']
        ...
        if self.reference_label:
            self.out.append('\\{}{{{}}}'.format(self.reference_label, href.replace('#', '')))
            raise nodes.SkipNode
        self.out.append(f'\\hyperref[{href}]{{')
    ...
    def visit_subtitle(self, node) -> None:
        ...
        # section subtitle: "starred" (no number, not in ToC)
            ...
            self.out.append(rf'\{self.d_class.section(self.section_level + 1)}*{{')
        else:
    ...
    def visit_thead(self, node) -> None:
        ...
        if self.thead_depth() == 1:
            self.out.append(f'{{{self.active_table.get_colspecs(node)}}}\n')
    ...
    def visit_title(self, node) -> None:
        ...
        elif (isinstance(node.parent, (nodes.topic, nodes.admonition, nodes.sidebar))):
            classes = node.parent['classes'] or [node.parent.tagname]
            if self.settings.legacy_class_functions:
                self.fallbacks['title'] = PreambleCmds.title_legacy
                self.out.append('\n\\DUtitle[{}]{{'.format(','.join(classes)))
            else:
                self.provide_fallback('title')
                self.out.append('\n\\DUtitle{')
            self.context.append('}\n')
        ...
        # Section title
            ...
            if level > len(self.d_class.sections):
                ...
                if self.settings.legacy_class_functions:
                    self.fallbacks['title'] = PreambleCmds.title_legacy
                    section_name += f'[section{RomanNumeral(level)}]'
                else:
                    self.provide_fallback('title')
                    self.provide_fallback('duclass', 'DUclass')
                    self.out.append(f'\\begin{{DUclass}}{{section{RomanNumeral(level)}}}\n')

            # System messages heading in red:
            if 'system-messages' in node.parent['classes']:
                self.requirements['color'] = PreambleCmds.color
                section_title = self.encode(node.astext())
                self.out.append(rf'\{section_name}[{section_title}]{{\color{{red}}')
            else:
                self.out.append(rf'\{section_name}{{')
    ...
    def visit_contents(self, node):
        ...
        if self.use_latex_toc and 'local' in node['classes']:
            ...
            try:
                ...
                self.warn(f'Skipping local ToC at "{section_name}" level.\n'
                          '  Feature not supported with option "use-latex-toc"', base_node=node)
                raise nodes.SkipNode
        ...
        title = self.encode(node[0].astext()) if isinstance(node.next_node(), nodes.title) else ''
        if 'local' in node['classes']:
            # use the "minitoc" package
            self.requirements['minitoc'] = PreambleCmds.minitoc
            self.requirements['minitoc-'+mtc_name] = rf'\do{mtc_name}toc'
            self.requirements[f'minitoc-{mtc_name}-depth'] = (
            ...
            # title:
            self.out.append(f'\\mtcsettitle{{{mtc_name}toc}}{{{title}}}\n')
            # the toc-generating command:
            self.out.append(f'\\{mtc_name}toc\n')
        else:
            if depth:
                self.out.append('\\setcounter{tocdepth}{%d}\n'
                                % self.d_class.latex_section_depth(depth))
            if title != 'Contents':
                self.out.append(f'\\renewcommand{{\\contentsname}}{{{title}}}\n')
    ...
    def unimplemented_visit(self, node):
        raise NotImplementedError(f'visiting unimplemented node type: {node.__class__.__name__}')
