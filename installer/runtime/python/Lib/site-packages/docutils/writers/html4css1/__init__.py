...
class Writer(writers._html_base.Writer):
    ...
    settings_spec = frontend.filter_settings_spec(
        ...
        stylesheet_path=(
            ...
            '(default: "{}")'.format(','.join(default_stylesheets)),
            ['--stylesheet-path'],
        ...
        stylesheet_dirs=(
            ...
            'arguments. (default: "{}")'.format(','.join(default_stylesheet_dirs)),
            ['--stylesheet-dirs'],
            {'metavar': '<dir[,dir,...]>',
             'validator': frontend.validate_comma_separated_list,
        ...
        initial_header_level=(
            ...
            {'choices': ['1', '2', '3', '4', '5', '6', 'auto'], 'default': '1',
             'metavar': '<level>'}),
        math_output=(
...
class HTMLTranslator(writers._html_base.HTMLTranslator):
    ...
    def visit_docinfo_item(self, node, name, meta=True) -> None:
        if meta:
            meta_tag = f'<meta name="{name}" content="{self.attval(node.astext())}" />\n'
            self.meta.append(meta_tag)
        self.body.append(self.starttag(node, 'tr', ''))
        self.body.append(f'<th class="docinfo-name">{self.language.labels[name]}:</th>\n<td>')
        if len(node):
    ...
    def visit_field_list(self, node) -> None:
        ...
        self.compact_p = None
        if 'compact' in node['classes'] or (self.settings.compact_field_lists
              and 'open' not in node['classes']):
    ...
    def footnote_backrefs(self, node) -> None:
        ...
        if self.settings.footnote_backlinks and backrefs:
            if len(backrefs) == 1:
                ...
                self.context.append(f'<a class="fn-backref" href="#{backrefs[0]}">')
            else:
                for (i, backref) in enumerate(backrefs, 1):
                    backlinks.append(f'<a class="fn-backref" href="#{backref}">{i}</a>')
                self.context.append('<em>({})</em> '.format(', '.join(backlinks)))
                self.context += ['', '']
        else:
            self.context.append('')
            self.context += ['', '']
    ...
    def visit_label(self, node) -> None:
        self.body.append(self.starttag(node, 'td', f'{self.context.pop()}[',
                                       CLASS='label'))
    ...
    def should_be_compact_paragraph(self, node) -> bool:
        ...
        return bool(self.compact_simple or self.compact_field_list or (self.compact_p and parent_length == 1))
    ...
    def visit_subtitle(self, node) -> None:
        if isinstance(node.parent, nodes.sidebar):
            ...
            self.context.append(f'</span></{tag}>\n')
    ...
    def visit_system_message(self, node) -> None:
        ...
        if len(node['backrefs']):
            backrefs = node['backrefs']
            if len(backrefs) == 1:
                backref_text = (f'; <em><a href="#{backrefs[0]}">backlink</a></em>')
            else:
                i = 1
                backlinks = []
                for backref in backrefs:
                    ...
                    i += 1
                backref_text = ('; <em>backlinks: {}</em>'.format(', '.join(backlinks)))
        line = ', line {}'.format(node['line']) if node.hasattr('line') else ''
        self.body.append('System Message: {}/{} '
                         '(<tt class="docutils">{}</tt>{}){}</p>\n'.format(node['type'], node['level'],
    ...
    def visit_table(self, node) -> None:
        ...
        if 'align' in node:
            classes.append('align-{}'.format(node['align']))
        if 'width' in node:
            width = node['width']
