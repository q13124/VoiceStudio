...
class Node:
    ...
    def walkabout(self, visitor: NodeVisitor) -> bool:
        ...
        stop = False
        visitor.document.reporter.debug(
            f'docutils.nodes.Node.walkabout calling dispatch_visit for {self.__class__.__name__}')
        try:
            try:
                visitor.dispatch_visit(self)
            except SkipNode:
...
class Text(Node, str):
    ...
    def shortrepr(self, maxlen: int = 18) -> str:
        ...
        if len(data) > maxlen:
            data = data[:maxlen-4] + ' ...'
        return f'<{self.tagname}: {str(data)!r}>'
...
class Element(Node):
    ...
    def _dom_node(self, domroot: minidom.Document) -> minidom.Element:
        ...
        for attribute, value in self.attlist():
            ...
            element.setAttribute(attribute, f'{value}')
        for child in self.children:
    ...
    def shortrepr(self) -> str:
        if self['names']:
            return '<{} "{}"...>'.format(self.tagname, '; '.join(self['names']))
        else:
    ...
    def emptytag(self) -> str:
        attributes = (f'{n}="{v}"' for n, v in self.attlist())
        return '<{}/>'.format(' '.join((self.tagname, *attributes)))
    ...
    def is_not_default(self, key: str) -> bool:
        return not (self[key] == [] and key in self.list_attributes)
    ...
    def update_all_atts(self,
        ...
        filter_fun = self.is_not_list_attribute if and_source else self.is_not_known_attribute
    ...
    def note_substitution_def(self,
        ...
        if name in self.substitution_defs:
            msg = self.reporter.error(
                      f'Duplicate substitution definition name: "{name}".',
                      base_node=subdef)
            if msgnode is not None:
                msgnode += msg
...
class pending(Invisible, Element):
    ...
    def pformat(self, indent: str = '    ', level: int = 0) -> str:
        internals = ['.. internal attributes:',
                     f'     .transform: {self.transform.__module__}.{self.transform.__name__}',
                     '     .details:']
...
class NodeVisitor:
    ...
    def dispatch_departure(self, node) -> None:
        ...
        self.document.reporter.debug(
            f'docutils.nodes.NodeVisitor.dispatch_departure calling {method.__name__} for {node_name}')
        return method(node)
    ...
    def unknown_visit(self, node) -> None:
        ...
        if (self.document.settings.strict_visitor
            ...
            raise NotImplementedError(
                f'{self.__class__} visiting unknown node type: {node.__class__.__name__}')
    ...
    def unknown_departure(self, node) -> None:
        ...
        if (self.document.settings.strict_visitor
            or node.__class__.__name__ not in self.optional):
            raise NotImplementedError(
                f'{self.__class__} departing unknown node type: {node.__class__.__name__}')
