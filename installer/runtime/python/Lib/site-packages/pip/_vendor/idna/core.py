...
def valid_label_length(label: bytes | str) -> bool:
    return not len(label) > 63


def valid_string_length(label: bytes | str, trailing_dot: bool) -> bool:
    return not len(label) > (254 if trailing_dot else 253)
...
def valid_contextj(label: str, pos: int) -> bool:
    ...
    if cp_value == 0x200D:
        return bool(pos > 0 and _combining_class(ord(label[pos - 1])) == _virama_combining_class)
...
def valid_contexto(label: str, pos: int, exception: bool = False) -> bool:
    ...
    if cp_value == 0x00B7:
        ...
        return all(not 1776 <= ord(cp) <= 1785 for cp in label)

    elif 0x6F0 <= cp_value <= 0x6F9:
        return all(not 1632 <= ord(cp) <= 1641 for cp in label)
