...
def Callback(hras, msg, state, error, exterror):
    ...
    if error != 0 or int(state) == win32ras.RASCS_Disconnected:
        ...
        print(f"Detected call failure: {win32ras.GetErrorString(error)}")
        HangUp(hras)
        win32event.SetEvent(callbackEvent)
...
def EditEntry(entryName):
    try:
        ...
        (_rc, _function, msg) = xxx_todo_changeme.args
...
def Connect(entryName, bUseCallback):
    ...
    try:
        dp, _b = win32ras.GetEntryDialParams(None, entryName)
    except:
        print(f"Couldn't find DUN entry: {entryName}")
    else:
...
def Disconnect(rasEntry):
    ...
    name = rasEntry.lower()
    for hcon, entryName, _devName, _devType in win32ras.EnumConnections():
