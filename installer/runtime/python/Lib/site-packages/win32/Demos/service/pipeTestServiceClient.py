...
def testClient(server, msg):
    ...
    data = CallPipe(
        CallNamedPipe,
        (f"\\\\{server}\\pipe\\PyPipeTest", msg, 256, NMPWAIT_WAIT_FOREVER),
    )
    if verbose:
        print(f"Server sent back '{data}'")
...
def testLargeMessage(server, size=4096):
    ...
    data = CallPipe(
        CallNamedPipe,
        (f"\\\\{server}\\pipe\\PyPipeTest", msg, 512, NMPWAIT_WAIT_FOREVER),
    )
    if len(data) - size:
...
def stressTestClient(server, numThreads, numMessages):
    ...
    thread_waits = []
    for _t_num in range(numThreads):
...
def main():
    ...
    try:
        ...
        print(
            f"Usage: {my_name} [-v] [-s server] [-t thread_count=0] [-m msg_count=500] msg ..."
        )
        print("       -v = verbose")
        print(
