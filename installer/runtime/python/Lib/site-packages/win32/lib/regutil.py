...
def RegisterModule(modName, modPath):
    ...
    try:
        ...
        print(f"Warning: Registering non-existant module {modPath}")
    win32api.RegSetValue(
        GetRootKey(),
        BuildDefaultPythonKey() + f"\\Modules\\{modName}",
        win32con.REG_SZ,
        modPath,
    )
...
def UnregisterModule(modName):
    ...
    try:
        win32api.RegDeleteKey(
            GetRootKey(), BuildDefaultPythonKey() + f"\\Modules\\{modName}"
        )
    except win32api.error as exc:
        import winerror
...
def RegisterHelpFile(helpFile, helpPath, helpDesc=None, bCheckFile=1):
    ...
    win32api.RegSetValue(
        GetRootKey(),
        BuildDefaultPythonKey() + f"\\Help\\{helpDesc}",
        win32con.REG_SZ,
        fullHelpFile,
    )
...
def UnregisterHelpFile(helpFile, helpDesc=None):
    ...
    try:
        win32api.RegDeleteKey(
            GetRootKey(), BuildDefaultPythonKey() + f"\\Help\\{helpDesc}"
        )
    except win32api.error as exc:
        import winerror
...
def RegisterFileExtensions(defPyIcon, defPycIcon, runCommand):
    ...
    win32api.RegSetValue(
        ...
        f"{pythonFileId}\\CLSID",
        win32con.REG_SZ,
        CLSIDPyFile,
    )
    win32api.RegSetValue(
        win32con.HKEY_CLASSES_ROOT,
        f"{pythonFileId}\\DefaultIcon",
        win32con.REG_SZ,
        defPyIcon,
    )
    base = f"{RegistryIDPyFile}\\Shell"
    win32api.RegSetValue(
...
def RegisterDDECommand(shellCommand, ddeApp, ddeTopic, ddeCommand):
    ...
    win32api.RegSetValue(
        win32con.HKEY_CLASSES_ROOT,
        base + f"\\{shellCommand}\\ddeexec",
        win32con.REG_SZ,
        ddeCommand,
    )
    win32api.RegSetValue(
        win32con.HKEY_CLASSES_ROOT,
        base + f"\\{shellCommand}\\ddeexec\\Application",
        win32con.REG_SZ,
        ddeApp,
    )
    win32api.RegSetValue(
        win32con.HKEY_CLASSES_ROOT,
        base + f"\\{shellCommand}\\ddeexec\\Topic",
        win32con.REG_SZ,
        ddeTopic,
    )
