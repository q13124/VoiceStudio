        ...
        if rp is not None:
            ...
            if self.token == "BEGIN":
                ...
                if rp is not None:
                    ...
                    # so push it back.
                    self.debug(f"Skipping top-level '{base_token}'")
                    self.ungetToken()
        ...
        while (
            ...
            if self.token != "|":
                if self.token in win32con.__dict__:
                    ...
                    value = getattr(commctrl, self.token) if self.token in commctrl.__dict__ else 0
                if Not:
...
def GenerateFrozenResource(rc_name, output_name, h_name=None):
    ...
    out.write(f"#This is a generated file. Please edit {rc_name} instead.\n")
...
if __name__ == "__main__":
    if len(sys.argv) <= 1:
        ...
        print(f"Dumping all resources in '{filename}'")
