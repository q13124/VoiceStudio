...
def LocateSpecificServiceExe(serviceName):
    ...
    hkey = win32api.RegOpenKey(
        ...
        f"SYSTEM\\CurrentControlSet\\Services\\{serviceName}",
        0,
...
def InstallService(
    ...
    exeName = f'"{LocatePythonServiceExe(exeName)}"'
...
def InstallPythonClassString(pythonClassString, serviceName):
    ...
    if pythonClassString:
        key = win32api.RegCreateKey(
            win32con.HKEY_LOCAL_MACHINE,
            f"System\\CurrentControlSet\\Services\\{serviceName}\\PythonClass",
        )
        try:
...
def SetServiceCustomOption(serviceName, option, value):
    ...
    key = win32api.RegCreateKey(
        win32con.HKEY_LOCAL_MACHINE,
        f"System\\CurrentControlSet\\Services\\{serviceName}\\Parameters",
    )
    try:
...
def RemoveService(serviceName):
    ...
    with contextlib.suppress(win32api.error):
...
def __FindSvcDeps(findName):
    ...
    while 1:
        ...
        try:
            deps, _typ = win32api.RegQueryValueEx(sk, "DependOnService")
        except win32api.error:
            deps = ()
        for dep in deps:
            dep = dep.lower()
...
def GetServiceClassString(cls, argv=None):
    ...
    if modName == "__main__":
        try:
            ...
            raise error(
                f"Could not resolve the path name '{argv[0]}' to a full path"
            )
...
def usage():
    try:
        ...
        fname = sys.argv[0]
    print(
        f"Usage: '{fname} [options] install|update|remove|start [...]|stop|restart [...]|debug [...]'"
    )
...
def HandleCommandLine(
    ...
    if arg == "start":
        ...
        knownArg = 1
        print(f"Restarting service {serviceName}")
    ...
    if arg == "install":
        ...
        try:
            ...
            print(f"Error installing service: {msg!s}")
            err = -1
    ...
    if arg == "update":
        ...
        knownArg = 1
        print(f"Removing service {serviceName}")
        try:
            RemoveService(serviceName)
        ...
        knownArg = 1
        print(f"Stopping service {serviceName}")
        try:
            if waitSecs:
