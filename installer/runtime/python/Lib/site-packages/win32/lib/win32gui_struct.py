...
def UnpackMENUITEMINFO(s):
    ...
    if fMask & win32con.MIIM_BITMAP == 0:
        hbmpItem = None
    text = win32gui.PyGetString(lptext, cch) if fMask & win32con.MIIM_STRING else None
    return _MakeResult(
...
def UnpackTVITEM(buffer):
    ...
    text = win32gui.PyGetString(item_textptr) if item_textptr else None
    return _MakeResult(
...
def UnpackLVCOLUMN(lparam):
    mask, fmt, cx, text_addr, _text_size, subItem, image, order = struct.unpack(
    ...
    if not (mask & commctrl.LVCF_ORDER):
        order = None
    text = win32gui.PyGetString(text_addr) if text_addr else None
    return _MakeResult(
...
def UnpackDEV_BROADCAST(lparam):
    ...
    size, devtype, _reserved = struct.unpack("iii", hdr_buf)
