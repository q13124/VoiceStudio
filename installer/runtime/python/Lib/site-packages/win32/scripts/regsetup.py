...
def FindPackagePath(packageName, knownFileName, searchPaths):
    ...
    for pathLook in searchPaths:
        if IsPackageDir(pathLook, packageName, knownFileName):
            ...
            return ret, ret
    raise error(f"The package {packageName} can not be located")
...
def FindHelpPath(helpFile, helpDesc, searchPaths):
    ...
    for pathLook in searchPaths:
        ...
        if FileExists(os.path.join(pathLook, helpFile)):
            return os.path.abspath(pathLook)
    raise error(f"The help file {helpFile} can not be located")
...
def LocateFileName(fileNamesString, searchPaths):
    ...
    for path in searchPaths:
        ...
        try:
            ...
            raise error(
                f"Need to locate the file {fileName}, but the win32ui module is not available\nPlease run the program again, passing as a parameter the path to this file."
            )
...
def LocatePythonCore(searchPaths):
    ...
    presearchPaths = currentPath.split(";") if currentPath else [os.path.abspath(".")]
    libPath = None
    ...
    suffix = IsDebug()
    for path in presearchPaths:
        if FileExists(os.path.join(path, f"unicodedata{suffix}.pyd")):
            corePath = path
            break
    if corePath is None and searchPaths is not None:
        corePath = LocatePath(f"unicodedata{suffix}.pyd", searchPaths)
    if corePath is None:
...
def RegisterShellInfo(searchPaths):
    ...
    exePath = FindRegisterPythonExe(f"Python{suffix}.exe", searchPaths)
    ...
    regutil.SetRegistryDefaultValue(
        "Python.File\\DefaultIcon", f"{exePath},0", win32con.HKEY_CLASSES_ROOT
    )
