...
class TestSimpleOps(unittest.TestCase):
    ...
    def testMoreFiles(self):
        ...
        attr, _ct, _at, _wt, size = win32file.GetFileAttributesEx(testName)
        self.assertTrue(
            size == newSize,
...
class TestOverlapped(unittest.TestCase):
    def testSimpleOverlapped(self):
        ...
        while 1:
            try:
                _hr, data = win32file.ReadFile(h, buffer, overlapped)
                win32event.WaitForSingleObject(overlapped.hEvent, win32event.INFINITE)
    ...
    def testCompletionPortsQueued(self):
        ...
        errCode, _bytes, _key, overlapped = win32file.GetQueuedCompletionStatus(
...
class TestFindFiles(unittest.TestCase):
    ...
    def testEmptySpec(self):
        ...
        num = 0
        for _i in win32file.FindFilesIterator(spec):
            num += 1
    ...
    def testEmptyDir(self):
        ...
        try:
            num = 0
            for _i in win32file.FindFilesIterator(os.path.join(test_path, "*")):
                num += 1
            # Expecting "." and ".." only
