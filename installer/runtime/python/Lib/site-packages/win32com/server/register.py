...
def _find_localserver_exe(mustfind):
    ...
    if not os.path.exists(exeName):
        ...
        try:
            key = f"SOFTWARE\\Python\\PythonCore\\{sys.winver}\\InstallPath"
    ...
    if not os.path.exists(exeName):
        if mustfind:
            raise RuntimeError(f"Can not locate the program '{exeBaseName}'")
        return None
    return exeName
...
def GetUnregisterServerKeys(clsid, progID=None, verProgID=None, customKeys=None):
    ...
    ret = [(f"CLSID\\{clsid!s}", win32con.HKEY_CLASSES_ROOT)]
    ...
    ret.append((f"AppID\\{clsid!s}", win32con.HKEY_CLASSES_ROOT))
    # Any custom keys?
    if customKeys:
...
def ReExecuteElevated(flags):
    ...
    try:
        ...
        if exit_code:
            ...
            print(f"Error: registration failed (exit code {exit_code}).")
...
if not pythoncom.frozen:
    try:
        win32api.RegQueryValue(
            ...
            f"Component Categories\\{CATID_PythonCOMServer}",
        )
    except win32api.error:
        try:
