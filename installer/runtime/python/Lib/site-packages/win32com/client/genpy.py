    ...
    def WriteVTableMap(self, generator):
        ...
        for v in self.vtableFuncs:
            ...
            for arg in desc.args:
                ...
                arg3_repr = None if arg[3] is None else repr(arg[3])
                print(
...
class DispatchItem(build.DispatchItem, WritableItem):
    ...
    def WriteClass(self, generator):
        if (
            ...
            and self.type_attr.typekind != pythoncom.TKIND_DISPATCH
        ):
            return
    ...
    def WriteClassHeader(self, generator):
        ...
        try:
            ...
            print(
                f"\t# This class is creatable by the name '{progId}'", file=stream
            )
    ...
    def WriteClassBody(self, generator):
        ...
        for name in sorted(self.mapFuncs):
            ...
            if generator.bBuildHidden or not entry.hidden:
                ...
                if entry.wasProperty:
                    print(
                        f"\t# The method {name} is actually a property, but must be used as a method to correctly pass the arguments",
                        file=stream,
                    )
        ...
        print(
            f"\t\treturn win32com.client.util.Iterator(ob, {resultCLSID})",
            file=stream,
        )
        ...
        if specialItems["count"]:
            ...
            print(
                f"\t#This class has Count() {typename} - allow len(ob) to provide this",
                file=stream,
            )
            for line in ret:
                print(line, file=stream)
...
class CoClassItem(build.OleItem, WritableItem):
    ...
    def __init__(self, typeinfo, attr, doc=None, sources=None, interfaces=None, bForUser=1):
    ...
    def WriteClass(self, generator):
        ...
        try:
            progId = pythoncom.ProgIDFromCLSID(self.clsid)
            print(f"# This CoClass is known by the name '{progId}'", file=stream)
        except pythoncom.com_error:
            pass
        print(
            f"class {self.python_name}(CoClassBaseClass): # A CoClass", file=stream
        )
        if doc and doc[1]:
...
class Generator:
    ...
    def _Build_CoClass(self, type_info_tuple):
        info, _infotype, doc, attr = type_info_tuple
    ...
    def _Build_CoClassChildren(self, coclass, coclass_info, oleItems, vtableItems):
        sources = {}
        interfaces = {}
        for _info, _info_type, refType, doc, refAttr, flags in coclass_info:
    ...
    def do_gen_file_header(self):
        ...
        print(f"# On {time.ctime(time.time())}", file=self.file)
    ...
    def generate_child(self, child, dir):
        ...
        try:
            ...
            if not found:
                ...
                for type_info_tuple in infos:
                    _info, infotype, doc, _attr = type_info_tuple
                    if infotype in [
                        pythoncom.TKIND_INTERFACE,
            ...
            for key, value in vtableItems.items():
                ...
                new_val = (existing[0], value) if existing is not None else (None, value)
                items[key] = new_val
