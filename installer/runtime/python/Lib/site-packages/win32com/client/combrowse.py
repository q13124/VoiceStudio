...
class HLICategory(HLICOM):
    ...
    def GetSubList(self):
        ...
        catid, _lcid, _desc = self.myobject
        catinf = pythoncom.CoCreateInstance(
            pythoncom.CLSID_StdComponentCategoriesMgr,
            None,
            pythoncom.CLSCTX_INPROC,
...
class HLIHelpFile(HLICOM):
    ...
    def GetText(self):
        ...
        fname, _ctx = self.myobject
        base = os.path.split(fname)[1]
        return f"Help reference in {base}"

    def TakeDefaultAction(self):
        fname, ctx = self.myobject
        cmd = win32con.HELP_CONTEXT if ctx else win32con.HELP_FINDER
...
class HLIRegisteredTypeLibrary(HLICOM):
    def GetSubList(self):
        ...
        try:
            ...
            while 1:
                ...
                try:
                    ...
                    value = ""
                if subKey == "HELPDIR" or subKey == "Flags":
                    pass
                else:
                    try:
                        lcid = int(subKey)
        ...
        for lcid, platform, fname in collected:
            ...
            if lcid:
                extraDescs.append(f"locale={lcid}")
            extraDesc = ""
            if extraDescs:
...
class HLITypeLibEntry(HLICOM):
    def GetText(self):
        ...
        name, _doc, _ctx, _helpFile = tlb.GetDocumentation(index)
        typedesc = HLITypeKinds.get(tlb.GetTypeInfoType(index), (None, "Unknown!"))[1]
        return name + " - " + typedesc

    def GetSubList(self):
        tlb, index = self.myobject
        _name, doc, ctx, helpFile = tlb.GetDocumentation(index)
        ret = []
        if doc:
...
class HLITypeLib(HLICOM):
    def GetSubList(self):
        ...
        try:
            ...
            return [browser.MakeHLI(f"{self.myobject} can not be loaded")]
