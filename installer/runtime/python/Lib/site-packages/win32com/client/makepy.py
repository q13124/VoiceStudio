...
def ShowInfo(spec):
    if not spec:
        ...
        try:
            ...
            sys.stderr.write(
                f"Warning - could not load registered typelib '{tlbSpec.clsid}'\n"
            )
            tlb = None
    ...
    for tlb, tlbSpec in infos:
        ...
        if desc is None:
            if tlb is None:
                desc = f"<Could not load typelib {tlbSpec.dll}>"
            else:
...
class SimpleProgress(genpy.GeneratorProgress):
    ...
    def LogBeginGenerate(self, filename):
        self.VerboseProgress(f"Generating to {filename}", 1)
        ...
        try:
            ...
            if len(tlbs) == 0:
                ...
                try:
                    ...
                    tlb, _index = ob._oleobj_.GetTypeInfo().GetContainingTypeLib()
                    spec = selecttlb.TypelibSpec(None, 0, 0, 0)
                    ...
                    pass
            if len(tlbs) == 0:
                print(f"Could not locate a type library matching '{arg}'")
            for spec in tlbs:
