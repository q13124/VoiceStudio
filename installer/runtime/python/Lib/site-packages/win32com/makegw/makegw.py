...
"""
    ...
    for method in interface.methods:
        f.write(
            f"\tstatic PyObject *{method.name}(PyObject *self, PyObject *args);\n"
        )
    f.write(
        f"""\
        ...
        for arg in method.args:
            try:
                ...
                if comArgDeclString:  # If we should declare a variable
                    codeCobjects += f"\t{comArgDeclString};\n"
                argsCOM += ", " + comArgName
...
""".format(**strdict)
    ...
    f.write(f"// @object Py{name}|Description of the interface\n")
    f.write(f"static struct PyMethodDef Py{name}_methods[] =\n{{\n")
    for method in interface.methods:
        f.write(
...
def _write_gw_h(f, interface):
    gname = "PyG" + interface.name[1:] if interface.name[0] == "I" else "PyG" + interface.name
    name = interface.name
...
"""
    ...
    for method in interface.methods:
        f.write(f"\tSTDMETHOD({method.name})(\n")
        if method.args:
            for arg in method.args[:-1]:
                f.write(f"\t\t{arg.GetRawDeclaration()},\n")
            arg = method.args[-1]
            f.write(f"\t\t{arg.GetRawDeclaration()});\n\n")
        else:
            f.write("\t\tvoid);\n\n")
...
"""
            ...
            for arg in method.args:
                if arg.HasAttribute("out"):
                    ...
                    if arg.indirectionLevel == 2:
                        f.write(f"\tif ({arg.name}==NULL) return E_POINTER;\n")
                if arg.HasAttribute("in"):
                    try:
                        argCvt = makegwparse.make_arg_converter(arg)
                        ...
                        codePost += f"\tPy_DECREF(ob{arg.name});\n"
                        formatChars += "O"
                        argStr += f", ob{arg.name}"
        ...
        fullArgStr = f'{resStr}, "{formatChars}"{argStr}' if formatChars else resStr
        ...
        if cout:
            ...
            if formatChars:  # If I have any to actually process.
                parseFn = "PyArg_Parse" if len(formatChars) == 1 else "PyArg_ParseTuple"
                if codePobjects:
                    f.write(codePobjects)
                f.write(
            ...
            if codePost:
                ...
                f.write(codePost)
                f.write(
                    f'\tif (!bPythonIsHappy) hr = MAKE_PYCOM_GATEWAY_FAILURE_CODE("{method.name}");\n'
                )
