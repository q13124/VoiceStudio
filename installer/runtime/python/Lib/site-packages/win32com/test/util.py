...
def RegisterPythonServer(filename, progids=None, verbose=0):
    if progids:
        ...
        for progid in progids:
            ...
            try:
                ...
                hk = winreg.OpenKey(HKCR, f"CLSID\\{clsid}")
    ...
    if not is_admin:
        msg = (
            f"{progids[0]!r} isn't registered, but I'm not an administrator who can register it."
        )
        if why_not:
    ...
    if rc:
        ...
        print(cmd)
        raise RuntimeError(f"Registration of engine '{filename}' failed")
...
def ExecuteShellCommand(
    ...
    try:
        ...
        print(f"Failed to exec command '{cmd!r}'")
        print("Failed as", why)
...
def assertRaisesCOM_HRESULT(testcase, hresult, func, *args, **kw):
    try:
        ...
        if details.hresult == hresult:
            return
    testcase.fail(f"Excepected COM exception with HRESULT 0x{hresult:x}")
...
class _CapturingFunctionTestCase(unittest.FunctionTestCase):  # , TestCaseMixin):
    ...
    def checkOutput(self, output, result):
        if output.find("Traceback") >= 0:
            msg = f"Test output contained a traceback\n---\n{output}\n---"
...
class ShellTestCase(unittest.TestCase):
    ...
    def __str__(self):
        max = 30
        cmd_repr = self.__cmd[:max] + "..." if len(self.__cmd) > max else self.__cmd
