...
def TestTrickyTypesWithVariants(o, is_generated):
    ...
    vals = [1, 2, 3, 4]
    arg = vals if is_generated else VARIANT(pythoncom.VT_ARRAY | pythoncom.VT_UI1, vals)
    TestApplyResult(o.SetBinSafeArray, (arg,), len(vals))

    # safearrays of doubles and floats
    vals = [0, 1.1, 2.2, 3.3]
    arg = vals if is_generated else VARIANT(pythoncom.VT_ARRAY | pythoncom.VT_R8, vals)
    TestApplyResult(o.SetDoubleSafeArray, (arg,), len(vals))

    arg = vals if is_generated else VARIANT(pythoncom.VT_ARRAY | pythoncom.VT_R4, vals)
...
def TestCounter(counter, bIsGenerated):
    ...
    for _i in range(50):
        ...
        try:
            ...
            ret = counter.Item(num + 1) if bIsGenerated else counter[num]
            assert ret == num + 1, (
...
def TestVTable2():
    ...
    win32com.client.Dispatch("Python.Test.PyCOMTest")
    iid = pythoncom.InterfaceNames["IPyCOMTest"]
    clsid = "Python.Test.PyCOMTest"
    clsctx = pythoncom.CLSCTX_SERVER
    try:
        pythoncom.CoCreateInstance(clsid, None, clsctx, iid)
    except TypeError:
