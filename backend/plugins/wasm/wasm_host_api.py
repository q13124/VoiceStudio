...
class CapabilityViolation(Exception):
    ...
    def __init__(self, capability: CapabilityToken, function: str):
        ...
        super().__init__(
            f"Capability violation: {function} requires {capability.name}"
        )
...
class WasmHostAPI:
    ...
    def _build_function_registry(self):
        ...
        self._functions.append(
            HostFunctionSpec(
                ...
                module="host",
                params=[
                    "i32", "i32",  # method_ptr, method_len
                    "i32", "i32",  # url_ptr, url_len
                    "i32", "i32",  # headers_ptr, headers_len
                    "i32", "i32",  # body_ptr, body_len
        ...
        for spec in self._functions:
            ...
            if spec.required_capability is not None:
                if not capabilities.has(spec.required_capability):
                    logger.debug(
                        f"Skipping {spec.name}: missing {spec.required_capability.name}"
                    )
                    continue
    ...
    def _http_request(
        ...
        logger.debug(
            f"[Plugin] http_request: url_ptr={url_ptr}, method_ptr={method_ptr}"
        )
...
class WasmMemoryHelper:
    ...
    def write_bytes(self, ptr: int, data: bytes) -> int:
        ...
        if ptr < 0 or ptr + len(data) > data_len:
            raise IndexError(
                f"Memory access out of bounds: {ptr}+{len(data)} > {data_len}"
            )
