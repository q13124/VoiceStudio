...
class PluginStorage:
    ...
    def __init__(
        ...
        self._paths: Dict[StorageType, Path] = {
            st: self._root / st.value for st in StorageType
        }
    ...
    def validate_path(self, path: Path, storage_type: StorageType) -> Path:
        ...
        try:
            ...
            raise PathValidationError(
                f"Path '{path}' escapes storage directory. "
    ...
    def write_file(
        ...
        if self.quota.max_file_size_bytes:
            if len(data) > self.quota.max_file_size_bytes:
                raise QuotaExceededError(
                    ...
                    f"{self.quota.max_file_size_bytes}"
                )
        ...
        if self.quota.max_total_bytes:
            ...
            if new_total > self.quota.max_total_bytes:
                raise QuotaExceededError(
                    f"Total storage {new_total} would exceed limit "
                    f"{self.quota.max_total_bytes}"
                )
        ...
        if self.quota.max_file_count and not path.exists():
            ...
            if usage.file_count >= self.quota.max_file_count:
                raise QuotaExceededError(
                    f"File count {usage.file_count} at limit "
                    f"{self.quota.max_file_count}"
                )
...
class StorageManager:
    ...
    def get_total_usage(self) -> Dict[str, StorageUsage]:
        ...
        return {
            plugin_id: storage.get_usage()
            for plugin_id, storage in self._storages.items()
        }
