...
class NetworkPolicyEnforcer:
    ...
    def _record_attempt(
        ...
        if action == NetworkAction.ALLOW:
            ...
            logger.debug(
                f"Network ALLOW: {self.plugin_id} -> {host}:{port} ({reason})"
            )
        else:
            self._denied_count += 1
            logger.warning(
                f"Network DENY: {self.plugin_id} -> {host}:{port} ({reason})"
            )
        ...
        class PolicySocket(original):
            ...
            def connect(self, address):
                ...
                if enforcer is not None:
                    ...
                    allowed, reason = enforcer.check_connection(
                        host, port, ConnectionType.TCP
                    )
                    
                    if not allowed:
                        raise ConnectionRefusedError(
                            f"Network policy violation: {reason}"
                        )
