...
class SynthesisRequest(BaseModel):
    ...
    voice_id: Optional[str] = Field(None, description="Voice ID for standard mode")
    reference_audio_url: Optional[str] = Field(None, description="URL to reference audio for cloning")
...
async def synthesize_with_reference(
    ...
    """Synthesize with uploaded reference audio."""
    import uuid
    import time
    import tempfile
    import os
...
@router.post(
    "/batch",
    response_model=BatchSynthesisResponse,
...
async def batch_synthesize(
    ...
    """Process batch synthesis requests."""
    import uuid
    ...
    for item in request.items:
        try:
            result = await synthesize(item, engine_service)
