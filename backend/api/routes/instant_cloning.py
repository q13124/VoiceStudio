...
def resolve_audio_id_to_path(audio_id: str) -> str:
    ...
    if not upload_path.exists():
        raise HTTPException(
            status_code=404, 
            detail=f"Audio uploads directory not found. Upload audio first."
        )
...
async def zero_shot_clone(request: ZeroShotCloneRequest):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Zero-shot cloning failed: {str(e)}"
        ) from e
...
async def instant_preview(request: InstantPreviewRequest):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Instant preview failed: {str(e)}"
        ) from e
...
async def estimate_quality(request: QualityEstimationRequest):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Quality estimation failed: {str(e)}"
        ) from e
