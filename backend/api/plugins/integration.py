...
def unregister_hook(hook_name: str, callback: Callable) -> None:
    ...
    if hook_name in _plugin_hooks:
        if callback in _plugin_hooks[hook_name]:
...
def unregister_event_handler(event_type: str, callback: Callable) -> None:
    ...
    if event_type in _plugin_events:
        if callback in _plugin_events[event_type]:
...
def get_hook_count(hook_name: Optional[str] = None) -> int:
    """
...
def get_event_handler_count(event_type: Optional[str] = None) -> int:
    """
