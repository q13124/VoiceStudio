    ...
    public class VideoEditViewModel : BaseViewModel
    {
        ...
        private VideoEditRequest? CreateEditRequest()
        {
            ...
            switch (SelectedOperation)
            {
                ...
                case "Transcode":
                    request.Format = "mp4";
                    request.Quality = QualityPresets.IndexOf(SelectedQuality) + 1; // 1-4
                    break;
                case "Upscale":
                    request.Format = "mp4";
        ...
        private async Task ApplyEffectAsync(CancellationToken cancellationToken = default)
        {
            ...
            IsLoading = true;
            ErrorMessage = null;
            StatusMessage = ResourceHelper.FormatString("VideoEdit.ApplyingEffect", SelectedEffect);
            ...
            try
            {
                ...
                if (response.Success && !string.IsNullOrWhiteSpace(response.OutputPath))
                {
                    StatusMessage = response.Message ?? ResourceHelper.FormatString("VideoEdit.EffectAppliedSuccess", SelectedEffect);
                    SelectedVideoPath = response.OutputPath; // Update to video with effect
                    await LoadVideoInfoAsync(cancellationToken); // Reload info
                }
                else
                {
                    ErrorMessage = response.Message ?? ResourceHelper.FormatString("VideoEdit.ApplyEffectFailed", SelectedEffect);
                }
            }
            catch (Exception ex)
            {
        ...
        private async Task ApplyTransitionAsync(CancellationToken cancellationToken)
        {
            ...
            IsLoading = true;
            ErrorMessage = null;
            StatusMessage = ResourceHelper.FormatString("VideoEdit.ApplyingTransition", SelectedTransition);
            ...
            try
            {
                ...
                if (response.Success && !string.IsNullOrWhiteSpace(response.OutputPath))
                {
                    StatusMessage = response.Message ?? ResourceHelper.FormatString("VideoEdit.TransitionAppliedSuccess", SelectedTransition);
                    SelectedVideoPath = response.OutputPath; // Update to video with transition
                    await LoadVideoInfoAsync(cancellationToken); // Reload info
                }
                else
                {
                    ErrorMessage = response.Message ?? ResourceHelper.FormatString("VideoEdit.ApplyTransitionFailed", SelectedTransition);
                }
            }
            catch (OperationCanceledException)
            {
