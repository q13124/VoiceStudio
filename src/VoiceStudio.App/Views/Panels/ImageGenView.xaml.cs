...
namespace VoiceStudio.App.Views.Panels
{
    public sealed partial class ImageGenView : UserControl
    {
        ...
        public ImageGenView()
        {
            ...
            ViewModel = new ImageGenViewModel(backendClient);
        ...
        private void ImageGenView_Loaded(object sender, RoutedEventArgs e)
        {
            ...
            if (sender is ListView listView && e.OriginalSource is FrameworkElement element)
            {
                ...
                if (image != null)
                {
                    ...
                    if (_contextMenuService != null)
                    {
                        ...
                        viewItem.Click += async (s, e2) => await HandleImageMenuClick("View", image);
                        menu.Items.Add(viewItem);

                        var exportItem = new MenuFlyoutItem { Text = "Export" };
                        exportItem.Click += async (s, e2) => await HandleImageMenuClick("Export", image);
                        menu.Items.Add(exportItem);

                        var upscaleItem = new MenuFlyoutItem { Text = "Upscale" };
                        upscaleItem.Click += async (s, e2) => await HandleImageMenuClick("Upscale", image);
                        menu.Items.Add(upscaleItem);

                        var duplicateItem = new MenuFlyoutItem { Text = "Duplicate" };
                        duplicateItem.Click += async (s, e2) => await HandleImageMenuClick("Duplicate", image);
                        menu.Items.Add(duplicateItem);

                        menu.Items.Add(new MenuFlyoutSeparator());

                        var deleteItem = new MenuFlyoutItem { Text = "Delete" };
                        deleteItem.Click += async (s, e2) => await HandleImageMenuClick("Delete", image);
