...
namespace VoiceStudio.App.Views.Panels
{
    public sealed partial class TrainingView : Microsoft.UI.Xaml.Controls.UserControl
    {
        ...
        private void ProgressChart_Loaded(object sender, RoutedEventArgs e)
        {
            UpdateProgressChart();
        }
        ...
        private void TrainingView_KeyboardNavigation_Loaded(object sender, RoutedEventArgs e)
        {
            ...
            if (sender is ListView listView && e.OriginalSource is FrameworkElement element)
            {
                ...
                if (dataset != null)
                {
                    ...
                    if (_contextMenuService != null)
                    {
                        ...
                        startTrainingItem.Click += async (s, e2) => await HandleDatasetMenuClick("Start Training", dataset);
                        menu.Items.Add(startTrainingItem);

                        menu.Items.Add(new MenuFlyoutSeparator());

                        var duplicateItem = new MenuFlyoutItem { Text = "Duplicate" };
                        duplicateItem.Click += async (s, e2) => await HandleDatasetMenuClick("Duplicate", dataset);
                        menu.Items.Add(duplicateItem);

                        var exportItem = new MenuFlyoutItem { Text = "Export" };
                        exportItem.Click += async (s, e2) => await HandleDatasetMenuClick("Export", dataset);
                        menu.Items.Add(exportItem);

                        menu.Items.Add(new MenuFlyoutSeparator());

                        var deleteItem = new MenuFlyoutItem { Text = "Delete" };
                        deleteItem.Click += async (s, e2) => await HandleDatasetMenuClick("Delete", dataset);
            ...
            if (sender is ListView listView && e.OriginalSource is FrameworkElement element)
            {
                ...
                if (job != null)
                {
                    ...
                    if (_contextMenuService != null)
                    {
                        ...
                        cancelItem.Click += async (s, e2) => await HandleTrainingJobMenuClick("Cancel", job);
                        menu.Items.Add(cancelItem);

                        menu.Items.Add(new MenuFlyoutSeparator());

                        var viewLogsItem = new MenuFlyoutItem { Text = "View Logs" };
                        viewLogsItem.Click += async (s, e2) => await HandleTrainingJobMenuClick("View Logs", job);
                        menu.Items.Add(viewLogsItem);

                        var exportItem = new MenuFlyoutItem { Text = "Export Model" };
                        exportItem.Click += async (s, e2) => await HandleTrainingJobMenuClick("Export", job);
                        menu.Items.Add(exportItem);

                        menu.Items.Add(new MenuFlyoutSeparator());

                        var deleteItem = new MenuFlyoutItem { Text = "Delete" };
                        deleteItem.Click += async (s, e2) => await HandleTrainingJobMenuClick("Delete", job);
                        menu.Items.Add(deleteItem);
