...
namespace VoiceStudio.App.Views.Panels
{
    public partial class EffectsMixerViewModel : ObservableObject, IPanelView
    {
        ...
        private async Task AddEffectToChainAsync(string? effectType, CancellationToken cancellationToken)
        {
        ...
        private async Task RemoveEffectFromChainAsync(string? effectId, CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                if (effect != null)
                {
                    ...
                    SelectedEffectChain.Effects.Remove(effect);
                    if (SelectedEffect?.Id == effectId)
                    {
                        SelectedEffect = null;
                    }
                    // Reorder remaining effects
                    for (int i = 0; i < SelectedEffectChain.Effects.Count; i++)
                    {
                        SelectedEffectChain.Effects[i].Order = i;
                    }
        ...
        private async Task MoveEffectUpAsync(string? effectId, CancellationToken cancellationToken)
        {
            if (string.IsNullOrWhiteSpace(effectId) || SelectedEffectChain == null)
                return;
        ...
        private async Task MoveEffectDownAsync(string? effectId, CancellationToken cancellationToken)
        {
            if (string.IsNullOrWhiteSpace(effectId) || SelectedEffectChain == null)
                return;
