...
namespace VoiceStudio.App.Views.Panels
{
    ...
    public sealed partial class TagManagerView : UserControl
    {
            ...
            if (sender is ListView listView && e.OriginalSource is FrameworkElement element)
            {
                ...
                if (tag != null)
                {
                    ...
                    if (_contextMenuService != null)
                    {
                        ...
                        editItem.Click += async (s, e2) => await HandleTagMenuClick("Edit", tag);
                        menu.Items.Add(editItem);

                        var duplicateItem = new MenuFlyoutItem { Text = "Duplicate" };
                        duplicateItem.Click += async (s, e2) => await HandleTagMenuClick("Duplicate", tag);
                        menu.Items.Add(duplicateItem);

                        menu.Items.Add(new MenuFlyoutSeparator());

                        var deleteItem = new MenuFlyoutItem { Text = "Delete" };
                        deleteItem.Click += async (s, e2) => await HandleTagMenuClick("Delete", tag);
        ...
        private void TagManagerView_KeyboardNavigation_Loaded(object sender, RoutedEventArgs e)
        {
