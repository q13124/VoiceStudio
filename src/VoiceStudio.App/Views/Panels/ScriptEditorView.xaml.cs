...
namespace VoiceStudio.App.Views.Panels
{
    ...
    public sealed partial class ScriptEditorView : UserControl
    {
        ...
        {
            InitializeComponent();
            ViewModel = new ScriptEditorViewModel(ServiceProvider.GetBackendClient());
        ...
        private void ScriptEditorView_KeyDown(object sender, KeyRoutedEventArgs e)
        {
            ...
            if (sender is ListView listView && e.OriginalSource is FrameworkElement element)
            {
                ...
                if (script != null)
                {
                    ...
                    if (_contextMenuService != null)
                    {
                        ...
                        synthesizeItem.Click += async (s, e2) => await HandleScriptMenuClick("Synthesize", script);
                        menu.Items.Add(synthesizeItem);

                        var editItem = new MenuFlyoutItem { Text = "Edit" };
                        editItem.Click += async (s, e2) => await HandleScriptMenuClick("Edit", script);
                        menu.Items.Add(editItem);

                        menu.Items.Add(new MenuFlyoutSeparator());

                        var duplicateItem = new MenuFlyoutItem { Text = "Duplicate" };
                        duplicateItem.Click += async (s, e2) => await HandleScriptMenuClick("Duplicate", script);
                        menu.Items.Add(duplicateItem);

                        var exportItem = new MenuFlyoutItem { Text = "Export" };
                        exportItem.Click += async (s, e2) => await HandleScriptMenuClick("Export", script);
                        menu.Items.Add(exportItem);

                        menu.Items.Add(new MenuFlyoutSeparator());

                        var deleteItem = new MenuFlyoutItem { Text = "Delete" };
                        deleteItem.Click += async (s, e2) => await HandleScriptMenuClick("Delete", script);
        ...
        private void Segment_DragLeave(object sender, DragEventArgs e)
        {
            if (_dragDropService != null)
            {
                _dragDropService.HideDropTargetIndicator();
        ...
        private void ScriptEditorView_KeyboardNavigation_Loaded(object sender, RoutedEventArgs e)
        {
