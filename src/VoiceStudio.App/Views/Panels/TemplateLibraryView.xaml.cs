...
namespace VoiceStudio.App.Views.Panels
{
    ...
    public sealed partial class TemplateLibraryView : UserControl
    {
        ...
        private void TemplateLibraryView_KeyboardNavigation_Loaded(object sender, RoutedEventArgs e)
        {
            ...
            if (sender is ListView listView && e.OriginalSource is FrameworkElement element)
            {
                ...
                if (template != null)
                {
                    ...
                    if (_contextMenuService != null)
                    {
                        ...
                        applyItem.Click += async (s, e2) => await HandleTemplateMenuClick("Apply", template);
                        menu.Items.Add(applyItem);

                        var previewItem = new MenuFlyoutItem { Text = "Preview" };
                        previewItem.Click += async (s, e2) => await HandleTemplateMenuClick("Preview", template);
                        menu.Items.Add(previewItem);

                        var editItem = new MenuFlyoutItem { Text = "Edit" };
                        editItem.Click += async (s, e2) => await HandleTemplateMenuClick("Edit", template);
                        menu.Items.Add(editItem);

                        var duplicateItem = new MenuFlyoutItem { Text = "Duplicate" };
                        duplicateItem.Click += async (s, e2) => await HandleTemplateMenuClick("Duplicate", template);
                        menu.Items.Add(duplicateItem);

                        var exportItem = new MenuFlyoutItem { Text = "Export" };
                        exportItem.Click += async (s, e2) => await HandleTemplateMenuClick("Export", template);
                        menu.Items.Add(exportItem);

                        menu.Items.Add(new MenuFlyoutSeparator());

                        var deleteItem = new MenuFlyoutItem { Text = "Delete" };
                        deleteItem.Click += async (s, e2) => await HandleTemplateMenuClick("Delete", template);
