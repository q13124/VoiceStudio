...
def generate_panel(
    ...
    # Add manual steps
    manual_steps.append(
        f"1. Register panel in PanelRegistry.cs or AdvancedPanelRegistrationService.cs:"
    )
    manual_steps.append(
        f"   _panels.Add(new PanelDescriptor(\"{panel_id}\", \"{display_name}\", typeof({name}View)));"
    )
    manual_steps.append(f"2. Build: dotnet build VoiceStudio.sln -c Debug -p:Platform=x64")
...
def main() -> int:
    ...
    if result.success:
        ...
        print(f"[ERROR] Panel scaffold generation failed.")
