...
class QualityDatabase:
    ...
    def get_week_ago(self) -> QualitySnapshot | None:
...
def ingest_scorecard(scorecard_path: Path | None = None) -> QualitySnapshot | None:
...
Examples:
  ...
  python -m tools.quality.dashboard --export html > report.html
    ...
    days = int(period_match[:-1]) if period_match.endswith("d") else int(period_match)
