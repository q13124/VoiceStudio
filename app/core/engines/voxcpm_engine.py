"""
VoxCPM Engine for VoiceStudio
Chinese and multilingual TTS integration

VoxCPM is a Chinese and multilingual text-to-speech system that provides
high-quality synthesis with support for Chinese and other languages.

Compatible with:
- Python 3.10+
- torch>=2.0.0
- transformers>=4.20.0
- voxcpm package or direct model loading
"""
...
try:
    ...
    logger.debug("General model cache not available, using VoxCPM-specific cache")

# Fallback: VoxCPM-specific cache (for backward compatibility)
_VOXCPM_MODEL_CACHE: OrderedDict = OrderedDict()
_MAX_CACHE_SIZE = 2  # Maximum number of models to cache in memory


def _get_cache_key(model_name: str, device: str) -> str:
    """Generate cache key for VoxCPM model."""
    return f"voxcpm::{model_name}::{device}"


def _get_cached_voxcpm_model(model_name: str, device: str):
    """Get cached VoxCPM model if available."""
    ...
    if cache_key in _VOXCPM_MODEL_CACHE:
        ...
        return _VOXCPM_MODEL_CACHE[cache_key]
    return None


def _cache_voxcpm_model(model_name: str, device: str, models: dict):
    """Cache VoxCPM model with LRU eviction."""
    # Try general model cache first
    if HAS_MODEL_CACHE and _model_cache is not None:
        try:
            _model_cache.set("voxcpm", model_name, models, device=device)
            return
...
class VoxCPMEngine(EngineProtocol):
    ...
    def _load_model(self) -> bool:
        ...
        if self._caching_enabled:
            cached_models = _get_cached_voxcpm_model(self.model_name, self.device)
            if cached_models is not None:
                logger.debug(f"Using cached VoxCPM model: {self.model_name}")
...
def create_voxcpm_engine(
    model_name: str = "voxcpm/base", device: str | None = None, gpu: bool = True
) -> VoxCPMEngine:
