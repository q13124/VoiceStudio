...
try:
    ...
    TextToSpeech = None
    logging.warning(
        "Tortoise TTS not installed. "
        "Install with: pip install tortoise-tts"
    )
...
try:
    ...
    _model_cache = get_model_cache(max_models=3, max_memory_mb=3072.0)  # 3GB max (Tortoise models are larger)
...
def _get_cached_voice_embedding(voice_samples: List[Union[str, Path]]) -> Optional[np.ndarray]:
...
def _cache_voice_embedding(voice_samples: List[Union[str, Path]], embedding: np.ndarray):
    ...
    logger.debug(f"Cached voice embedding: {cache_key[:8]} (cache size: {len(_VOICE_EMBEDDING_CACHE)})")
...
class TortoiseEngine(EngineProtocol):
    ...
    QUALITY_PRESETS = {
        ...
        "high_quality": {"num_autoregressive_samples": 256, "diffusion_iterations": 200},
        "ultra_quality": {"num_autoregressive_samples": 512, "diffusion_iterations": 400}
    }
            ...
            if self.enable_caching and voice_refs and len(voice_refs) > 1:
                ...
                if voice_embedding is None and hasattr(self.tts, 'get_voice_embedding'):
                    ...
                    try:
                        ...
                        logger.debug(f"Cached voice embedding for {len(voice_refs)} samples")
                    except Exception as e:
