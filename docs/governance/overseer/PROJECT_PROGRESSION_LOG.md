## VoiceStudio — Project Progression Log (living)

**Last updated:** 2026-01-11  
**Snapshot:** `docs/governance/overseer/PROJECT_PROGRESSION_LOG_2026-01-11.md` (immutable copy)

> This file is the **canonical living log**. When you update it, also cut a dated snapshot.

---

## Non‑negotiables (must be honored in all plans/changes)

- **Single packaging lane**: **unpackaged apphost EXE + installer only** (no MSIX).
  - Gate C proof artifact: `scripts/gatec-publish-launch.ps1`
  - Release/installer lane: `scripts/prepare-release.ps1` + `installer/build-installer.ps1`
  - MSIX is historical only: `docs/archive/msix/` (do not reintroduce a second lane)
- **Deterministic evidence (not “fallback behavior”)**:
  - Keep proof artifacts: `.buildlogs/` (binlogs + `gatec-latest.txt`) and crash artifacts under `%LOCALAPPDATA%\VoiceStudio\crashes\*` / `%LOCALAPPDATA%\VoiceStudio\dumps\*`.
  - Crash bundle export is a supported workflow and must remain usable (explicit user action; no automatic uploads): `docs/release/CRASH_BUNDLE_EXPORT_FEATURE.md`.
- **No placeholders / no incomplete implementations**:
  - RuleGuard runs at build time and must remain green: `Directory.Build.targets` + `tools/verify_no_stubs_placeholders.py`.
  - Avoid introducing placeholder patterns in production code or docs.
- **UI invariants must not drift** (layout + separation + tokens):
  - 3-row grid structure, 4 PanelHosts, MVVM separation, and **VSQ.\*** design tokens (no hardcoded styling).
  - Canonical rules: `docs/governance/AGENT_SETTINGS_RULES_COMMANDS_2025-01-28.md`.
- **Localization-ready UI**:
  - User-facing strings should be resource-backed (`.resw`) and XAML should use `x:Uid` where appropriate.
  - References: Worker 2 localization tasks in `docs/governance/worker2/WORKER_2_*_PROMPT_2025-01-28.md` and `docs/governance/COMPREHENSIVE_FINISH_PLAN_2025-01-28.md`.
- **Accessibility requirements**:
  - AutomationProperties (Name/HelpText/LabeledBy/LiveSetting), keyboard navigation, minimum hit targets, and contrast expectations.
  - Helper utilities: `src/VoiceStudio.App/Utilities/AccessibilityHelpers.cs`.
- **Analytics/telemetry constraints (local-first + privacy)**:
  - Analytics and telemetry are intended for local diagnostics and must respect privacy boundaries (no background uploading by default).
  - App analytics: `src/VoiceStudio.App/Services/AnalyticsService.cs`; backend analytics routes: `backend/api/routes/analytics.py`.
  - Dev toggle exists: `VOICESTUDIO_ENABLE_ANALYTICS` (see `src/VoiceStudio.App/Properties/launchSettings.json`).
- **Least privilege / security**:
  - Frontend/backend are expected to run as standard user (no Administrator requirement for normal runtime).
  - Canonical guidance: `docs/security/MINIMAL_PRIVILEGES.md`.
- **Toolchain pinning and controlled upgrades**:
  - .NET SDK is pinned via `global.json`; WinAppSDK and key packages are pinned in `Directory.Build.props`.
  - Any upgrades must include proof runs (build + Gate C publish+launch + regression checks) and updated documentation.
- **Python environment isolation**:
  - Some tooling uses different Python requirements than the engine stack (e.g., `backend/mcp_servers/mcp-unlock-pdf/pyproject.toml` requires Python `>=3.12`).
  - Treat tooling as isolated venvs unless/until a verified migration plan exists.
...
- **Production build plan:** `VoiceStudio_Production_Build_Plan.md`
