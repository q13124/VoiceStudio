# VoxCPM Engine Performance Optimization Complete
...
**Features:**
- LRU cache for loaded VoxCPM models (model_name + device aware)
...
**Usage:**
```python
engine = VoxCPMEngine(batch_size=4)
...
- `app/core/engines/voxcpm_engine.py` - Complete optimization with caching, lazy loading, batch processing
...
- `_get_cached_voxcpm_model()` - Get cached model
- `_cache_voxcpm_model()` - Cache model
