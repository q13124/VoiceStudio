...
**Target:** Python 3.11.9 + PyTorch 2.9.0 + RTX 30/40 Series
...
| --------------------- | --------------- | ------------------------------------------------- | ----- |
| **Python runtime**    | 3.11.9          | Latest tested with Torch 2.9 + Transformers 4.57  |       |
| **PyTorch**           | `2.9.0 + cu128` | Fully GPU-optimized for RTX 30/40 series          |       |
| **Torchaudio**        | `2.9.0 + cu128` | Must match Torch exactly                          |       |
| **Coqui-TTS**         | `0.27.2`        | Current release w/ XTTS v2 engine                 |       |
| **coqui-tts-trainer** | `0.3.1`         | Finetuning & dataset manager                      |       |
| **Transformers**      | `4.57.1`        | Stable with XTTS v2 and Torch ≥ 2.8               |       |
| **huggingface_hub**   | `0.36.0`        | Matches Transformers 4.57+ API                    |       |
| **hf-xet**            | `1.2.0`         | (installed by hub) XetHub remote dataset backend  |       |
| **fsspec**            | `2025.9.0`      | For remote asset loading                          |       |
| **NumPy**             | `1.26.4`        | Perfect bridge between Torch 2.9 and Librosa 0.11 |       |
| **Librosa**           | `0.11.0`        | Audio feature extraction for XTTS and QC          |       |
| **SoundFile**         | `0.12.1`        | WAV/FLAC I/O                                      |       |
| **Faster-Whisper**    | `1.2.0`         | Newer Whisper variant; GPU-ready                  |       |
| **pyloudnorm**        | `0.1.1`         | LUFS metering                                     |       |
| **noisereduce**       | `3.0.2`         | NR chain                                          |       |
| **ffmpeg-python**     | `0.2.0`         | Audio/video conversion                            |       |
| **tokenizers**        | `0.22.1`        | Required by Transformers 4.57                     |       |
...
| --------------------- | ---------------------------------- | -------------------------------------------------------------------- |
| **MyShell OpenVoice** | `0.0.0 (patched)`                  | Keep isolated in its own venv → conflicts w/ NumPy 1.26+ & Torch 2.9 |
...
| **WhisperX**          | `3.7.4`                            | Works if Torch ≈ 2.8 — stay on 1.2.0 Faster-Whisper instead          |
...
| ---------------------------- | ---------------- | ---------------------------------------- |
| WinUI 3 / .NET SDK           | 8.0.303          | Verified for Windows 11 build 26100      |
| WinAppSDK / CommunityToolkit | 1.5.0 / 8.1.2409 | Native controls + XAML utilities         |
| PySide6 / Addons             | 6.8.0.1          | Bridge for Python UI tools               |
| qfluentwidgets               | 1.4.3            | Fluent style toolkit for VoiceStudio GUI |
...
pip install transformers==4.57.1 huggingface_hub==0.36.0
pip install tokenizers==0.22.1 safetensors==0.6.2
```
...
### Must Match Exactly
1. **Torch == Torchaudio** (2.9.0+cu128)
2. **Transformers >= 4.57.1** (for XTTS v2)
...
- [x] PyTorch 2.9.0+cu128 verified
- [x] Torchaudio matches Torch version
- [x] Transformers 4.57.1 compatible
...
**Status:** ✅ All components verified compatible
...
**Next Review:** When major version updates available
