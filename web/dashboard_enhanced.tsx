...
// Router Dashboard — React Preview (with A/B test + WebSocket progress)
// Talks to:
//   GET  /health, /engines
//   POST /tts  { text, language, quality, mode: "sync" }
//   POST /abtest { text, language, quality, engines? }
// And listens (optionally) to WebSocket progress at ws://<host>:<port>/ws (placeholder until backend enables it)
...
function ProgressPanel({ baseUrl }: { baseUrl: string }) {
  ...
  useEffect(() => {
    // Placeholder path — wire your backend to serve ws://host:port/ws
  ...
  return (
    <div className="space-y-2">
      ...
      <div className="border rounded-lg bg-white p-2 h-48 overflow-auto text-xs font-mono">
        {messages.length === 0 ? (
          <div className="text-gray-500">Waiting for progress events… (enable WS at /ws on the router)</div>
        ) : (
          messages.map((m, i) => (
    ...
    <div className="min-h-screen bg-gray-50 text-gray-900">
      <div className="max-w-6xl mx-auto p-4 md:p-8 space-y-6">
        ...
        <footer className="pt-6 text-xs text-gray-500">
          Router endpoints: <code>/health</code>, <code>/engines</code>, <code>/tts</code>, <code>/abtest</code>. Optional WebSocket: <code>/ws</code>
        </footer>
      </div>
    </div>
  );
