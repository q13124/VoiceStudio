...
from backend.plugins.gallery.verification import (  # Main class; Data classes; Enums; Convenience functions
...
class TestPluginVerificationService:
    ...
    async def test_verify_nonexistent_package(self):
        ...
        assert result.passed is False
        assert any(
            c.name == "file_exists" and c.status == VerificationStatus.FAILED for c in result.checks
        )
