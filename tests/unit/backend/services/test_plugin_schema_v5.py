    ...
    def test_supply_chain_vulnerability_scan_invalid_scanner(self, validator):
        ...
        manifest = {
            **BASE_VALID_MANIFEST,
            "supply_chain": {
                "vulnerability_scan": {
                    "scanner": "unknown-scanner"  # Not in enum
                }
            }
        }
        is_valid, errors = validator.validate(manifest)
    ...
    def test_supply_chain_license_invalid_category(self, validator):
        ...
        manifest = {
            **BASE_VALID_MANIFEST,
            "supply_chain": {
                "license_compliance": {
                    "license_category": "invalid_category"  # Not in enum
                }
            }
        }
        is_valid, errors = validator.validate(manifest)
...
class TestProvenanceSection:
    ...
    def test_provenance_invalid_build_type(self, validator):
        ...
        manifest = {
            **BASE_VALID_MANIFEST,
            "provenance": {
                "build_type": "invalid_type"  # Not in enum
            }
        }
        is_valid, errors = validator.validate(manifest)
        assert not is_valid
...
class TestCertificationSection:
    ...
    def test_certification_invalid_level(self, validator):
        ...
        manifest = {
            **BASE_VALID_MANIFEST,
            "certification": {
                "certification_level": "invalid_level"  # Not in enum
            }
        }
        is_valid, errors = validator.validate(manifest)
        assert not is_valid
