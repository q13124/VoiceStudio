...
VALID_CAPABILITIES = {
    ...
    "upscaling", "style_transfer", "controlnet", "lora",
    # Video capabilities
...
class TestRequiredFields:
    ...
    def test_engine_id_format(self, all_manifests):
        ...
        assert not errors, f"Invalid engine_id format:\n" + "\n".join(errors[:20])
    ...
    def test_engine_id_matches_directory(self, all_manifests):
        ...
        if len(mismatches) > 5:
            import warnings
            warnings.warn(f"Engine ID/directory mismatches: {mismatches[:5]}")
    ...
    def test_version_format(self, all_manifests):
        ...
        if len(errors) > 5:
            import warnings
            warnings.warn(f"Non-standard versions: {errors[:5]}")
...
class TestTypeValidation:
    ...
    def test_audio_subtypes_valid(self, audio_manifests):
        ...
        if invalid:
            import warnings
            warnings.warn(f"Non-standard audio subtypes: {invalid[:5]}")
...
class TestCapabilities:
    ...
    def test_capabilities_is_list(self, all_manifests):
        ...
        assert not errors, f"Invalid capabilities type:\n" + "\n".join(errors)
    ...
    def test_capabilities_not_empty(self, all_manifests):
        ...
        if len(empty) > 5:
            import warnings
            warnings.warn(f"Engines with empty capabilities: {empty[:5]}")
    ...
    def test_known_capabilities(self, all_manifests):
        ...
        if unknown:
            import warnings
            warnings.warn(f"Custom capabilities found: {list(unknown.keys())[:10]}")
...
class TestContractSchema:
    ...
    def test_contract_structure(self, all_manifests):
        ...
        assert not errors, f"Contract structure errors:\n" + "\n".join(errors)
    ...
    def test_input_contract(self, all_manifests):
        ...
        if len(errors) > 10:
            import warnings
            warnings.warn(f"Input contract issues: {errors[:5]}")
    ...
    def test_output_contract(self, all_manifests):
        ...
        if len(errors) > 10:
            import warnings
            warnings.warn(f"Output contract issues: {errors[:5]}")
    ...
    def test_resource_requirements(self, all_manifests):
        ...
        assert not errors, f"Resource requirement issues:\n" + "\n".join(errors)
...
class TestConfigSchema:
    ...
    def test_config_schema_structure(self, all_manifests):
        ...
        assert not errors, f"Config schema errors:\n" + "\n".join(errors[:20])
    ...
    def test_config_types_valid(self, all_manifests):
        ...
        assert not errors, f"Invalid config types:\n" + "\n".join(errors)
    ...
    def test_enum_fields_have_values(self, all_manifests):
        ...
        assert not errors, f"Enum errors:\n" + "\n".join(errors)
...
class TestCrossManifestConsistency:
    ...
    def test_unique_engine_ids(self, all_manifests):
        ...
        assert not duplicates, f"Duplicate engine_ids:\n" + "\n".join(duplicates)
    ...
    def test_consistent_license_format(self, all_manifests):
        ...
        if len(non_standard) > 10:
            import warnings
            warnings.warn(f"Non-standard licenses: {non_standard[:5]}")
    ...
    def test_python_version_format(self, all_manifests):
        ...
        if len(errors) > 5:
            import warnings
            warnings.warn(f"Non-standard python_version: {errors[:5]}")
    ...
    def test_audio_sample_rates_consistent(self, audio_manifests):
        ...
        if sample_rates:
            ...
            if non_standard:
                import warnings
                warnings.warn(f"Non-standard sample rates: {non_standard}")
...
class TestDependencyValidation:
    ...
    def test_dependencies_format(self, all_manifests):
        ...
        assert not errors, f"Dependency format errors:\n" + "\n".join(errors)
    ...
    def test_no_conflicting_torch_versions(self, all_manifests):
        ...
        if len(torch_versions) > 5:
            import warnings
            warnings.warn(f"Multiple torch version requirements: {list(torch_versions.keys())}")
...
class TestEntryPointValidation:
    ...
    def test_entry_point_format(self, all_manifests):
        ...
        if len(errors) > 10:
            import warnings
            warnings.warn(f"Non-standard entry points: {errors[:5]}")
    ...
    def test_entry_point_consistency(self, all_manifests):
        ...
        if patterns:
            ...
            if dominant / total < 0.5:
                import warnings
                warnings.warn(f"Inconsistent entry point patterns: {patterns}")
