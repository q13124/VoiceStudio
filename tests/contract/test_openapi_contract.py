...
class TestOpenAPISchemaValidation:
    ...
    def test_all_refs_resolve(self, openapi_schema: Dict[str, Any]):
        ...
        assert not unresolved, f"Unresolved $ref references:\n" + "\n".join(unresolved[:20])
    ...
    def test_array_items_defined(self, openapi_schema: Dict[str, Any]):
        ...
        if len(all_errors) > 5:
            pytest.fail(f"Arrays without items:\n" + "\n".join(all_errors[:10]))
        ...
        def check_enums(obj: Any, path: str):
            if isinstance(obj, dict):
                if "enum" in obj:
                    ...
                    elif len(values) != len(set(str(v) for v in values)):
...
class TestSecurityDefinitions:
    ...
    def test_sensitive_endpoints_secured(self, openapi_schema: Dict[str, Any]):
        ...
        if unsecured:
            ...
            import warnings
            warnings.warn(f"Potentially unsecured endpoints: {unsecured[:5]}")
...
class TestDocumentation:
    ...
    def test_schemas_have_descriptions(self, openapi_schema: Dict[str, Any]):
        ...
        if total > 0:
            ...
            if coverage < 0.3:
                import warnings
                warnings.warn(f"Schema description coverage: {coverage:.0%}")
    ...
    def test_examples_provided(self, openapi_schema: Dict[str, Any]):
        ...
        if total > 0:
            ...
            if coverage < 0.2:
                import warnings
                warnings.warn(f"Example coverage: {coverage:.0%} ({with_examples}/{total})")
