...
class WorkflowTracer:
    ...
    def audio_import(
        ...
        """
        Record an audio import checkpoint.

        Args:
            audio_path: Path to imported audio file.
            asset_id: Backend asset ID if available.
        ...
        entry = {
            ...
            "asset_id": asset_id,
            "metadata": self._safe_serialize(metadata),
    ...
    def audio_validate(
        ...
        """
        Record an audio validation checkpoint.

        Args:
            audio_path: Path to validated audio file.
        ...
        entry = {
            "event": "audio_checkpoint",
            "checkpoint_type": "validate",
            "category": "audio",
            "audio_path": str(path),
            "validation_passed": passed,
            "validation_errors": errors or [],
            "validation_result": self._safe_serialize(validation_result),
            "current_step": self._current_step,
            "current_phase": self._current_phase,
            "timestamp": datetime.now().isoformat(),
            "elapsed_ms": self._elapsed_ms(),
    ...
    def audio_process(
        ...
        """
        Record an audio processing checkpoint.

        Args:
            operation: Type of processing (transcription, conversion, etc.).
            input_path: Input audio path.
            output_path: Output path if different from input.
            asset_id: Backend asset ID.
        ...
        self.trace_log.append(entry)
        self.performance_metrics.setdefault("audio_process", [])
        return self
    ...
    def audio_export(
        ...
        """
        Record an audio export checkpoint.

        Args:
            source_asset_id: Source asset ID.
            export_path: Path where audio was exported.
            export_format: Export format (wav, mp3, etc.).
    ...
    def audio_checkpoint(
        ...
        """
        Generic audio checkpoint with optional validation.
        ...
        if validate and path and path.exists():
            validation_info = self._quick_audio_validate(path)
    ...
    def _quick_audio_validate(self, path: Path) -> dict[str, Any]:
        """Quick WAV validation for checkpoints."""
        result = {"valid": False, "format": None, "errors": []}
        ...
        try:
            ...
            with wave.open(str(path), "rb") as wf:
                ...
                result["duration"] = wf.getnframes() / wf.getframerate()
        except Exception as e:
            result["errors"].append(str(e))

        return result

    def get_audio_checkpoints(self) -> list[dict[str, Any]]:
        """
        Get all audio checkpoints from the trace log.
    ...
    def get_audio_summary(self) -> dict[str, Any]:
        """
        Get a summary of audio operations in this trace.

        Returns:
            Summary dict with counts and status.
        """
        checkpoints = self.get_audio_checkpoints()
        ...
        for cp in checkpoints:
            ...
            if cp_type == "import":
                summary["imports"] += 1
            elif cp_type == "validate":
                summary["validations"] += 1
                if cp.get("validation_passed"):
                    summary["validations_passed"] += 1
                else:
                    summary["errors"].extend(cp.get("validation_errors", []))
            elif cp_type == "process":
                summary["processes"] += 1
                if not cp.get("success") and cp.get("error"):
                    summary["errors"].append(cp["error"])
            elif cp_type == "export":
                summary["exports"] += 1
                if not cp.get("success") and cp.get("error"):
                    summary["errors"].append(cp["error"])
    ...
    # Alias for consistency with phase naming
    complete_phase = end_phase
