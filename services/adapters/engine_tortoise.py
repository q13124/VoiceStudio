"""
Real Tortoise TTS Engine Adapter for VoiceStudio Router
Compatible with Tortoise TTS v2.4+
"""
...
class TortoiseRealAdapter:
    id = "tortoise"
    languages = ["en"]  # Tortoise is English-only
    ...
    def _init_model(self):
        ...
        if not TextToSpeech:
            print("Warning: Tortoise TTS not available, using stub")
            return
        ...
        try:
            ...
            print(f"Warning: Could not initialize Tortoise TTS model: {e}")
            self.tts = None
    ...
    def supports(self, language: str, tier: QualityTier) -> bool:
        return language.lower() == "en"  # English only
    ...
    def tts(self, *, text: str, voice_profile: Dict, params: Dict) -> bytes:
        ...
        try:
            ...
            # Get voice samples
            voice_samples = voice_profile.get("voice_samples", [])
            if not voice_samples:
                voice_samples = self.cfg.get("default_voice_samples", [])

            # Generate speech
            if voice_samples:
                # Use provided voice samples
                voice_data = load_voice(voice_samples[0])
                audio = self.tts.tts_with_preset(
                    text, voice_samples=voice_data, preset=self._get_preset(quality)
                )
            else:
                # Use default voice
                audio = self.tts.tts_with_preset(text, preset=self._get_preset(quality))

            # Convert to WAV bytes
            return self._audio_to_wav_bytes(audio, sample_rate)

        except Exception as e:
            print(f"Tortoise TTS generation failed: {e}")
            return self._generate_stub_audio(text, params)

    def _get_preset(self, quality: str) -> str:
        """Get Tortoise preset based on quality tier"""
        presets = {"fast": "fast", "balanced": "standard", "quality": "high_quality"}
        return presets.get(quality, "standard")

    def _generate_stub_audio(self, text: str, params: Dict) -> bytes:
        """Generate stub audio when Tortoise TTS is not available"""
        import math
        import struct
    ...
    def _load_cfg(self, config_path: Optional[Path]) -> Dict:
        ...
        for p in paths:
            if p and p.exists() and yaml is not None:
                ...
                return (data.get("engines", {}) or {}).get("tortoise", {})
